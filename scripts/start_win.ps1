...
Start-Process -FilePath $exe -ArgumentList "--role","channel","--config",(Join-Path $win "channel.toml") -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logs "channel.$ts.out.log") -RedirectStandardError (Join-Path $logs "channel.$ts.err.log")
