...
# database. Proves the backup is restorable — a dump that has never been restored
# is not a backup.
#
# Usage:
...
# Credentials via the environment so they never appear in the command line
$env:PGUSER     = "mt2"
$env:PGPASSWORD = "mt2"

# 1. Pick the dump (explicit path or newest nightly)
if (-not $DumpFile) {
    $latest = Get-ChildItem -LiteralPath $backupDir -Filter "metin2_*.dump" -ErrorAction Stop |
        Sort-Object Name -Descending | Select-Object -First 1
    if (-not $latest) { throw "No metin2_*.dump found in $backupDir" }
...
try {
    ...
    # 4. Key-table counts (a missing table here means schema drift -> fail the drill)
...
"@
    ...
    $counts | ForEach-Object {
        Write-Host "  $_"
        if ($_ -match "=(0|)$") { throw "Key table EMPTY or missing (row: '$_') — schema drift or bad dump" }
    }
    Write-Host "OK: restore drill PASSED ($DumpFile -> $drillDb)"
}
finally {
    if (-not $KeepDb) {
        & $dropdb -h 127.0.0.1 -p 5432 -d $drillDb --force 2>$null
