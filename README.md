...
> **Un Metin2 reimaginado desde cero: servidor en Rust, PostgreSQL 18, arquitectura server-authoritative y un diseño pensado para crecer.**
>
> English: a from-scratch rewrite of the Metin2 game server in Rust. Server-authoritative, PostgreSQL-backed, engineered for scale and built to revive a classic.
...
**Metin2 Rust** es la reescritura completa del servidor de Metin2 (MMORPG de 2004) en **Rust**, con las tecnologías de 2026 y un objetivo claro: **hacer más con menos**.
...
- 🚀 **Rinde al máximo**: tokio + ECS por región, sin locks compartidos, sin SQL en el hot path. Techo de **1.000+ jugadores por instancia** (el legacy: ~300-500).
...
| Fase | Estado |
|---|---|
| Línea base C++ verificada (login completo contra el cliente real) | ✅ |
...
```
Cliente (C++ v40999 congelado + 2 paquetes aditivos) ──► Servidor Rust
                                                            │  net (tokio)
...
```
source/
├── metin2_server/    # Fuente C++ del servidor legacy (la referencia a portar)
├── metin2_client/    # Fuente C++ del cliente v40999 (contrato de protocolo)
├── metin2_pack/      # Fuentes del pack (python, uiscript, PackMakerLite)
└── metin2_proto/     # Metadatos de protocolo
...
> **Binarios y packs no están en git.** El cliente instalado, los `.epk` y los builds se distribuyen como Releases.
...
Este proyecto quiere **revivir Metin2 con la comunidad**: documentación pública de protocolo, anti-bot efectivo, y un servidor moderno que los operadores puedan adoptar. Issues, PRs y opiniones sobre el plan son bienvenidos — las decisiones de arquitectura se discuten en los documentos de `docs/` antes de implementarse.
