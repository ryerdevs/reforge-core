...
A row may be working while a related feature remains limited. This static
summary does not replace the live state sources.
...
|---|---|---|---|---|
| Runtime, login, and entry | ✅ Working and verified | Native Windows runtime with PostgreSQL 18; login → character selection → world → movement verified with an external compatible client (`test` / `1234`). | Listener state and deployed-binary identity are volatile; see the [live handoff](documentation/progress.md) and [Gap Registry](documentation/plans/gap-registry.md). The G0.1b/c/d real-client checks remain operator actions. | [ADR-0012](documentation/adr/0012-windows-native-runtime-wsl-on-demand.md) |
...
| Database and persistence | ✅ Working and verified | PostgreSQL 18.4 configuration at `127.0.0.1:5432` (service `postgresql-metin2`); `tokio-postgres` repositories; WAL durability with `ON CONFLICT DO NOTHING` idempotency; `Batcher::flush()` for one-tx commits; selected ACID paths for items, economy, and safebox are covered by focused tests. | Current PostgreSQL availability and WAL-test results are recorded in the [live handoff](documentation/progress.md) and [Gap Registry](documentation/plans/gap-registry.md). Not every legacy domain is persisted; the remaining persistence gates are open. `CHECK gold>=0` is enforced in Rust (`economy.rs:97`); the PG-side `CHECK` constraint is a follow-up. Nightly `pg_dump` is covered by `scripts/restore_drill.ps1`; an off-host copy of the dumps is still an operator action. | [ADR-0008](documentation/adr/0008-data-layer.md); [ADR-0005](documentation/adr/0005-postgresql-cutover-and-legacy-adapter.md); [Backup/restore runbook](documentation/reference/backup-restore.md); [Gap Registry](documentation/plans/gap-registry.md) |
...
| Caps and storage | 🟡 Partial or limited | The five caps (item stack 200, absolute move distance 6000, spawn view 300k/310k, boot speed 200, gold 2 000 000 000) have focused, mutation-tested verifier coverage (12 + 13 + 22 + 147 tests in the recorded baseline). `target/` is now 425 MB (release only), within the 5 GB budget. | The 2000 stack target remains blocked on the wire (G0.1a). Storage gate is closed but the off-host dump copy is still an operator action. The five real-client checks (G0.1a–G0.1d) are committed but unverified at the wire. | [Live handoff](documentation/progress.md); [Gap Registry — G0.1a–G0.1e, G0.2](documentation/plans/gap-registry.md) |
| Verification, documentation, and deployment | ✅ Working and verified | `scripts/verify.ps1` defines the standard sequence: `cargo fmt --check` + `cargo test --workspace` + ignored informative leg + `cargo clippy --workspace -- -D warnings` + `git diff --check`. The CI workflow `.github/workflows/docs.yml` runs the same script on every push and every PR, plus `check_docs.ps1` (metadata) and the handoff check (source touched ⇒ `progress.md` / `CHANGELOG.md` / `gap-registry.md` updated). | Current verifier results and deployment availability are recorded in the [live handoff](documentation/progress.md) and [Gap Registry](documentation/plans/gap-registry.md). The `--ignored` leg is informative; full live-PG execution needs the runbook. The WSL parity leg of G1.18 still needs an operator action. The three `channel_pg` follow-ups (G3.2g/h/i) and the party drain test (G3.2c) are excluded from the gate. | [Gap Registry](documentation/plans/gap-registry.md); [AGENTS.md](AGENTS.md) |
...
The claims in this README were last checked on **2026-09-02** against the
