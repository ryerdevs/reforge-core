...
- The "0 items" inventory observation is NOT a bug: the item query contract is correct â€” `player.item` holds 22 rows, all `owner_id 2` (other characters); the test chars have none.

**Parallel wave (2026-08-13, 39th part):**

- **(1) ECS migration DONE** â€” bevy_ecs 0.19 World replaces `MobCache`: `game_core/src/ecs.rs` (986 lines) â€” components Vid/Position/Hp/Aggro/Mob/Item/Player, resources Tick/Rand/NpcOutbox/SpawnCache, systems `chase_attack`/`aggro_detect`/`patrol` (parity order), `WorldSim` wrapper; channel.rs refactored (AI tick â†’ `world.update`). Workspace **359 passed / 0 failed**, clippy clean, release green, deployed + running (30001/30003). Accepted deviations: `multi_threaded` off until the F5 benchmark (one-line toggle); SpawnCache stays `Arc<Mutex<>>` as a resource (PG-row cache, not world state); World per-connection (shared-World = **spawn dinÃ¡mico IN PROGRESS**); armor at entry/equip/unequip (no per-tick PG); mpsc intents deferred with spawn dinÃ¡mico (ecs.rs:34).
- **(2) WAL idempotency DONE** â€” `safebox.rs`/`messenger.rs` INSERTs now `ON CONFLICT DO NOTHING` (legacy quirks preserved); `replay_wal` PG test UN-GATED, 2/2 green (2.34 s); database 79 test attributes.
- **(3) dwLoginKey real flow DONE** â€” real `LoginKeyStore` in auth.rs (per-process Mutex\<HashMap\>, keys die with the process â€” C++ parity `ClientManagerLogin.cpp:81-178`); LOGIN_BY_KEY via the existing LOGIN3 `passwd[17]` (no wire change, 68/88 B intact); wrong key â†’ rejection; password path byte-for-byte unchanged; 18 auth unit tests. F2a debt closed at the auth (channel LOGIN2 consumption stays for the channel lane).
- **(4) Client UTF-8 overrides DONE** â€” `net.SetLocaleName`/`net.SetItemLocaleName` hooks (PythonNetworkStreamModule.cpp:1668-1708); exe 5,130,752 B deployed; lookup override â†’ bundle cache â†’ pack; server does not send overrides yet (data channel future) â€” API inert.
- **(5) Benchmark harness DONE** â€” new `bench_bot` crate (27 attrs; smoke: 1 bot loginâ†’world 1321 ms, 11 mobs, no speedhack kick; 2/2 concurrent; `--cleanup-accounts` 0 rows) + `bench_capture.rs`/`--bench-capture <dir>` (4 channel call sites to wire). `start_win.ps1` is LAUNCH-ONLY (rule 16).

**Wave 2 (2026-08-13, 40th part):**

- **(1) Spawn dinÃ¡mico DONE + deployed** â€” channel-level shared bevy World; `spawn_despawn_system` in `game_core/src/ecs.rs` (SPAWN_VIEW 2500 materialize, DESPAWN_RADIUS 4000 â€” ecs.rs:449 â€” combat mobs never despawn); entry via `Intent::Join` mpsc (ecs.rs:337-340); no static SPAWN_VIEW filter in channel.rs; bench_capture wired (all 4 call sites).
- **(2) Walkability + speed envelope DONE** â€” NEW `game_core/src/map.rs` (553 lines: server_attr LZO1X parsing â€” real map 41 decoded: 2,176,848 blocked + 1,076,378 water cells; C++ cell arithmetic SECTREE_SIZE 6400 / CELL_SIZE 50; `MapStore` lazy + cached failures) + `game_core/src/movement.rs` (`PlayerMotion.speed` 300 u/s default; `MoveError::ExceedsEnvelope` movement.rs:53-67; envelope = speedÃ—(dt+100ms)/1000Ã—1.20, inert without anchor); CG_MOVE validates walkability BEFORE `process_move` (reject â†’ stands, no ban). Workspace **418 passed / 0 failed**; clippy clean; release built; deployed 06:11 (binary 3,818,496 B) â€” live stack runs it (auth7/chan7 logs 06:33). Follow-ups: N-violations auto-ban (config), path-line sampling, buffs/mounts recompute speed, warp re-anchor.
- **(3) Quest DSL core DONE** (orchestrator direct â€” after 3 failed agent attempts that returned empty reports) â€” NEW crate `quest_dsl/` (ast/parser/family/render; typed catalog per spec Â§3-Â§5; families expand; Â§11 decisions resolved â€” between native, if 1-level+else, select as-capture, @key, .quest, timer alias); **13 tests green, clippy clean**; workspace member. qcâ†’DSL converter IN PROGRESS (lane).
- **(4) F3 tail ACID DONE** (fix-2) â€” `Batcher::flush()` (wal.rs:398), `ItemExchange::exchange_mutated()` (materialsâ†’resultâ†’gold one tx â€” proven vs real PG: 4 audit rows same applied_at; item.rs:233), SocialRepo (`GuildRepo` load/ranking â€” social.rs:58; `player.guild_member/grade/comment` NOT migrated to PG, documented gap), EconomyRepo (money_log append-only + `checked_gold_mutation` â€” economy.rs:97; `CHECK gold>=0` NOT in PG, Rust guard is enforcement; G-PG ALTER follow-up); database 95 test attributes.

**Wave 3 (2026-08-13, 41st part):**

- **(1) Skills + server-timed buffs DONE** â€” NEW `game_core/src/skill.rs` (732 lines: SkillRepo from `player.skill_proto`, poly evaluator, `skill_damage`; 15 tests); ecs.rs `Mp`/`SkillLevels`/`SkillCooldowns`/`Affects` (:190-209) + `SkillTable` (:377) + `process_skill` + `affects_system` (:870, 19 tests); channel `CG_USE_SKILL` (52, 9 B â€” Packet.h:854; channel.rs:800-810) + `GC_AFFECT_ADD` (126)/`GC_AFFECT_REMOVE` (127) (channel.rs:1958-2002). Workspace **481 passed / 0 failed** (game_core 106 per run); clippy no new; deployed 07:23 (binary 3,935,232 B; auth8/chan8 07:26). GAPs: SPLASH/PARTY/HORSE families, `skill_power.txt`, MOV_SPEED/ATT_SPEED/CRITICAL buffs stored+shown but numeric application pending, quest-granted/passive skills; test chars have all-zero skill_level blobs â€” DB grant note: `UPDATE player.player SET skill_level = overlay(skill_level placing '\x01' from 8 for 1) WHERE id = 2`.
- **(2) qcâ†’DSL converter DONE** â€” `quest_dsl/src/convert/` (qc.rs real-grammar parser â€” Lua 5.0 dialect with begin/end, multi-line when heads, inline ifs, while/repeat AST; map.rs â€” 22 actions/10 triggers/10 conditions; `convert_corpus` CLI); **44 tests / 0 failed**; **corpus 194/194 files, 0 failed (~2 s)**; 5,513 unmapped items (journal `q.*` ~944, UI setskin/makequestbutton ~625, target.pos, say_pc_name, pc.give_exp2/change_money â€” Rust-module territory per spec Â§8); **6 family proposals = 112/194 files** (collect_quest 11, main_quest 32, subquest 44, collect_herb 6, new_quest 12, main_quest_flame 7). Family parameter extraction + Value-as-Expr IN PROGRESS (~160 pc.setqf unblocked).
- **(3) Benchmark FIRST LOAD SIGNAL** â€” `source\deploy\win\logs\bench-run-2026-08-13.md` (raw JSONs beside it; run vs the 06:11 binary on auth7/chan7): smoke 1 bot world_ms 1524; 5Ã—30 s 5/5 OK (median 2254); **20Ã—60 s 20/20 OK, 0 failures, 0 panics** (median 7829, p95 8214, ~13 % spread); cleanup 0 rows. **FINDING: spawn-dinÃ¡mico materialization reaches only FIRST-COMERS under concurrency (19/20 bots got 0 entries) â€” fix IN PROGRESS (channel lane).** Envelope rejects the harness walk (300 u/s) â€” harness fix in progress. Latency 1.5 s â†’ 7.8 s world_ms (1 â†’ 20 bots) â€” per-connection entry bottleneck candidate. Full ladder pending (WorldSim::metrics, sharded-region, 100â†’1000 bots).

**Wave 6 (2026-08-13, 42nd part):** the crate **`realm` â†’ `game_core`** (user decision â€” `source/reforge/game_core/`; 78 code refs; workspace **512 passed / 0 failed**; `server_realms`/`quest_dsl`/`bench_bot`/`mysql_proxy` unchanged). channel.rs split â†’ `server_realms/src/channel/` (13 files; `Session` + `Outcome`), ecs.rs split â†’ `game_core/src/ecs/` (components/events/resources/systems; `Intent`/`NpcEvent` wrapper sub-enums); **N1 trap guard**: `game_core/src/ecs/systems/{social,quest}.rs` + `channel/{social,quest}.rs` (empty-sub-enum delegates). PG migrations: `scripts/gpg/alter_gold_check.sql` (CHECK gold>=0, 3 wallet tables; money_log excluded) + `migrate_guild_tables.sql` (guild_member/grade/comment). Quest similarity engine (spec Â§9.3; quest_dsl 66 tests). **Deploy note: servers still run the pre-refactor binary (3,935,232 B) â€” redeploy pending.**

**Wave 7 (2026-08-13, 43rd part):**

- **(1) NPC shops + player trade DONE** (social lane, cod-3) â€” `game_core/src/shop.rs` (406 lines: buy = `item_proto.goldÃ—count` parity shop.cpp:166-180; sell = `shop_buy_priceÃ—count/5 âˆ’3%` parity shop_manager.cpp:297-319; rejects SoldOut/NotEnoughMoney/InventoryFull/NotSellable/Equipped/GoldOverflow) + `game_core/src/trade.rs` (370 lines: `TradeSession` 12-cap, gold once, 2-phase accept, **commit via `ItemExchange::exchange_mutated` + `Batcher::flush()` â€” ONE tx, ids 100M-200M**); `ecs/systems/social.rs` ShopTable + `channel/{shop,trade}.rs` (wire byte-exact: GC_SHOP 1888 B, GC_EXCHANGE 47 B, CG_SHOP 50 B, CG_EXCHANGE 27 B); `WorldStore::exchange`. Workspace **548 passed**.
- **(2) Quest runtime engine DONE** (cod-1) â€” `game_core/src/quest/engine.rs` (859 lines): `{quest}.__status` 1-based (parity questpc.cpp:115-118), `wait()`/`select()` suspension (CG_SCRIPT_ANSWER re-enter), conditions + actions subset, `player.quest` persistence (value 0 = DELETE); GC_SCRIPT 45 / CG_SCRIPT_ANSWER 29. Pending actions: say_reward, send_letter, set_quest_state, target_vid, affect_*, input_number. Workspace **564 passed**.
- **(3) GM commands DONE (subset, fix-4)** â€” `gm.rs` (221 lines) + `channel/gm.rs`: `/` prefix parity input_main.cpp:661-665, per-command DB permission check (common.gmlist); warp/item/notice/level; mob spawn deferred (needs a new intent); unknown/rejected â†’ EN message (locale pending, documented divergence).
- **(4) dw_arrow DONE** (orchestrator direct) â€” `USE_ARROW_DAMAGE` gate (parity GetArrowAndBow char_battle.cpp:2919-2941), `consume_arrow` (parity UseArrow char_battle.cpp:2770-2789), `pending_arrow_shot` (session.rs:237), **`dw_arrow` in GC_CHARACTER_ADDITIONAL_INFO = real equipped arrow count** (packets.rs:311-333; was 0). Verified: cargo check 4.52 s, game_core 154 + server_realms 42 passed.
- **(5) Background subagents ENABLED** â€” `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` in `~/.config/opencode/opencode.jsonc`; tasks run in background, the conversation never blocks (restart opencode to take effect).
- **Deploy note: servers still run the pre-wave-7 binary (3,935,232 B, 07:23) â€” redeploy pending.**

**Backup cadence (oracle H.2) live:** nightly `scripts\backup_win.ps1` (native `pg_dump -Fc` of `metin2` â†’ `C:\projects\metin2-extra\backups\metin2_<yyyy-MM-dd>.dump`, retention 7). The 53-commit backlog is PUSHED: `origin/main = 294edb1` (0 ahead / 0 behind, verified 2026-08-12).

## Historical â€” login fix chain (2026-08-08, verified facts; still authoritative for protocol/data rules)
...
**Server â€” game binary (auth + cores):**
...
3. `main.cpp`: reverted the io_loop hack (fallthrough READâ†’WRITE + sys_err debug). `optresetâ†’optind` was kept (needed for glibc/Linux).
4. Encryption: `_IMPROVED_PACKET_ENCRYPTION_` OFF on both sides (client and server = plaintext). Sequence OFF on both.
5. `utils.cpp` `mysql5_password`: **THE REAL FORMAT INCLUDES THE ASTERISK** (`"*" + encoded2` uppercase â€” lines 51-57). The hash stored in `account.password` MUST have the `*` (native MySQL `PASSWORD()` format). Do not "fix" it by removing the asterisk â€” that breaks the auth strcmp.

**Server â€” db binary:**
6. `PeerBase.cpp` `Send()`: consume `result > 0` (same problem as the game).
7. `ClientManagerLogin.cpp` `QUERY_LOGIN`: the query MUST return 13 columns in the order `CreateAccountTableFromRes` expects â€” hash, id, login, password, social_id, bEmpire, pid1..pid5, status, lang: `SELECT mysql_hash_password('%s'), a.id, a.login, a.password, a.social_id, pi.empire, pi.pid1, pi.pid2, pi.pid3, pi.pid4, pi.pid5, a.status, a.lang FROM account a LEFT JOIN player.player_index pi ON pi.id = a.id WHERE a.login='%s' AND a.password=mysql_hash_password('%s')` (verified 2026-08-10, `ClientManagerLogin.cpp:395-426` + `CreateAccountTableFromRes:259-297` â€” the `lang` column comes from the Language System ALTER). The previous model used 6 columns in another order â†’ WRONGPWD always.
...
10. Main-loop flood: the WRITE interest of fdwatch is persistent (the `oneshot` flag is ignored in `fdwatch_add_fd`); after draining the peer buffer, re-register with READ only (`fdwatch_del_fd` + `add_fd(FDW_READ, false)` in the FDW_WRITE case) â€” otherwise the db spins on `AUTH_PEER_WRITE: size 0` and never processes the async results.
11. SQL function created in MariaDB: `account.mysql_hash_password(pw)` = `CONCAT('*', UPPER(SHA1(UNHEX(SHA1(pw)))))` (matches the C++).

**Client:**
12. `AccountConnector.cpp` `__AuthState_RecvPhase`: removed `rkNetStream.ClearLoginInfo()` â€” it deleted `m_stPassword` from CPythonNetworkStream DURING auth â†’ the channel connection sent an empty password â†’ "incorrect password" at the channel. (The client's `ClearLoginInfo` only clears the password, not the ID â€” that is why the channel received the correct login with an empty password.) Client RECOMPILED with this fix (Release|Win32, metin2client.exe ~5.1MB).
13. Pack `root.epk` (intrologin.py): re-SetLoginInfo with the edit-line values before connecting to the channel (lines ~1082 and ~1320). Repacked with `PackMakerLite.exe` (in `source\tools\pack\` â€” uses `PackMakerLite.json` with the pack keys).
...
15. **CHAT SPAM / monster_chat (new, 2026-08-08, DATA fix â€” no rebuild):** the chat spammed `SYSERR: LUA ScriptRunError (code:1 src:[(locale.monster_chat[vnum] ...)])` for every monster in combat. Double cause, both in the `share/locale/spain` runtime:
    ...
    - `quest/locale.lua` had a SYNTAX error for the server's lua 5.0: the lexer is MODIFIED for Korean EUC-KR (2 bytes per char â€” `read_string` in `liblua/5.0/src/llex.c` consumes 2 bytes when `b_current & 0x80`). The Spanish file was UTF-8 with 349 lines of Korean of **3 bytes** â†’ parity misalignment â†’ the closing quote gets "eaten" â†’ `unfinished string` (LUA_ERRSYNTAX=3) â†’ `LoadQuestLocale returns 3` â†’ `locale.monster_chat` NEVER defined. Fix: convert ONLY the Korean lines to CP949 (script `fix_locale_enc.py`: 349 lines, 0 failures); the Spanish (2-byte UTF-8 accents) stays intact. Verified: `/home/m2/luaparse` (the server's real lua 5.0 harness) â†’ `SYNTAX_OK`; after core restart: `LoadQuestLocale(...) returns 0` and 0 new ScriptRunErrors.
    - **RULE: server locale lua files containing Korean MUST use CP949/EUC-KR (2 bytes/char), NOT UTF-8 â€” the server lexer breaks on UTF-8.**
16. **MOB NAMES IN KOREAN (new, 2026-08-08, DATA fix â€” no rebuild):** mobs showed Korean/mojibake names in game. Double cause:
    ...
    - The MySQL data was Korean CP949 double-encoded: the original `mob_proto.txt` has Korean CP949 names (e.g. `b5 e9 b0 b3` = ë“¤ê°œ) and importing them produced `C2B5C3A9...` (each CP949 byte â†’ latin-1 â†’ UTF-8) in the utf8mb4 `name`/`locale_name` columns.
    - **FINAL FIX (multilanguage):** names were taken from the CLIENT PACK (`locale.epk` â†’ `locale/es/mob_proto`, decoded with `DumpProto.exe` â€” `Srcs\Tools\DumpProto`, mob keys {4813894,18955,552631,6822045}, item keys {173217,72619434,408587239,27973291}; MMPT0/MIPX format + TEA-ECB 32 rounds + LZO1X). The S3llMetin2 v24 Spanish pack already ships the 2864 Spanish names (Perro Salvaje, JabalÃ­, Zorro del Desierto...). `UPDATE mob_proto SET name=..., locale_name=...` from the dump's `mob_names.txt` (script `gen_pack_sql.py`), truncated to 24 bytes (`varbinary(24)` â€” a longer name fails with `ERROR 1406 Data too long`). 14 mobs untranslated in the pack (Korean) were left intact. Verified: `MOB: #101 Perro Salvaje`, `#2101 Zorro del Desierto`, `#20001 Alquimista` in the core boot.
17. **MULTILANGUAGE ARCHITECTURE (verified 2026-08-08) â€” the client translates, the server does NOT:**
    ...
    - The dbmanager in `source\tools\DBManager` (PHP/bash suite) exists but is only txtâ†”mysql import/export â€” it does not translate.
...
**WSL (Debian-M2) â€” ON-DEMAND ORACLE BOX ONLY (ADR-0012, 2026-08-12):**
...
- `/home/m2/source` â€” was the build + deploy copy (THE source of truth for compiling the server until 2026-08-12); archived to `metin2-extra\archive`, full delete at F6 (`wsl --unregister Debian-M2`).
- `/home/m2/source/metin2_svfiles/main/srv1` â€” the frozen C++ instances (binaries via symlinks to `share/bin/{game,db}`), used only in parity sessions.
- `/home/m2/tmp4-server` â€” upstream old-metin2.com git repo (reference, NOT the active source).
...
- `serverinfo.py`: host `127.0.0.1` (repacked 2026-08-12, ADR-0012 â€” client and servers share the Windows host), auth 30001, ch1 30003, ch2 30007, ch3 30011, ch4 30015.
...
- Parity sessions (frozen C++ in the WSL oracle box): the old `serverinfo.py` host `172.25.104.175` (WSL eth0 IP â€” **CHECK after every WSL restart**).
- Test account: `test` / `1234` (hash `*A4B6157319038724E3560894F7F932C8886EBFCF` in `account.account`).

## Runbook

### Primary path (native Windows, ADR-0012) â€” the daily stack
...
### Parity path (on-demand WSL oracle box, ADR-0012) â€” frozen C++ only

```powershell
# 1. Start the frozen C++ oracle (db + auth + ch1-core1) â€” parity sessions only
...
- ~~The deployed db binary included temporary debug logs (`DBG_AQR`, `DBG_RESULT_LOGIN`, `DBG_PARSE` in ClientManager.cpp / ClientManagerLogin.cpp)~~ â€” **CLEANED (session 2):** 0 DBG lines in the new db boot; item award logs clean.
- **Language System (1.2.6 mod multilingual engine):** integrated and loading â€” 16 languages, 764-775 entries each, evidence in `core1/stdout` (NOT syslog: `sys_log` is silent in `config_init` because the logfile is not open yet and `DB_LOG_LEVEL: 1` â†’ `log_level_bits=1` blocks `sys_log` stdout; that is why `LocaleService_LoadLocaleStringFile` uses `fprintf(stdout, "Load LocaleString %s (%d entries)")`). Full detail in `documentation/reference/legacy/language-system.md` Â§5 (historical). **Partial end-to-end test (session 2):** the user played in the world and server texts come out in SPANISH âœ“ (including `monster_chat`); `login_success: lang 'es' -> 5` confirms account.lang â†’ g_iCurrentLang propagation. The client (build 18:36, 5,115,392 bytes) sends its language in the auth LOGIN3 (68 bytes: 65 + `szLanguage[3]`) â†’ auth does `UPDATE account SET lang=...` â†’ **the client OVERWRITES account.lang on every login** (to test another server language, change the client locale or the send).
- ~~**INTERMITTENT WORLD-ENTRY CRASH â€” DIAGNOSIS IN PROGRESS (2026-08-09, session 3):** the `string_replace_word` over-read (PythonSkill.cpp:62) was a REAL corruptor and is fixed (bounds check, build 14:12, hash C7EAD7CC)...~~ â€” **CLOSED (2026-08-09):** see the "World-entry crash" section below and CHANGELOG. Final state: string_replace_word bounds check (build C7EAD7CC) + coordinate fix (`UPDATE player SET x=969600, y=278400` for both old characters) â†’ **field test 2/2 consecutive entries** (2026-08-09, session 3 4th part). The cdb/WER findings (granny2.dll, igc32.dll detectors) were symptoms of the same corrupted heap, not independent causes. Tools remain installed for future diagnostics (Debugging Tools + LocalDumps C:\dumps + PageHeap via gflags).
- **COORDINATE CONVENTION (CRITICAL for creating/moving characters):** `player.x/y` = **UNITS** (village c1 of map 41 = `969600, 278400`). The `AddGotoInfo` boot values (e.g. c1 `(9696, 2784)`) are **cells (Ã·100)** â€” do NOT use them directly in the DB. `GetValidLocation` validates the position against the map sectree; on failure â†’ fallback to `EMPIRE_START_*(empire)` (0,0 for empire 0). A character saved with garbage coordinates crashes the CLIENT with `0xc0000374` (heap corruption) while loading the map.
- LS pending items (full audit 2026-08-09, see CHANGELOG): language selector at login (flag column â€” user request; the native `IDD_SELECT_LOCALE` dialog is compiled but dead: `LOCALE_SERVICE_GLOBAL` not defined). ~~Multilanguage NPCs~~ â€” **RESOLVED (2026-08-09):** the client resolves NPCs from its pack (server fallback); client rebuild 5,115,904 bytes (see Â§17 and CHANGELOG). **Real server gaps:** (A) broadcasts/notices use the last packet's language (`LC_TEXT_LANG` defined but never used â€” 26 `SendNotice` affected); (B) quest/monster_chat do NOT translate (lua fixed Spanish at boot â€” `locale_quest_find`/`LC_QUEST_TEXT` from the mod not integrated; **the real cause of the ES/EN mix the user saw**); (C) ~437 `ChatPacket` without `LC_TEXT` (mostly protocol commands, some visible: arena, battle, char.cpp:3045); (D) server NPC names fixed from `mob_proto.locale_name` without a GetLang branch; (E) ES lacks 11 keys EN has (10 used: exchange won â†’ `@0949`+English for ES players); (F) Windows svfiles copy out of sync (16 locale_string only in WSL). **mod locale_string compatibility (verified):** format 100% compatible with the parser; 11 base languages + AE/EN/GR cover ~75% of the code's 769 keys; **181 keys (23.5%) missing in ALL files â†’ `@0949`+key for everyone** (52 English: exchange won/dados/fishing; 129 Korean: chat bans, monarch); **PT (44%) and RU (19%) are from another mod base â€” useless**. Correction: EN covers 100% of the ES keys (the "732 missing" figure from that session was a parse error â€” pair format, not lines).
- The `test` account has 2 characters (slot 0 = `lkjsnlfknlsk`, slot 4 = `ninja`, both on map 41). **WATCH OUT:** the earlier "World entry VERIFIED" note was SERVER-side evidence (character-creation packet) â€” the CLIENT crashed during map load until the coordinate fix (`UPDATE player SET x=969600, y=278400`, 2026-08-09) and the `string_replace_word` bounds check; after both, **2/2 consecutive entries** (see "World-entry crash"). The chat spam (monster_chat) was fixed by the data fix of item 15 (`LoadQuestLocale returns 0` after core restart). Pending to test: full combat (partial: the user fought and killed mobs âœ“), NPCs, items, drops.
- The `intrologin.py` pack has the password-restoration fixes (lines 1082/1320) that became redundant after the client rebuild â€” can be kept or reverted.
- ~~Evaluate environment stability (more RAM, WSL update/downgrade, or Docker Desktop) before depending on long sessions~~ â€” **RESOLVED by ADR-0012 (2026-08-12):** the runtime is native Windows; WSL is on-demand only (cap 1 GB, off when unused). The 4 GB host constraint remains â€” nightly `scripts\backup_win.ps1` covers the data-loss risk (oracle H.2).

## World-entry crash (0xc0000374 heap corruption) â€” RESOLVED (2026-08-09)
...
**Deterministic part â€” RESOLVED (session 2):** the 2 characters were in the DB with garbage coordinates `(960155, 269313)` / `(960970, 271421)` on map 41 (real village = `(969600, 278400)` in UNITS). Fix: `UPDATE player SET x=969600, y=278400`.

**Intermittent part â€” ROOT CAUSE FOUND AND FIXED (2026-08-09, session 3):**

1. **Decisive evidence: the client's own minidumps** (`client\logs\metin2client_*.dmp`, written by `EterExceptionFilter` in `EterBase\error.cpp` â€” they were always there, nobody had read them). Two dumps of today's crash (13:15:00, 13:15:25) identical: exception `0xC0000005` in `string_replace_word` (`PythonSkill.cpp:62`), instruction `mov eax,[ecx]` at RVA 0x95110 (`disasm` with dumpbin + PDB), with ECX=0x96510FFD â€” garbage pointer.
...
4. Lessons: (a) the server syserr will NEVER see client crashes (local memory; the server only sees the socket close) â€” client close errors are in `C:\projects\metin2-extra\client\logs\*.dmp` (binary, parseable with the session's `parse_dump3.py` script or dumpbin/cdb); (b) App Verifier Heaps changes the detection timing (guard pages detect the over-read at the write) â€” useful to isolate, not to reproduce the original symptom.
...
6. **The C++ server is frozen â€” never rebuilt (ADR-0012).** The old "sync the two source copies" rule is dead for the server: `C:\projects\Metin2\source` (Windows) is the only source copy; Rust + client + docs live only on Windows. (WSL parity sessions still require `sync` + md5sum after touching the frozen oracle copy.)
7. Confirm before destructive operations (deleting volumes, databases, build caches).
8. **Docs after every change:** every task ends with its documentation updated â€” canonical docs, doc-comments and metadata `Last verified` (policy: `documentation/DOCUMENTATION.md`; hub: `documentation/README.md`). Never leave docs describing the old behavior; if a doc is outside the lane's write scope, the agent lists the exact required updates in its report. Update documentation/ADRs when project knowledge changes (see "Documentation methodology" below).
...
> **Ops note (2026-08-13, 43rd part):** `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` is set in `~/.config/opencode/opencode.jsonc` â€” agent tasks now run in the BACKGROUND and the conversation never blocks while one works (root fix for the recurring "agent stuck / chat frozen" problem). Takes effect after an opencode restart.

## Documentation methodology (how the count is kept)

The project follows the standard pattern for AI-agent projects (AGENTS.md + CHANGELOG.md + ROADMAP.md + documentation/), the methodology the user asked to adopt (reference: "purely" from gestorify â€” identical pattern to Keep a Changelog + ROADMAP + AGENTS.md).
