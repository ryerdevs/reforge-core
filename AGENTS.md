...
10. **Work in parallel (speed):** when there are independent tasks, deploy specialized agents in the background simultaneously (@explorer for discovery, @librarian for docs/research, @build for implementation, @fixer (build's adversary) for review, @oracle for supervision). Do not serialize work that can run in parallel; reconcile results when returning.
