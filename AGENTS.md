...
**Wave 7 (2026-08-13, 43rd part):**

- **(1) NPC shops + player trade DONE** (social lane, cod-3) — `game_core/src/shop.rs` (406 lines: buy = `item_proto.gold×count` parity shop.cpp:166-180; sell = `shop_buy_price×count/5 −3%` parity shop_manager.cpp:297-319; rejects SoldOut/NotEnoughMoney/InventoryFull/NotSellable/Equipped/GoldOverflow) + `game_core/src/trade.rs` (370 lines: `TradeSession` 12-cap, gold once, 2-phase accept, **commit via `ItemExchange::exchange_mutated` + `Batcher::flush()` — ONE tx, ids 100M-200M**); `ecs/systems/social.rs` ShopTable + `channel/{shop,trade}.rs` (wire byte-exact: GC_SHOP 1888 B, GC_EXCHANGE 47 B, CG_SHOP 50 B, CG_EXCHANGE 27 B); `WorldStore::exchange`. Workspace **548 passed**.
- **(2) Quest runtime engine DONE** (cod-1) — `game_core/src/quest/engine.rs` (859 lines): `{quest}.__status` 1-based (parity questpc.cpp:115-118), `wait()`/`select()` suspension (CG_SCRIPT_ANSWER re-enter), conditions + actions subset, `player.quest` persistence (value 0 = DELETE); GC_SCRIPT 45 / CG_SCRIPT_ANSWER 29. Pending actions: say_reward, send_letter, set_quest_state, target_vid, affect_*, input_number. Workspace **564 passed**.
- **(3) GM commands DONE (subset, fix-4)** — `gm.rs` (221 lines) + `channel/gm.rs`: `/` prefix parity input_main.cpp:661-665, per-command DB permission check (common.gmlist); warp/item/notice/level; mob spawn deferred (needs a new intent); unknown/rejected → EN message (locale pending, documented divergence).
- **(4) dw_arrow DONE** (orchestrator direct) — `USE_ARROW_DAMAGE` gate (parity GetArrowAndBow char_battle.cpp:2919-2941), `consume_arrow` (parity UseArrow char_battle.cpp:2770-2789), `pending_arrow_shot` (session.rs:237), **`dw_arrow` in GC_CHARACTER_ADDITIONAL_INFO = real equipped arrow count** (packets.rs:311-333; was 0). Verified: cargo check 4.52 s, game_core 154 + server_realms 42 passed.
- **(5) Background subagents ENABLED** — `OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` in `~/.config/opencode/opencode.jsonc`; tasks run in background, the conversation never blocks (restart opencode to take effect).
- **Deploy note: servers still run the pre-wave-7 binary (3,935,232 B, 07:23) — redeploy pending.**
...
17. **Conventional Commits in English (permanent, 2026-08-15):** every commit message follows [Conventional Commits](https://www.conventionalcommits.org/) — `type(scope): description` in English, imperative present tense, no period at the end. Types: `feat` (new behavior), `fix` (bug fix), `docs` (documentation only), `refactor` (no behavior change), `perf` (performance), `test` (tests only), `chore` (build/tooling/backlog, e.g. committing work that was never committed), `revert`. One logical change = one commit (**atomicity** — no "fix A + fix B" mixes: enables `git bisect` and clean reverts). The scope is optional but recommended (`fix(movement)`, `feat(channel)`). Historical commits using the old styles (`[fix]`, Spanish, multi-purpose) are preserved verbatim — never rewritten (published history). Do not rewrite pushed history.
