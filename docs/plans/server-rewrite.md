...
| [`../decisions/`](../decisions/) (ADRs 0001–0011) | Architecture decision records — the statuses below refer to them |
...
**Preparation for F7 (no C++ refactor — reusable artifacts):** spec of the client walls (limits/structs/headers), reproducible client build (script/CI), client as thin viewer (server data), UI design in Slint standalone (the `.slint` files survive; the standalone app is **deferred from F5 to F7 — 2026-08-12 review H.3**). Refactoring "well-made" C++ modules = wasted work, not done. **No Rust embedded in the legacy client** (ADR-0007, Accepted).
...
| **F3** Data layer + data channel | `database` crate by domain on PG; port by QID; pull-based packets 162+ (CG_QUERY/GC_RESPONSE) | **Redefined 2026-08-12 (review H.4):** ported QIDs behave identically on PG through the Rust `database` crate (parity harness — the C++ game operates on PG via `mysql_proxy`, never the Rust crate); the recompiled client receives additive data without desynchronizing | **IN PROGRESS (2026-08-12)** — account/world repos + WAL phase 2 (Batcher + WalSink + replay); milestone pending: items/social QIDs, active data channel, PROTO_FROM_DB |
| **F4** World entry + names | CG_PLAYER_SELECT, spawn, map, stats; UTF-8 name overrides | The real client enters the world against the Rust core with correct names | **Milestone MET 2026-08-11** (real client world entry, 50+ s sustained); domain-boundary ADR = ADR-0010 (Accepted 2026-08-12, ratifies the implemented architecture; ECS implementation = next slice) |
| **F5** Gameplay | Movement, combat, drops, items, NPCs, quests, chat, shops, trade, GM — by domains, side-by-side; channel list from auth; config via manifest; **ECS migration slice first** (ADR-0010) + provisional N-bot benchmark after it; scale benchmark (N bots × N regions); benchmark instrumentation; **Slint standalone → F7 and REST/metrics → post-cutover (deferred 2026-08-12, review H.3; ADR-0007 amended)** (**Docker removed 2026-08-11 — user decision**) | Full session without divergences (defined real-client session script) + benchmark passed | **IN PROGRESS (2026-08-12)** — F5.3 slices 1–17 (combat/items/NPC AI); pending: **ECS migration + provisional benchmark**, walkability, skills, quests (DSL), shops; Slint → F7 |
...
- Rust stack: tokio 1.x + **tokio-postgres 0.7 (ADR-0008, Accepted 2026-08-11 — replaces the sqlx 0.9 candidate; pool later via deadpool-postgres if measured)** + config-rs + clap + tracing + proptest. No scripting (own DSL). WAL: **DONE 2026-08-12** (F3 phase 2, ADR-0008); RLS post-WAL, Patroni F5/F6; bevy_ecs **ADOPTED 2026-08-12** (ADR-0010 §2 — F5 benchmark validates); REST deferred until justified [Proposed] (**Docker removed 2026-08-11 — user decision**).
...
3. **Concurrency**: is world-per-region with systems (ECS deferred) the right choice for scaling? Or actors from the start? (Position: regions + deferred ECS; YAGNI on multi-process until benchmark.) — **RESOLVED 2026-08-12 (ADR-0010, Accepted):** pure-function modules + per-connection session state + `WorldStore` ratified; actors rejected; bevy_ecs **adopted** (user decision — mob density is the core requirement); the F5 benchmark (1,000+ players/instance, ≥2–5× CPU headroom, AI tick < 500 ms) validates the choice (ADR-0010 §2).
...
3. Write the pending ADRs: domain boundaries (char.cpp by systems), concurrency (regions + deferred ECS), quest engine (own DSL), anti-hack model, regional channels (central DB + process per region), data layer (transactional batches; deferral list), server→client data (manifest + delta), migration review of ADR-0002. — **PARTIAL (2026-08-12):** written — data layer (ADR-0008, Accepted 2026-08-11), domain boundaries (ADR-0010, Accepted 2026-08-12), anti-hack model (ADR-0011, Accepted 2026-08-12). Pending — quest engine (DSL), regional channels, server→client data (manifest + delta), migration review of ADR-0002 (concurrency ratified by ADR-0010 §1).
