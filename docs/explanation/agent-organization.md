...
- Global skills exist for everyone (opencode mechanics), but each agent's mission limits it to its lane's skills (fixer → adversarial + rust-* + diagnose + improve-codebase-architecture + **C++ debug: gdb/sanitizers/strace-ltrace** + **zeroize-audit**; coder → rust-* + clean-code; librarian → documentation-*; explorer → graphify/codemap/**clonedeps**; designer → impeccable + brainstorming; orchestrator → planning/verification + **verification-planning/deepwork** — docker-development removed 2026-08-12, Docker out of the plan). **MCPs: all agents get graphify + context7 + gh_grep** (harness-provisioned, verified `opencode mcp list` 2026-08-12: graphify local, context7 `mcp.context7.com`, gh_grep `mcp.grep.app`). GitHub ops stay on the `gh` CLI (authenticated as `ryerdevs`; repo `github.com/ryerdevs/reforge-core` is PUBLIC — a GitHub MCP server is not needed while the CLI covers the workflow).
