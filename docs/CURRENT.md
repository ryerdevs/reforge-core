...
| Item | Value | Evidence |
|---|---|---|
| Workspace tests | **168 passing / 0 failed / 7 ignored** (database 18 + 7 gated, mysql_proxy 66, network 24 + 2 f16 smoke, protocol 37 + 3 golden, server_realms 14 + 4 auth smoke) | `cargo test` in `source/reforge` |
...
| **F3+ — Data, world, parity** | Planned only (`database`, `realm` are scaffolds) | Depends on F2a; details in [plans/server-rewrite.md](plans/server-rewrite.md) |
