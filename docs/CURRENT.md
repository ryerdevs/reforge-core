...
- **Date:** 2026-08-12
- **Commit:** `c0954c5` (HEAD, 2026-08-12) + working tree ahead — the G-PG cutover session changes are committed (snapshot `03c03ad` superseded); F5.3/WAL/locale work since the last commit may be uncommitted.
...
5. **Data layer driver + durability** — **ADR-0008 Accepted 2026-08-11**: tokio-postgres 0.7 as the `database` driver (sqlx deferred with measurements; pool via deadpool-postgres if needed); PostgreSQL-only repos, no direct-sql backend; durable = **save-by-event** (event → Batcher ≤100 ms → local WAL → PG, replay idempotent — implemented 2026-08-12, F3 phase 2); RLS post-WAL; Patroni F5/F6.
6. **Server-side locale** — **ADR-0009 Proposed (2026-08-11)**: the server owns all text per player language (one `common.*` table per domain, `CG_LOCALE_REQUEST`/`GC_LOCALE`, EN fallback); importer live (`locale_import`, 2026-08-12), wire slice pending.
...
**F5.3 gameplay slices — 17 done (2026-08-12, direct orchestrator implementation; per-crate `cargo test` green + clippy clean):** s1 kill rewards + chat + client locale cache; s2 item drops + pickup; s3 NPC AI aggro + chase + `GC_MOVE`; s4 mob attack in range; s5 PC death + revive; s6 warp-to-city + de-aggro; s7 idle patrol; s8 stacking pickup; s9 player DEF in mob damage; s10 proactive aggro + `aggressive_sight`; s11 potions + latent framer fix; s12 item move/stack/split; s13 equip/unequip; s14 equipped items affect combat; s15 ComputeParts; s16 FindEquipCell; s17 weapon attack_speed. Pending: walkability (`IsMovablePosition`), `dw_arrow` (quiver), skills, NPCs interactivos/tiendas, quests, safebox, trade, GM, benchmark, Slint, REST.

## Next gates

1. **F3 milestone** (data layer + data channel): port the remaining QIDs to the `database` crate (items, social), activate the pull data channel (162/163) end-to-end, maintain `PROTO_FROM_DB`. WAL phase 2 (durable-first + idempotent replay) is **DONE (2026-08-12, 11th part)**; gated `replay_wal` PG test pending (user directive).
2. **F5 gameplay tail**: walkability (`IsMovablePosition`, anti-speedhack envelope), `dw_arrow` (quiver), skills, interactive NPCs/shops, quests (DSL engine), safebox, trade, GM; then scale benchmark (N bots × N regions), Slint standalone, REST/metrics.
