...
| Versioned documentation hub and policy: [`documentation/README.md`](../README.md) | Canonical documentation rules, authority, and boundary references. | Versioned Markdown, relative links, and concise evidence citations only. | [`documentation/README.md`](../README.md) |
...
| Deploy executables and generated build binaries, including `source/deploy/**/*.exe`, `source/tools/proto/**/item_proto`, `source/tools/proto/**/mob_proto`, and `**/target/**` | Generated executables and client proto output are local or release outputs, not versioned public source inputs. | [`.gitignore`](../../.gitignore):41-59,121-133; [`scripts/build_admin_tui.ps1`](../../scripts/build_admin_tui.ps1):45-68; [`scripts/check_boundary.ps1`](../../scripts/check_boundary.ps1) | Do not stage them; remove them from a package manifest or index, regenerate from versioned source or supplied tooling, and keep any local copy outside the versioned tree. |
...
The workflow uses read-only repository permissions and runs the boundary,
boundary-mutation, and dependency-review checks without repository or production
secrets. It installs the pinned `1.97.0` Rust toolchain with `rustfmt` and
`clippy` before invoking the gate. The active `Protect main and beta` repository
ruleset applies to `refs/heads/main` and `refs/heads/beta` (so it covers a future
`beta` branch), requires one fresh pull-request approval, and requires the
`verify` and `dependency-review` checks. Verify the hosting state with
`gh api repos/ryerdevs/reforge-core/rules/branches/main` and the corresponding
`beta` path; hosting settings are not represented in this checkout.
