...
int socket_read(socket_t desc, char* read_point, size_t space_left)
{
    ...
    sys_err("socket_read: recv failed: %s (fd %d)", strerror(errno), desc);
    return -1;
}

int socket_write_tcp(socket_t desc, const char *txt, int length)
{
	const int bytes_written = send(desc, txt, length, 0);
...
int socket_write(socket_t desc, const char *data, size_t length)
{
	...
	{
	    /* Non-blocking socket would block (EAGAIN). Return what was
	       written so far; ProcessOutput() will re-register the fd and
	       try again later. */
...
socket_t socket_accept(socket_t s, struct sockaddr_in *peer)
{
	...
	if (errno == EAGAIN || errno == EWOULDBLOCK)
	    return -1; /* No pending connection: normal for non-blocking sockets */
