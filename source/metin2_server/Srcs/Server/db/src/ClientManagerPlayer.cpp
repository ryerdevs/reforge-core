...
{
	const auto it = m_map_playerCache.find(id);
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
		...
		const auto pkInfo = new ClientHandleInfo(dwHandle, packet->player_id);
...
void CClientManager::ItemAward(CPeer * peer,char* login)
{
	...
	while(it != pSet->end() )
	{
		const TItemAward * pItemAward = *(it++);
...
char* CClientManager::GetCommand(char* str, char* command) const
// @fixme203
{
	char* tok;
...
void CClientManager::RESULT_COMPOSITE_PLAYER(CPeer * peer, SQLMsg * pMsg, DWORD dwQID)
{
	...
	const std::unique_ptr<ClientHandleInfo> info((ClientHandleInfo *) qi->pvData);
...
void CClientManager::RESULT_ITEM_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID)
{
	...
	const DWORD dwCount = s_items.size();
...
void CClientManager::RESULT_AFFECT_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwRealPID) const
{
	int iNumRows;
	...
	const DWORD dwCount = s_elements.size();
...
void CClientManager::RESULT_QUEST_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle, DWORD pid) const
{
	int iNumRows;
	...
	const DWORD dwCount = s_table.size();
 ...
 * PLAYER CREATE
 */
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet) const
{
	char	queryStr[QUERY_MAX_LEN];
	int		queryLen;
	int		player_id;

	const auto it = s_createTimeByAccountID.find(packet->account_id);
	...
	const auto pMsg0(CDBManager::instance().DirectQuery(queryStr));
	if (pMsg0->Get()->uiNumRows != 0)
	{
		...
		const MYSQL_ROW row = mysql_fetch_row(pMsg0->Get()->pSQLResult);
	...
	const auto pMsg1(CDBManager::instance().DirectQuery(queryStr));
	if (pMsg1->Get()->uiNumRows)
	{
		...
		const MYSQL_ROW row = mysql_fetch_row(pMsg1->Get()->pSQLResult);
	...
	const auto pMsg2(CDBManager::instance().DirectQuery(queryStr));
	if (g_test_server)
		sys_log(0, "Create_Player queryLen[%d] TEXT[%s]", queryLen, text);
	...
	const auto pMsg3(CDBManager::instance().DirectQuery(queryStr));
...
void CClientManager::__QUERY_PLAYER_DELETE(CPeer* peer, DWORD dwHandle, TPlayerDeletePacket* packet)
{
	...
	const auto pi = new ClientHandleInfo(dwHandle, packet->player_id);
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	const auto pi = (ClientHandleInfo *) qi->pvData;
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		const auto pIns(CDBManager::instance().DirectQuery(queryStr));
		...
		const CPlayerTableCache * pkPlayerCache = GetPlayerCache(pi->player_id);
		...
		const auto pMsg(CDBManager::instance().DirectQuery(queryStr));
...
void CClientManager::QUERY_ADD_AFFECT(CPeer * peer, TPacketGDAddAffect * p) const
{
	char queryStr[QUERY_MAX_LEN];
	snprintf(queryStr, sizeof(queryStr),
...
void CClientManager::QUERY_REMOVE_AFFECT(CPeer * peer, TPacketGDRemoveAffect * p) const
{
	char queryStr[QUERY_MAX_LEN];
...
{
	const auto it = m_map_logout.find(pid);
...
void CClientManager::UpdateLogoutPlayer()
{
	...
	{
		const TLogoutPlayer* pLogout = it->second;
...
{
	const auto it = m_map_playerCache.find(pid);
