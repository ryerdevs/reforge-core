...
bool CClientManager::InitializeTables()
{
	...
	if (g_bMirror2DB)
	{
		CDBManager::instance().AsyncQuery("SET sql_mode = '';"); // @fixme328
...
bool CClientManager::InitializeRefineTable()
{
	...
	const auto pkMsg(CDBManager::instance().DirectQuery(query));
...
bool CClientManager::InitializeShopTable()
{
	...
	const auto pkMsg2(CDBManager::instance().DirectQuery(s_szQuery));

	const SQLResult * pRes2 = pkMsg2->Get();
...
bool CClientManager::InitializeQuestItemTable()
{
	...
	static auto s_szQuery = "SELECT vnum, name, %s FROM quest_item_proto ORDER BY vnum";
...
bool CClientManager::InitializeBanwordTable()
{
	...
	const auto pkMsg(CDBManager::instance().DirectQuery("SELECT word FROM banword"));
...
bool CClientManager::InitializeObjectProto()
{
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			{
				const std::pair<int, int> & r = vec[i];
...
bool CClientManager::InitializeMonarch() const
{
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				"value0, value1, value2, value3, value4, value5, specular "
				") values ("
...
{	char query[2048];
	...
	const DWORD addNumber = pRes->uiNumRows;
	if (addNumber == 0)
		return false;
