...
{
	const auto it = m_map_MonarchElection.find(pid);

	if (it == m_map_MonarchElection.end())
	{
		auto p = new MonarchElectionInfo;
		p->pid = pid;
...
{
	const int size = GetVecMonarchCandidacy().size();

	const auto s = new int[size];
...
bool CMonarch::IsMonarch(int Empire, DWORD pid) const
{
...
bool CMonarch::AddMoney(int Empire, int64_t Money)
{
	...
	const int64_t Money64 = m_MonarchInfo.money[Empire];
