...
namespace marriage
{
	...
	{
		const DWORD now = CClientManager::instance().GetCurrentTime();
	...
	{
		const DWORD dwStartTime = CClientManager::instance().GetCurrentTime();
	...
	void CManager::Update()
	{
		...
		if (!m_pqWeddingEnd.empty())
		{
			while (!m_pqWeddingEnd.empty() && m_pqWeddingEnd.top().dwTime <= now)
			{
				...
				{
					const TMarriage* pMarriage = it_marriage->second;
					if (!pMarriage->is_married)
					{
