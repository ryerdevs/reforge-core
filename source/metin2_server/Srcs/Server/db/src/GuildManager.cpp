...
namespace
{
	struct FSendPeerWar
	{
		...
		void operator() (CPeer* peer) const
		{
...
void CGuildManager::ParseResult(SQLResult * pRes)
{
	...
	{
		const DWORD GID = strtoul(row[0], nullptr, 10);
...
void CGuildManager::QueryRanking() const
{
	char szQuery[256];
	...
	CDBManager::instance().ReturnQuery(szQuery, QID_GUILD_RANKING, 0, nullptr);
}
...
void CGuildManager::Update()
{
	...
	if (!m_pqOnWar.empty())
	{
		...
		{
			const TGuildWarPQElement * e = m_pqOnWar.top().second;
...
void CGuildManager::ProcessWinLose(DWORD dwGuildWinner, DWORD dwGuildLoser)
{
	...
	const int loss = (int)(iPoint * 0.07);
...
void CGuildManager::RecvWarOver(DWORD dwGuildWinner, DWORD dwGuildLoser, bool bDraw, long lWarPrice)
{
	...
	const TGuildWarInfo & gw = it->second;
...
void CGuildManager::MoneyChange(DWORD dwGuild, DWORD dwGold) const
{
...
void CGuildManager::BootReserveWar()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		const auto pmsg(CDBManager::instance().DirectQuery(c_apszQuery[i]));
...
int GetAverageGuildMemberLevel(DWORD dwGID)
{
	...
	const auto msg(CDBManager::instance().DirectQuery(szQuery));
...
bool CGuildManager::ReserveWar(TPacketGuildWar * p)
{
	...
	const TGuild & k1 = TouchGuild(GID1);
...
void CGuildManager::ProcessReserveWar()
{
	...
	while (it != m_map_kWarReserve.end())
	{
		...
		{
			const int iMin = (int) ceil((int)(r.dwTime - dwCurTime) / 60.0);
...
void CGuildWarReserve::Initialize()
{
	...
	const auto msgbet(CDBManager::instance().DirectQuery(szQuery));
...
void CGuildWarReserve::End(int iScoreFrom, int iScoreTo)
{
	...
	const DWORD dwTotalBet = m_data.dwBetFrom + m_data.dwBetTo;
	DWORD dwWinnerBet = 0;
	...
	while (1)
	{
		...
		while (it != mapBet.end())
		{
			...
			const DWORD dwGold = (DWORD) (dwTotalBet * ratio * 0.9);
