...
bool CPeerBase::Accept(socket_t fd_accept)
{
	...
	const std::string targetIP = inet_ntoa(peer.sin_addr);
...
int CPeerBase::Recv()
{
	...
	const int bytes_to_read = buffer_has_space(m_inBuffer);
	const ssize_t bytes_read = socket_read(m_fd, (char *) buffer_write_peek(m_inBuffer), bytes_to_read);
...
int CPeerBase::Send()
{
	...
	const int iBufferLeft = fdwatch_get_buffer_size(m_fdWatcher, m_fd);
	const int iBytesToWrite = MIN(iBufferLeft, buffer_size(m_outBuffer));

	if (iBytesToWrite == 0)
		return 0;

	const int result = socket_write(m_fd, (const char *) buffer_read_peek(m_outBuffer), iBytesToWrite);

	/* socket_write() returns: >0 = bytes actually written (== iBytesToWrite on
	   full success), 0 = EAGAIN (nothing written), -1 = error. Consume only the
	   bytes that really left the buffer, otherwise the output buffer never
	   drains and the peer never receives the replies. */
	if (result > 0)
	{
