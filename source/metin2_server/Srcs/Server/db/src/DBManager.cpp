...
SQLMsg * CDBManager::PopResult() const
{
	SQLMsg * p;
...
SQLMsg * CDBManager::PopResult(eSQL_SLOT slot) const
{
	SQLMsg * p;
...
std::unique_ptr<SQLMsg> CDBManager::DirectQuery(const char * c_pszQuery, int iSlot) const
{
...
void CDBManager::ReturnQuery(const char * c_pszQuery, int iType, IDENT dwIdent, void * udata, int iSlot) const
{
	assert(iSlot < SQL_MAX_NUM);
	//sys_log(0, "ReturnQuery %s", c_pszQuery);
	const auto p = new CQueryInfo;
...
void CDBManager::AsyncQuery(const char * c_pszQuery, int iSlot) const
{
	assert(iSlot < SQL_MAX_NUM);
...
unsigned long CDBManager::EscapeString(void *to, const void *from, unsigned long length, int iSlot) const
{
	assert(iSlot < SQL_MAX_NUM);
	return mysql_real_escape_string(m_directSQL[iSlot]->GetSQLHandle(), (char *) to, (const char *) from, length);
}

void CDBManager::SetLocale(const char * szLocale) const
{
...
void CDBManager::QueryLocaleSet() const
{
	for (int n = 0; n < SQL_MAX_NUM; ++n)
	{
