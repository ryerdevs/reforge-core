...
{
	const auto itr(m_Index2Name.find(index));
...
{
	const auto itr(m_Name2Index.find(Lower(name)));
...
bool cCsvFile::Save(const char* fileName, bool append, char seperator, char quote) const
{
    ...
    const char special_chars[5] = { seperator, quote, '\r', '\n', 0 };
    const char quote_escape_string[3] = { quote, quote, 0 };
