...
namespace quest
{
	...
	ALUA(guild_name)
	{
		...
		const CGuild * pkGuild = CGuildManager::instance().FindGuild((DWORD) lua_tonumber(L, 1));
	...
	ALUA(guild_is_bet)
	{
		...
		const bool bBet = CGuildManager::instance().IsBet((DWORD) lua_tonumber(L, 1),
	...
	ALUA(guild_get_reserve_war_table)
	{
		...
		while (it != con.end())
		{
			const TGuildWarReserve * p = &(*(it++))->data;
	...
	ALUA(guild_change_master)
	{
		...
		if ( pGuild != nullptr)
		{
			if ( pGuild->GetMasterPID() == ch->GetPlayerID() )
			{
				...
				{
					const bool ret = pGuild->ChangeMasterTo(pGuild->GetMemberPID(lua_tostring(L, 1)));
	...
	ALUA(guild_change_master_with_limit)
	{
		...
		if ( pGuild != nullptr)
		{
			if ( pGuild->GetMasterPID() == ch->GetPlayerID() )
			{
				...
				{
					const LPCHARACTER pNewMaster = CHARACTER_MANAGER::instance().FindPC( lua_tostring(L,1) );
