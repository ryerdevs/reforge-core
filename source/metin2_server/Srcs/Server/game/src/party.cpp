...
{
	const auto it = m_map_pkParty.find(ch->GetPlayerID());

	if (it == m_map_pkParty.end())
		return false;

	const LPPARTY pParty = it->second;
	pParty->Link(ch);
	return true;
}
...
{
	const auto it = m_map_pkParty.find(pid);
...
{
	const auto it = m_map_pkParty.find(leader);
...
{
	const auto it = m_map_pkParty.find(dwPID);
...
{
	const auto info = dynamic_cast<party_update_event_info*>( event->info );
	...
	{
		const LPPARTY pParty = leader->GetParty();
...
void CParty::Destroy()
{
	...
	const DWORD dwTime = get_dword_time();

	while (it != m_memberMap.end())
	{
		const TMember & rMember = it->second;
		++it;
...
DWORD CParty::GetMemberCount() const
{
...
{
	const auto it = m_memberMap.find(dwPID);
...
void CParty::P2PQuit(DWORD dwPID)
{
	...
	const BYTE bRole = it->second.bRole;
...
void CParty::RequestSetMemberLevel(DWORD pid, BYTE level) const
{
...
void CParty::SendMessage(LPCHARACTER ch, BYTE bMsg, DWORD dwArg1, DWORD dwArg2)
{
	...
	switch (bMsg)
	{
		...
		case PM_RETURN:
			{
				...
				while (it != m_memberMap.end())
				{
					...
					{
						const DWORD x = dwArg1 + number(-500, 500);
						const DWORD y = dwArg2 + number(-500, 500);
						...
						if (pkChr->Goto(x, y))
						{
							const LPCHARACTER victim = pkChr->GetVictim();
		...
		case PM_ATTACKED_BY:
			{
				const LPCHARACTER pkChrVictim = ch->GetVictim();
		...
		case PM_AGGRO_INCREASE:
			{
				const LPCHARACTER victim = CHARACTER_MANAGER::instance().Find(dwArg2);
...
{
	const auto it = m_memberMap.find(pid);
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	const LPCHARACTER ch = m_memberMap[pid].pCharacter;
...
LPCHARACTER CParty::GetNextOwnership(LPCHARACTER ch, long x, long y)
{
	...
	while (size-- > 0)
	{
		const LPCHARACTER pkMember = m_itNextOwner->second.pCharacter;
...
void CParty::ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd) const
{
	if (!bAdd)
	{
	...
	const float k = (float) ch->GetSkillPowerByLevel( MIN(SKILL_MAX_LEVEL, m_iLeadership ) )/ 100.0f;
	...
	switch (bRole)
	{
		...
		case PARTY_ROLE_TANKER:
			{
				const int iBonus = (int) (50 + 1450 * k);
...
void CParty::Update()
{
	...
	if (!m_bPartyHealReady)
	{
		...
		const DWORD PartyHealCoolTime = (m_iLeadership >= 40) ? PARTY_HEAL_COOLTIME_SHORT * 60 * 1000 : PARTY_HEAL_COOLTIME_LONG * 60 * 1000;
...
{
	const auto it = m_map_iFlag.find(name);
...
LPDUNGEON CParty::GetDungeon_for_Only_party() const
{
	return m_pkDungeon_for_Only_party;
}

bool CParty::IsPositionNearLeader(LPCHARACTER ch) const
{
	if (!m_pkChrLeader)
		return false;
...
void CParty::SendParameter(LPCHARACTER ch) const
{
...
LPCHARACTER CParty::GetExpCentralizeCharacter() const
{
...
bool CParty::IsPartyInDungeon(int mapIndex)
{
	for(auto it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		const LPDUNGEON d = ch->GetDungeon();
