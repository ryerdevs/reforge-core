...
{
	const auto info = dynamic_cast<dungeon_regen_event_info*>( event->info );
	...
	const LPREGEN	regen = info->regen;
	if (regen->time == 0)
	{
		regen->event = nullptr;
	}
...
{
	const auto info = dynamic_cast<regen_event_info*>( event->info );
