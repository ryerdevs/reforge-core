...
int CHARACTER::ProcessAffect()
{
	...
	{
		const int aff_idx = i + AFFECT_PREMIUM_START;
...
{
	const auto info = dynamic_cast<load_affect_login_event_info*>( event->info );
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement * pElements)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++pElements)
	{
		...
		{
			const LPITEM item = FindItemByID( pElements->dwFlag );
