...
{
	const auto info = dynamic_cast<TargetInfo *>( event->info );
...
// <Factor> Raplaced direct pointer reference with key searching.
	...
	const LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindByPID(info->dwPID);
	if (pkChr == nullptr) {
		...
		int iSendFlag)
{
	...
	if (it != m_map_kListEvent.end())
	{
		...
		{
			const LPEVENT pkEvent = *(it2++);
			const auto existInfo = dynamic_cast<TargetInfo*>(pkEvent->info);
...
void CTargetManager::DeleteTarget(DWORD dwPID, DWORD dwQuestIndex, const char * c_pszTargetName)
{
	...
	{
		LPEVENT pkEvent = *it2;
		const auto info = dynamic_cast<TargetInfo*>(pkEvent->info);
