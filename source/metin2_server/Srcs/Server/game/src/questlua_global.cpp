...
namespace quest
{
	...
	ALUA(_time_to_str)
	{
		const time_t curTime = (time_t)lua_tonumber(L, -1);
	...
	ALUA(_chat_in_map0)
	{
		const DWORD dwMapIndex = lua_tonumber(L, 1);
		const std::string sText = lua_tostring(L, 2);

		FSendChatPacket f(CHAT_TYPE_TALKING, sText);
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap(dwMapIndex);
		if (pSecMap)
			pSecMap->for_each(f);
		return 0;
	}
	...
	ALUA(_set_server_timer)
	{
		...
		const double t = lua_tonumber(L, 2);
		DWORD arg = 0;
		...
		const int timernpc = q.LoadTimerScript(name);

		const LPEVENT event = quest_create_server_timer_event(name, t, timernpc, false, arg);
	...
	ALUA(_set_server_loop_timer)
	{
		...
		const LPEVENT event = quest_create_server_timer_event(name, t, timernpc, true, arg);
		q.AddServerTimer(name, arg, event);
		return 0;
	}
	...
	ALUA(_clear_server_timer)
	{
		CQuestManager & q = CQuestManager::instance();
		const char * name = lua_tostring(L, 1);
		const DWORD arg = (DWORD) lua_tonumber(L, 2);
	...
	ALUA(_item_log)
	{
		...
		const LPITEM item = ITEM_MANAGER::instance().Find(dwItemID);
	...
	ALUA(_syslog)
	{
		...
		const PC* pc = CQuestManager::instance().GetCurrentPC();
	...
	ALUA(_add_bgm_info)
	{
		...
		const int mapIndex		= (int)lua_tonumber(L, 1);

		const char*	bgmName	= lua_tostring(L, 2);
		if (!bgmName)
			return 0;

		const float bgmVol = lua_isnumber(L, 3) ? lua_tonumber(L, 3) : (1.0f/5.0f)*0.1f;
	...
	ALUA(_add_goto_info)
	{
		...
		const int empire 	= (int)lua_tonumber(L, 2);
		const int mapIndex 	= (int)lua_tonumber(L, 3);
		const int x 		= (int)lua_tonumber(L, 4);
		const int y 		= (int)lua_tonumber(L, 5);
	...
	ALUA(_refine_pick)
	{
		...
		const int ret = mining::RealRefinePick(ch, item);
	...
	ALUA(_fish_real_refine_rod)
	{
		...
		const int ret = fishing::RealRefineRod(ch, item);
		lua_pushnumber(L, ret);
		return 1;
	}
	...
	{
		const int pid = CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerID();
	...
	{
		const int empire = (int)lua_tonumber(L,1);
		const int type = (int)lua_tonumber(L, 2);
		const int value = (int)lua_tonumber(L, 3);
		const int time = (int) lua_tonumber(L,4);
		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		if (ch)
			sys_log(0, "_give_empire_privilege(empire=%d, type=%d, value=%d, time=%d), by quest, %s",
					empire, type, value, time, ch->GetName());
		else
			sys_log(0, "_give_empire_privilege(empire=%d, type=%d, value=%d, time=%d), by quest, Null",
	...
	{
		const int guild_id = (int)lua_tonumber(L,1);
		const int type = (int)lua_tonumber(L, 2);
		const int value = (int)lua_tonumber(L, 3);
		const int time = (int)lua_tonumber( L, 4 );
	...
	ALUA(_get_empire_privilege_string)
	{
		...
		{
			const CPrivManager::SPrivEmpireData* pkPrivEmpireData = CPrivManager::instance().GetPrivByEmpireEx(empire, type);
	...
	ALUA(_get_empire_privilege)
	{
		...
		const int empire = (int)lua_tonumber(L,1);
		const int type = (int)lua_tonumber(L,2);
		const int value = CPrivManager::instance().GetPrivByEmpire(empire,type);
	...
	{
		const int guild = (int) lua_tonumber(L,1);
		ostringstream os;
		bool found = false;
	...
	ALUA(_get_guildid_byname)
	{
		...
		const CGuild* pFindGuild = CGuildManager::instance().FindGuildByName( pszGuildName );
		if ( pFindGuild )
	...
	ALUA(_get_guild_privilege)
	{
		...
		const int guild = (int)lua_tonumber(L,1);
		const int type = (int)lua_tonumber(L,2);
		const int value = CPrivManager::instance().GetPrivByGuild(guild,type);
		lua_pushnumber(L, value);
		return 1;
	}
	...
	ALUA(_find_pc_cond)
	{
		...
		const int iMinLev = (int) lua_tonumber(L, 1);
		const int iMaxLev = (int) lua_tonumber(L, 2);
		const unsigned int uiJobFlag = (unsigned int) lua_tonumber(L, 3);
	...
	ALUA(_find_npc_by_vnum)
	{
		...
		const DWORD race = (DWORD) lua_tonumber(L, 1);
	...
	ALUA(_get_quest_state)
	{
		...
		if (pPC)
		{
			...
			const int nRet = pPC->GetFlag( stQuestName.c_str() );
	...
	ALUA(_big_notice_all)
	{
		...
		SendNoticeRaw(s.str().c_str(), true); // text built by the quest script: verbatim
		return 1;
	}
#endif
	...
	ALUA(_notice_all)
	{
		...
		SendNoticeRaw(s.str().c_str(), false); // text built by the quest script: verbatim
		return 1;
	}
	...
	EVENTFUNC(warp_all_to_village_event)
	{
		...
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( info->dwWarpMapIndex );
	...
	ALUA(_warp_all_to_village)
	{
		...
		const int iSec		= static_cast<int>(lua_tonumber(L, 2));
	...
	ALUA(_say_in_map)
	{
		...
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( iMapIndex );
	...
	{
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( lua_tonumber(L,1) );
	...
	{
		const int iMapIndex = static_cast<int>(lua_tonumber(L, 1));
		const std::string szFilename(lua_tostring(L, 2));

		const LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(iMapIndex);
	...
	ALUA(_add_ox_quiz)
	{
		...
		const bool answer = lua_toboolean(L, 3);
	...
	{
		const auto info = dynamic_cast<warp_all_to_map_my_empire_event_info *>(event->info);
		...
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( info->m_lMapIndexFrom );
	...
	ALUA(_block_chat)
	{
		...
		if (pChar != nullptr)
		{
			...
			const std::string strName(lua_tostring(L, 1));
			const std::string strTime(lua_tostring(L, 2));

			const std::string strArg = strName + " " + strTime;
	...
	// Language System: lowercase code of the current player's language
	// ("es", "en", ...). Used by the gameforge/locale proxies to pick the
	// per-language table (gameforge_EN, locale_ES, ...).
	ALUA(_get_current_lang)
	{
		const BYTE bLang = g_iCurrentLang < LANGUAGE_MAX_NUM ? g_iCurrentLang : (BYTE) LANGUAGE_DEFAULT;
		const std::string& strLang = arstLocaleStringNames[bLang];

		char szLower[4] = { 0, 0, 0, 0 };
		for (size_t i = 0; i < strLang.size() && i < 3; ++i)
			szLower[i] = (strLang[i] >= 'A' && strLang[i] <= 'Z') ? (char)(strLang[i] - 'A' + 'a') : strLang[i];

		lua_pushstring(L, szLower);
	...
	ALUA(_purge_area)
	{
		const int x1 = lua_tonumber(L, 1);
		const int y1 = lua_tonumber(L, 2);
		const int x2 = lua_tonumber(L, 3);
		const int y2 = lua_tonumber(L, 4);
		...
		const LPSECTREE_MAP pSectree = SECTREE_MANAGER::instance().GetMap(mapIndex);
	...
	{
		const int from_x1 = lua_tonumber(L, 1);
		const int from_y1 = lua_tonumber(L, 2);
		const int from_x2 = lua_tonumber(L, 3);
		const int from_y2 = lua_tonumber(L, 4);

		const int to_x1 = lua_tonumber(L, 5);
		const int to_y1 = lua_tonumber(L, 6);
		const int to_x2 = lua_tonumber(L, 7);
		const int to_y2 = lua_tonumber(L, 8);
	...
	ALUA(_get_special_item_group)
	{
		...
		const int count = pItemGroup->GetGroupSize();
...
#define NAN (INFINITY-INFINITY)
	...
	ALUA(_mysql_direct_query)
	{
		...
		const auto pMsg(DBManager::instance().DirectQuery("%s", lua_tostring(L, 1)));
		if (pMsg.get())
		{
	...
	ALUA(_map_instance_find)
	{
		lua_pushboolean(L, SECTREE_MANAGER::instance().GetMap(lua_tonumber(L, 1)) != nullptr);
		return 1;
	}
#endif
	...
	void RegisterGlobalFunctionTable(lua_State* L)
	{
		...
		const luaL_reg global_functions[] =
		{
			...
			{	"get_current_lang",				_get_current_lang				},
			...
			{	"map_instance_find", 			_map_instance_find				},
#endif
