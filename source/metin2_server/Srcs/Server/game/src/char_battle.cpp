...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
...

	{
		...
		{
			const LPITEM item = vec_item[i].first;
			const int window = vec_item[i].second;
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			if (!(pkKiller && pkKiller->IsPC() && pkKiller->GetGuild() && pkKiller->GetGuild()->UnderAnyWar(GUILD_WAR_TYPE_FIELD)))
			{
				if (GetMobTable().dwResurrectionVnum)
				{
					...
					const LPCHARACTER chResurrect = CHARACTER_MANAGER::instance().SpawnMob(GetMobTable().dwResurrectionVnum, GetMapIndex(), GetX(), GetY(), GetZ(), true, (int) GetRotation());
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	const rate_t lvFactor = static_cast<rate_t>(NEW_GET_LVDELTA(to->GetLevel(), from->GetLevel())) / 100.0L;
	iExp *= lvFactor;
...
typedef struct SDamageInfo
{
	...
	inline void Distribute(LPCHARACTER ch, int iExp) const
	{
		if (pAttacker)
		...
		else if (pParty)
		{
			...
			if (pParty->GetExpCentralizeCharacter())
			{
				...
				{
					const int iExpCenteralize = (int) (iExp * 0.05f);
...
LPCHARACTER CHARACTER::GetMostAttacked() {
	...
	while (it != m_map_kDamage.end()){
		...
		const LPCHARACTER pAttacker = CHARACTER_MANAGER::instance().Find(c_VID);
...
int CHARACTER::GetArrowAndBow(LPITEM * ppkBow, LPITEM * ppkArrow, int iArrowCount/* = 1 */) const
{
	LPITEM pkBow;
	...
	if (pkArrow->GetSubType() == WEAPON_QUIVER && pkArrow->GetRealUseLimit()) // no arrows if expired - 1 otherwise
		iArrowCount = ((pkArrow->GetSocket(0) - time(nullptr)) <= 0) ? 0 : 1;
#endif
...
void CHARACTER::UseArrow(LPITEM pkArrow, DWORD dwArrowCount)
{
	...
	if (iCount == 0)
	{
		const LPITEM pkNewArrow = FindSpecifyItem(dwVnum);
...
LPCHARACTER CHARACTER::GetNearestVictim(LPCHARACTER pkChr)
{
	...
	while (it != m_map_kDamage.end())
	{
		...
		if (!pAttacker || pAttacker->IsDead()) // @fixme337
			continue;
		...
		const float fDist = DISTANCE_APPROX(pAttacker->GetX() - pkChr->GetX(), pAttacker->GetY() - pkChr->GetY());
...
struct FuncPullMonster
{
	...
	void operator()(LPENTITY ent) const
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			//if (ch->GetVictim() && ch->GetVictim() != m_ch)
			//return;
			const float fDist = DISTANCE_APPROX(m_ch->GetX() - ch->GetX(), m_ch->GetY() - ch->GetY());
			...
			const long tx = (long)(ch->GetX() + fx);
			const long ty = (long)(ch->GetY() + fy);
