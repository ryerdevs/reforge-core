...
size_t Cipher::Prepare(void* buffer, size_t* length) {
  ...
  assert(key_agreement_ != nullptr);
  const size_t agreed_length = key_agreement_->Prepare(buffer, length);
  if (agreed_length == 0) {
    delete key_agreement_;
    key_agreement_ = nullptr;
  }
...
bool Cipher::SetUp(bool polarity) {
  ...
  const int hint_0 = shared.BytePtr()[*(shared.BytePtr()) % shared.size()];
  const int hint_1 = shared.BytePtr()[*(shared.BytePtr() + 1) % shared.size()];
  ...
  const std::unique_ptr<BlockCipherAlgorithm> algorithm_0(detail_0);
  const std::unique_ptr<BlockCipherAlgorithm> algorithm_1(detail_1);
...
BlockCipherAlgorithm* BlockCipherAlgorithm::Pick(int hint) {
  ...
  const int selector = hint % kMaxAlgorithms;
  switch (selector) {
    case kRC6:
...
size_t DH2KeyAgreement::Prepare(void* buffer, size_t* length) {
  ...
  const Integer v = ModularExponentiation(g, q, p);
  if(v != Integer::One()) {
...
bool DH2KeyAgreement::Agree(size_t agreed_length, const void* buffer, size_t length) {
  ...
  const auto buf = (const byte*)buffer;
