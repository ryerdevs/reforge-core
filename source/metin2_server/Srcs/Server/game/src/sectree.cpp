...
void SECTREE::DecreasePC()
{
	...
	while (it_tree != m_neighbor_list.end())
	{
		...
		if (--tree->m_iPCCount <= 0)
		{
			...
			{
				const LPENTITY pkEnt = *(it_entity++);

				if (pkEnt->IsType(ENTITY_CHARACTER))
				{
					const auto ch = (LPCHARACTER) pkEnt;
					ch->StopStateMachine();
				}
			}
		}
...
bool SECTREE::InsertEntity(LPENTITY pkEnt)
{
	...
	if (pkEnt->IsType(ENTITY_CHARACTER))
	{
		const auto pkChr = (LPCHARACTER) pkEnt;
...
{
	const auto it = m_set_entity.find(pkEnt);
...
void SECTREE::SetAttribute(DWORD x, DWORD y, DWORD dwAttr) const
{
	assert(m_pkAttribute != nullptr);
	m_pkAttribute->Set(x, y, dwAttr);
}

void SECTREE::RemoveAttribute(DWORD x, DWORD y, DWORD dwAttr) const
{
	assert(m_pkAttribute != nullptr);
	m_pkAttribute->Remove(x, y, dwAttr);
}

DWORD SECTREE::GetAttribute(long x, long y) const
{
	assert(m_pkAttribute != nullptr);
