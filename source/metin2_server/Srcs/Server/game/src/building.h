...
namespace building
{
	...
	class CObject : public CEntity
	{
		public:
			...
			CLand *	GetLand() const { return m_pkLand; }

			DWORD	GetVnum() const { return m_pProto ? m_pProto->dwVnum : 0; }
			DWORD	GetGroup() const { return m_pProto ? m_pProto->dwGroupVnum : 0; }
			...
			LPCHARACTER GetNPC() const { return m_chNPC; }
			// END_OF_BUILDING_NPC
	...
	class CLand
	{
		public:
			...
			bool	RequestCreateObject(DWORD dwVnum, long lMapIndex, long x, long y, float xRot, float yRot, float zRot, bool checkAnother) const;
			void	RequestDeleteObject(DWORD dwID);
			void	RequestDeleteObjectByVID(DWORD dwVID);

			void	RequestUpdate(DWORD dwGuild) const;
			...
			// BUILD_WALL
			bool RequestCreateWall(long nMapIndex, float rot) const;
			void RequestDeleteWall();

			bool RequestCreateWallBlocks(DWORD dwVnum, long nMapIndex, char wallSize, bool doorEast, bool doorWest, bool doorSouth, bool doorNorth) const;
			void RequestDeleteWallBlocks(DWORD dwVnum);
			// END_BUILD_WALL

			DWORD GetMapIndex() const { return m_data.lMapIndex; }
		...
		protected:
			...
			// BUILD_WALL
		private :
			void DrawWall(DWORD dwVnum, long nMapIndex, long& centerX, long& centerY, char length, float zRot) const;
			// END_BUILD_WALL
	};
