...
namespace mining
{
	...
	{
		const DWORD dwRawOreVnum = GetRawOreFromLoad(dwLoadVnum);
		...
		const LPITEM item = ITEM_MANAGER::instance().CreateItem(dwRawOreVnum, GetFractionCount());
	...
	int GetOrePct(LPCHARACTER ch)
	{
		...
		const LPITEM pick = ch->GetWear(WEAR_WEAPON);
	...
	{
		const auto info = dynamic_cast<mining_event_info*>( event->info );
	...
	bool OreRefine(LPCHARACTER ch, LPCHARACTER npc, LPITEM item, int cost, int pct, LPITEM metinstone_item)
	{
		...
		const DWORD dwRefinedVnum = GetRefineFromRawOre(item->GetVnum());
