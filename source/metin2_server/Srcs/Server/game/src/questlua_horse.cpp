...
namespace quest
{
	...
	ALUA(horse_summon)
	{
		...
		const DWORD horseVnum= lua_isnumber(L, 2) ? lua_tonumber(L, 2) : 0;
	...
	{
		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const LPCHARACTER horse = CQuestManager::instance().GetCurrentNPCCharacterPtr();
	...
	ALUA(horse_set_level)
	{
		...
		const int newlevel = MINMAX(0, (int)lua_tonumber(L, 1), HORSE_MAX_LEVEL);
	...
	ALUA(horse_get_health_pct)
	{
		...
		const int pct = MINMAX(0, ch->GetHorseHealth() * 100 / ch->GetHorseMaxHealth(), 100);
	...
	{
		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const int pct = MINMAX(0, ch->GetHorseStamina() * 100 / ch->GetHorseMaxStamina(), 100);
	...
	ALUA(horse_set_name)
	{
		...
		if ( ch->GetHorseLevel() > 0 )
		{
			...
			{
				const int nHorseNameDuration = test_server == true ? 60*5 : 60*60*24*30;
	...
	{
		const int iHealth = MINMAX(0, lua_tonumber(L, 1), 50);
		const int iStamina = MINMAX(0, lua_tonumber(L, 2), 200);
