...
bool TSkillUseInfo::UseSkill(bool isGrandMaster, DWORD vid, DWORD dwCooltime, int splashcount, int hitcount, int range)
{
	...
	const DWORD dwCur = get_dword_time();
...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	const BYTE bLastLevel = GetSkillLevel(dwSkillVnum);
	...
	const DWORD nextTime = get_global_time() + number(g_dwSkillBookNextReadMin, g_dwSkillBookNextReadMax);
...
void CHARACTER::ComputePassiveSkill(DWORD dwVnum)
{
	...
	const int iAmount = (int) pkSk->kPointPoly.Eval();
...
struct FFindNearVictim
{
	...
	void operator ()(LPENTITY ent)
	{
		...
		const float fDist = DISTANCE_APPROX(m_pkChrCenter->GetX() - pkChr->GetX(), m_pkChrCenter->GetY() - pkChr->GetY());
...
{
	const auto info = dynamic_cast<chain_lightning_event_info *>( event->info );

	const LPCHARACTER pkChrVictim = CHARACTER_MANAGER::instance().Find(info->dwVictim);
	const LPCHARACTER pkChr = CHARACTER_MANAGER::instance().Find(info->dwChr);
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		auto pkChrVictim = (LPCHARACTER) ent;
		...
		{
			const int iPenetratePct = (int) m_pkSk->kPointPoly2.Eval();
		...
		{
			const int iPct = (int) m_pkSk->kPointPoly2.Eval();
		...
		const bool bUnderEunhyung = m_pkChr->GetAffectedEunhyung() > 0;
...

		{
			...
			if (0 != AntiSkillID)
			{
				const BYTE AntiSkillLevel = pkChrVictim->GetSkillLevel(AntiSkillID);
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun() && !pkChrVictim->IsDead()) // @fixme312 IsDead
		{
				...
				!IS_SET(pkChrVictim->GetAIFlag(), AIFLAG_NOMOVE))
			{
				...
				const long tx = (long)(pkChrVictim->GetX()+fx);
				const long ty = (long)(pkChrVictim->GetY()+fy);
...
{
	const auto info = dynamic_cast<skill_gwihwan_info*>( event->info );
	...
	const DWORD pid = info->pid;
	const BYTE sklv= info->bsklv;
...
int CHARACTER::ComputeSkillAtPosition(DWORD dwVnum, const PIXEL_POSITION& posTarget, BYTE bSkillLevel)
{
	...
	const int iAmount2 = (int) pkSk->kPointPoly2.Eval();

	// ADD_GRANDMASTER_SKILL
	const int iAmount3 = (int) pkSk->kPointPoly3.Eval();
	...
	if (false == m_bDisableCooltime)
	{
		if (false ==
				m_SkillUseInfo[dwVnum].UseSkill(
					bUseGrandMaster,
				   	(nullptr != pkVictim && SKILL_HORSE_WILDATTACK != dwVnum) ? pkVictim->GetVID() : VID(),
...
{
	const auto info = dynamic_cast<mob_skill_event_info *>( event->info );
...
bool CHARACTER::UseMobSkill(unsigned int idx)
{
	...
	const DWORD dwVnum = pInfo->dwSkillVnum;
	CSkillProto * pkSk = CSkillManager::instance().Get(dwVnum);
	...
	pkSk->kCooldownPoly.SetVar("k", k);
	const int iCooltime = (int) (pkSk->kCooldownPoly.Eval() * 1000);
...
{
	const DWORD selfJobGroup = (GetJob()+1) * 10 + GetSkillGroup();
#ifdef ENABLE_WOLFMAN_CHARACTER
	...
	const DWORD skillCount = *skillVNums++;
	...
	for (DWORD skillIndex = 0; skillIndex != skillCount; ++skillIndex)
	{
		...
		const DWORD eachSkillVNum = skillVNums[skillIndex];
		if ( eachSkillVNum != 0 )
		{
			const DWORD eachJobGroup = s_anSkill2JobGroup[eachSkillVNum];
...
const DWORD GetRandomSkillVnum(BYTE bJob)
{
	...
	do
	{
		...
		const DWORD tmpSkillGroup = number(0, SKILL_GROUP_MAX_NUM-1);
		const DWORD tmpSkillCount = number(0, SKILL_COUNT-1);
		// set skill
...
{
	const auto iter = m_SkillUseInfo.find(SkillID);
	...
	const auto iterTargetMap = rSkillUseInfo.TargetVIDMap.find(TargetVID);
