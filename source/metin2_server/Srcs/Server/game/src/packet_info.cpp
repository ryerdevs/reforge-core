...
{
	const auto it = m_pPacketMap.find(header);
...
CPacketInfoCG::CPacketInfoCG()
{
	...
	// Language System: the recompiled client appends szLanguage[3] ("es\0")
	// to LOGIN3 ONLY on the auth server; the channel keeps sending 65 bytes.
	// g_bAuthServer is set during config reading, before any desc/input
	// processor (and its packet table) is created.
	Set(HEADER_CG_LOGIN3, sizeof(TPacketCGLogin3) + (g_bAuthServer ? 3 : 0), "Login3", true);
