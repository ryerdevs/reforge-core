...
namespace quest
{
	...
	int	PC::GetCurrentQuestIndex() const
	{
	...
	void PC::SetFlag(const string& name, int value, bool bSkipSave)
	{
		...
		const auto it = m_FlagMap.find(name);
	...
	{
		const auto it = m_FlagSaveMap.find(name);
	...
	{
		const int iNowState = GetFlag(quest_name + ".__status");
	...
	{
		const DWORD dwQuestIndex = CQuestManager::instance().GetQuestIndexByName(quest_name);
	...
	void PC::SetQuest(const string& quest_name, QuestState& qs)
	{
		...
		const auto it = m_QuestInfo.find(qi);
	...
	void PC::SendQuestInfoPakcet()
	{
		...
		{
			const BYTE temp = m_RunningQuestState->bStart?1:0;
			buf.write(temp);
			qi.size+=sizeof(temp);
	...
	void PC::EndRunning()
	{
		...
		const int iNowState = m_RunningQuestState->st;
		...
		{
			const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			const DWORD dwQuestIndex = CQuestManager::instance().GetQuestIndexByName(m_stCurQuest);
			if (ch)
			{
	...
	void PC::SetCurrentQuestBeginFlag()
	{
		...
		const int iQuestIndex = q.GetQuestIndexByName(m_stCurQuest);
	...
	int PC::GetCurrentQuestBeginFlag() const
	{
	...
	{
		//SetSendFlag(QUEST_SEND_TITLE);
		const auto it = m_QuestInfo.find(CQuestManager::instance().GetQuestIndexByName(quest));

		if (it != m_QuestInfo.end())
		{
			//(*it)->_title = title;
			QuestState* old = m_RunningQuestState;
			const int old2 = m_iSendToClient;
			const std::string oldquestname = m_stCurQuest;
			m_stCurQuest = quest;
	...
	void PC::Save()
	{
		...
		while (it != m_FlagSaveMap.end())
		{
			...
			const long lValue = it->second;

			++it;

			const int iPos = stComp.find(".");
	...
	{
		const unsigned int qi = CQuestManager::instance().GetQuestIndexByName(quest_name);
	...
	{
		const string quest_name_with_dot = quest_name + '.';
