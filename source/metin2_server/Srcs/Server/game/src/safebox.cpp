...
LPITEM CSafebox::Get(DWORD dwPos) const
{
...
bool CSafebox::IsEmpty(DWORD dwPos, BYTE bSize) const
{
	if (!m_pkGrid)
		return false;
...
void CSafebox::ChangeSize(int iSize)
{
	...
	const std::unique_ptr<CGrid> pkOldGrid = std::move(m_pkGrid); // @fixme191
...
LPITEM CSafebox::GetItem(BYTE bCell) const
{
	if (bCell >= 5 * m_iSize)
	{
...
bool CSafebox::IsValidPosition(DWORD dwPos) const
{
	if (!m_pkGrid)
		return false;
