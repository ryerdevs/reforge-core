...
BOOL CTextFileLoader::GetTokenVector(const std::string & c_rstrKey, TTokenVector ** ppTokenVector) const
{
	if (!m_pcurNode)
	{
		assert(!"Node to access has not set!");
	...
	const auto it = m_pcurNode->LocalTokenVectorMap.find(c_rstrKey);
