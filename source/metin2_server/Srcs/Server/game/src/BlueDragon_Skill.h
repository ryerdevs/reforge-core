			...
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					if (ch->GetJob() == Set1)
					{
						...
						const int firstDamagePercent =  number(BlueDragon_GetSkillFactor(4, "Skill0", "damage", ptr, "min"), BlueDragon_GetSkillFactor(4, "Skill0", "damage", ptr, "max"));
					...
					if (ch->GetJob() == Set2)
					{
						...
						const int secondDamagePercent = number(BlueDragon_GetSkillFactor(4, "Skill0", "damage", ptr, "min"), BlueDragon_GetSkillFactor(4, "Skill0", "damage", ptr, "max"));
					...
					if (GET_SEX(ch) == gender)
					{
						...
						const int thirdDamagePercent = number(BlueDragon_GetSkillFactor(4, "Skill0", "gender", ptr, "min"), BlueDragon_GetSkillFactor(4, "Skill0", "gender", ptr, "max"));
					...
					const int addPct = BlueDragon_GetRangeFactor("hp_damage", pAttacker->GetHPPct());
		...
		if (nullptr != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					const long dist = DISTANCE_APPROX( pAttacker->GetX() - ch->GetX(), pAttacker->GetY() - ch->GetY() );
