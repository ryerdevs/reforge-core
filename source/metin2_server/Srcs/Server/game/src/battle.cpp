...
{
	const long distance = DISTANCE_APPROX(x - tx, y - ty);
...
bool battle_is_icedamage(LPCHARACTER pAttacker, LPCHARACTER pVictim)
{
	...
	{
		const DWORD race = pAttacker->GetRaceNum();
		...
		{
			for (auto it = tmp_setNDAFlag.begin(); it != tmp_setNDAFlag.end(); ++it)
			{
...
bool battle_is_attackable(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	if (victim->IsObserverMode()) // @fixme327
		return false;
...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	int dam;
	const int ret = battle_hit(ch, victim, dam);
	return (ret);
}
...
float CalcAttackRating(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, bool bIgnoreTargetRating)
{
	...
	{
		const int attacker_dx = pkAttacker->GetPolymorphPoint(POINT_DX);
		const int attacker_lv = pkAttacker->GetLevel();

		const int victim_dx = pkVictim->GetPolymorphPoint(POINT_DX);
		const int victim_lv = pkVictim->GetLevel(); // @fixme320 (pkAttacker -> pkVictim)

		iARSrc = MIN(90, (attacker_dx * 4	+ attacker_lv * 2) / 6);
		iERSrc = MIN(90, (victim_dx	  * 4	+ victim_lv   * 2) / 6);
	}

	const float fAR = ((float) iARSrc + 210.0f) / 300.0f;

	if (bIgnoreTargetRating)
		return fAR;

	const float fER = ((float) (iERSrc * 2 + 5) / (iERSrc + 95)) * 3.0f / 10.0f;
...
{
	const LPITEM pkWeapon = pkAttacker->GetWear(WEAR_WEAPON);
	if (pkWeapon)
	{
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	if (!pkAttacker->IsPC())
	{
		const int iReduceDamagePct = pkVictim->GetMarriageBonus(UNIQUE_ITEM_MARRIAGE_TRANSFER_DAMAGE);
...
{
	const LPITEM pWeapon = pkAttacker->GetWear(WEAR_WEAPON);
	const bool bPolymorphed = pkAttacker->IsPolymorphed();
	...
	int iDam = 0;
	const float fAR = CalcAttackRating(pkAttacker, pkVictim, bIgnoreTargetRating);
	...
	if (bPolymorphed && !pkAttacker->IsPolyMaintainStat())
	{
		...
		const DWORD dwMobVnum = pkAttacker->GetPolymorphVnum();
		const CMob * pMob = CMobManager::instance().Get(dwMobVnum);

		if (pMob)
		{
			const int iPower = pkAttacker->GetPolymorphPower();
	...
	if (test_server)
	{
		const int DEBUG_iLV = pkAttacker->GetLevel()*2;
		const int DEBUG_iST = int((pkAttacker->GetPoint(POINT_ATT_GRADE) - DEBUG_iLV) * fAR);
		const int DEBUG_iPT = pkAttacker->GetPoint(POINT_PARTY_ATTACKER_BONUS);
		int DEBUG_iWP = 0;
...
int CalcArrowDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, LPITEM pkBow, LPITEM pkArrow, bool bIgnoreDefense)
{
	...
	const int iDist = (int) (DISTANCE_SQRT(pkAttacker->GetX() - pkVictim->GetX(), pkAttacker->GetY() - pkVictim->GetY()));
	//int iGap = (iDist / 100) - 5 - pkBow->GetValue(5) - pkAttacker->GetPoint(POINT_BOW_DISTANCE);
	const int iGap = (iDist / 100) - 5 - pkAttacker->GetPoint(POINT_BOW_DISTANCE);
	...
	const float fAR = CalcAttackRating(pkAttacker, pkVictim, false);
...
DWORD GET_ATTACK_SPEED(LPCHARACTER ch)
{
	...
	const LPITEM item = ch->GetWear(WEAR_WEAPON);
	const DWORD default_bonus = SPEEDHACK_LIMIT_BONUS;
	...
	const DWORD ani_speed = ani_attack_speed(ch);
