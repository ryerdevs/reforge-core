...
namespace quest
{
	...
	{
		const auto info = dynamic_cast<quest_server_event_info *>( event->info );
	...
	{
		const auto info = dynamic_cast<quest_event_info *>( event->info );
	...
	{
		const long ltime_cycle = (long) (rint(PASSES_PER_SEC(when)));
	...
	{
		const long ltime_cycle = (int) (rint(PASSES_PER_SEC(when)));
