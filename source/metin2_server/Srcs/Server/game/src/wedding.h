...
namespace marriage
{
	...
	class WeddingMap
	{
		public:
			...
			DWORD GetMapIndex() const { return m_dwMapIndex; }
			...
			void SendLocalEvent(LPCHARACTER ch) const;

			void ShoutInMap(BYTE type, const char* szMsg);
		private:

			const char* __BuildCommandPlayMusic(char* szCommand, size_t nCmdLen, BYTE bSet, const char* c_szMusicFileName) const;
	...
	class WeddingManager : public singleton<WeddingManager>
	{
		public:
			...
			bool IsWeddingMap(DWORD dwMapIndex) const;
