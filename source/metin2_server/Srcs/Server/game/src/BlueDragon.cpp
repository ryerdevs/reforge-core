...
time_t UseBlueDragonSkill(LPCHARACTER pChar, unsigned int idx)
{
	...
	const int addPct = BlueDragon_GetRangeFactor("hp_period", pChar->GetHPPct());
...
int BlueDragon_StateBattle(LPCHARACTER pChar)
{
	...
	const time_t timeNow = static_cast<time_t>(get_dword_time());
	...
	if (pAttacker->IsMonster() && BlueDragon::BossVnum == pAttacker->GetMobTable().dwVnum)
	{
		for (int i=1; i <= BlueDragon::StoneCount; ++i)
		{
			if (ATK_BONUS == BlueDragon_GetIndexFactor("DragonStone", i, "effect_type"))
			{
				...
				const size_t cnt = SECTREE_MANAGER::instance().GetMonsterCountInMap( pAttacker->GetMapIndex(), dwDragonStoneID );
	...
	if (me->IsMonster() && BlueDragon::BossVnum == me->GetMobTable().dwVnum)
	{
		for (int i=1; i <= BlueDragon::StoneCount; ++i)
		{
			if (DEF_BONUS == BlueDragon_GetIndexFactor("DragonStone", i, "effect_type"))
			{
				const DWORD dwDragonStoneID = BlueDragon_GetIndexFactor("DragonStone", i, "vnum");
				const size_t val = BlueDragon_GetIndexFactor("DragonStone", i, "val");
				const size_t cnt = SECTREE_MANAGER::instance().GetMonsterCountInMap( me->GetMapIndex(), dwDragonStoneID );
	...
	if (me->IsStone() && 0 != pAttacker->GetMountVnum())
	{
		for (int i=1; i <= BlueDragon::StoneCount; ++i)
		{
			if (me->GetMobTable().dwVnum == BlueDragon_GetIndexFactor("DragonStone", i, "vnum"))
			{
				...
				{
					const size_t val = BlueDragon_GetIndexFactor("DragonStone", i, "enemy_val");
