...
#define ENABLE_SAFEBOX_MOVE_ANTIFLOOD
...
EVENTFUNC(block_chat_by_ip_event)
{
	...
	const auto it = spam_score_of_ip.find(host);
...
bool SpamBlockCheck(LPCHARACTER ch, const char* const buf, const size_t buflen)
{
	if (ch->GetLevel() < g_iSpamBlockMaxLevel)
	{
		...
		if (it == spam_score_of_ip.end())
		{
			spam_score_of_ip.emplace(ch->GetDesc()->GetHostName(), std::make_pair(0, (LPEVENT)nullptr));
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes) const
{
	...
	const LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindPC(pinfo->szNameTo);
...
struct RawPacketToCharacterFunc
{
	...
	void operator () (LPCHARACTER c) const
	{
		if (!c->GetDesc())
			return;
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes) const
{
...
void CInputMain::ItemUse(LPCHARACTER ch, const char * data) const
{
...
void CInputMain::ItemDrop(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_item_drop *) data;
	if (!ch)
		return;
...
void CInputMain::ItemDrop2(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (TPacketCGItemDrop2 *) data;
	if (!ch)
		return;
...
{
	const auto pinfo = (struct command_quickslot_add *) data;
	ch->SetQuickslot(pinfo->pos, pinfo->slot);
}

void CInputMain::QuickslotDelete(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_quickslot_del *) data;
	ch->DelQuickslot(pinfo->pos);
}

void CInputMain::QuickslotSwap(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_quickslot_swap *) data;
	ch->SwapQuickslot(pinfo->pos, pinfo->change_pos);
}

int CInputMain::Messenger(LPCHARACTER ch, const char* c_pData, size_t uiBytes) const
{
	const auto p = (TPacketCGMessenger*) c_pData;
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_VID:
			{
				...
				const auto p2 = (TPacketCGMessengerAddByVID *) c_pData;
...
void CInputMain::OnClick(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_on_click *) data;
	LPCHARACTER			victim;
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_exchange *) data;
...
{
	const auto pinfo = (struct command_position *) data;
...
void CInputMain::Move(LPCHARACTER ch, const char * data) const
{
	if (!ch->CanMove())
		return;
	...
	else
	{
		...
		else if (pinfo->bFunc & FUNC_SKILL)
		{
			...
			const unsigned int motion = pinfo->bFunc & MASK_SKILL_MOTION;

			if (!ch->IsUsableSkillMotion(motion))
			{
				const char* name = ch->GetName();
				const unsigned int job = ch->GetJob();
				const unsigned int group = ch->GetSkillGroup();
...
void CInputMain::Attack(LPCHARACTER ch, const BYTE header, const char* data) const
{
	if (nullptr == ch)
		return;
	...
	const auto type = reinterpret_cast<const struct type_identifier*>(data);
	...
	switch (header)
	{
		case HEADER_CG_ATTACK:
			{
				...
				const auto packMelee = reinterpret_cast<const TPacketCGAttack*>(data);

				ch->GetDesc()->AssembleCRCMagicCube(packMelee->bCRCMagicCubeProcPiece, packMelee->bCRCMagicCubeFilePiece);

				const LPCHARACTER	victim = CHARACTER_MANAGER::instance().Find(packMelee->dwVID);
		...
		case HEADER_CG_SHOOT:
			{
				const auto packShoot = reinterpret_cast<const TPacketCGShoot*>(data);
...
int CInputMain::SyncPosition(LPCHARACTER ch, const char * c_pcData, size_t uiBytes) const
{
	...
	const auto pHeader = (TPacketGCSyncPosition *) buffer_write_peek(lpBuf);
	...
	{
		const LPCHARACTER victim = CHARACTER_MANAGER::instance().Find(e->dwVID);
...
void CInputMain::FlyTarget(LPCHARACTER ch, const char * pcData, BYTE bHeader) const
{
	const auto p = (TPacketCGFlyTargeting *) pcData;
	ch->FlyTarget(p->dwTargetVID, p->x, p->y, bHeader);
}

void CInputMain::UseSkill(LPCHARACTER ch, const char * pcData) const
{
	const auto p = (TPacketCGUseSkill *) pcData;
	ch->UseSkill(p->dwVnum, CHARACTER_MANAGER::instance().Find(p->dwVID));
}

void CInputMain::ScriptButton(LPCHARACTER ch, const void* c_pData) const
{
	const auto p = (TPacketCGScriptButton *) c_pData;
...
void CInputMain::ScriptAnswer(LPCHARACTER ch, const void* c_pData) const
{
	const auto p = (TPacketCGScriptAnswer *) c_pData;
...
// SCRIPT_SELECT_ITEM
void CInputMain::ScriptSelectItem(LPCHARACTER ch, const void* c_pData) const
{
	const auto p = (TPacketCGScriptSelectItem*) c_pData;
...
void CInputMain::QuestInputString(LPCHARACTER ch, const void* c_pData) const
{
	const auto p = (TPacketCGQuestInputString*) c_pData;
...
void CInputMain::QuestConfirm(LPCHARACTER ch, const void* c_pData) const
{
	const auto p = (TPacketCGQuestConfirm*) c_pData;
	const LPCHARACTER ch_wait = CHARACTER_MANAGER::instance().FindByPID(p->requestPID);
	if (p->answer)
...
void CInputMain::Target(LPCHARACTER ch, const char * pcData) const
{
	const auto p = (TPacketCGTarget *) pcData;
...
void CInputMain::Warp(LPCHARACTER ch, const char * pcData) const
{
	ch->WarpEnd();
}

void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData) const
{
	#ifdef ENABLE_SAFEBOX_MOVE_ANTIFLOOD
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::SafeboxMove, std::chrono::milliseconds(200))) {
	...
	const auto p = (TPacketCGSafeboxCheckin *) c_pData;

	if (!ch->CanHandleItem())
		return;

	CSafebox * pkSafebox = ch->GetSafebox();
	const LPITEM pkItem = ch->GetItem(p->ItemPos);
	...
	if (pkItem->IsEquipped())
	{
		...
		{
			const LPITEM costumeWeapon = ch->GetWear(WEAR_COSTUME_WEAPON);
	...
	// @fixme140 BEGIN
	if (pkItem->IsEquipped() && ITEM_BELT == pkItem->GetType() && CBeltInventoryHelper::IsExistItemInBeltInventory(ch))
	{
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall) const
{
	#ifdef ENABLE_SAFEBOX_MOVE_ANTIFLOOD
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::SafeboxMove, std::chrono::milliseconds(200))) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Remaining time: %.2f"), PULSEMANAGER_CLOCK_TO_SEC2(ch->GetPlayerID(), ePulse::SafeboxMove));
	...
	const auto p = (TPacketCGSafeboxCheckout *) c_pData;
	...
	const LPITEM pkItem = pkSafebox->Get(p->bSafePos);
...
void CInputMain::SafeboxItemMove(LPCHARACTER ch, const char * data) const
{
	const auto pinfo = (struct command_item_move *) data;
...
// PARTY_JOIN_BUG_FIX
void CInputMain::PartyInvite(LPCHARACTER ch, const char * c_pData) const
{
	if (ch->GetArena())
	{
	...
	const auto p = (TPacketCGPartyInvite*) c_pData;

	const LPCHARACTER pInvitee = CHARACTER_MANAGER::instance().Find(p->vid);
...
void CInputMain::PartyInviteAnswer(LPCHARACTER ch, const char * c_pData) const
{
	if (ch->GetArena())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("����忡�� ����Ͻ� �� �����ϴ�."));
...
void CInputMain::PartySetState(LPCHARACTER ch, const char* c_pData) const
{
	...
	const auto p = (TPacketCGPartySetState*) c_pData;
...
void CInputMain::PartyRemove(LPCHARACTER ch, const char* c_pData) const
{
	if (ch->GetArena())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("����忡�� ����Ͻ� �� �����ϴ�."));
	...
	const auto p = (TPacketCGPartyRemove*) c_pData;
	...
	if (pParty->GetLeaderPID() == ch->GetPlayerID())
	{
		...
		else
		{
			...
			{
				const LPCHARACTER B = CHARACTER_MANAGER::instance().FindByPID(p->pid);
				if (B)
				{
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData) const
{
	const auto p = (TPacketCGAnswerMakeGuild*) c_pData;
...
void CInputMain::PartyUseSkill(LPCHARACTER ch, const char* c_pData) const
{
	const auto p = (TPacketCGPartyUseSkill*) c_pData;
	if (!ch->GetParty())
		return;
	...
	switch (p->bySkillIndex)
	{
		...
		case PARTY_SKILL_WARP:
			{
				const LPCHARACTER pch = CHARACTER_MANAGER::instance().Find(p->vid);
				if (pch)
...
void CInputMain::PartyParameter(LPCHARACTER ch, const char * c_pData) const
{
	const auto p = (TPacketCGPartyParameter *) c_pData;
...
int CInputMain::Guild(LPCHARACTER ch, const char * data, size_t uiBytes) const
{
		...
		case GUILD_SUBHEADER_CG_ADD_MEMBER:
			{
				...
				const LPCHARACTER newmember = CHARACTER_MANAGER::instance().Find(vid);
		...
		case GUILD_SUBHEADER_CG_REMOVE_MEMBER:
			{
				...
				const LPCHARACTER member = CHARACTER_MANAGER::instance().FindByPID(pid);
		...
		case GUILD_SUBHEADER_CG_POST_COMMENT:
			{
				...
				{
					const std::string str(c_pData + 1, length);
...
void CInputMain::Fishing(LPCHARACTER ch, const char* c_pData) const
{
	const auto p = (TPacketCGFishing*)c_pData;
	ch->SetRotation(p->dir * 5);
	ch->fishing();
	return;
}

void CInputMain::ItemGive(LPCHARACTER ch, const char* c_pData) const
{
	const auto p = (TPacketCGGiveItem*) c_pData;
	const LPCHARACTER to_ch = CHARACTER_MANAGER::instance().Find(p->dwTargetVID);
...
void CInputMain::Hack(LPCHARACTER ch, const char * c_pData) const
{
	const auto p = (TPacketCGHack *) c_pData;
...
int CInputMain::MyShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes) const
{
	const auto p = (TPacketCGMyShop *) c_pData;
	...
	else if (p->type == REFINE_TYPE_MONEY_ONLY)
	{
		...
		if (nullptr != item)
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("�ٸ� Ÿ���� �������� �ѹ��� ����� �Ұ����մϴ�."));
				}
			}
		}
	}
...
void CInputMain::Acce(LPCHARACTER pkChar, const char* c_pData) const
{
	quest::PC * pPC = quest::CQuestManager::instance().GetPCForce(pkChar->GetPlayerID());
	if (pPC->IsRunning())
		return;

	const auto sPacket = (TPacketAcce*) c_pData;
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	// Language System: set the per-player language context for this packet, so
	// every single-arg LC_TEXT(str) call made while handling it (chat, combat
	// messages, etc.) resolves to the sender's language.
	// LIMITATION: messages emitted from timers or on behalf of other players
	// (broadcasts, notices, chat of other characters) use the language of the
	// last processed packet; use LC_TEXT_LANG(lang, str) for those.
	...
	switch (bHeader)
	{
		...
		case HEADER_CG_DRAGON_SOUL_REFINE:
			{
				const auto p = reinterpret_cast <TPacketCGDragonSoulRefine*>((void*)c_pData);
				switch(p->bSubType)
				{
