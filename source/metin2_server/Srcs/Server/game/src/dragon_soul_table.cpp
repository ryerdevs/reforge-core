...
{
	const auto it = m_map_type_to_name.find (bType);
...
bool DragonSoulTable::ReadVnumMapper()
{
	...
	{
		const int n = pGroupNode->GetRowCount();
		if (0 == n)
		{
...
{
	const CGroupNode* pGroupNode = m_pLoader->GetGroup("basicapplys");
	...
	for (uint i = 0; i < m_vecDragonSoulNames.size(); i++)
	{
		...
		const int n = pChild->GetRowCount();
...
{
	const CGroupNode* pGroupNode = m_pLoader->GetGroup("additionalapplys");
	if (nullptr == pGroupNode)
	{
...
bool DragonSoulTable::CheckApplyNumSettings () const
{
...
bool DragonSoulTable::CheckWeightTables () const
{
...
bool DragonSoulTable::CheckRefineGradeTables() const
{
...
bool DragonSoulTable::CheckRefineStepTables () const
{
...
bool DragonSoulTable::CheckRefineStrengthTables() const
{
	const CGroupNode* pGroupNode = m_pRefineStrengthTableNode;
...
bool DragonSoulTable::CheckDragonHeartExtTables() const
{
...
{
	const auto it = m_map_basic_applys_group.find(ds_type);
...
{
	const auto it = m_map_additional_applys_group.find(ds_type);
...
bool DragonSoulTable::GetApplyNumSettings(BYTE ds_type, BYTE grade_idx, OUT int& basis, OUT int& add_min, OUT int& add_max) const
{
...
bool DragonSoulTable::GetWeight(BYTE ds_type, BYTE grade_idx, BYTE step_index, BYTE strength_idx, OUT float& fWeight) const
{
	...
	const CGroupNode* pDragonSoulGroup = m_pWeightTableNode->GetChildNode(stDragonSoulName);
...
bool DragonSoulTable::GetRefineGradeValues(BYTE ds_type, BYTE grade_idx, OUT int& need_count, OUT int& fee, OUT std::vector<float>& vec_probs) const
{
...
bool DragonSoulTable::GetRefineStepValues(BYTE ds_type, BYTE step_idx, OUT int& need_count, OUT int& fee, OUT std::vector<float>& vec_probs) const
{
...
bool DragonSoulTable::GetRefineStrengthValues(BYTE ds_type, BYTE material_type, BYTE strength_idx, OUT int& fee, OUT float& prob) const
{
	...
	const auto stStrengthIdx = boost::lexical_cast <std::string> ((int)strength_idx);
...
bool DragonSoulTable::GetDragonHeartExtValues(BYTE ds_type, BYTE grade_idx, OUT std::vector<float>& vec_chargings, OUT std::vector<float>& vec_probs) const
{
	if (grade_idx >= DRAGON_SOUL_GRADE_MAX)
	{
		sys_err ("Invalid dragon soul grade_idx(%d).", grade_idx);
...
bool DragonSoulTable::GetDragonSoulExtValues(BYTE ds_type, BYTE grade_idx, OUT float& prob, OUT DWORD& by_product) const
{
	if (grade_idx >= DRAGON_SOUL_GRADE_MAX)
	{
		sys_err ("Invalid dragon soul grade_idx(%d).", grade_idx);
