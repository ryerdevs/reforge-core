...
class CWarMap
{
	public:
		...
		bool	GetTeamIndex(DWORD dwGuild, BYTE & bIdx) const;

		void	IncMember(LPCHARACTER ch);
		void	DecMember(LPCHARACTER ch);

		CGuild * GetGuild(BYTE bIdx) const;
		DWORD	GetGuildID(BYTE bIdx) const;

		BYTE	GetType() const;
		long	GetMapIndex() const;
		DWORD	GetGuildOpponent(LPCHARACTER ch) const;

		DWORD	GetWinnerGuild() const;
		...
		bool	IsFlagOnBase(BYTE bIdx) const;
		void	ResetFlag();
	...
	private:
		...
		typedef struct STeamData
		{
			...
			int GetAccumulatedJoinerCount() const;
			int GetCurJointerCount() const;
