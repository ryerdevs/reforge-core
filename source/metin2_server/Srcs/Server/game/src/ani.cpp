...
class ANI
{
	...
	public:
		...
		DWORD	load_one_weapon(const char *dir_name, int weapon, BYTE combo, bool horse) const;
		DWORD	attack_speed(int race, int weapon, BYTE combo = 0, bool horse = false) const;
...
DWORD FN_attack_speed_from_file(const char *file)
{
	...
	const auto key	= "DirectInputTime";
	const auto delim	= " \t\r\n";
	...
	while (fgets(buf, 1024, fp))
	{
		...
		if (field && value)
		{
			...
			{
				const float f_speed = strtof(value, nullptr);
...
DWORD ANI::load_one_weapon(const char *dir_name, int weapon, BYTE combo, bool horse) const
{
	char format[128];
	char filename[256];
	...
	const DWORD speed = FN_attack_speed_from_file(filename);
...
DWORD ANI::attack_speed(int race, int weapon, BYTE combo, bool horse) const
{
	switch (race)
	{
