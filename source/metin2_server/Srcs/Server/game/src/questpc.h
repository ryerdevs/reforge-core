...
namespace quest
{
	...
	class PC
	{
		public:
			...
			inline QuestState *	GetRunningQuestState() const;
		...
		public:
			...
			bool		HasReward() const { return !m_vRewardData.empty() || m_bIsGivenReward; }
			...
			bool		GetAndResetDoneFlag() {
				const bool temp = m_bShouldSendDone; m_bShouldSendDone = false; return temp; }
		...
		private:
			...
			int			GetCurrentQuestBeginFlag() const;
	...
	inline bool PC::IsRunning() const
	{
		return m_RunningQuestState != nullptr;
	}

	inline QuestState* PC::GetRunningQuestState() const
	{
