		...
		if (m_pkDungeon) {
			...
			if (m_pkDungeon->IsValidRegen(m_pkRegen, regen_id_)) {
				...
				if (0 == m_pkRegen->count)
				{
					...
					event_create(dungeon_regen_event, info, PASSES_PER_SEC(number(0, 16)) + PASSES_PER_SEC(m_pkRegen->time));
				}
				#endif
			}
		} else {
			...
			if (0 == m_pkRegen->count)
			{
				...
				info->regen = m_pkRegen;
				event_create(regen_event, info, PASSES_PER_SEC(number(0, 16)) + PASSES_PER_SEC(m_pkRegen->time));
			}
			#endif
		}
		m_pkRegen = nullptr;
