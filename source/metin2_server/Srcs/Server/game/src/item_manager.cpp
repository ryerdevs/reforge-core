...
void ITEM_MANAGER::SaveSingleItem(LPITEM item) const
{
	if (!item->GetOwner())
	{
		const DWORD dwID = item->GetID();
		const DWORD dwOwnerID = item->GetLastOwnerPID();
...
void ITEM_MANAGER::DecreaseItem(LPITEM item, const char * c_pszReason)
{
	LPCHARACTER owner = item->GetOwner();

	char szHint[64];
	snprintf(szHint, sizeof(szHint), "%s %u ", item->GetName(), item->GetCount());
	LogManager::instance().ItemLog(owner, item, c_pszReason ? c_pszReason : "DECREASE", szHint);

	item->SetCount(item->GetCount() - 1);
}

void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason) const
{
	LPCHARACTER o;
...
int ITEM_MANAGER::RealNumber(DWORD vnum) const
{
	int bot, top, mid;
...
static void __DropEvent_CharStone_DropItem(CHARACTER & killer, CHARACTER & victim, ITEM_MANAGER& itemMgr, std::vector<LPITEM>& vec_item)
{
	...
	if (number(1, MaxRange) <= dropPercent)
	{
		...
		const int item_vnum = Stones[number(0, _countof(Stones)-1)]; // @fixme189
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	if (quest::CQuestManager::instance().GetEventFlag("xmas_sock"))
	{
		...
		const int iDropPerKill[MOB_RANK_MAX_NUM] =
		{
			2000,
			1000,
			300,
		...
		{
			const int iPercent = 40000 * iDeltaPercent / iDropPerKill[pkChr->GetMobRank()];
	...
	{
		const int pct = quest::CQuestManager::instance().GetEventFlag("hc_drop");
	...
	if (GetDropPerKillPct(100, 500, iDeltaPercent, "newyear_moon") >= number(1, iRandRange))
	{
		...
		const DWORD dwVnum = wonso_items[number(0,5)];
	...
	if (GetDropPerKillPct(1, 2000, iDeltaPercent, "valentine_drop") >= number(1, iRandRange))
	{
		...
		const DWORD dwVnum = valentine_items[number(0, 1)];
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "whiteday_drop") >= number(1, iRandRange))
	{
		...
		const DWORD dwVnum = whiteday_items[number(0,1)];
	...
	if (pkKiller->GetLevel()>=50)
	{
		...
		{
			const DWORD ITEM_QUIZ_BOX = 50034;
	...
	if (pkKiller->GetLevel() >= 15 && quest::CQuestManager::instance().GetEventFlag("mars_drop"))
	{
		...
		const int iDropMultiply[MOB_RANK_MAX_NUM] =
		{
			50,
			30,
			5,
