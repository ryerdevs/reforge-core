...
namespace quest
{
	...
	ALUA(dungeon_get_flag_from_map_index)
	{
		...
		const DWORD dwMapIndex = (DWORD) lua_tonumber(L, 2);
		if (dwMapIndex)
		{
	...
	ALUA(dungeon_jump_all)
	{
		...
		const LPDUNGEON pDungeon = CQuestManager::instance().GetCurrentDungeon();
	...
	ALUA(dungeon_set_unique)
	{
		...
		const DWORD vid = (DWORD) lua_tonumber(L, 2);
	...
	ALUA(dungeon_get_damage_from_race)
	{
		...
		const LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(vid);
	...
	ALUA(dungeon_spawn_mob)
	{
		...
		if (pDungeon)
		{
			...
			const float radius = lua_isnumber(L, 4) ? (float) lua_tonumber(L, 4) : 0;
	...
	ALUA(dungeon_spawn_mob_dir)
	{
		...
		if (pDungeon)
		{
			...
			const long x = (long) lua_tonumber(L, 2);
			const long y = (long) lua_tonumber(L, 3);
			const BYTE dir = (int) lua_tonumber(L, 4);
	...
	ALUA(dungeon_spawn_goto_mob)
	{
		...
		const long lFromX = (long)lua_tonumber(L, 1);
		const long lFromY = (long)lua_tonumber(L, 2);
	...
	ALUA(dungeon_spawn_group)
	{
		...
		if (pDungeon)
		{
			...
			const DWORD count = (DWORD) lua_tonumber(L, 6);
	...
	ALUA(dungeon_select)
	{
		const DWORD dwMapIndex = (DWORD) lua_tonumber(L, 1);
		if (dwMapIndex)
		{
			const LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(dwMapIndex);
			if (pDungeon)
	...
	ALUA(dungeon_set_warp_location)
	{
		...
		const FSetWarpLocation f ((int)lua_tonumber(L, 1), (int)lua_tonumber(L, 2), (int)lua_tonumber(L, 3));
		pDungeon->ForEachMember (f);
