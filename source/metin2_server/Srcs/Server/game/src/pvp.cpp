...
void CPVP::Packet(bool bDelete) const
{
...
{
	const int iSlot = m_players[0].dwPID != dwPID ? 1 : 0;
...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	// NOTIFY_PVP_MESSAGE
	const LPDESC pkVictimDesc = pkVictim->GetDesc();
	if (pkVictimDesc)
	{
		TPacketGCWhisper pack;

		const int len = MIN(CHAT_MAX_LEN, strlen(msg) + 1);
...
{
	const auto it = m_map_pkPVPSetByID.find(dwPID);
...
void CPVPManager::ConnectEx(LPCHARACTER pkChr, bool bDisconnect)
{
	...
	const DWORD dwVID = bDisconnect ? 0 : pkChr->GetVID();
...
void CPVPManager::Disconnect(LPCHARACTER pkChr) const
{
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	else
	{
		...
		const eMountType eIsMount = GetMountLevelByVnum(pkChr->GetMountVnum(), false);
		switch (eIsMount)
		{
	...
	{
		const BYTE bMapEmpire = SECTREE_MANAGER::instance().GetEmpireFromMapIndex(pkChr->GetMapIndex());
	...
	const CPVP kPVP(pkChr->GetPlayerID(), pkVictim->GetPlayerID());
...
{
	const auto it = m_map_pkPVP.find(pkPVP->m_dwCRC);
...
void CPVPManager::SendList(LPDESC d)
{
	...
	const DWORD dwVID = d->GetCharacter()->GetVID();
	...
	while (it != m_map_pkPVP.end())
	{
		...
		{
			const LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(pkPVP->m_players[1].dwVID);
			if (ch && ch->GetDesc())
			{
		...
		{
			const LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(pkPVP->m_players[0].dwVID);
			if (ch && ch->GetDesc())
			{
				const LPDESC d = ch->GetDesc();
				d->Packet(&pack, sizeof(pack));
