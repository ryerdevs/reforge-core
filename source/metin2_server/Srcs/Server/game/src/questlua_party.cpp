...
namespace quest
{
	...
	ALUA(party_syschat)
	{
		const LPPARTY pParty = CQuestManager::Instance().GetCurrentCharacterPtr()->GetParty();
	...
	ALUA(party_is_map_member_flag_lt)
	{
		...
		const LPPARTY pParty = q.GetCurrentCharacterPtr()->GetParty();
		const LPCHARACTER ch = q.GetCurrentCharacterPtr();
		const PC* pPC = q.GetCurrentPC();
		...
		if (pParty)
		{
			...
			const bool returnBool = pParty->ForEachOnMapMemberBool(f, ch->GetMapIndex());
	...
	ALUA(party_give_buff)
	{
		...
		const DWORD dwType = lua_tonumber(L, 1);
		const BYTE bApplyOn = lua_tonumber(L, 2);
		const long lApplyValue = lua_tonumber(L, 3);
		const DWORD dwFlag = lua_tonumber(L, 4);
