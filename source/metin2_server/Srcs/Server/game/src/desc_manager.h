...
class DESC_MANAGER : public singleton<DESC_MANAGER>
{
	...
	public:
		...
		DWORD			GetLocalUserCount() const { return m_iLocalUserCount; }
		...
		bool			IsDisconnectInvalidCRC() const { return m_bDisconnectInvalidCRC; }
		...
		bool			LoadClientPackageCryptInfo(const char* pDirName) const;
		void			SendClientPackageCryptKey( LPDESC desc ) const;
		void			SendClientPackageSDBToLoadMap( LPDESC desc, const char* pMapName ) const;
