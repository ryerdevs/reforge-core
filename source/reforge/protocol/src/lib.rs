...
pub mod header {
    ...
    /// `HEADER_CG_EMPIRE` (Packet.h:102 — 90): elegir imperio —
    /// `TPacketCGEmpire` 2 B (header+bEmpire; Packet.h:2100-2105).
    pub const CG_EMPIRE: u8 = 90;
    /// `HEADER_CG_CHANGE_NAME` (Packet.h:121 — 106): renombrar personaje —
    /// `TPacketCGChangeName` 27 B (header+index+name[25]; Packet.h:980-984).
    pub const CG_CHANGE_NAME: u8 = 106;
    ...
    /// `HEADER_GC_CHARACTER_CREATE_SUCCESS` (packet.h:118 — 8): create OK —
    /// `TPacketGCPlayerCreateSuccess` 73 B (header+slot+TSimplePlayer).
    pub const GC_CHARACTER_CREATE_SUCCESS: u8 = 8;
    /// `HEADER_GC_CHARACTER_CREATE_FAILURE` (packet.h:119 — 9): create
    /// fallido — `TPacketGCCreateFailure` 2 B (header+bType).
    pub const GC_CHARACTER_CREATE_FAILURE: u8 = 9;
    /// `HEADER_GC_CHARACTER_DELETE_SUCCESS` (packet.h:120 — 10): 2 B — el
    /// C++ los manda como DOS paquetes de 1 B (input_db.cpp:296-297) y el
    /// cliente los lee como UN struct (header+account_index).
    pub const GC_CHARACTER_DELETE_SUCCESS: u8 = 10;
    /// `HEADER_GC_CHARACTER_DELETE_WRONG_SOCIAL_ID` (packet.h:121 — 11):
    /// 1 B (fallo del delete: social_id mal o slot vacío).
    pub const GC_CHARACTER_DELETE_WRONG_SOCIAL_ID: u8 = 11;
    pub const GC_PHASE: u8 = 0xfd;
    pub const GC_HANDSHAKE: u8 = 0xff;
    pub const GC_EMPIRE: u8 = 90;
    pub const GC_LOGIN_KEY: u8 = 118;
    /// `HEADER_GC_CHANGE_NAME` (packet.h:208 — 107): renombre OK —
    /// `TPacketGCChangeName` 30 B (header+pid+name[25]).
    pub const GC_CHANGE_NAME: u8 = 107;
...
/// `TPacketCGEmpire` (2 B, `header::CG_EMPIRE` = 90).
/// Layout: `BYTE bHeader; BYTE bEmpire` (Packet.h:2100-2105 — el mismo shape
/// que el `TPacketGCEmpire` S→C; el cliente lo manda desde la ventana de
/// selección de imperio, `SendSelectEmpirePacket`).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketCGEmpire {
    pub header: u8,
    pub b_empire: u8,
}

impl TPacketCGEmpire {
    pub const SIZE: usize = 2;
    pub const HEADER: u8 = header::CG_EMPIRE;
...
/// `TPacketCGChangeName` (27 B, `header::CG_CHANGE_NAME` = 106).
/// Layout: `BYTE header; BYTE index; char name[25]` (Packet.h:980-984).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketCGChangeName {
    pub header: u8,
    pub index: u8,
    pub name: [u8; CHARACTER_NAME_MAX_LEN + 1],
}

impl TPacketCGChangeName {
    pub const SIZE: usize = 27;
    pub const HEADER: u8 = header::CG_CHANGE_NAME;
...
/// `TPacketGCPlayerCreateSuccess` (73 B, `header::GC_CHARACTER_CREATE_SUCCESS`
/// = 8). Layout: `BYTE bHeader; BYTE bAccountCharacterSlot; TSimplePlayer
/// player` (Packet.h:1160-1164 — el cliente actualiza el slot del select con
/// el TSimplePlayer recibido, `__RecvPlayerCreateSuccessPacket`).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketGCPlayerCreateSuccess {
    pub header: u8,
    pub b_account_character_slot: u8,
    pub player: TSimplePlayer,
}

impl TPacketGCPlayerCreateSuccess {
    /// 1 + 1 + 71 = 73 (packed).
    pub const SIZE: usize = 73;
    pub const HEADER: u8 = header::GC_CHARACTER_CREATE_SUCCESS;
    /// Offset del `TSimplePlayer` dentro del paquete.
    pub const PLAYER_OFFSET: usize = 2;

    pub fn new(b_account_character_slot: u8, player: TSimplePlayer) -> Self {
        Self {
            header: Self::HEADER,
            b_account_character_slot,
            player,
        }
    }
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            header: data[0],
            b_account_character_slot: data[1],
            player: TSimplePlayer::from_bytes(&data[Self::PLAYER_OFFSET..])?,
        })
    }
    ...
    pub fn to_bytes(&self) -> [u8; Self::SIZE] {
        let mut b = [0u8; Self::SIZE];
        b[0] = self.header;
        b[1] = self.b_account_character_slot;
        b[Self::PLAYER_OFFSET..].copy_from_slice(&self.player.to_bytes());
        b
    }
}

/// `TPacketGCCreateFailure` (2 B, `header::GC_CHARACTER_CREATE_FAILURE` = 9).
/// Layout: `BYTE header; BYTE bType` (Packet.h:1166-1170). `bType`: 0 =
/// genérico (nombre inválido/job/shape, `PlayerCreateFailure(d, 0)`),
/// 1 = nombre o slot YA EXISTEN (`PlayerCreateFailure(d, 1)` — el cliente
/// pinta "el nombre ya existe", OnCreateFailure(bType)).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketGCCreateFailure {
    pub header: u8,
    pub b_type: u8,
}

impl TPacketGCCreateFailure {
    pub const SIZE: usize = 2;
    pub const HEADER: u8 = header::GC_CHARACTER_CREATE_FAILURE;

    pub fn new(b_type: u8) -> Self {
        Self {
            header: Self::HEADER,
            b_type,
        }
    ...
    pub fn to_bytes(&self) -> [u8; Self::SIZE] {
        [self.header, self.b_type]
    }
}

/// `TPacketGCDestroyCharacterSuccess` (2 B,
/// `header::GC_CHARACTER_DELETE_SUCCESS` = 10). Layout: `BYTE header; BYTE
/// account_index` (Packet.h:1179-1183 — el C++ los manda como dos paquetes
/// de 1 B, input_db.cpp:296-297; el cliente los lee como UN struct de 2 B).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketGCDestroyCharacterSuccess {
    pub header: u8,
    pub account_index: u8,
}

impl TPacketGCDestroyCharacterSuccess {
    pub const SIZE: usize = 2;
    pub const HEADER: u8 = header::GC_CHARACTER_DELETE_SUCCESS;
    ...
    pub fn to_bytes(&self) -> [u8; Self::SIZE] {
        [self.header, self.account_index]
    }
}

/// `TPacketGCChangeName` (30 B, `header::GC_CHANGE_NAME` = 107).
/// Layout: `BYTE header; DWORD pid; char name[25]` (Packet.h:2375-2380 — el
/// cliente matchea el pid contra sus slots y actualiza el nombre,
/// `__RecvChangeName`).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(C)]
pub struct TPacketGCChangeName {
    pub header: u8,
    pub pid: u32,
    pub name: [u8; CHARACTER_NAME_MAX_LEN + 1],
}

impl TPacketGCChangeName {
    pub const SIZE: usize = 30;
    pub const HEADER: u8 = header::GC_CHANGE_NAME;
...
mod tests {
    ...
    /// Wire byte-exacto del create del cliente (34 B, header 4): layout
    /// `BYTE header; BYTE index; char name[25]; WORD job; BYTE shape, Con,
    /// Int, Str, Dex` (packet.h:534-545).
    #[test]
    fn cg_player_create_wire_layout() {
        let mut raw = [0u8; 34];
        raw[0] = 4;
        raw[1] = 2;
        raw[2..7].copy_from_slice(b"Ninja"); // name "Ninja" + NUL
        raw[27] = 1;
        raw[28] = 0; // job u16 LE = 1
        raw[29] = 0; // shape
        raw[30] = 30; // Con
        raw[31] = 20; // Int
        raw[32] = 40; // Str
        raw[33] = 10; // Dex
        let p = TPacketCGPlayerCreate::from_bytes(&raw).unwrap();
        assert_eq!(p.header, header::CG_CHARACTER_CREATE);
        assert_eq!(p.index, 2);
        assert_eq!(p.job, 1);
        assert_eq!((p.con, p.int_, p.str_, p.dex), (30, 20, 40, 10));
        assert_eq!(p.to_bytes(), raw, "round-trip byte-exacto");
    }

    /// Wire byte-exacto del delete del cliente (10 B, header 5): `BYTE
    /// header; BYTE index; char private_code[8]` (packet.h:528-532).
    #[test]
    fn cg_player_delete_wire_layout() {
        let mut raw = [0u8; 10];
        raw[0] = 5;
        raw[1] = 3;
        raw[2..10].copy_from_slice(b"12345678");
        let p = TPacketCGPlayerDelete::from_bytes(&raw).unwrap();
        assert_eq!(p.header, header::CG_CHARACTER_DELETE);
        assert_eq!(p.index, 3);
        assert_eq!(p.private_code, *b"12345678");
        assert_eq!(p.to_bytes(), raw, "round-trip byte-exacto");
    }

    /// `TPacketCGEmpire` C→S (2 B, header 90): `BYTE bHeader; BYTE bEmpire`.
    #[test]
    fn roundtrip_cg_empire() {
        let p = TPacketCGEmpire::new(3);
        assert_eq!(TPacketCGEmpire::SIZE, 2);
        assert_eq!(p.to_bytes(), [90, 3]);
        let p2 = TPacketCGEmpire::from_bytes(&p.to_bytes()).unwrap();
        assert_eq!(p, p2);
        assert!(TPacketCGEmpire::from_bytes(&[90]).is_err(), "1 B rechazado");
    }

    /// `TPacketCGChangeName` C→S (27 B, header 106): `BYTE header; BYTE
    /// index; char name[25]` (Packet.h:980-984).
    #[test]
    fn roundtrip_cg_change_name() {
        let p = TPacketCGChangeName::new(2, "Renamed");
        assert_eq!(TPacketCGChangeName::SIZE, 27);
        let b = p.to_bytes();
        assert_eq!(b[0], 106);
        assert_eq!(b[1], 2);
        assert_eq!(b[2..27], from_cstr::<25>("Renamed")[..]);
        let p2 = TPacketCGChangeName::from_bytes(&b).unwrap();
        assert_eq!(p, p2);
        assert_eq!(p2.to_bytes(), b);
    }

    /// `TPacketGCPlayerCreateSuccess` S→C (73 B, header 8): `BYTE header;
    /// BYTE bAccountCharacterSlot; TSimplePlayer player` (Packet.h:1160-1164).
    #[test]
    fn roundtrip_gc_player_create_success() {
        let player = TSimplePlayer {
            dw_id: 42,
        ...
        let p = TPacketGCPlayerCreateSuccess::new(2, player);
        assert_eq!(TPacketGCPlayerCreateSuccess::SIZE, 73, "1 + 1 + 71");
        let b = p.to_bytes();
        assert_eq!(b[0], 8, "header GC_CHARACTER_CREATE_SUCCESS");
        assert_eq!(b[1], 2, "slot");
        assert_eq!(b[2..], player.to_bytes(), "TSimplePlayer@2");
        let p2 = TPacketGCPlayerCreateSuccess::from_bytes(&b).unwrap();
        assert_eq!(p, p2);
        assert_eq!(p2.to_bytes(), b);
        assert!(TPacketGCPlayerCreateSuccess::from_bytes(&b[..72]).is_err());
    }

    /// `TPacketGCCreateFailure` S→C (2 B, header 9): `BYTE header; BYTE
    /// bType` (Packet.h:1166-1170).
    #[test]
    fn roundtrip_gc_create_failure() {
        let p = TPacketGCCreateFailure::new(1);
        assert_eq!(TPacketGCCreateFailure::SIZE, 2);
        assert_eq!(p.to_bytes(), [9, 1]);
        let p2 = TPacketGCCreateFailure::from_bytes(&p.to_bytes()).unwrap();
        assert_eq!(p, p2);
    }

    /// `TPacketGCDestroyCharacterSuccess` S→C (2 B, header 10): `BYTE
    /// header; BYTE account_index` (Packet.h:1179-1183).
    #[test]
    fn roundtrip_gc_destroy_character_success() {
        let p = TPacketGCDestroyCharacterSuccess::new(3);
        assert_eq!(TPacketGCDestroyCharacterSuccess::SIZE, 2);
        assert_eq!(p.to_bytes(), [10, 3]);
        let p2 = TPacketGCDestroyCharacterSuccess::from_bytes(&p.to_bytes()).unwrap();
        assert_eq!(p, p2);
    }

    /// `TPacketGCChangeName` S→C (30 B, header 107): `BYTE header; DWORD
    /// pid; char name[25]` (Packet.h:2375-2380).
    #[test]
    fn roundtrip_gc_change_name() {
        let p = TPacketGCChangeName::new(0xDEAD_BEEF, "Renamed");
        assert_eq!(TPacketGCChangeName::SIZE, 30);
        let b = p.to_bytes();
        assert_eq!(b[0], 107, "header GC_CHANGE_NAME");
        assert_eq!(
            u32::from_le_bytes([b[1], b[2], b[3], b[4]]),
            0xDEAD_BEEF,
            "pid@1"
        );
        assert_eq!(b[5..30], from_cstr::<25>("Renamed")[..], "name@5");
        let p2 = TPacketGCChangeName::from_bytes(&b).unwrap();
        assert_eq!(p, p2);
        assert_eq!(p2.to_bytes(), b);
    }
