...
async fn mark_login_handshake(addr: &str, handle: u32, random_key: u32) {
    ...
    assert_eq!(
        n, 0,
        "el canal cierra la conexión mark sin paquetes (EOF limpio)"
    );
}
...
async fn connect_login_449(addr: &str) -> Result<(Connection<TcpStream>, u32), String> {
    ...
    let empire_pkt = read_exact_size(&mut conn, 2)
        .await
        .map_err(|e| format!("GC_EMPIRE: {e}"))?;
    assert_eq!(empire_pkt[0], 0x5a, "header GC_EMPIRE");
    assert_eq!(empire_pkt[1], 3, "empire de la cuenta test (pi.empire)");

    // GC_PHASE(SELECT) (2 B).
    let sel_phase = read_exact_size(&mut conn, 2)
        .await
    ...
    let mut pkt = read_exact_size(&mut conn, 1)
        .await
        .map_err(|e| format!("hdr 449: {e}"))?;
    assert_eq!(
        pkt[0],
        TPacketGCLoginSuccess::HEADER,
        "header 0x20 (NEWSLOT)"
    );
    ...
    assert_eq!(
        success.players[3].dw_id, 0,
        "slot 3 vacio (pid4=0) -> zeroed"
    );
    ...
    assert_ne!(
        success.players[0].l_addr, 0,
        "lAddr = IP del server de juego (DirectEnter)"
    );
    assert_ne!(
        success.players[0].w_port, 0,
        "wPort = puerto del canal (DirectEnter)"
    );
...
async fn enter_and_read_spawns(
    ...
    let _ = read_exact_size(conn, 2)
        .await
        .map_err(|e| format!("loading: {e}"))?; // GC_PHASE(LOADING)
    let _ = read_exact_size(conn, 47)
        .await
        .map_err(|e| format!("main_char: {e}"))?; // MAIN_CHARACTER (15, 47 B)
    for _ in 0..36 {
        let _ = read_exact_size(conn, 4)
            .await
            .map_err(|e| format!("QS: {e}"))?;
    }
    let _ = read_exact_size(conn, 1021)
        .await
        .map_err(|e| format!("16: {e}"))?; // POINTS
    let _ = read_exact_size(conn, 1531)
        .await
        .map_err(|e| format!("76: {e}"))?; // SKILLS
    ...
    conn.send(&[10u8])
        .await
        .map_err(|e| format!("ENTERGAME: {e}"))?;
    // Cola del ENTERGAME (tamaños fijos).
    let _ = read_exact_size(conn, 37)
        .await
        .map_err(|e| format!("ADD: {e}"))?;
    let _ = read_exact_size(conn, 70)
        .await
        .map_err(|e| format!("INFO: {e}"))?;
    let _ = read_exact_size(conn, 2)
        .await
        .map_err(|e| format!("GAME: {e}"))?;
    let land_hdr = read_exact_size(conn, 3)
        .await
        .map_err(|e| format!("land: {e}"))?;
    let size = u16::from_le_bytes([land_hdr[1], land_hdr[2]]);
    let _ = read_exact_size(conn, (size - 3) as usize)
        .await
        .map_err(|e| format!("lands: {e}"))?;
    let _ = read_exact_size(conn, 5)
        .await
    ...
    loop {
        let hdr = read_exact_size(conn, 1)
            .await
            .map_err(|e| format!("spawn hdr: {e}"))?;
        match hdr[0] {
            1 => {
                let body = read_exact_size(conn, 36)
                    .await
                    .map_err(|e| format!("spawn add: {e}"))?;
            ...
            other => {
                ...
                eprintln!(
                    "fin de spawns ({} adds); siguiente header 0x{other:02x}",
                    spawns.len()
                );
                break;
            }
...
async fn full_login_select_entry_flow(addr: &str) -> Result<(), String> {
    ...
    let loading = read_exact_size(&mut conn, 2)
        .await
    ...
    let main_pkt = read_exact_size(&mut conn, 47)
        .await
        .map_err(|e| format!("main_char: {e}"))?;
    assert_eq!(main_pkt[0], 15, "header GC_MAIN_CHARACTER (Packet.h:160)");
    assert_eq!(
        u32::from_le_bytes([main_pkt[1], main_pkt[2], main_pkt[3], main_pkt[4]]),
        1,
        "dwVID"
    );
    let name_end = main_pkt[9..34].iter().position(|&b| b == 0).unwrap_or(25);
    assert_eq!(
        &main_pkt[9..9 + name_end],
        b"lkjsnlfknlsk",
        "szName del main character"
    );
    ...
    for i in 0..36u8 {
        let q = read_exact_size(&mut conn, 4)
            .await
            .map_err(|e| format!("QS {i}: {e}"))?;
    ...
    let points_pkt = read_exact_size(&mut conn, 1021)
        .await
        .map_err(|e| format!("16: {e}"))?;
    ...
    assert!(
        max_hp > 0,
        "POINT_MAX_HP > 0 (ComputePoints subset): {max_hp}"
    );
    let mov = i32::from_le_bytes([
        points_pkt[77],
        points_pkt[78],
        points_pkt[79],
        points_pkt[80],
    ...
    let skills_pkt = read_exact_size(&mut conn, 1531)
        .await
        .map_err(|e| format!("76: {e}"))?;
    ...
    conn.send(&version)
        .await
        .map_err(|e| format!("CG_CLIENT_VERSION2: {e}"))?;
    conn.send(&[10u8])
        .await
        .map_err(|e| format!("CG_ENTERGAME: {e}"))?;
    ...
    loop {
        let hdr = read_exact_size(&mut conn, 1)
            .await
            .map_err(|e| format!("cola: {e}"))?;
        match hdr[0] {
            21 => {
                // GC_ITEM_SET (51 B packed): window/cell/vnum spot.
                let body = read_exact_size(&mut conn, 50)
                    .await
                ...
                assert!(
                    u32::from_le_bytes([pkt[4], pkt[5], pkt[6], pkt[7]]) > 0,
                    "vnum del item"
                );
                items_seen += 1;
            }
            126 => {
                // GC_AFFECT_ADD (22 B).
                let body = read_exact_size(&mut conn, 21)
                    .await
                    .map_err(|e| format!("affect: {e}"))?;
                assert_eq!(body.len(), 21, "TPacketAffectElement");
                affects_seen += 1;
            }
            1 => {
                // ADD (37 B): vid = 1, type PC (6).
                let add_pkt = read_exact_size(&mut conn, 36)
                    .await
                    .map_err(|e| format!("ADD: {e}"))?;
                let mut pkt = hdr.clone();
            ...
            136 => {
                ...
                let info_pkt = read_exact_size(&mut conn, 69)
                    .await
                    .map_err(|e| format!("INFO: {e}"))?;
                let mut pkt = hdr.clone();
            ...
            0xfd => {
                // GC_PHASE(GAME) (2 B).
                let phase_byte = read_exact_size(&mut conn, 1)
                    .await
                    .map_err(|e| format!("GAME: {e}"))?;
            ...
            130 => {
                ...
                let size_bytes = read_exact_size(&mut conn, 2)
                    .await
                    .map_err(|e| format!("land size: {e}"))?;
                ...
                assert_eq!(
                    u32::from_le_bytes([land_body[0], land_body[1], land_body[2], land_body[3]]),
                    201u32,
                    "primer land del mapa 41"
                );
                landed = true;
            }
            106 => {
                // GC_TIME (106, 5 B): el reloj del server.
                let time_body = read_exact_size(&mut conn, 4)
                    .await
                    .map_err(|e| format!("TIME: {e}"))?;
                let server_time =
                    u32::from_le_bytes([time_body[0], time_body[1], time_body[2], time_body[3]]);
                assert!(
                    server_time > 1_700_000_000,
                    "get_global_time (unix now): {server_time}"
                );
            }
            121 => {
                // GC_CHANNEL (121, 2 B): el canal 1.
                let chan_byte = read_exact_size(&mut conn, 1)
                    .await
                    .map_err(|e| format!("CHANNEL: {e}"))?;
    ...
    conn.send(&move_pkt)
        .await
        .map_err(|e| format!("CG_MOVE: {e}"))?;
    let mut timesync = [0u8; 13];
    timesync[0] = 0xfc;
    conn.send(&timesync)
        .await
        .map_err(|e| format!("TIME_SYNC: {e}"))?;
    conn.send(&[0xfeu8])
        .await
    ...
    conn.send(&attack)
        .await
        .map_err(|e| format!("CG_ATTACK: {e}"))?;
    let pickup = [0x0fu8, 0, 0, 0, 0];
    conn.send(&pickup)
        .await
        .map_err(|e| format!("CG_ITEM_PICKUP: {e}"))?;
...
async fn channel_deployed_30003_full_flow() {
    let addr =
        std::env::var("CHANNEL_TEST_ADDR").unwrap_or_else(|_| "172.25.104.175:30003".to_string());
...
async fn channel_select_empty_slot_closes() {
    ...
    let result = async {
        ...
        let _ = read_exact_size(&mut conn, 2)
            .await
            .map_err(|e| format!("empire: {e}"))?;
        let _ = read_exact_size(&mut conn, 2)
            .await
            .map_err(|e| format!("phase: {e}"))?;
        ...
        tokio::time::timeout(Duration::from_millis(2000), async {
            loop {
                ...
                let size = match hdr[0] {
                    ...
                    _ => {
                        return Err(format!(
                            "header inesperado en el ENTERGAME: 0x{:02x}",
                            hdr[0]
                        ));
                    }
                };
                let _ = read_exact_size(&mut conn, size)
                    .await
                    .map_err(|e| format!("entergame body: {e}"))?;
            }
            Ok::<(), String>(())
        })
        .await
