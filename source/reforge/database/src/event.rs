...
const LIST_SQL: &str =
    "SELECT id, name, start_ts, end_ts, trigger_type, trigger_value FROM player.event ORDER BY id";
...
impl EventRepo {
    ...
    pub async fn list(&self) -> Result<Vec<EventRow>, String> {
        ...
        let rows = c
            .query(LIST_SQL, &[])
            .await
            .map_err(|e| pg_err("EVENT_LIST", &e))?;
        rows.iter()
            .map(|r| {
                Ok(EventRow {
                    id: r.try_get(0).map_err(|e| format!("col0 id:{e}"))?,
                    name: r.try_get(1).map_err(|e| format!("col1 name:{e}"))?,
                    start_ts: r.try_get(2).map_err(|e| format!("col2:{e}"))?,
                    end_ts: r.try_get(3).map_err(|e| format!("col3:{e}"))?,
                    trigger_type: r.try_get(4).map_err(|e| format!("col4:{e}"))?,
                    trigger_value: r.try_get(5).map_err(|e| format!("col5:{e}"))?,
                })
            })
            .collect()
    }
    pub async fn create(&self, r: &EventRow) -> Result<i64, String> {
        ...
        let row = c
            ...
            .await
            .map_err(|e| pg_err("EVENT_CREATE", &e))?;
    ...
    pub async fn delete(&self, id: i64) -> Result<u64, String> {
        ...
        c.execute(DELETE_SQL, &[&id])
            .await
            .map_err(|e| pg_err("EVENT_DELETE", &e))
    }
    pub fn create_mutated(&self, b: &Batcher, r: &EventRow) {
        b.push(event_mutation(r));
    }
}
...
pub(crate) fn event_mutation(r: &EventRow) -> Mutation {
    Mutation::new(
        INSERT_SQL,
        vec![
            Param::Text(r.name.clone()),
            Param::Int(r.start_ts),
            Param::Int(r.end_ts),
            Param::Int(i64::from(r.trigger_type)),
            Param::Int(i64::from(r.trigger_value)),
        ],
    )
}
...
mod tests {
    ...
    fn insert_sql_is_id_lane_returning() {
        ...
        assert!(
            !INSERT_SQL.contains("ON CONFLICT"),
            "insert id lane sin upsert"
        );
    }
    #[test]
    fn delete_sql_shape() {
        assert_eq!(DELETE_SQL, "DELETE FROM player.event WHERE id = $1");
    }
    #[test]
