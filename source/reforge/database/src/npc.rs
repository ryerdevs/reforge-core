...
pub struct MobRow {
    ...
    pub gold_max: i32,
    // ---- F5.3 (drops): el drop primario del mob. ----
    /// `drop_item` (integer) — el item que dropea el mob al morir
    /// (→ `item_proto.vnum`; 0 = sin drop). El C++ usa además las tablas
    /// `etc_drop_item.txt`/`common_drop_item.txt` (por nombre CP949 — TRAP
    ...
    pub move_speed: i32,
    // ---- C29: la velocidad de ataque del mob (cooldown del golpe). ----
    /// `attack_speed` (smallint, default 100) — el `sAttackSpeed` del
    /// mob_proto → `POINT_ATT_SPEED` del C++. El cooldown del golpe =
    /// `CalculateDuration(attack_speed, 2000)` ms (char_state.cpp:1005-1012).
    pub attack_speed: i32,
    // ---- F5.3 (NPC AI): el daño del ataque del mob. ----
    /// `damage_min` (smallint) — el daño mínimo del ataque del mob
    /// (parity `mob_proto.damage_min`; el C++ sortea `number(min,max)`).
    pub damage_min: i32,
    ...
    pub rank: i32,
    // ---- AIFLAGs de combate del mob (sp_* — puntos de activación HP%). ----
    /// `sp_berserk` (smallint, 0..40) — HP% bajo el cual el mob hace daño
    /// ×2 (`IsBerserker` + `GetHPPct() < bBerserkPoint` → `SetBerserk`,
    /// char_state.cpp:1016-1018; `GetMobDamageMultiply` ×2, char.cpp:1963).
    pub sp_berserk: i32,
    /// `sp_stoneskin` (smallint, 0..40) — HP% bajo el cual el daño recibido
    /// se divide por 2 (`IsStoneSkinner` + `GetHPPct() < bStoneSkinPoint` →
    /// `dam /= 2`, char_battle.cpp:2082-2084).
    pub sp_stoneskin: i32,
    /// `sp_godspeed` (smallint, 0..40) — HP% bajo el cual el ataque del mob
    /// sube a 250 (`IsGodSpeeder` + `GetHPPct() < bGodSpeedPoint` →
    /// `POINT_ATT_SPEED = 250`, char_state.cpp:1021-1023 + char.cpp:6867).
    pub sp_godspeed: i32,
}
...
attack_speed, damage_min, damage_max, aggressive_sight, rank, sp_berserk, \
sp_stoneskin, sp_godspeed \
...
mod tests {
    ...
    /// C29: `attack_speed` (posición 18, entre move_speed y damage_min — el
    /// cooldown del golpe del mob). La query batch comparte el MISMO orden
    /// (mismo mapeo).
    #[test]
