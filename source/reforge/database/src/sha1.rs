...
//! `server_realms/src/sha1.rs`; ADR-0008 defers the unification into a common
//! crate until a third consumer appears (do not touch the auth copy).
//!
//! Use in F3: MySQL hash `mysql5_password` = `"*" + UPPER(hex(SHA1(SHA1(pw))))`
//! (parity `utils.cpp:30-58`; the stored value in `account.password` uses this
//! format, `db.cpp:340` strcmp).

/// SHA-1 de `data` (20 bytes, big-endian).
pub fn digest(data: &[u8]) -> [u8; 20] {
    const K: [u32; 4] = [0x5A82_7999, 0x6ED9_EBA1, 0x8F1B_BCDC, 0xCA62_C1D6];

    // Padding (FIPS 180-1 Â§5.1.1): 0x80, ceros hasta mÃºltiplo de 64 con 8 bytes
...
mod tests {
    ...
    /// Vector cruzado con .NET SHA1 (sesiÃ³n 2026-08-10):
