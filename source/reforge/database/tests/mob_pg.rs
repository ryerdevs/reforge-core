...
use database::npc::{MobRepo, wire_b_type};
...
async fn mob_repo_loads_map41_vnums() {
    ...
    for vnum in [20340i64, 20001, 9001, 11004, 101, 191, 151, 8001, 5001] {
        ...
        assert!(
            !m.locale_name.is_empty(),
            "locale_name presente para {vnum}"
        );
...
async fn mob_repo_batch_load_map41_vnums() {
    ...
    let vnums: Vec<i64> = vec![
        ...
        9006, 9007, 9008, 9009, 9012, 10005, 11004, 11005, 20001, 20002, 20003, 20005, 20006,
        20008, 20009, 20011, 20016, 20018, 20023, 20025, 20029, 20030, 20041, 20047, 20049, 20050,
        20051, 20052, 20084, 20086, 20087, 20090, 20095, 20340, 20341, 20342, 20343, 20344, 20345,
        20346, 20347, 20349, 20354, 20355, 20357, 20358, 60003,
    ];
    let map = repo.load_by_vnums(&vnums).await.expect("batch");
    assert_eq!(
        map.len(),
        vnums.len(),
        "todos los vnums del mapa 41 existen"
    );
    for v in &vnums {
