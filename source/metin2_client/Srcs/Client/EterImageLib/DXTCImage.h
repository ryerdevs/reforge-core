...
class CDXTCImage
{
	...
	public:
		...
		bool LoadFromMemory(const BYTE* c_pbMap, int iSize); // @fixme042
		bool LoadHeaderFromMemory(const BYTE* c_pbMap, int iSize); // @fixme042
		bool Copy(int miplevel, BYTE * pbDest, long lDestPitch) const;
		...
		void DecompressARGB(int miplevel, DWORD * pdwDest) const;

		VOID DecodePixelFormat(CHAR* strPixelFormat, XDDPIXELFORMAT* pddpf);

		void Unextract(BYTE * pbDest, int iWidth, int iHeight, int iPitch) const;
};
