...
CImage::CImage(CImage & image)
{
	...
	const DWORD * pdwSrc = image.GetBasePointer();
...
void CImage::PutImage(int x, int y, CImage* pImage) const
{
	...
	const int len = pImage->GetWidth() * sizeof(DWORD);
...
DWORD* CImage::GetBasePointer() const
{
	assert(m_pdwColors != nullptr);
	return m_pdwColors;
}

DWORD* CImage::GetLinePointer(int line) const
{
	assert(m_pdwColors != nullptr);
...
void CImage::Clear(DWORD color) const
{
	assert(m_pdwColors != nullptr);
...
{
	const auto swap = new DWORD[m_width * m_height];
