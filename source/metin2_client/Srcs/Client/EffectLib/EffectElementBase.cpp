...
void CEffectElementBase::GetPosition(float fTime, D3DXVECTOR3 & rPosition)
{
	...
	const TEffectPosition & rEffectPosition = *result;
	auto rPrev = result;
	...
	const TEffectPosition & rPrevEffectPosition = *rPrev;
	const int iMovingType = rPrevEffectPosition.m_iMovingType;

	if (MOVING_TYPE_DIRECT == iMovingType)
	{
		const float Head = fabs(rEffectPosition.m_fTime - fTime) / fabs(rEffectPosition.m_fTime - rPrevEffectPosition.m_fTime);
		const float Tail = 1.0f - fabs(rEffectPosition.m_fTime - fTime) / fabs(rEffectPosition.m_fTime - rPrevEffectPosition.m_fTime);
	...
	{
		const float ft = (fTime - rPrevEffectPosition.m_fTime) / (rEffectPosition.m_fTime - rPrevEffectPosition.m_fTime);
