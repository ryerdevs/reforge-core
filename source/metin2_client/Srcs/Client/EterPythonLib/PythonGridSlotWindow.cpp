...
void CGridSlotWindow::OnRenderPickingSlot()
{
	...
	{
		const DWORD dwSlotNumber = UI::CWindowManager::Instance().GetAttachingSlotNumber();
		const DWORD dwItemIndex = UI::CWindowManager::Instance().GetAttachingIndex();

		if (m_isUseMode)
		{
			const TSlot * pSlot = *SlotList.begin();
			TSlot * pCenterSlot;
...
BOOL CGridSlotWindow::GetPickedSlotList(int iWidth, int iHeight, std::list<TSlot*> * pSlotPointerList)
{
	...
	if (GetPickedGridSlotPosition(lx, ly, &ix, &iy))
	{
		...
		{
			const int ixTemporary = DWORD(ixEnd) - m_dwxCount + 1;
			ixStart -= ixTemporary;
			ixEnd -= ixTemporary;
		}
		...
		{
			const int iyTemporary = DWORD(iyEnd) - m_dwyCount + 1;
			iyStart -= iyTemporary;
			iyEnd -= iyTemporary;
		}
		...
		{
			const TSlot * pMainSlot = *pSlotPointerList->begin();
...
BOOL CGridSlotWindow::GetGridSlotPointer(int ix, int iy, TSlot ** ppSlot) const
{
	const DWORD dwSlotIndex = ix + iy*m_dwxCount;
...
BOOL CGridSlotWindow::GetPickedGridSlotPosition(int ixLocal, int iyLocal, int * pix, int * piy) const
{
...
void CGridSlotWindow::ArrangeGridSlot(DWORD dwStartIndex, DWORD dwxCount, DWORD dwyCount, int ixSlotSize, int iySlotSize, int ixTemporarySize, int iyTemporarySize)
{
	...
	{
		const DWORD dwIndex = dwStartIndex + x + y * dwxCount;
		const int ixPosition = x * (ixSlotSize + ixTemporarySize);
		const int iyPosition = y * (iySlotSize + iyTemporarySize);
	...
	const int iWidth = dwxCount * (ixSlotSize + ixTemporarySize);
	const int iHeight = dwyCount * (iySlotSize + iyTemporarySize);
...
BOOL CGridSlotWindow::CheckMoving(DWORD dwSlotNumber, DWORD dwItemIndex, const std::list<TSlot*> & c_rSlotList) const
{
