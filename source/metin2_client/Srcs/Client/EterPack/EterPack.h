...
namespace eterpack
{
	constexpr DWORD	c_PackCC = MAKEFOURCC('E', 'P', 'K', 'D');
	constexpr DWORD	c_IndexCC = MAKEFOURCC('E', 'P', 'K', 'D');
	constexpr DWORD c_Version = 2;
								// FourCC + Version + m_indexCount
	constexpr DWORD c_HeaderSize = sizeof(DWORD) + sizeof(DWORD) + sizeof(long);
};
...
class CEterPack
{
	...
	private:
		...
		int					GetFreeBlockIndex(long size) const;
		void				PushFreeIndex(TEterPackIndex * index);

		bool				CreateDataFile() const;
		long				GetNewDataPosition(CFileBase& file) const;
		bool				ReadData(CFileBase& file, TEterPackIndex * index, LPVOID data, long maxsize) const;
		bool				WriteData(CFileBase& file, TEterPackIndex * index, LPCVOID data) const;
		bool				WriteNewData(CFileBase& file, TEterPackIndex * index, LPCVOID data) const;
	...
	private:
		void				__CreateFileNameKey_Panama(const char * filename, BYTE * key, unsigned int keySize) const;
