...
class CSoundManager3D : public CSoundBase
{
	...
	public:
		...
		void				SetListenerDirection(float fxDir, float fyDir, float fzDir, float fxUp, float fyUp, float fzUp) const;
		void				SetListenerPosition(float x, float y, float z) const;
		void				SetListenerVelocity(float fDistanceX, float fDistanceY, float fDistanceZ, float fNagnitude) const;

		void				Lock(int iIndex);
		void				Unlock(int iIndex);

	protected:
		bool				IsValidInstanceIndex(int iIndex) const;
