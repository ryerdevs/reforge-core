...
public:
	...
	void SetDirection(float fxDir, float fyDir, float fzDir, float fxUp, float fyUp, float fzUp) const;
	void Update();

	float GetSoundScale() const;
	...
	// Sound
	void PlaySound2D(const char * c_szFileName) const;
	void PlaySound3D(float fx, float fy, float fz, const char * c_szFileName, int iPlayCount = 1) const;
	void StopSound3D(int iIndex) const;
	int  PlayAmbienceSound3D(float fx, float fy, float fz, const char * c_szFileName, int iPlayCount = 1) const;
	void PlayCharacterSound3D(float fx, float fy, float fz, const char * c_szFileName, BOOL bCheckFrequency = FALSE);
	void SetSoundVolume3D(int iIndex, float fVolume) const;
	void StopAllSound3D();
	...
	// Sound Node
	void UpdateSoundData(DWORD dwcurFrame, const NSound::TSoundDataVector * c_pSoundDataVector) const;
	void UpdateSoundData(float fx, float fy, float fz, DWORD dwcurFrame, const NSound::TSoundDataVector * c_pSoundDataVector) const;
	...
	void UpdateSoundInstance(DWORD dwcurFrame, const NSound::TSoundInstanceVector * c_pSoundInstanceVector) const;
...
protected:
	...
	BOOL GetMusicIndex(const char * c_szFileName, DWORD * pdwIndex) const;

protected:
	float __ConvertGradeVolumeToApplyVolume(int nVolumeGrade) const;
	float __ConvertRatioVolumeToApplyVolume(float fVolumeRatio) const;
	void __SetMusicVolume(float fVolume);
	BOOL GetSoundInstance2D(const char * c_szSoundFileName, ISoundInstance ** ppInstance) const;
	BOOL GetSoundInstance3D(const char * c_szFileName, ISoundInstance ** ppInstance) const;
