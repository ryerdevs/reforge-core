												 ...
												 float &parameter)
{
	...
	const float D = D3DXVec3LengthSq(&L);
...
{
	float temp = 0.f;
	constexpr float epsilon = MY_EPSILON;
	constexpr float epsilon_squared = MY_EPSILON*MY_EPSILON;

	// Compute parameters from Equations (1) and (2) in the text
	const D3DXVECTOR3 La = A2-A1;
	const D3DXVECTOR3 Lb = B2-B1;
	// From Equation (15)
                           ...
                           float &PointOnSegBy, float &PointOnSegBz)
{
	...
	// Compute parameters from Equations (1) and (2) in the text
	const float Lax = A2x - A1x;
	const float Lay = A2y - A1y;
	const float Laz = A2z - A1z;
	const float Lbx = B2x - B1x;
	const float Lby = B2y - B1y;
	const float Lbz = B2z - B1z;
	// From Equation (15)
	...
	{
		// Compute more parameters from Equation (3) in the text.
		const float ABx = B1x - A1x;
		const float ABy = B1y - A1y;
		const float ABz = B1z - A1z;
...
{
	// Line/Segment is degenerate --- special case #1
	const float D = Lx * Lx + Ly * Ly + Lz * Lz;
	...
	const float ABx = Bx - A1x;
	const float ABy = By - A1y;
	const float ABz = Bz - A1z;
