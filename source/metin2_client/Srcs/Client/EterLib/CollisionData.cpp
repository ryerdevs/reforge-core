...
constexpr float gc_fReduceMove = 0.5f;
...
{
	const auto val = (s.v3Position-m_attribute.v3Position);
	if (D3DXVec3LengthSq(&val)>=(s.fRadius+m_attribute.fRadius)*(m_attribute.fRadius+s.fRadius))
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	D3DXVECTOR3 c;
	const auto val1 = (s.v3Position-s.v3LastPosition);
	const auto val2 = D3DXVECTOR3(0.0f,0.0f,1.0f);
	D3DXVec3Cross(&c, &val1, &val2);

	const auto val3 = (s.v3Position-m_attribute.v3Position);
	const float sum = - D3DXVec3Dot(&c,&val3);
	const auto val4 = (s.v3Position-m_attribute.v3Position);
	const float mul = (s.fRadius+m_attribute.fRadius)*(s.fRadius+m_attribute.fRadius)-D3DXVec3LengthSq(&val4);

	if (sum*sum-4*mul<=0)
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	const float sq = sqrt(sum*sum-4*mul);
...
{
	const D3DXVECTOR3 v3SpherePosition = s.v3Position - m_attribute.v3Position;
	const D3DXVECTOR3 v3SphereLastPosition = s.v3LastPosition - m_attribute.v3Position;

	const float fPosition1 = D3DXVec3Dot(&m_attribute.v3Normal, &v3SpherePosition);
	const float fPosition2 = D3DXVec3Dot(&m_attribute.v3Normal, &v3SphereLastPosition);

	if (fPosition1 >0.0f && fPosition2 < 0.0f  || fPosition1 <0.0f && fPosition2 >0.0f
		|| (fPosition1) <= s.fRadius && fPosition1 >= -s.fRadius)
	{
		const D3DXVECTOR3 v3QuadPosition1 = s.v3Position - m_attribute.v3QuadPosition[0];
		const D3DXVECTOR3 v3QuadPosition2 = s.v3Position - m_attribute.v3QuadPosition[3];
		...
		if (D3DXVec3Dot(&v3QuadPosition1, &m_attribute.v3InsideVector[0]) > - s.fRadius/*0.0f*/)
			if (D3DXVec3Dot(&v3QuadPosition1, &m_attribute.v3InsideVector[1]) > -s.fRadius/*0.0f*/)
				if (D3DXVec3Dot(&v3QuadPosition2, &m_attribute.v3InsideVector[2]) > - s.fRadius/*0.0f*/)
					...
					{
						const auto val1 = (s.v3Position - m_attribute.v3Position);
						const auto val2 = (s.v3LastPosition - m_attribute.v3Position);
...
{
	const D3DXVECTOR3 advance = s.v3Position-s.v3LastPosition;

	const float d = D3DXVec3Dot(&m_attribute.v3Normal, &advance);
	if (d>=-0.0001 && d<=0.0001)
		return D3DXVECTOR3(0.0f,0.0f,0.0f);
	const auto val = (s.v3Position-m_attribute.v3Position);
	const float t= - D3DXVec3Dot(&m_attribute.v3Normal, &val)/d;
...
bool CCylinderCollisionInstance::OnMovementCollisionDynamicSphere(const CDynamicSphereInstance & s) const
{
	...
	const D3DXVECTOR3 v3Distance = s.v3Position - s.v3LastPosition;
	const float fDistance = D3DXVec3Length(&v3Distance);
	...
	if (fDistance >= s.fRadius*2.0f)
	{
		...
		const int iStep = fDistance / s.fRadius*2.0f;
		const D3DXVECTOR3 v3Step = v3Distance / float(iStep);
...
D3DXVECTOR3 CCylinderCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	const auto val4 = (s.v3Position-v3Position);
	...
	advance.z = 0;
	const auto val3 = D3DXVECTOR3(0.0f,0.0f,1.0f);
	D3DXVec3Cross(&c, &advance, &val3);

	const auto val2 = (s.v3Position-v3Position);
	const float sum = - D3DXVec3Dot(&c,&val2);
	const auto val = (s.v3Position-v3Position);
	const float mul = (s.fRadius+m_attribute.fRadius)*(s.fRadius+m_attribute.fRadius)-D3DXVec3LengthSq(&val);
...
D3DXVECTOR3 CAABBCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	*/
	...
	const auto val = (v3Temp - s.v3Position);
...
{
	const D3DXVECTOR3 v3Center = 0.5f * (m_attribute.v3Min + m_attribute.v3Max);
