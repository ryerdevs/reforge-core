...
{
	const auto pWnd = (CMSWindow *) GetWindowLong(hWnd, GWL_USERDATA);
...
bool CMSWindow::IsVisible() const
{
	return m_isVisible;
}

bool CMSWindow::IsActive() const
{
	return m_isActive;
}

HINSTANCE CMSWindow::GetInstance() const
{
	return ms_hInstance;
}

HWND CMSWindow::GetWindowHandle() const
{
	return m_hWnd;
}

int	CMSWindow::GetScreenWidth() const
{
	return GetSystemMetrics(SM_CXSCREEN);
}

int	CMSWindow::GetScreenHeight() const
{
	return GetSystemMetrics(SM_CYSCREEN);
}

void CMSWindow::GetWindowRect(RECT* prc) const
{
	::GetWindowRect(m_hWnd, prc);
}

void CMSWindow::GetClientRect(RECT* prc) const
{
	::GetClientRect(m_hWnd, prc);
}

void CMSWindow::GetMousePosition(POINT* ppt) const
{
	GetCursorPos(ppt);
	ScreenToClient(m_hWnd, ppt);
}

void CMSWindow::SetPosition(int x, int y) const
{
	SetWindowPos(m_hWnd, nullptr, x, y, 0, 0, SWP_NOZORDER|SWP_NOSIZE);
}

void CMSWindow::SetCenterPosition() const
{
	RECT rc;
...
void CMSWindow::SetText(const char* c_szText) const
{
	SetWindowText(m_hWnd, c_szText);
}

void CMSWindow::SetSize(int width, int height) const
{
	SetWindowPos(m_hWnd, nullptr, 0, 0, width, height, SWP_NOZORDER|SWP_NOMOVE);
}

const char * CMSWindow::RegisterWindowClass(DWORD style, int brush, WNDPROC pfnWndProc, HICON hIcon, int iCursorResource) const
{
	char szClassName[1024];
	sprintf(szClassName, "eter - s%x:b%x:p:%x", style, brush, (DWORD) pfnWndProc);

	const auto f = ms_stWCSet.find((char*) szClassName);
