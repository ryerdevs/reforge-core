...
class CGraphicTextInstance
{
	...
	public:
		...
		void GetTextSize(int* pRetWidth, int* pRetHeight) const;
		const std::string& GetValueStringReference();
		WORD GetTextLineCount();

		int PixelPositionToCharacterPosition(int iPixelPosition) const;
		int GetHorizontalAlign() const;
#ifdef ENABLE_NEW_EVENT_STRUCT
		WORD GetLineHeight() const { return m_textHeight; }
		WORD GetLineWidth() const { return m_textWidth; }
#endif

	protected:
		void __Initialize();
		int  __DrawCharacter(CGraphicFontTexture * pFontTexture, WORD codePage, wchar_t text, DWORD dwColor);
		void __GetTextPos(DWORD index, float* x, float* y) const;
