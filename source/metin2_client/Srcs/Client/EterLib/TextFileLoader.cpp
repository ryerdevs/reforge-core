...
{
	const DWORD dwGroupNameKey=GenNameKey(c_rstGroupName.c_str(), c_rstGroupName.length());
...
bool CTextFileLoader::SGroupNode::IsGroupNameKey(DWORD dwGroupNameKey) const
{
...
CTextFileLoader* CTextFileLoader::Cache(const char* c_szFileName)
{
	...
	if (ms_kMap_dwNameKey_pkTextFileLoader.end()!=f)
	{
		if (!ms_isCacheMode)
		{
			...
			const auto pkNewTextFileLoader=new CTextFileLoader;
...
const char * CTextFileLoader::GetFileName() const
{
	return m_strFileName.c_str();
}

bool CTextFileLoader::IsEmpty() const
{
...
DWORD CTextFileLoader::GetChildNodeCount() const
{
	if (!m_pcurNode)
	{
...
BOOL CTextFileLoader::SetChildNode(const char * c_szKey)
{
	...
	const DWORD dwKey=SGroupNode::GenNameKey(c_szKey, strlen(c_szKey));
...
BOOL CTextFileLoader::GetCurrentNodeName(std::string * pstrName) const
{
	if (!m_pcurNode)
		return FALSE;
	if (nullptr == m_pcurNode->pParentNode)
...
BOOL CTextFileLoader::IsToken(const std::string & c_rstrKey) const
{
	if (!m_pcurNode)
	{
		assert(!"Node to access has not set!");
...
BOOL CTextFileLoader::GetTokenVector(const std::string & c_rstrKey, CTokenVector ** ppTokenVector) const
{
	if (!m_pcurNode)
	{
		assert(!"Node to access has not set!");
