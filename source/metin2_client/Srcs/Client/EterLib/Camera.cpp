...
constexpr float c_fDefaultResistance = 0.5f;
...
float CCamera::GetTargetHeight() const
{
	return m_fTarget_;
}
...
bool CCamera::EndDrag()
{
	...
	const float fSum=sqrt(m_fPitchSum*m_fPitchSum+m_fRollSum*m_fRollSum);
...
bool CCamera::Drag(int nMouseX, int nMouseY, LPPOINT lpReturnPoint)
{
	...
	const long lMouseX = nMouseX;
	const long lMouseY = nMouseY;
...
void CCamera::SetViewMatrix()
{
	...
	const D3DXVECTOR3 v3CenterRay = -m_v3View;
 	CalculateRoll();
	...
	const auto val5 = (v3CenterRay - m_fTerrainCollisionRadius * m_v3Up);
...
void CCamera::RotateEyeAroundTarget(float fPitchDegree, float fRollDegree)
{
	...
	const D3DXVECTOR3 v3Temp = m_v3Eye - m_v3Target;
...
void CCamera::CalculateRoll()
{
	...
	const auto val2 = D3DXVECTOR2(0.0f, 1.0f);
	...
	fDot = acosf(fDot);
	fDot *= (180.0f / D3DX_PI);
	const auto val = D3DXVECTOR2(0.0f, 1.0f);
	const float fCross = D3DXVec2CCW (&v2ViewXY, &val);
	if ( 0 > fCross)
	{
		fDot = -fDot;
	}
...
{
	const auto itor = m_CameraMap.find(ucCameraNum);
