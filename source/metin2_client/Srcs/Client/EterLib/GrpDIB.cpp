...
HDC	CGraphicDib::GetDCHandle() const
{
	return m_hDC;
}

void CGraphicDib::SetBkMode(int iBkMode) const
{
	::SetBkMode(m_hDC, iBkMode);
}

void CGraphicDib::TextOut(int ix, int iy, const char * c_szText) const
{
	::SetBkColor(m_hDC, 0);
	::TextOut(m_hDC, ix, iy, c_szText, strlen(c_szText));
}

void CGraphicDib::Put(HDC hDC, int x, int y) const
{
	SetDIBitsToDevice(
	  hDC,
	  x,
...
void* CGraphicDib::GetPointer() const
{
	return m_pvBuf;
}

int CGraphicDib::GetWidth() const
{
	return m_width;
}

int CGraphicDib::GetHeight() const
{
	return m_height;
}
