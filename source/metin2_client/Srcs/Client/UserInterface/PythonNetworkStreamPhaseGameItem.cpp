...
bool CPythonNetworkStream::SendSafeBoxMoneyPacket(BYTE byState, DWORD dwMoney) const
{
...
bool CPythonNetworkStream::RecvItemGroundAddPacket()
{
	...
	CPythonItem::Instance().CreateItem(packet_item_ground_add.dwVID,
									   ...
									   , packet_item_ground_add.attrs
									   , packet_item_ground_add.sockets
									   #endif
	);
	return true;
}
...
{
	IAbstractPlayer& rkPlayer=IAbstractPlayer::GetSingleton();
	const DWORD dwItemID=rkPlayer.GetItemIndex(uSlotPos);

	const CPythonItem& rkItem=CPythonItem::Instance();
...
void CPythonNetworkStream::__PlaySafeBoxItemDropSound(UINT uSlotPos) const
{
	DWORD dwItemID;
	const CPythonSafeBox& rkSafeBox=CPythonSafeBox::Instance();
...
void CPythonNetworkStream::__PlayMallItemDropSound(UINT uSlotPos) const
{
	DWORD dwItemID;
	const CPythonSafeBox& rkSafeBox=CPythonSafeBox::Instance();
...
bool CPythonNetworkStream::RecvSpecialEffect()
{
	...
	switch (kSpecialEffect.type)
	{
		...
		case SE_SPEEDUP_GREEN:
			...
			//bPlayPotionSound = true; // @fixme040 commented line
			break;
		case SE_DXUP_PURPLE:
