...
constexpr float c_fStartDisappearingTime = 5.0f;
constexpr int c_iMaxLineCount = 5;
...
void CPythonChat::SetChatColor(UINT eType, UINT r, UINT g, UINT b)
{
	...
	const DWORD dwColor=(0xff000000)|(r<<16)|(g<<8)|(b);
...
void CPythonChat::UpdateViewMode(DWORD dwID)
{
	...
	const float fcurTime = rApp.GetGlobalTime();
	...
	for (itor = pLineList->begin(); itor != pLineList->end();)
	{
		...
		const float fElapsedTime = (fcurTime - pChatLine->fAppendedTime);
...
void CPythonChat::Update(DWORD dwID)
{
	...
	const DWORD dwcurTime = CTimer::Instance().GetCurrentMillisecond();
...
{
	const TChatLineList * pLineList = GetChatLineListPtr(dwID);
	if (!pLineList)
		return 0;
...
{
	const auto itor = m_ChatSetMap.find(dwID);
...
void CPythonChat::ArrangeShowingChat(DWORD dwID)
{
	...
	const int icurLineCount = TempChatLineDeque.size();
	const int iVisibleLineCount = min(icurLineCount, (pChatSet->m_iHeight + pChatSet->m_iStep) / pChatSet->m_iStep);
	const int iEndLine = iVisibleLineCount + int(float(icurLineCount - iVisibleLineCount - 1) * pChatSet->m_fEndPos);
...
DWORD CPythonChat::GetChatColor(int iType) const
{
...
{
	const auto itor = m_WhisperMap.find(c_szName);
...
void CWhisper::Render(float fx, float fy) const
{
