...
{
	const int race = CPythonPlayer::Instance().GetRace();
	const int job = RaceToJob(race);
...
PyObject * playerGetSkillCoolTime(PyObject* poSelf, PyObject* poArgs)
{
	...
	const float fCoolTime = CPythonPlayer::Instance().GetSkillCoolTime(iSlotIndex);
	const float fElapsedCoolTime = CPythonPlayer::Instance().GetSkillElapsedCoolTime(iSlotIndex);
...
{
	const DWORD dwEXP = CPythonPlayer::Instance().GetStatus(POINT_EXP);
...
PyObject * playerGetStatus(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		const CInstanceBase * pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
...
PyObject * playerGetGuildName(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (pInstance)
	{
		const DWORD dwID = pInstance->GetGuildID();
		std::string strName;
	...
	case 1:
		{
			...
			const int ItemIndex = CPythonPlayer::Instance().GetItemIndex(TItemPos (INVENTORY, iSlotIndex));
	...
	case 2:
		{
			...
			const int ItemIndex = CPythonPlayer::Instance().GetItemIndex(Cell);
			return Py_BuildValue("i", ItemIndex);
		}
	default:
		return Py_BuildException();
	...
	case 1:
		{
			...
			const DWORD flags = CPythonPlayer::Instance().GetItemFlags(TItemPos(INVENTORY, iSlotIndex));
	...
	case 2:
		{
			...
			const DWORD flags = CPythonPlayer::Instance().GetItemFlags(Cell);
			return Py_BuildValue("i", flags);
		}
	default:
		return Py_BuildException();
	...
	case 1:
		{
			...
			const int ItemNum = CPythonPlayer::Instance().GetItemCount(TItemPos (INVENTORY, iSlotIndex));
	...
	case 2:
		{
			...
			const int ItemNum = CPythonPlayer::Instance().GetItemCount(Cell);
...
PyObject * playerGetItemCountByVnum(PyObject* poSelf, PyObject* poArgs)
{
	...
	const int ItemNum = CPythonPlayer::Instance().GetItemCountByVnum(ivnum);
	return Py_BuildValue("i", ItemNum);
}
...
PyObject * playerGetItemMetinSocket(PyObject* poSelf, PyObject* poArgs)
{
	...
	const int nMetinSocketValue = CPythonPlayer::Instance().GetItemMetinSocket(Cell, iMetinSocketIndex);
...
PyObject * playerisItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	const char Flag = CPythonPlayer::Instance().IsItem(TItemPos(INVENTORY, iSlotIndex));
...
PyObject * playerIsBeltInventorySlot(PyObject* poSelf, PyObject* poArgs)
{
	...
	const char Flag = CPythonPlayer::Instance().IsBeltInventorySlot(TItemPos(INVENTORY, iSlotIndex));
...
PyObject * playerIsValuableItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	const DWORD dwItemIndex = CPythonPlayer::Instance().GetItemIndex(SlotIndex);
	...
	const DWORD dwValue = pItemData->GetISellItemPrice();
	if (dwValue > 5000)
		isHighPrice = TRUE;
...
{
	const std::string strName = c_szItemName;
	if (strName.empty())
		return 0;
...
PyObject * playerCanRefine(PyObject * poSelf, PyObject * poArgs)
{
	...
	const CItemData * pScrollItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pScrollItemData)
	...
	// Target Item
	const int iTargetItemIndex = CPythonPlayer::Instance().GetItemIndex(TargetSlotIndex);
	CItemManager::Instance().SelectItemData(iTargetItemIndex);
	const CItemData * pTargetItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pTargetItemData)
		return Py_BuildValue("i", REFINE_CANT);
...
PyObject * playerCanAttachMetin(PyObject* poSelf, PyObject* poArgs)
{
	...
	const int iSubType = pMetinItemData->GetSubType();
	for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
	{
		const DWORD dwSocketType = CPythonPlayer::Instance().GetItemMetinSocket(TargetSlotIndex, i);
...
PyObject * playerCanUnlock(PyObject * poSelf, PyObject * poArgs)
{
	...
	const CItemData * pKeyItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pKeyItemData)
		return Py_BuildException("Can't find item data");
	const int iKeyType = pKeyItemData->GetType();
...
PyObject * playerRegisterEmotionIcon(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto pImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(szFileName);
...
PyObject * playerGetAutoPotionInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	const CPythonPlayer::SAutoPotionInfo& potionInfo = player->GetAutoPotionInfo(potionType);
...
void initPlayer()
{
	...
	PyModule_AddIntConstant(poModule, "DRAGON_SOUL_STEP_COUNT", DRAGON_SOUL_STEP_MAX);
