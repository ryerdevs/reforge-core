...
void ScanProcessList(std::map<DWORD, DWORD>& rkMap_crcProc, std::vector<CRCPair>* pkVct_crcPair)
{
	...
	const HANDLE process = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);

    BOOL bOK = Process32First(process, Entry);

    while(bOK)
    {
	    const HANDLE hProc = OpenProcess(PROCESS_VM_READ, FALSE, Entry->th32ProcessID);
		if (hProc)
		{
			const HANDLE hModuleSnap = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, Entry->th32ProcessID);
