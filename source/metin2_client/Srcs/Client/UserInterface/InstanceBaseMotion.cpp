...
constexpr int c_iFishingRotStep = 8;
constexpr float c_fFishingDistance = 600.0f;
...
void CInstanceBase::ActDualEmotion(CInstanceBase & rkDstInst, WORD wMotionNumber1, WORD wMotionNumber2)
{
	...
	const TPixelPosition kDirection = c_rMainPosition - c_rTargetPosition;
	const float fDistance = sqrtf((kDirection.x*kDirection.x) + (kDirection.y*kDirection.y));
	...
	const DWORD dwCurTime = ELTimer_GetServerMSec() + 500;
