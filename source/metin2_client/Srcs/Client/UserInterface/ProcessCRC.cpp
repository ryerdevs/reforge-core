...
{
	const char * t = big;
	const size_t len = strlen(little) - 1;
...
{
	const HANDLE hModuleSnap = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, GetCurrentProcessId());
...
DWORD GetProcessMemoryCRC(LPCVOID c_pvBaseAddress)
{
	...
	const auto pBuf = new char[1024*1024];
	DWORD dwBytesRead;
	...
	if (bRet)
	{
		const DWORD dwCRC = GetCRC32(pBuf, dwBytesRead);
		delete [] pBuf;
		return dwCRC;
	}
