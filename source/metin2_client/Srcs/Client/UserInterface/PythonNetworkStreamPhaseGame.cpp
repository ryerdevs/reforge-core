...
void CPythonNetworkStream::GamePhase()
{
	...
	constexpr DWORD MAX_RECV_COUNT = 8; // @warme669
	constexpr DWORD SAFE_RECV_BUFSIZE = 8192;
	DWORD dwRecvCount = 0;
...
void CPythonNetworkStream::Warp(LONG lGlobalX, LONG lGlobalY)
{
	...
	CPythonBackground::Instance().Update(float(lLocalX), float(lLocalY), 0.0f);
#endif
	const float fHeight = CPythonBackground::Instance().GetHeight(float(lLocalX), float(lLocalY));
...
#ifdef ENABLE_AREA_OPTIMIZATION
	m_fWarpPosX = float(lLocalX);
	m_fWarpPosY = float(lLocalY);
	CInstanceBase* pMain = CPythonCharacterManager::Instance().GetMainInstancePtr();
	if (pMain)
		pMain->NEW_SetPixelPosition(TPixelPosition(float(lLocalX), float(lLocalY), fHeight));
	CPythonCharacterManager::Instance().RefreshAllCharacterHeights();
	m_bRefreshHeightsAfterWarp = true;
	m_iRefreshHeightsAfterWarpFramesLeft = 30;
#endif
}

#ifdef ENABLE_AREA_OPTIMIZATION
void CPythonNetworkStream::TryRefreshHeightsAfterWarp()
{
	if (!m_bRefreshHeightsAfterWarp)
		return;
	CPythonCharacterManager::Instance().RefreshAllCharacterHeights();
	if (--m_iRefreshHeightsAfterWarpFramesLeft <= 0)
		m_bRefreshHeightsAfterWarp = false;
}
#endif

void CPythonNetworkStream::__ShowMapName(LONG lLocalX, LONG lLocalY) const
{
...
bool CPythonNetworkStream::RecvDuelStartPacket()
{
	...
	const DWORD count = (kDuelStartPacket.wSize - sizeof(kDuelStartPacket))/sizeof(DWORD);
	...
	const DWORD dwVIDSrc = pkInstMain->GetVirtualID();
	DWORD dwVIDDest;
...
bool CPythonNetworkStream::RecvPVPPacket()
{
	...
	switch (kPVPPacket.bMode)
	{
		...
		case PVP_MODE_REVENGE:
		{
			...
			const DWORD dwKiller = kPVPPacket.dwVIDSrc;
			const DWORD dwVictim = kPVPPacket.dwVIDDst;
...
bool CPythonNetworkStream::SendChatPacket(const char * c_szChat, BYTE byType)
{
	...
	const int iTextLen = strlen(c_szChat) + 1;
...
bool CPythonNetworkStream::ParseEmoticon(const char * pChatMsg, DWORD * pdwEmoticon) const
{
...
void CPythonNetworkStream::__ConvertEmpireText(DWORD dwEmpireID, char* szText) const
{
	...
	const STextConvertTable& rkTextConvTable=m_aTextConvTable[dwEmpireID-1];
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	const UINT uChatSize=kChat.size - sizeof(kChat);
		...
		case CHAT_TYPE_WHISPER:
			{
				...
				else
				{
					...
					{
						const CInstanceBase* pkInstMain=rkChrMgr.GetMainInstancePtr();
						if (pkInstMain)
...
bool CPythonNetworkStream::RecvCharacterPositionPacket()
{
	...
	const CInstanceBase * pChrInstance = CPythonCharacterManager::Instance().GetInstancePtr(PositionPacket.vid);
...
bool CPythonNetworkStream::RecvMotionPacket()
{
	...
	const CInstanceBase * pMainInstance = CPythonCharacterManager::Instance().GetInstancePtr(MotionPacket.vid);
	const CInstanceBase * pVictimInstance = nullptr;
...
bool CPythonNetworkStream::RecvShopPacket()
{
	...
	const int iSize = packet_shop.size - sizeof(packet_shop);
	if (iSize > 0)
	{
	...
	switch (packet_shop.subheader)
	{
		case SHOP_SUBHEADER_GC_START:
			{
				...
				const DWORD dwVID = *(DWORD *)&vecBuffer[0];

				const auto pShopStartPacket = (TPacketGCShopStart *)&vecBuffer[4];
		...
		case SHOP_SUBHEADER_GC_START_EX:
			{
				...
				const auto pShopStartPacket = (TPacketGCShopStartEx *)&vecBuffer[0];
				size_t read_point = sizeof(TPacketGCShopStartEx);

				const DWORD dwVID = pShopStartPacket->owner_vid;
				...
				{
					const auto pPackTab = (TPacketGCShopStartEx::TSubPacketShopTab*)&vecBuffer[read_point];
		...
		case SHOP_SUBHEADER_GC_UPDATE_ITEM:
			{
				const auto pShopUpdateItemPacket = (TPacketGCShopUpdateItem *)&vecBuffer[0];
...
bool CPythonNetworkStream::RecvExchangePacket()
{
	...
	switch (exchange_packet.subheader)
	{
		case EXCHANGE_SUBHEADER_GC_START:
			...
			{
				const CInstanceBase * pCharacterInstance = CPythonCharacterManager::Instance().GetInstancePtr(exchange_packet.arg1);
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
			...
			{
				const int iSlotIndex = exchange_packet.arg2.cell;
...
void CPythonNetworkStream::ToggleGameDebugInfo() const
{
...
bool CPythonNetworkStream::SendExchangeItemDelPacket(BYTE pos) const
{
...
bool CPythonNetworkStream::SendPointResetPacket() const
{
...
bool CPythonNetworkStream::RecvScriptPacket()
{
	...
	const int iIndex = CPythonEventManager::Instance().RegisterEventSetFromString(str);
...
bool CPythonNetworkStream::RecvMountPacket()
{
	...
	const CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetInstancePtr(MountPacket.vid);
...
bool CPythonNetworkStream::RecvChangeSpeedPacket()
{
	...
	const CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetInstancePtr(SpeedPacket.vid);
...
bool CPythonNetworkStream::SendSpecial(int nLen, void * pvBuf)
{
	...
	switch (bHeader)
	{
		case HEADER_CG_ATTACK:
			{
				const auto pkPacketAtk = (TPacketCGAttack *) pvBuf;
...
bool CPythonNetworkStream::RecvAddFlyTargetingPacket()
{
	...
	{
		const float h = CPythonBackground::Instance().GetHeight(kPacket.lX,kPacket.lY) + 60.0f; // TEMPORARY HEIGHT
...
bool CPythonNetworkStream::RecvPartyInvite()
{
	...
	const CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetInstancePtr(kPartyInvitePacket.leader_pid);
	if (!pInstance)
	{
...
bool CPythonNetworkStream::RecvFishing()
{
	...
	switch (FishingPacket.subheader)
	{
		...
		{
			const DWORD dwFishID = FishingPacket.info;
...
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock)
{
	...
	for (auto itor = c_rSellingItemStock.begin(); itor < c_rSellingItemStock.end(); ++itor)
	{
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
	...
	const TPacketAffectElement & rkElement = kAffectAdd.elem;
...
bool CPythonNetworkStream::RecvLandPacket()
{
	...
	const CInstanceBase * pMainInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
...
void CPythonNetworkStream::Discord_Start() const
{
	DiscordStartTime = time(nullptr);
	DiscordEventHandlers handlers{};
	Discord_Initialize(Discord::DiscordClientID, &handlers, 1, nullptr);
	Discord_Update(false);
}

void CPythonNetworkStream::Discord_Update(const bool ingame) const
{
...
void CPythonNetworkStream::Discord_Close() const
{
	Discord_Shutdown();
}
#endif
