...
{
	const int isInterfaceConfig = CPythonSystem::Instance().isInterfaceConfig();
...
{
	const CPythonSystem::TConfig *tmp = CPythonSystem::Instance().GetConfig();

	const int iRes = CPythonSystem::Instance().GetResolutionIndex(tmp->width, tmp->height, tmp->bpp);
	const int iFrequency = CPythonSystem::Instance().GetFrequencyIndex(iRes, tmp->frequency);
...
{
	const int value = CPythonSystem::Instance().IsSaveID();
...
{
	const int ret = CPythonSystem::Instance().SaveConfig();
