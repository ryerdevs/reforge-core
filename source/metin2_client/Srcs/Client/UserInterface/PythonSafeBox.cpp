...
DWORD CPythonSafeBox::GetMoney() const
{
	return m_dwMoney;
}
...
BOOL CPythonSafeBox::GetSlotItemID(DWORD dwSlotIndex, DWORD* pdwItemID) const
{
	if (dwSlotIndex >= m_ItemInstanceVector.size())
	{
...
BOOL CPythonSafeBox::GetSlotMallItemID(DWORD dwSlotIndex, DWORD * pdwItemID) const
{
	if (dwSlotIndex >= m_MallItemInstanceVector.size())
	{
		TraceError("CPythonSafeBox::GetMallSlotItemID(dwSlotIndex=%d) - Strange slot index", dwSlotIndex);
...
DWORD CPythonSafeBox::GetMallSize() const
{
