...
class CPythonSkill : public CSingleton<CPythonSkill>
{
	public:
		...
		typedef struct SSkillData
		{
			...
			const char * GetAffectDescription(DWORD dwIndex, float fSkillLevel) const;
			DWORD GetSkillCoolTime(float fSkillPoint) const;
			int GetNeedSP(float fSkillPoint) const;
			DWORD GetContinuationSP(float fSkillPoint) const;
			DWORD GetMotionLoopCount(float fSkillPoint) const;
			DWORD GetTargetCount(float fSkillPoint) const;
			DWORD GetDuration(float fSkillPoint) const;
