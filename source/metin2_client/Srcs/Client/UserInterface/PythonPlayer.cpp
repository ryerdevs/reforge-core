...
constexpr DWORD POINT_MAGIC_NUMBER = 0xe73ac1da;
...
long CPythonPlayer::SPlayerStatus::GetPoint(UINT ePoint) const
{
...
{
	const DWORD dwSkillIndex=m_kMap_dwAffectIndexToSkillIndex[uAffect];
...
{
	const CPythonCharacterManager& rkChrMgr=CPythonCharacterManager::Instance();
...
bool CPythonPlayer::__GetPickedActorID(DWORD* pdwActorID) const
{
	const CPythonCharacterManager& rkChrMgr=CPythonCharacterManager::Instance();
	return rkChrMgr.OLD_GetPickedInstanceVID(pdwActorID);
}

bool CPythonPlayer::__GetPickedItemID(DWORD* pdwItemID) const
{
	const CPythonItem& rkItemMgr=CPythonItem::Instance();
...
void CPythonPlayer::NEW_ShowEffect(int dwEID, TPixelPosition kPPosDst) const
{
	if (dwEID>=EFFECT_NUM)
		return;

	const D3DXVECTOR3 kD3DVt3Pos(kPPosDst.x, -kPPosDst.y, kPPosDst.z);
	const D3DXVECTOR3 kD3DVt3Dir(0.0f, 0.0f, 1.0f);

	CEffectManager& rkEftMgr=CEffectManager::Instance();
	rkEftMgr.CreateEffect(m_adwEffect[dwEID], kD3DVt3Pos, kD3DVt3Dir);
}

CInstanceBase* CPythonPlayer::NEW_FindActorPtr(DWORD dwVID) const
{
...
void CPythonPlayer::Update()
{
	...
	{
		const float fElapsedTime = CTimer::Instance().GetElapsedSecond();
...
bool CPythonPlayer::__IsUsingChargeSkill()
{
	...
	const TSkillInstance & rkSkillInst = m_playerStatus.aSkill[m_dwSkillSlotIndexReserved];
...
void CPythonPlayer::__Update_NotifyGuildAreaEvent()
{
	...
	if (pkInstMain)
	{
		...
		const DWORD dwAreaID = CPythonMiniMap::Instance().GetGuildAreaID(
...
{
	const CInstanceBase* pkInstMain=NEW_GetMainActorPtr();
	if (!pkInstMain)
		return 0xffffffff;
...
DWORD CPythonPlayer::GetRace() const
{
	return m_dwRace;
}
...
DWORD CPythonPlayer::__GetWeaponAtk(DWORD dwWeaponPower) const
{
...
{
	const DWORD dwLvAtk=__GetLevelAtk();
	const DWORD dwStAtk=__GetStatAtk();
...
{
	const int src = (GetStatus(POINT_DX) * 4 + GetStatus(POINT_LEVEL) * 2) / 6;
...
void CPythonPlayer::NotifyCharacterUpdate(DWORD dwVID)
{
	if (__IsSameTargetVID(dwVID))
	{
		const CInstanceBase * pMainInstance = NEW_GetMainActorPtr();
...
void CPythonPlayer::MoveItemData(TItemPos SrcCell, TItemPos DstCell)
{
	...
	const TItemData src_item(*GetItemData(SrcCell));
	const TItemData dst_item(*GetItemData(DstCell));
...
DWORD CPythonPlayer::GetItemCountByVnum(DWORD dwVnum) const
{
	DWORD dwCount = 0;
...
DWORD CPythonPlayer::GetItemMetinSocket(TItemPos Cell, DWORD dwMetinSocketIndex) const
{
	if (!Cell.IsValidCell())
		return 0;
...
void CPythonPlayer::GetItemAttribute(TItemPos Cell, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue) const
{
	*pbyType = 0;
	*psValue = 0;
...
void CPythonPlayer::SetItemCount(TItemPos Cell, BYTE byCount)
{
	...
	if (Cell.window_type == DRAGON_SOUL_INVENTORY) // @fixme052 reduce calls
		RefreshInventory();
}
...
DWORD	CPythonPlayer::LocalQuickSlotIndexToGlobalQuickSlotIndex(DWORD dwLocalSlotIndex) const
{
...
{
	const TQuickSlot& rkQuickSlot=__RefGlobalQuickSlot(dwGlobalSlotIndex);
...
{
	const TQuickSlot& rkQuickSlot=__RefLocalQuickSlot(dwSlotPos);
	*pdwWndType=rkQuickSlot.Type;
	*pdwWndItemPos=rkQuickSlot.Position;
}
...
void CPythonPlayer::RequestMoveGlobalQuickSlotToLocalQuickSlot(DWORD dwGlobalSrcSlotIndex, DWORD dwLocalDstSlotIndex) const
{
	//DWORD dwGlobalSrcSlotIndex=LocalQuickSlotIndexToGlobalQuickSlotIndex(dwLocalSrcSlotIndex);
	const DWORD dwGlobalDstSlotIndex=LocalQuickSlotIndexToGlobalQuickSlotIndex(dwLocalDstSlotIndex);

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendQuickSlotMovePacket((BYTE) dwGlobalSrcSlotIndex, (BYTE)dwGlobalDstSlotIndex);
}

void CPythonPlayer::RequestAddLocalQuickSlot(DWORD dwLocalSlotIndex, DWORD dwWndType, DWORD dwWndItemPos) const
{
	if (dwLocalSlotIndex>=QUICKSLOT_MAX_COUNT_PER_LINE)
		return;

	const DWORD dwGlobalSlotIndex=LocalQuickSlotIndexToGlobalQuickSlotIndex(dwLocalSlotIndex);
...
void CPythonPlayer::RequestAddToEmptyLocalQuickSlot(DWORD dwWndType, DWORD dwWndItemPos)
{
    ...
    {
	    const TQuickSlot& rkQuickSlot=__RefLocalQuickSlot(i);

        if (0 == rkQuickSlot.Type)
        {
	        const DWORD dwGlobalQuickSlotIndex=LocalQuickSlotIndexToGlobalQuickSlotIndex(i);
            CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
...
void CPythonPlayer::RequestDeleteGlobalQuickSlot(DWORD dwGlobalSlotIndex) const
{
...
bool CPythonPlayer::IsEquipItemInSlot(TItemPos Cell)
{
	...
	const DWORD dwItemIndex = pData->vnum;
...
int CPythonPlayer::GetSkillIndex(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;
...
{
	const auto f=m_skillSlotDict.find(dwSkillIndex);
...
int CPythonPlayer::GetSkillGrade(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;
...
float CPythonPlayer::GetSkillCurrentEfficientPercentage(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;
...
float CPythonPlayer::GetSkillNextEfficientPercentage(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;
...
void CPythonPlayer::SetSkillLevel_(DWORD dwSkillIndex, DWORD dwSkillGrade, DWORD dwSkillLevel)
{
	...
	constexpr DWORD SKILL_MAX_LEVEL = 40;
...
float CPythonPlayer::GetSkillCoolTime(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0.0f;
...
float CPythonPlayer::GetSkillElapsedCoolTime(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0.0f;
...
BOOL CPythonPlayer::IsSkillActive(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return FALSE;
...
BOOL CPythonPlayer::IsToggleSkill(DWORD dwSlotIndex) const
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return FALSE;

	const DWORD dwSkillIndex = m_playerStatus.aSkill[dwSlotIndex].dwIndex;
...
DWORD CPythonPlayer::GetPlayTime() const
{
	return m_dwPlayTime;
}
...
void CPythonPlayer::__SendClickActorPacket(CInstanceBase& rkInstVictim)
{
	...
	if (dwCurTime >= s_dwNextTCPTime)
	{
		...
		const DWORD dwVictimVID=rkInstVictim.GetVirtualID();
...
{
	const CInstanceBase * pkInstTarget = __GetAliveTargetInstancePtr();
	if (!pkInstTarget)
	{
...
void CPythonPlayer::AlarmHaveToGo()
{
	...
	const D3DXVECTOR3 v3Rotation(0.0f, 0.0f, fAngle);
...
bool CPythonPlayer::IsPartyMemberByVID(DWORD dwVID)
{
	...
	{
		const TPartyMemberInfo & rPartyMemberInfo = itor->second;
...
{
	const auto itor = m_PartyMemberMap.find(dwPID);
...
{
	const CInstanceBase * pInstance = NEW_GetMainActorPtr();
	if (!pInstance)
		return 0;
...
void CPythonPlayer::RefreshInventory() const
{
...
{
	const int iAffIndex = GetAffectDataIndex(dwType, kElem.bPointIdxApplyOn);
	if (iAffIndex != -1)
	{
