...
void CPythonPlayer::PickCloseItem()
{
	...
	for (auto& i : rkItem.vecMultiItemPick)
		SendClickItemPacket(i);
#else
	SendClickItemPacket(dwItemID);
#endif
}
...
bool CPythonPlayer::__IsSameTargetVID(DWORD dwVID) const
{
	return dwVID == __GetTargetVID();
}

DWORD CPythonPlayer::__GetTargetVID() const
{
	return m_dwTargetVID;
}

DWORD CPythonPlayer::GetTargetVID() const
{
...
void CPythonPlayer::SetTarget(DWORD dwVID, BOOL bForceChange)
{
	...
	const DWORD dwCurrentTime = CTimer::Instance().GetCurrentMillisecond();
...
CInstanceBase *	CPythonPlayer::__GetSkillTargetInstancePtr(CPythonSkill::TSkillData& rkSkillData) const
{
...
void CPythonPlayer::OpenCharacterMenu(DWORD dwVictimActorID) const
{
	const CInstanceBase * pkInstMain = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (!pkInstMain)
		return;
...
void CPythonPlayer::__OnClickItem(CInstanceBase& rkInstMain, DWORD dwItemID) const
{
}
...
{
	const float fDistance=rkInstMain.NEW_GetDistanceFromDestPixelPosition(c_rkPPosPickedGround);
...
bool CPythonPlayer::NEW_MoveToDirection(float fDirRot)
{
	...
	if (pkCmrCur)
	{
		const float fCmrCurRot=CameraRotationToCharacterRotation(pkCmrCur->GetRoll());
		...
		if (m_isCmrRot)
		{
			...
			const float fElapsedTime = CPythonApplication::Instance().GetGlobalElapsedTime();

			const float fRotDeg = -m_fCmrRotSpd * fElapsedTime * fRotRat / 90.0f;
...
bool CPythonPlayer::__CanAttack()
{
	...
	// @fixme054 BEGIN
	DWORD slotIndex{};
	if (pkInstMain->IsMountingHorse() && pkInstMain->IsNewMount() && GetSkillSlotIndex(c_iSkillIndex_Riding, &slotIndex) && (GetSkillGrade(slotIndex) < 1 && GetSkillLevel(slotIndex) < 11))
		return false;
	// @fixme054 END
...
{
	const long lWidth = UI::CWindowManager::Instance().GetScreenWidth();
	const long lHeight = UI::CWindowManager::Instance().GetScreenHeight();
	const int nScrPosX=lWidth*fScrX;
	const int nScrPosY=lHeight*fScrY;
	const int nScrWidth=lWidth;
	const int nScrHeight=lHeight;
	const int nScrCenterX=nScrWidth/2;
	const int nScrCenterY=nScrHeight/2;

	const float finputRotation = GetDegreeFromPosition(nScrPosX, nScrPosY, nScrCenterX, nScrCenterY);
...
float CPythonPlayer::GetDegreeFromPosition(int ix, int iy, int iHalfWidth, int iHalfHeight) const
{
	...
	const D3DXVECTOR3 vtStan(0, -1, 0);
...
bool CPythonPlayer::__IsReservedUseSkill(DWORD dwSkillSlotIndex) const
{
	if (MODE_USE_SKILL!=m_eReservedMode)
		return false;
