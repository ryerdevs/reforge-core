...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		void TryRefreshHeightsAfterWarp();
#endif
		...
		void Discord_Start() const;
		void Discord_Close() const;
		void Discord_Update(const bool ingame) const;
		#endif
		...
		DWORD GetGuildID() const;

		UINT UploadMark(const char* c_szImageFileName) const;
		UINT UploadSymbol(const char* c_szImageFileName) const;

		bool LoadInsultList(const char* c_szInsultListFileName);
		bool LoadConvertTable(DWORD dwEmpireID, const char* c_szFileName);

		UINT		GetAccountCharacterSlotDatau(UINT iSlot, UINT eType) const;
		...
		void ToggleGameDebugInfo() const;
...
#endif
		bool SendExchangeItemDelPacket(BYTE pos) const;
		...
		// SafeBox
		bool SendSafeBoxMoneyPacket(BYTE byState, DWORD dwMoney) const;
...
#endif
		// ETC
		DWORD GetMainActorVID() const;
		DWORD GetMainActorRace() const;
		DWORD GetMainActorEmpire() const;
		DWORD GetMainActorSkillGroup() const;
		void SetEmpireID(DWORD dwEmpireID);
		DWORD GetEmpireID() const;
	...
	protected:
		...
		void __DownloadSymbol(const std::vector<DWORD> & c_rkVec_dwGuildID) const;
		...
		void __PlaySafeBoxItemDropSound(UINT uSlotPos) const;
		void __PlayMallItemDropSound(UINT uSlotPos) const;
	...
	protected:
		bool ParseEmoticon(const char * pChatMsg, DWORD * pdwEmoticon) const;

		void OnConnectFailure();
		void OnScriptEventStart(int iSkin, int iIndex) const;
	...
	protected:
		...
		bool __IsEquipItemInSlot(TItemPos Cell) const;

		void __ShowMapName(LONG lLocalX, LONG lLocalY) const;
		...
		void __ConvertEmpireText(DWORD dwEmpireID, char* szText) const;
...
#ifdef ENABLE_AREA_OPTIMIZATION
		bool m_bRefreshHeightsAfterWarp;
		float m_fWarpPosX;
		float m_fWarpPosY;
		int m_iRefreshHeightsAfterWarpFramesLeft;
#endif
	...
	public:
		DWORD EXPORT_GetBettingGuildWarValue(const char* c_szValueName) const;
