...
PyObject * chrmgrRegisterCacheMotionData(PyObject* poSelf, PyObject* poArgs)
{
	...
	const char * c_szFullFileName = CRaceManager::Instance().GetFullPathFileName(szFileName);
	const CGraphicThing* pkMotionThing=pRaceData->RegisterMotionData(iMode, iMotion, c_szFullFileName, iWeight);
...
{
	const CInstanceBase * pkInstSel = (dwVID == 0xffffffff) ? GetSelectedInstancePtr() : GetInstancePtr(dwVID);
	if (!pkInstSel)
		return false;
...
PyObject * chrmgrIsPossibleEmoticon(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonCharacterManager & rkChrMgr = CPythonCharacterManager::Instance();
	const int result = rkChrMgr.IsPossibleEmoticon(nVID >= 0 ? nVID : 0xffffffff);
...
PyObject* chrmgrGetHorseVnumByVID(PyObject* poSelf, PyObject* poArgs)
{
	...
	const auto pkInstBase = CPythonCharacterManager::Instance().GetInstancePtr(iVID);
