...
void CPythonPlayerEventHandler::OnWaiting(const SState& c_rkState)
{
	...
	const float dx=m_kPPosPrevWaiting.x-c_rkPPosCurWaiting.x;
	const float dy=m_kPPosPrevWaiting.y-c_rkPPosCurWaiting.y;
...
void CPythonPlayerEventHandler::FlushVictimList()
{
	...
	const unsigned int uiVictimCount = m_kVctkVictim.size();
