...
DWORD GetHFILECRC32(HANDLE hFile)
{
	...
	const DWORD dataOffset=0;
	const DWORD mapSize=dwFileSize;

	SYSTEM_INFO SysInfo;
	GetSystemInfo(&SysInfo);

	const DWORD dwSysGran = SysInfo.dwAllocationGranularity;
	const DWORD dwFileMapStart = (dataOffset / dwSysGran) * dwSysGran;
	const DWORD dwMapViewSize = (dataOffset % dwSysGran) + mapSize;
	//INT iViewDelta = dataOffset - dwFileMapStart;

	const HANDLE hFM = CreateFileMapping(hFile,				// handle
	                                     nullptr,					// security
	...
	if (hFM)
	{
		const LPVOID lpMapData = MapViewOfFile(hFM,
		                                       FILE_MAP_READ,
		                                       0,
		                                       dwFileMapStart,
		                                       dwMapViewSize);
...
{
	const HANDLE hFile = CreateFile(c_szFileName,					// name of the file
	...
	const DWORD dwRetCRC32=GetHFILECRC32(hFile);
