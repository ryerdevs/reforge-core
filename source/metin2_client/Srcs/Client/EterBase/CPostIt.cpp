...
BOOL CPostIt::CopyTo( CPostIt *pPostIt, LPCSTR lpszKeyName ) const
{
	if( m_pMemoryBlock == nullptr)
		return FALSE;
	const LPSTR szText = m_pMemoryBlock->Find( lpszKeyName );
	if( szText == nullptr)
		return FALSE;
...
void CPostIt::Empty( void )
{
	...
	if( ::OpenClipboard(nullptr) == FALSE )
		return;
	const HANDLE hClipboardMemory = ::GetClipboardData( uDGPFormat );
	if( hClipboardMemory ) {
...
BOOL _CPostItMemoryBlock::Assign( HANDLE hBlock )
{
	if( hBlock == nullptr || hBlock == INVALID_HANDLE_VALUE )
		return FALSE;
	auto lpBuffer = (LPBYTE) ::GlobalLock( hBlock );

	if( lpBuffer == nullptr)
		return FALSE;
	const DWORD dwCount = *((LPDWORD) lpBuffer);				lpBuffer += sizeof( DWORD );
	for( DWORD dwI=0; dwI < dwCount; dwI++ ) {
		const WORD	wLen = *((LPWORD) lpBuffer);			lpBuffer += sizeof( WORD );

		const auto lpszText = new CHAR[ wLen + 1 ];
...
HANDLE _CPostItMemoryBlock::CreateHandle( void )
{
	...
	const HANDLE hBlock = ::GlobalAlloc( GMEM_ZEROINIT | GMEM_MOVEABLE, dwBlockSize );
	if( hBlock == nullptr)
		return INVALID_HANDLE_VALUE;
	auto lpBuffer = (LPBYTE) ::GlobalLock( hBlock );
	if( lpBuffer == nullptr) {
...
BOOL _CPostItMemoryBlock::Put( LPCSTR lpszKeyName, LPCSTR lpBuffer )
{
	...
	if( lpBuffer == nullptr || !*lpBuffer )
		return TRUE;
	const size_t nStrLen	= strlen( lpszKeyName ) + 1 /* '=' */ + strlen( lpBuffer );
...
BOOL _CPostItMemoryBlock::Put( LPCSTR lpBuffer )
{
	...
	if( lpBuffer == nullptr || !*lpBuffer )
		return TRUE;
	const size_t nStrLen	= strlen( lpBuffer );
	lpszText = new CHAR[ nStrLen + 1 ];
