...
{
	constexpr float LIMIT = 3276700.0f;
	if (x<LIMIT && x>-LIMIT)
		if (y<LIMIT && y>-LIMIT)
			if (z<LIMIT && z>-LIMIT)
				return true;
...
bool Sphere::RayIntersection(const Vector3d &rayOrigin,
														 const Vector3d &dir,
														 Vector3d *intersect) const
{
	//notation:
	//point E  = rayOrigin
	//point O  = sphere center

	const Vector3d EO = mCenter - rayOrigin;
  Vector3d V = dir;
	const float dist2 = EO.x*EO.x + EO.y*EO.y + EO.z * EO.z;
  // Bug Fix For Gem, if origin is *inside* the sphere, invert the
  // direction vector so that we get a valid intersection location.
  if ( dist2 < mRadius2 ) V*=-1;

	const float v = EO.Dot(V);

	const float disc = mRadius2 - (EO.Length2() - v*v);

	if (disc > 0.0f)
	{
		if ( intersect )
		{
			const float d = sqrtf(disc);
...
{
  Vector3d sect;
  const bool hit = RayIntersectionInFront(rayOrigin,V,&sect);

  if ( hit )
  {
	  const float d = rayOrigin.DistanceSq(sect);
...
{
  Vector3d sect;
  const bool hit = RayIntersection(rayOrigin,V,&sect);

  if ( hit )
  {
	  const Vector3d dir = sect - rayOrigin;

	  const float dot = dir.Dot(V);
