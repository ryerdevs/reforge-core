...
void SpherePackFactory::Process(void)
{
...

	{
		...
		const int maxintegrate = mIntegrate->GetCount();
...
void SpherePackFactory::AddIntegrate(SpherePack *pack) const
{
	if (pack->HasSpherePackFlag(SPF_ROOT_TREE))
		mRoot->AddChild(pack);
	else
...
void SpherePackFactory::Render(void) const
{
#if DEMO
...
void SpherePack::Render(unsigned int /*color*/) const
{
#if DEMO
...
bool SpherePack::Recompute(float gravy)
{
	...
	if (count)
	{
		const float recip = 1.0f / float(count);
		total*=recip;
		...
		while (pack)
		{
			const float dist = DistanceSquared(pack);
			const float radius = sqrtf(dist) + pack->GetRadius();
			if (radius > maxradius)
			{
				maxradius = radius;
...
void SpherePack::LostChild(SpherePack *t)
{
	...
	const SpherePack *pack = mChildren;
	bool found = false;
	while (pack)
	{
		if (pack == t)
	...
	else
	{
		SpherePack *next = t->_GetNextSibling();
		mChildren = next;
		if (mChildren) mChildren->SetPrevSibling(nullptr);
	}
...
#endif
		const auto link = (SpherePack *) pack->GetUserData();
                                  ...
                                  float node_size)
{
	...
	while (search)
	{
		...
		{
			const float dist = pack->DistanceSquared(search);

			if (nearest1)
			{
				if (dist < neardist1)
				{
					const float d = sqrtf(dist)+pack->GetRadius();
			...
			{
				const float d = (sqrtf(dist) + pack->GetRadius())-search->GetRadius();
...
#endif
			const auto link = (SpherePack *) nearest1->GetUserData();
	...
	else
	{
		...
		if (nearest2)
		{
			const float newsize = neardist2 + nearest2->GetRadius() + mSuperSphereGravy;
...
#endif
					const auto link = (SpherePack *) nearest2->GetUserData();
                           ...
                           ViewState state)
{
	if (state == VS_PARTIAL)
	{
		const float d = p.Distance(mCenter);
...
#endif
		SpherePack *pack = mChildren;
		while (pack) // In the World Editor it's null (in the launcher it's OK) if CCulling not processed / updated
		{
...
#endif
	const auto link = (SpherePack *) sphere->GetUserData();
...
void SpherePackFactory::Reset(void) const
{
	mRoot->Reset();
	mLeaf->Reset();
}
