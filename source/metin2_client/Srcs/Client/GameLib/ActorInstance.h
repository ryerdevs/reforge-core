...
class CActorInstance : public IActorInstance, public IFlyTargetableObject
{
	...
	public:
		...
		DWORD GetPartItemID(DWORD dwPartIndex) const;

		// Attach Effect
		BOOL GetAttachingBoneName(DWORD dwPartIndex, const char ** c_szBoneName) const;
		...
		BOOL CanCheckAttacking() const;
		BOOL CanCancelSkill() const;
		/////////////////////////////////////////////////////////////////////////////////////

		/////////////////////////////////////////////////////////////////////////////////////
		// Collison Detection
		bool CreateCollisionInstancePiece(DWORD dwAttachingModelIndex, const NRaceData::TAttachingData * c_pAttachingData, TCollisionPointInstance * pPointInstance) const;

		void UpdatePointInstance();
		void UpdatePointInstance(TCollisionPointInstance * pPointInstance) const;
	...
	public:
		...
		BOOL		isComboAttacking() const;
		BOOL		IsSplashAttacking() const;
		BOOL		IsUsingMovingSkill() const;
		BOOL		IsActEmotion();
		DWORD		GetComboIndex() const;
		...
		/////////////////////////////////////////////////////////////////////////////////////
		// Position
		const D3DXVECTOR3&	GetMovementVectorRef() const;
		...
		const		TPixelPosition& NEW_GetAtkPixelPositionRef() const;
		const		TPixelPosition& NEW_GetCurPixelPositionRef();
		const		TPixelPosition& NEW_GetSrcPixelPositionRef() const;
		const		TPixelPosition& NEW_GetDstPixelPositionRef() const;

		const		TPixelPosition& NEW_GetLastPixelPositionRef();

		void		GetPixelPosition(TPixelPosition * pPixelPosition) const;
		...
		float		GetAdvancingRotation() const;

		float		GetRotatingTime() const;
		...
		void		SoundEventProcess(BOOL bCheckFrequency) const;
		/////////////////////////////////////////////////////////////////////////////////////
		...
		void		BeginDiffuseRender() const;
		void		EndDiffuseRender() const;
		void		BeginOpacityRender() const;
		void		EndOpacityRender() const;

		void		BeginBlendRender() const;
		void		EndBlendRender() const;
		...
		void		BeginAddRender() const;
		void		EndAddRender() const;
		void		SetAddRenderMode();
		void		SetAddColor(const D3DXCOLOR & c_rColor);

		void		BeginModulateRender() const;
		void		EndModulateRender() const;
	...
	public:
		...
		float GetFlyTargetDistance() const;
	...
	public:
		WORD GetCurrentMotionIndex() const { return __GetCurrentMotionIndex(); }
#endif
	...
	protected:
		...
		MOTION_KEY	GetRandomMotionKey(MOTION_KEY dwMotionKey) const;
		...
		void ProcessMotionEventSound(const CRaceMotionData::TMotionEventData * c_pData) const;
		...
		void ProcessMotionEventUnk12(DWORD dwcurFrame, int iIndex, const CRaceMotionData::TMotionEventData * c_pData) const;
#endif

		void AddMovement(float fx, float fy, float fz);

		bool __IsLeftHandWeapon(DWORD type) const;
		bool __IsRightHandWeapon(DWORD type) const;
		bool __IsWeaponTrace(DWORD weaponType) const;
	...
	protected:
		...
		UINT		__GetMotionType() const;

		bool		__IsNeedFlyTargetMotion() const;
		bool		__HasMotionFlyEvent() const;
		bool		__IsWaitMotion() const;
		bool		__IsMoveMotion() const;
		bool		__IsAttackMotion() const;
		bool		__IsComboAttackMotion() const;
		bool		__IsDamageMotion() const;
		bool		__IsKnockDownMotion() const;
		bool		__IsDieMotion() const;
		bool		__IsStandUpMotion() const;
		bool		__IsMountingHorse() const;

		bool		__CanAttack() const;
		bool		__CanNextComboAttack() const;

		bool		__IsComboAttacking() const;
		void		__CancelComboAttack();

		WORD		__GetCurrentMotionIndex() const;
		DWORD		__GetCurrentMotionKey() const;

		int			__GetLoopCount() const;
		WORD		__GetCurrentComboType();

		void		__ShowEvent();
		void		__HideEvent();
		BOOL		__IsHiding() const;
		BOOL		__IsMovingSkill(WORD wSkillNumber) const;
	...
	public:
		IEventHandler* __GetEventHandlerPtr() const;
		IEventHandler& __GetEventHandlerRef() const;
		...
		void	__OnStop();
		void	__OnWarp();
		void	__OnClearAffects() const;
		void	__OnSetAffect(UINT uAffect) const;
		void	__OnResetAffect(UINT uAffect) const;
		void	__OnAttack(WORD wMotionIndex);
		void	__OnUseSkill(UINT uMotSkill, UINT uLoopCount, bool isMoving);

	protected:
		void	__OnHit(UINT uSkill, CActorInstance& rkInstVictm, BOOL isSendPacket) const;
	...
	public:
		...
		void __DestroyTree() const;
		void __SetTreePosition(float fx, float fy, float fz) const;
