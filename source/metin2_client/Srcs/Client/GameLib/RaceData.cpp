...
DWORD CRaceData::GetSmokeEffectID(UINT eSmoke) const
{
	if(eSmoke>=SMOKE_NUM)
	{
...
{
	const auto f=m_kMap_dwHairKey_kHair.find(eHair);
...
{
	const auto f=m_kMap_dwShapeKey_kShape.find(eShape);
...
{
	const TMotionModeDataIterator itor = m_pMotionModeDataMap.find(wMotionMode);
...
{
	const TModelDataMapIterator itor = m_ModelDataMap.find(dwModelIndex);
...
DWORD CRaceData::GetAttachingDataCount() const
{
	return m_AttachingDataVector.size();
}

BOOL CRaceData::GetAttachingDataPointer(DWORD dwIndex, const NRaceData::TAttachingData ** c_ppAttachingData) const
{
...
BOOL CRaceData::GetCollisionDataPointer(DWORD dwIndex, const NRaceData::TAttachingData ** c_ppAttachingData) const
{
	if (dwIndex >= GetAttachingDataCount())
		return FALSE;
...
BOOL CRaceData::GetBodyCollisionDataPointer(const NRaceData::TAttachingData ** c_ppAttachingData) const
{
...
{
	const auto pMotionThing = (CGraphicThing *)CResourceManager::Instance().GetResourcePointer(c_szFileName);
...
{
	const auto pThing = (CGraphicThing *)CResourceManager::Instance().GetResourcePointer(c_szFileName);
...
{
	const auto itor = m_NormalAttackIndexMap.find(wMotionModeIndex);
...
{
	const TComboAttackDataIterator itor = m_ComboAttackDataMap.find(MAKE_COMBO_KEY(wMotionModeIndex, wComboType));
...
CGraphicThing * CRaceData::GetLODModelThing()
{
	...
	if (!m_pLODModelThing)
	{
		const std::string strLODFileName = CFileNameHelper::NoExtension(m_strBaseModelFileName) + "_lod_01.gr2";
...
CAttributeData * CRaceData::GetAttributeDataPtr() const
{
...
{
	const auto itor = m_AttachingBoneNameMap.find(dwPartIndex);
...
BOOL CRaceData::IsTree() const
{
	return !m_strTreeFileName.empty();
}

const char * CRaceData::GetTreeFileName() const
{
