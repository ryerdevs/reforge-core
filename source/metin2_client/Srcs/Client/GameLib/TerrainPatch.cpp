...
void CTerrainPatch::BuildWaterVertexBuffer(SWaterVertex* akSrcVertex, UINT uWaterVertexCount)
{
	...
	{
		const UINT uVBSize=sizeof(SWaterVertex)*uWaterVertexCount;
...
void CTerrainPatch::__BuildHardwareTerrainVertexBuffer(HardwareTransformPatch_SSourceVertex* akSrcVertex)
{
	...
	{
		const UINT uVBSize=sizeof(HardwareTransformPatch_SSourceVertex)*TERRAIN_VERTEX_COUNT;
...
void CTerrainPatch::SoftwareTransformPatch_UpdateTerrainLighting(DWORD dwVersion, const D3DLIGHT9& c_rkLight, const D3DMATERIAL9& c_rkMtrl)
{
	...
	const D3DXVECTOR3 kLightDir=c_rkLight.Direction;

	DWORD dwDot;
	const DWORD dwAmbientR=(c_rkMtrl.Ambient.r*c_rkLight.Ambient.r+c_rkMtrl.Emissive.r)*255.0f;
	const DWORD dwAmbientG=(c_rkMtrl.Ambient.g*c_rkLight.Ambient.g+c_rkMtrl.Emissive.g)*255.0f;
	const DWORD dwAmbientB=(c_rkMtrl.Ambient.b*c_rkLight.Ambient.b+c_rkMtrl.Emissive.b)*255.0f;
	...
	{
		const float fDot=D3DXVec3Dot(&akSrcVertex[uIndex].kNormal, &kLightDir);

		constexpr float N=0xffffff;
		constexpr int S=24;
		if (fDot<0.0f)
			dwDot=(N*-fDot);
		else
...
UINT CTerrainPatch::GetWaterFaceCount() const
{
...
bool CTerrainPatchProxy::IsIn(const D3DXVECTOR3& c_rv3Target, float fRadius) const
{
	const float dx=m_v3Center.x-c_rv3Target.x;
	const float dy=m_v3Center.y-c_rv3Target.y;
	const float fDist=dx*dx+dy*dy;
	const float fCheck=fRadius*fRadius;
...
void CTerrainPatchProxy::SoftwareTransformPatch_UpdateTerrainLighting(DWORD dwVersion, const D3DLIGHT9& c_rkLight, const D3DMATERIAL9& c_rkMtrl) const
{
	if (m_pTerrainPatch)
...
SoftwareTransformPatch_SSourceVertex* CTerrainPatchProxy::SoftwareTransformPatch_GetTerrainVertexDataPtr() const
{
	if (m_pTerrainPatch)
...
UINT CTerrainPatchProxy::GetWaterFaceCount() const
{
	if (m_pTerrainPatch)
