...
{
	const auto pThread = (TEMP_CAreaLoaderThread *) pThis;
...
{
	const bool bProcessTerrain = true;
	while (!m_bShutdowned)
	{
		DWORD dwWaitResult;
...
void TEMP_CAreaLoaderThread::ProcessArea()	// called in loader thread
{
	...
	const DWORD dwID = (DWORD) (wAreaCoordX) * 1000L + (DWORD) (wAreaCoordY);
...
void TEMP_CAreaLoaderThread::ProcessTerrain()	// called in loader thread
{
	...
	const DWORD dwID = (DWORD) (wCoordX) * 1000L + (DWORD) (wCoordY);
