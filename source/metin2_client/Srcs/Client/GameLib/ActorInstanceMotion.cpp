...
UINT CActorInstance::__GetMotionType() const
{
...
void CActorInstance::__MotionEventProcess(BOOL isPC)
{
	if (isAttacking())
	{
		const DWORD dwNextFrame = DWORD(GetAttackingElapsedTime() * g_fGameFPS);
...
void CActorInstance::ReservingMotionProcess()
{
	...
	const TReservingMotionNode & rReservingMotionNode = m_MotionDeque.front();

	const float fCurrentTime = GetLocalTime();
	if (rReservingMotionNode.fStartTime > fCurrentTime)
		return;

	const DWORD dwNextMotionIndex = GET_MOTION_INDEX(rReservingMotionNode.dwMotionKey);
	...
	const SCurrentMotionNode kPrevMotionNode=m_kCurMotNode;

	const EMotionPushType iMotionType=rReservingMotionNode.iMotionType;
	const float fSpeedRatio=rReservingMotionNode.fSpeedRatio;
	const float fBlendTime=rReservingMotionNode.fBlendTime;

	const DWORD dwMotionKey=rReservingMotionNode.dwMotionKey;

	m_MotionDeque.pop_front();

	const DWORD dwCurrentMotionIndex=GET_MOTION_INDEX(dwMotionKey);
	...
	const DWORD dwRealMotionKey = __SetMotion(kSetMotData);

	if (0 == dwRealMotionKey)
		return;

	//float fDurationTime=rReservingMotionNode.fDuration;
	const float fDurationTime = GetMotionDuration(dwRealMotionKey) / fSpeedRatio;
	const float fStartTime = rReservingMotionNode.fStartTime;
	const float fEndTime = fStartTime + fDurationTime;
...
void CActorInstance::CurrentMotionProcess()
{
	...
	const DWORD dwMotionIndex=GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey);
...
int CActorInstance::GetMotionMode() const
{
...
bool CActorInstance::InterceptMotion(EMotionPushType iMotionType, WORD wMotion, float fBlendTime, UINT uSkill, float fSpeedRatio)
{
	...
	const float fDuration = GetMotionDuration(dwRealMotionKey) / fSpeedRatio;
...
BOOL CActorInstance::IsUsingSkill() const
{
	const DWORD dwCurMotionIndex=__GetCurrentMotionIndex();
...
BOOL CActorInstance::IsFishing() const
{
	if (!m_pkCurRaceMotionData)
		return FALSE;
...
BOOL CActorInstance::CanCancelSkill() const
{
	assert(IsUsingSkill());
	return m_pkCurRaceMotionData->IsCancelEnableSkill();
}

BOOL CActorInstance::isLock() const
{
	const DWORD dwCurMotionIndex=__GetCurrentMotionIndex();
...
float CActorInstance::GetLastMotionTime(float fBlendTime)
{
	...
	const TReservingMotionNode & rMotionNode = m_MotionDeque[m_MotionDeque.size()-1];
...
float CActorInstance::GetMotionDuration(DWORD dwMotionKey)
{
	...
	const CGrannyMotion * pGrannyMotion = pMotion->GetMotionPointer(0);
	return pGrannyMotion->GetDuration();
}

MOTION_KEY CActorInstance::GetRandomMotionKey(MOTION_KEY dwMotionKey) const
{
	const WORD wMode = GET_MOTION_MODE(dwMotionKey);
	const WORD wIndex = GET_MOTION_INDEX(dwMotionKey);
...
void CActorInstance::PreAttack() const
{
}
...
DWORD CActorInstance::__SetMotion(const SSetMotionData& c_rkSetMotData, DWORD dwRandMotKey)
{
	...
	const UINT uNextMot = GET_MOTION_INDEX(c_rkSetMotData.dwMotKey);
	...
	if (m_pkHorse)
	{
		const WORD wMotionIndex = GET_MOTION_INDEX(dwMotKey);
		const WORD wMotionSubIndex = GET_MOTION_SUB_INDEX(dwMotKey);
		const DWORD dwChildMotKey = MAKE_RANDOM_MOTION_KEY(m_pkHorse->m_wcurMotionMode, wMotionIndex, wMotionSubIndex);
	...
	{
		const int iLoopCount = __GetLoopCount();
...
int CActorInstance::__GetLoopCount() const
{
	if (!m_pkCurRaceMotionData)
	{
...
bool CActorInstance::__CanAttack() const
{
	if (!m_pkCurRaceMotionData)
	{
...
bool CActorInstance::__CanNextComboAttack() const
{
	if (!m_pkCurRaceMotionData)
	{
...
bool CActorInstance::__IsComboAttacking() const
{
...
bool CActorInstance::__IsNeedFlyTargetMotion() const
{
	if (!m_pkCurRaceMotionData)
		return true;
...
bool CActorInstance::__HasMotionFlyEvent() const
{
	if (!m_pkCurRaceMotionData)
		return true;
...
bool CActorInstance::__IsStandUpMotion() const
{
