...
void CGameEventManager::Update() const
{
}
...
void CGameEventManager::ProcessEventScreenWaving(CActorInstance * pActorInstance, const CRaceMotionData::TScreenWavingEventData * c_pData)
{
	...
	const float fdx = PixelPosition.x - m_CenterPosition.x;
	const float fdy = PixelPosition.y - m_CenterPosition.y;
	const float fdz = PixelPosition.z - m_CenterPosition.z;
