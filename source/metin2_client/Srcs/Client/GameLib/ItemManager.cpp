...
BOOL CItemManager::SelectItemData(DWORD dwIndex)
{
	...
	if (m_ItemMap.end() == f)
	{
		const int n = m_vec_ItemRange.size();
		for (int i = 0; i < n; i++)
		{
...
CItemData * CItemManager::GetSelectedItemDataPointer() const
{
...
BOOL CItemManager::GetItemDataPointer(DWORD dwItemID, CItemData ** ppItemData)
{
	...
	const auto f = m_ItemMap.find(dwItemID);
...
bool CItemManager::LoadItemList(const char * c_szFileName)
{
    ...
    for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		const DWORD dwItemVNum=atoi(c_rstrID.c_str());
		...
		if (USE_VIETNAM_CONVERT_WEAPON_VNUM)
		{
			...
			const DWORD dwMildItemVnum = Vietnam_ConvertWeaponVnum(dwItemVNum);
			...
			{
				const DWORD dwMildBaseVnum = dwMildItemVnum / 10 * 10;
...
{
	const UINT uSrcLen=c_rstSrc.length();
	if (uSrcLen<2)
		return c_rstSrc;
...
bool CItemManager::LoadItemTable(const char* c_szFileName)
{
	...
	const auto pbData = new BYTE[dwDataSize];
		...
		if (m_ItemMap.end() == f)
		{
			...
			if (CResourceManager::Instance().IsFileExist(szName) == false)
			{
				...
				if (CResourceManager::Instance().IsFileExist(szName) == false)
				{
					...
					#endif
					constexpr DWORD EmptyBowl = 27995;
...
#endif
		const BYTE bSex = c_rstrSex[0] == 'M';

		const float fScaleX = atof(c_rstrScaleX.c_str()) * 0.01f;
		const float fScaleY = atof(c_rstrScaleY.c_str()) * 0.01f;
		const float fScaleZ = atof(c_rstrScaleZ.c_str()) * 0.01f;
