...
bool Environment_Load(SEnvironmentData& envData, const char* envFileName)
{
	...
	if (textLoader.SetChildNode("skybox"))
	{
		...
		const BYTE byGradientCount = envData.bySkyBoxGradientLevelUpper+envData.bySkyBoxGradientLevelLower;
...
{
	const int idx = c_rsrcPosition.x - c_rdstPosition.x;
	const int idy = c_rsrcPosition.y - c_rdstPosition.y;
...
BOOL CEaseOutInterpolation::isPlaying() const
{
	return m_fRemainingTime > 0.0f;
}

float CEaseOutInterpolation::GetValue() const
{
	return m_fValue;
}

float CEaseOutInterpolation::GetChangingValue() const
{
