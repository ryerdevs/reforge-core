...
DWORD CActorInstance::GetRace() const
{
	return m_eRace;
}
...
bool CActorInstance::SetRace(DWORD eRace)
{
	...
	if (pRaceData->CreateMotionModeIterator(itor))
	{
		do
		{
			const WORD wMotionMode = itor->first;
			...
			{
				const WORD wMotionIndex = itorMotion->first;
				...
				{
					const DWORD dwMotionKey = MAKE_RANDOM_MOTION_KEY(wMotionMode, wMotionIndex, i);
...
)
{
	...
	const CRaceData::SHair* pkHair=pRaceData->FindHair(eHair);
	if (pkHair)
	{
		if (!pkHair->m_stModelFileName.empty())
		{
			const auto pkHairThing = (CGraphicThing *)CResourceManager::Instance().GetResourcePointer(pkHair->m_stModelFileName.c_str());
...
void CActorInstance::SetShape(DWORD eShape, float fSpecular)
{
	...
	const CRaceData::SShape* pkShape=pRaceData->FindShape(eShape);
	if (pkShape)
	{
		...
		{
			const auto pModelThing = (CGraphicThing *)rkResMgr.GetResourcePointer(pkShape->m_stModelFileName.c_str());
			RegisterModelThing(0, pModelThing);
		}
...
void CActorInstance::ChangeMaterial(const char * c_szFileName)
{
	...
	const CRaceData::SShape* pkShape=pRaceData->FindShape(m_eShape);
	if (!pkShape)
		return;
	...
	const auto i = c_rkVct_kSkin.begin();
	const CRaceData::SSkin& c_rkSkinItem = *i;
...
DWORD CActorInstance::GetPartItemID(DWORD dwPartIndex) const
{
	if (dwPartIndex>=CRaceData::PART_MAX_NUM)
	{
...
void CActorInstance::SetSpecularInfo(BOOL bEnable, int iPart, float fAlpha)
{
	...
	const CRaceData::SShape * pkShape = pkRaceData->FindShape(m_eShape);
