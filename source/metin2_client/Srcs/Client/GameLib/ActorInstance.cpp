...
constexpr float gc_fActorSlideMoveSpeed = 5.0f;
...
void CActorInstance::AdjustDynamicCollisionMovement(const CActorInstance * c_pActorInstance)
{
	...
	const UINT uActorType = c_pActorInstance->GetActorType();
		...
		for (; itMain != m_BodyPointInstanceList.end(); ++itMain)
		{
			...
			for (DWORD i = 0; i < c_rMainSphereVector.size(); ++i)
			{
				...
				for(;itOpp != c_pActorInstance->m_BodyPointInstanceList.end();++itOpp)
				{
					...
					const D3DXVECTOR3 v3Delta = s.GetCollisionMovementAdjust(c_rMainSphere);
...
void CActorInstance::__AdjustCollisionMovement(const CGraphicObjectInstance * c_pGraphicObjectInstance)
{
	...
	for (; itMain != m_BodyPointInstanceList.end(); ++itMain)
	{
		...
		for (DWORD i = 0; i < c_rMainSphereVector.size(); ++i)
		{
			...
			const D3DXVECTOR3 v3Delta = c_pGraphicObjectInstance->GetCollisionMovementAdjust(c_rMainSphere);
			m_v3Movement+=v3Delta;
			c_rMainSphere.v3Position+=v3Delta;
...
BOOL CActorInstance::IsMovement() const
{
	if (m_pkHorse)
...
bool CActorInstance::IntersectDefendingSphere()
{
	for (auto it = m_DefendingPointInstanceList.begin(); it != m_DefendingPointInstanceList.end(); ++it)
	{
		...
		{
			const CDynamicSphereInstance & rInstance = *it2;
			D3DXVECTOR3 v3SpherePosition = rInstance.v3Position;
			const float fRadius = rInstance.fRadius;
			...
			const float b = D3DXVec3Dot(&v3Dir, &v3Distance);
			const float c = D3DXVec3Dot(&v3Distance, &v3Distance) - fRadius*fRadius;
...
void CActorInstance::__CreateTree(const char * c_szFileName)
{
	...
	const DWORD dwCRC32 = GetCaseCRC32(strFileName.c_str(), strFileName.size());
...
void CActorInstance::__DestroyTree() const
{
	if (!m_pkTree)
		return;
...
void CActorInstance::__SetTreePosition(float fx, float fy, float fz) const
{
	if (!m_pkTree)
		return;
