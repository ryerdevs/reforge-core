...
class MysqlWrapper : public CSingleton<MysqlWrapper>
{
	...
	public:
		...
		int CountRows(MYSQL_RES * res = nullptr)
		{
			return (res) ? mysql_num_rows(res) : 0;
		}

		int CountFields(MYSQL_RES * res = nullptr)
		{
			return (res) ? mysql_num_fields(res) : 0;
		}

		MYSQL_ROW FetchRow(MYSQL_RES * res = nullptr)
		{
			return (res) ? mysql_fetch_row(res) : nullptr;
		}

		bool Connect(const char* hostname, const char* user, const char* password, const char* database = nullptr, unsigned int port = 0)
		{
			if (!mysql_real_connect(m_con, hostname, user, password, database, port, nullptr, 0))
			{
				fprintf(stderr, "Error: %s\n", mysql_error(m_con));
				// mysql_close(m_con);
				exit(0);
