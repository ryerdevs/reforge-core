class Sandbox(object):
	...
	WHITE_LIST = ['__builtin__', 'types', __name__.lower(), '__main__', 'sys']
	def __init__(self, prevent_imported_modules = False, allowed_modules = []):
		self.prevent_imported_modules = prevent_imported_modules
		self.allowed_modules = [module.lower() for module in allowed_modules]
		...
		if self.prevent_imported_modules:
			...
			for k, v in old_modules.items():
				if type(v) == types.ModuleType:
					if not (k.lower() in self.WHITE_LIST or k.lower() in self.allowed_modules):
						sys.modules[k] = None
