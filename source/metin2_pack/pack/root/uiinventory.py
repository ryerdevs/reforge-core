...
class InventoryWindow(ui.ScriptWindow):
				...
				for j in xrange(acce.WINDOW_MAX_MATERIALS):
					...
					if isHere:
						if iCell == slotNumber:
							...
							if not slotNumber in self.listHighlightedAcceSlot:
								self.listHighlightedAcceSlot.append(slotNumber)

							slotNumberChecked = 1
					else:
						if slotNumber in self.listHighlightedAcceSlot and not slotNumberChecked:
							self.wndItem.DeactivateSlot(i)
	...
	def OverInItem(self, overSlotPos):
		...
		if app.ENABLE_HIGHLIGHT_NEW_ITEM and overSlotPosGlobal in self.listHighlightedSlot:
