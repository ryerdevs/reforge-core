...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def OnMouseWheel(self, delta):
		for i in range(player.DRAGON_SOUL_PAGE_COUNT):
			if self.inventoryTab[i].IsInPosition():
				return self.OnScrollWheelInventoryPage(delta)
		if self.wndItem.IsInPosition():
			return self.OnScrollWheelInventoryPage(delta)
		if self.GetChild("TabControl").IsInPosition():
			return self.OnScrollWheelInventoryKind(delta)

		return True

	def OnScrollWheelInventoryPage(self, delta):
		if delta < 0:
			next_page = min(self.inventoryPageIndex+1, player.DRAGON_SOUL_PAGE_COUNT-1)
	...
	def OnScrollWheelInventoryKind(self, delta):
		if delta < 0:
			next_page = min(self.DSKindIndex+1, len(self.tabButtonDict)-1)
			self.SetDSKindIndex(next_page)
			self.tabButtonDict[next_page].Down()
		else:
			prev_page = max(self.DSKindIndex-1, 0)
			self.SetDSKindIndex(prev_page)
			self.tabButtonDict[prev_page].Down()
		return True
