...
bool CTerrainImpl::LoadAttrMap(const char *c_szFileName)
{
	...
	const DWORD dwFileSize = kMappedFile.Size();
...
#pragma pack(pop)
		...
		constexpr WORD c_wAttrMapMagic = 2634;
		...
		const DWORD dwFileRestSize=dwFileSize-sizeof(kAttrMapHeader);
		const DWORD dwFileNeedSize=sizeof(m_abyAttrMap);
...
#pragma pack(pop)
		...
		constexpr WORD c_wWaterMapMagic = 5426;
		...
		const DWORD dwFileRestSize = dwFileSize - sizeof(kWaterMapHeader);
		const DWORD dwFileNeedSize = sizeof(m_abyWaterMap) + sizeof(long) * m_byNumWater;
		const DWORD dwFileNeedSize2 = sizeof(m_abyWaterMap) + sizeof(WORD) * m_byNumWater;
...
DWORD CTerrainImpl::GetShadowMapColor(float fx, float fy) const
{
