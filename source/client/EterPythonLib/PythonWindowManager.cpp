...
namespace UI
{
	...
	{
		const long ldx = abs(m_lMouseX - m_lPickedX);
		const long ldy = abs(m_lMouseY - m_lPickedY);
		if (ldx+ldy < 10)
			return FALSE;
	...
	DWORD CWindowManager::GetAttachingType() const
	{
		return m_dwAttachingType;
	}

	DWORD CWindowManager::GetAttachingIndex() const
	{
		return m_dwAttachingIndex;
	}

	DWORD CWindowManager::GetAttachingSlotNumber() const
	{
		return m_dwAttachingSlotNumber;
	}

	void CWindowManager::GetAttachingIconSize(BYTE * pbyWidth, BYTE * pbyHeight) const
	{
	...
	void CWindowManager::SetParent(CWindow * pWindow, CWindow * pParentWindow) const
	{
		if (!pWindow)
		{
	...
	void CWindowManager::SetTop(CWindow * pWin) const
	{
		if (!pWin->HasParent())
			return;
	...
	void CWindowManager::RunMouseMove(long x, long y)
	{
		...
		if (m_pLeftCaptureWindow)
		{
			...
			{
				const long x = m_lMouseX - m_lDragX;
				const long y = m_lMouseY - m_lDragY;
	...
	{
		const auto itor = m_KeyCaptureWindowMap.find(vkey);
	...
	#ifdef ENABLE_UI_CIRCLE
	CWindow* CWindowManager::RegisterCircle(PyObject* po, const char* c_szLayer)
	{
		assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));

		CWindow* pWin = new CCircle(po);
		m_LayerWindowMap[c_szLayer]->AddChild(pWin);
