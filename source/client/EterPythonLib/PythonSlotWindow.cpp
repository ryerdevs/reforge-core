...
void CSlotWindow::SetSlotCount(DWORD dwIndex, DWORD dwCount)
{
	...
	else
	{
		...
		if (!pSlot->pNumberLine)
		{
			const auto pNumberLine = new UI::CNumberLine(this);
...
#ifdef ENABLE_HIGHLIGHT_NEW_ITEM
	const int slot_index = MINMAX(0, pSlot->byyPlacedItemSize - 1, SLOT_ACTIVE_EFFECT_COUNT-1);
...
void CSlotWindow::ClearSlot(TSlot * pSlot) const
{
	pSlot->bActive = FALSE;
...
DWORD CSlotWindow::GetSlotCount() const
{
...
DWORD CSlotWindow::GetSelectedSlotCount() const
{
...
BOOL CSlotWindow::OnMouseLeftButtonUp()
{
	...
	if (IsIn())
	{
		...
		{
			const auto pSlotWin = (UI::CSlotWindow *)pWin;
...
const CWindow * pPointWindow = UI::CWindowManager::Instance().GetPointWindow();
...
void CSlotWindow::OnUpdate()
{
	...
	if (m_bEnableMask)
	{
		...
		for (TSlot& slot : m_SlotList)
		{
			if (slot.pNumberLine)
				slot.pNumberLine->SetClippingMaskRect(m_rMaskRect);

			if (slot.pSignImage)
				slot.pSignImage->SetClippingMaskRect(m_rMaskRect);

			if (slot.pFinishCoolTimeEffect)
				slot.pFinishCoolTimeEffect->SetClippingMaskRect(m_rMaskRect);

			if (slot.pSlotButton)
				slot.pSlotButton->SetClippingMaskRect(m_rMaskRect);

			if (slot.pCoverButton)
				slot.pCoverButton->SetClippingMaskRect(m_rMaskRect);
		}
	}
#endif
}
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
...
#if defined(__BL_CLIP_MASK__)
			rSlot.pInstance->Render(m_bEnableMask ? &m_rMaskRect : nullptr);
#else
		...
		{
			const int ix = rSlot.byxPlacedItemSize*ITEM_WIDTH + rSlot.ixPosition - 4;
			const int iy = rSlot.iyPosition + rSlot.byyPlacedItemSize*ITEM_HEIGHT - 12 + 2;
		...
		if (rSlot.bActive)
		{
			const int slot_index = MINMAX(0, rSlot.byyPlacedItemSize - 1, SLOT_ACTIVE_EFFECT_COUNT-1);
			if (m_pSlotActiveEffect[slot_index])
			{
				...
				m_pSlotActiveEffect[slot_index]->SetDiffuseColor(rSlot.d3Color);
				m_pSlotActiveEffect[slot_index]->Render();
			}
		}
#else
		if (rSlot.bActive && m_pSlotActiveEffect)
		{
			const int ix = m_rect.left + rSlot.ixPosition;
			const int iy = m_rect.top + rSlot.iyPosition;
...
#if defined(__BL_CLIP_MASK__)
		m_pBaseImageInstance->Render(m_bEnableMask ? &m_rMaskRect : nullptr);
#else
...
BOOL CSlotWindow::GetPickedSlotPointer(TSlot ** ppSlot)
{
	...
	const int ixLocal = lx - m_rect.left;
	const int iyLocal = ly - m_rect.top;
...
void CSlotWindow::__CreateBaseImage(const char * c_szFileName, float fr, float fg, float fb, float fa)
{
	...
	const auto pImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(c_szFileName);
...
#ifdef ENABLE_HIGHLIGHT_NEW_ITEM
void CSlotWindow::__DestroyAllSlotEnableEffect()
{
	for (int i = 0; i < SLOT_ACTIVE_EFFECT_COUNT; ++i)
...
void CSlotWindow::__DestroyFinishCoolTimeEffect(TSlot * pSlot) const
{
	if (pSlot->pFinishCoolTimeEffect)
	{
