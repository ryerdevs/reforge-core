...
#endif
{
	...
	float v3 = m_v2Position.x - 0.5f;
	float v4 = m_v2Position.y - 0.5f;

	if (!m_fRotation)
	{
		v1 -= m_RenderingRect.left;
		v2 -= m_RenderingRect.top;
		v3 += float(pImage->GetWidth()) * m_v2Scale.x + m_RenderingRect.right;
		v4 += float(pImage->GetHeight()) * m_v2Scale.y + m_RenderingRect.bottom;
	...
	if (m_fRotation)
	{
		const float fimgHalfWidth = float(pImage->GetWidth()) / 2.0f * m_v2Scale.x;
		const float fimgHalfHeight = float(pImage->GetHeight()) / 2.0f * m_v2Scale.y;
		...
		const float fRadian = D3DXToRadian(m_fRotation);
