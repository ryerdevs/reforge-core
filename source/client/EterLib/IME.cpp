...
public:
	...
	void DisableCiceroOnThisWnd( HWND hwnd ) const
	{
		if ( m_ptim == nullptr)
			return;
...
bool CIME::Initialize(HWND hWnd)
{
	...
	if(GetSystemDirectoryA(szPath, MAX_PATH+1)) {
		...
		if(ms_hImm32Dll)
		{
			...
			const auto _ImmDisableTextFrameService = (BOOL (WINAPI*)(DWORD))GetProcAddress(ms_hImm32Dll, "ImmDisableTextFrameService");
...
bool CIME::IsIMEEnabled() const
{
	return ms_bImeEnabled;
}

void CIME::EnableIME(bool bEnable) const
{
...
void CIME::DisableIME() const
{
	EnableIME(false);
}

void CIME::EnableCaptureInput() const
{
	ms_bCaptureInput = true;
}

void CIME::DisableCaptureInput() const
{
	ms_bCaptureInput = false;
}

bool CIME::IsCaptureEnabled() const
{
...
int CIME::GetReading(std::string & rstrText) const
{
	char reading[IMEREADING_MAXLEN];

	if(ms_wstrReading.size() == 0)
		return 0;
	const int readingLen = WideCharToMultiByte(ms_uOutputCodePage, 0, &ms_wstrReading[0], ms_wstrReading.size(), reading, sizeof(reading), nullptr, nullptr);
...
int CIME::GetReadingError() const
{
...
void CIME::SetText(const char* szText, int len) const
{
	ms_compLen = 0;
	ms_ulbegin = 0;
	ms_ulend = 0;
...
int  CIME::GetText(std::string & rstrText, bool addCodePage) const
{
...
const char* CIME::GetCodePageText() const
{
...
int CIME::GetCodePage() const
{
	return ms_uOutputCodePage;
}

int CIME::GetCandidatePageCount() const
{
	return ms_dwCandidatePageSize;
}

int CIME::GetCandidateCount() const
{
	return ms_dwCandidateCount;
}

int  CIME::GetCandidate(DWORD index, std::string & rstrText) const
{
	if(index >= MAX_CANDLIST)
		return 0;

	const LPCWSTR wszText = ms_wszCandidate[index];
	if(wszText == nullptr)
		return 0;

	const int wTextLen = wcslen(wszText);
	if(wTextLen == 0)
		return 0;

	char text[IMESTR_MAXLEN];
	const int len = ::WideCharToMultiByte(CP_UTF8, 0, wszText, wTextLen, text, sizeof(text), nullptr, nullptr);
...
int CIME::GetCandidateSelection() const
{
	return ms_dwCandidateSelection;
}

void CIME::SetInputMode(DWORD dwMode) const
{
	const HIMC hImc = ImmGetContext(ms_hWnd);
...
void CIME::PasteTextFromClipBoard() const
{
	if (!m_bEnablePaste)
		return;
	...
	const auto buffer = (char*)GlobalLock(handle);
	if (!buffer) // @fixme008
	{
		GlobalUnlock(handle);
		CloseClipboard();
	...
	const std::string strClipboard = buffer;
	GlobalUnlock(handle);
	CloseClipboard();
...
{
	const UINT uLanguage = (UINT) GETLANG();
	CheckToggleState();
	ChangeInputLanguageWorker();
...
void CIME::SetCurPos(int offset) const
{
...
void CIME::PasteString(const char * str) const
{
...
/*---------------------------------------------------------------------------*/ /* Private */
void CIME::InsertString(wchar_t* wString, int iSize) const
{
...
UINT CIME::GetCodePageFromLang(LANGID langid) const
{
...
void CIME::ResultProcess(HIMC hImc) const
{
...
void CIME::AttributeProcess(HIMC hImc) const
{
	BYTE	attribute[IMESTR_MAXLEN];
	const LONG	attributeLen = ImmGetCompositionStringW(hImc, GCS_COMPATTR, &attribute, sizeof(attribute)) / sizeof(BYTE);
...
void CIME::CandidateProcess(HIMC hImc) const
{
	...
	if(dwCandidateLen > 0) {
		...
		if( GETLANG() == LANG_CHS ) {
			...
			constexpr int maxCandChar = 18 * (3 - sizeof(TCHAR));
			UINT cChars = 0;
			UINT i;
			for (i = 0; i < ms_dwCandidateCount; i++)
			{
				const UINT uLen = lstrlenW((LPWSTR)((DWORD)lpCandidateList + lpCandidateList->dwOffset[i])) + (3 - sizeof(WCHAR));
...
void CIME::ReadingProcess(HIMC hImc) const
{
    if (!ms_adwId[0])
	{
		return;
    ...
    if (_GetReadingString)
	{
        ...
        const int wstrLen = _GetReadingString(hImc, 0, nullptr, (PINT)&dwErr, &bVertical, &uMaxUiLen);

		if(wstrLen == 0) {
			ms_wstrReading.resize(0);
		} else {
			const auto wstr = (wchar_t*)alloca(sizeof(wchar_t) * wstrLen);
        ...
        const INPUTCONTEXT *lpIC = _ImmLockIMC(hImc);
		...
		if(tempLen == 0) {
			...
			if(bUnicodeIme) {
				...
				const int wstrLen = MultiByteToWideChar(ms_uInputCodePage, 0, (char*)temp, tempLen, nullptr, 0);
				const auto wstr = (wchar_t*)alloca(sizeof(wchar_t)*wstrLen);
...
bool CIME::IsMax(const wchar_t* wInput, int len) const
{
...
DWORD CIME::GetImeId( UINT uIndex ) const
{
    ...
    DWORD   dwVerHandle;
	const DWORD   dwVerSize = GetFileVersionInfoSize(szTmp, &dwVerHandle);
	const LANGID	langId = LOWORD(ms_hklCurrent);

    if (dwVerSize)
	{
		const LPVOID lpVerBuffer = alloca(dwVerSize);
...
bool CIME::GetReadingWindowOrientation() const
{
    ...
    if(!bHorizontalReading && (GETLANG() == LANG_CHT))
    {
        ...
        HKEY hKey;
        const DWORD dwVer = ms_adwId[0] & 0xFFFF0000;
		...
		if (bstr)
		{
			...
			if ( fVertical )
			{
				...
				for ( UINT i = 0; i < cchMax; i++ )
				{
					...
					if ( *pszSource )
					{
						const LPWSTR pszNextSrc = CharNextW(pszSource);
						const SIZE_T size = (LPSTR)pszNextSrc - (LPSTR)pszSource;
...
/* IME Message Handler */
LRESULT CIME::WMInputLanguage(HWND hWnd, UINT /*uiMsg*/, WPARAM /*wParam*/, LPARAM lParam) const
{
	ChangeInputLanguage();
	return 0;
}

LRESULT CIME::WMStartComposition(HWND /*hWnd*/, UINT /*uiMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/) const
{
	return 1L;
}
...
LRESULT CIME::WMEndComposition(HWND /*hWnd*/, UINT /*uiMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/) const
{
	ms_compLen = 0;
	ms_ulbegin = 0;
	ms_ulend = 0;
...
LRESULT CIME::WMNotify(HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam) const
{
	LRESULT		result = 0;
