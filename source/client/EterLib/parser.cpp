...
const char* LocaleString_FindChar(const char* base, int len, char test)
{
	...
	const DWORD codePage = GetDefaultCodePage();
...
bool Group::GetArg(const char *c_arg_base, int arg_len, TArgList & argList) const
{
    char szName[32 + 1];
    char szValue[64 + 1];
...
int Group::Create(const std::string & stSource)
{
	...
	const int str_len = stSource.length();
	int str_pos = 0;
	int len_line = 0;
	...
	while (str_pos < str_len)
	{
		...
		const int word_len = word_next - word;
		if (word_len > 1)
		{
			lastIsLine = false;
			str_pos += word_len;
		...
		else if (word_len == 1)
		{
			...
			if ('[' == cur)
			{
				...
				if (space)
				{
					const int name_len = space - box_data;
					...
					const int arg_len = data_len - (arg - box_data);
...
bool Group::ReadCmd(TCmd & cmd) const
{
    if (m_cmdList.empty())
        return false;
