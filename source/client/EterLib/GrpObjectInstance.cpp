...
bool CGraphicObjectInstance::isIntersect(const CRay & c_rRay, float * pu, float * pv, float * pt) const
{
	...
	const TIndex Indices[36] = {0, 1, 2, 1, 3, 2,
						  2, 0, 6, 0, 4, 6,
						  0, 1, 4, 1, 5, 4,
						  1, 3, 5, 3, 7, 5,
						  3, 2, 7, 2, 6, 7,
...
DWORD CGraphicObjectInstance::GetCollisionInstanceCount() const
{
	return m_StaticCollisionInstanceVector.size();
}

CBaseCollisionInstance * CGraphicObjectInstance::GetCollisionInstanceData(DWORD dwIndex) const
{
...
int CGraphicObjectInstance::GetPortal(DWORD dwIndex) const
{
	if (dwIndex >= PORTAL_ID_MAX_NUM)
	{
		assert(dwIndex < PORTAL_ID_MAX_NUM);
