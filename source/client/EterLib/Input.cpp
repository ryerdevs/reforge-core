...
HRESULT CInputDevice::CreateDevice(HWND /*hWnd*/) const
{
	if (ms_lpDI)
	{
		ms_lpDI->AddRef();
...
void CInputKeyboard::ResetKeyboard() const
{
	memset(ms_diks, 0, sizeof(ms_diks));
	memset(ms_bPressedKey, 0, sizeof(ms_bPressedKey));
}

bool CInputKeyboard::InitializeKeyboard(HWND hWnd) const
{
	if (ms_lpKeyboard)
		return true;
	...
	const DWORD dwCoopFlags = DISCL_FOREGROUND | DISCL_NONEXCLUSIVE;
...
bool CInputKeyboard::IsPressed(int iIndex) const
{
