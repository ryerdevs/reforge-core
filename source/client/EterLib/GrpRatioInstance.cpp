...
{
	const DWORD curTime = GetTime();
	const DWORD elapsedTime = curTime - m_baseTime;

	if (elapsedTime < m_blendTime)
	{
		const float diff = m_dstRatio - m_srcRatio;
		const float rate = elapsedTime / float(m_blendTime);
...
DWORD CGraphicRatioInstance::GetTime() const
{
