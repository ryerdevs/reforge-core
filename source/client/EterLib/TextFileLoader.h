...
class CTextFileLoader
{
	public:
		typedef struct SGroupNode
		{
			...
			bool IsGroupNameKey(DWORD dwGroupNameKey) const;
	...
	public:
		...
		BOOL SetParentNode();
		BOOL GetCurrentNodeName(std::string * pstrName) const;

		BOOL IsToken(const std::string & c_rstrKey) const;
		BOOL GetTokenVector(const std::string & c_rstrKey, CTokenVector ** ppTokenVector) const;
