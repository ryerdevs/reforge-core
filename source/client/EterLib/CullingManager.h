...
public:
	...
	void RangeTest(const Vector3d& p, float radius, SpherePackCallback* callback) const
	{
		m_Factory->RangeTest(p, radius, callback);
	}

	void PointTest2d(const Vector3d& p, SpherePackCallback* callback) const
	{
	...
	CullingHandle Register(CGraphicObjectInstance * ob) const;
	void Unregister(CullingHandle h) const;
