...
BOOL CMappedFile::Create(const char * filename, const void** dest, int offset, int size)
{
	...
	const int ret = Map(dest, offset, size);
	return (ret) > 0;
}
...
int CMappedFile::Map(const void **dest, int offset, int size)
{
	...
	const DWORD dwFileMapStart = (m_dataOffset / dwSysGran) * dwSysGran;
	const DWORD dwMapViewSize = (m_dataOffset % dwSysGran) + m_mapSize;
	const INT iViewDelta = m_dataOffset - dwFileMapStart;
...
DWORD CMappedFile::GetPosition() const
{
	return m_dataOffset;
}
...
DWORD CMappedFile::GetSeekPosition(void) const
{
	return m_seekPosition;
}
