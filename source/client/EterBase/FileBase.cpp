...
DWORD CFileBase::Size() const
{
	return (m_dwSize);
}

void CFileBase::SeekCur(DWORD size) const
{
	SetFilePointer(m_hFile, size, nullptr, FILE_CURRENT);
}

void CFileBase::Seek(DWORD offset) const
{
	if (offset > m_dwSize)
		offset = m_dwSize;
...
DWORD CFileBase::GetPosition() const
{
...
{
	DWORD dwUseless;
	const BOOL ret = WriteFile(m_hFile, src, bytes, &dwUseless, nullptr);
