...
int CPoly::my_irandom(double start, double end) const
{
	...
	const int is = int(start + 0.5);
	const int ie = int(end - start + 0.5) + 1;
...
int CPoly::find(const string & s) const
{
    int l, m, r;
...
int CPoly::SetVar(const string & strName, double dVar) const
{
    if (ErrorOccur) return false;
    const int index=find(strName);
...
int CPoly::GetVarCount() const
{
	return lSymbol.size() - MathSymbolCount;
}

const char * CPoly::GetVarName(unsigned int dwIndex) const
{
