...
{
	const auto fRaceName=m_kMap_dwRaceKey_stRaceName.find(dwRaceIndex);
...
bool CRaceManager::__LoadRaceMotionList(CRaceData & rkRaceData, const char * pathName, const char * motionListFileName) const
{
	...
	const UINT uLineCount=kTextFileLoader.GetLineCount();
	for (UINT uLineIndex=0; uLineIndex<uLineCount; ++uLineIndex)
	{
		...
		{
			constexpr size_t c_cutLengthLimit = 2;
			bool bFound = false;
...
{
	const TRaceDataIterator itor = m_RaceDataMap.find(dwRaceIndex);
...
CRaceData * CRaceManager::GetSelectedRaceDataPointer() const
{
...
const char * CRaceManager::GetFullPathFileName(const char * c_szFileName) const
{
...
void CRaceManager::Create() const
{
...
{
	const auto it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(iVnum);
