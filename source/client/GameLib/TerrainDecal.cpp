...
void CTerrainDecal::Make(D3DXVECTOR3 v3Center, D3DXVECTOR3 v3Normal, D3DXVECTOR3 v3Tangent, float fWidth, float fHeight, float fDepth)
{
	...
	const float fSearchRadius = fMAX(fWidth, fHeight);// 0.75f >= sqrtf(2)/2;
	const float fMinX = v3Center.x - fSearchRadius;
	const float fMaxX = v3Center.x + fSearchRadius;
	const float fMinY = fabs(v3Center.y) - fSearchRadius;
	const float fMaxY = fabs(v3Center.y) + fSearchRadius;
	...
	const float fOne_over_w = 1.0f / fWidth;
	const float fOne_over_h = 1.0f / fHeight;
	for (DWORD dwi = 0; dwi < m_dwVertexCount; ++dwi)
	{
		D3DXVECTOR3 v3 = m_Vertices[dwi].position - m_v3Center;
		const float fu = -D3DXVec3Dot(&v3, &v3Binormal) * fOne_over_w + 0.5f;
		const float fv = -D3DXVec3Dot(&v3, &v3Tangent) * fOne_over_h + 0.5f;
...
void CTerrainDecal::SearchAffectedTerrainMesh(float fMinX,
											  ...
											  D3DXVECTOR3 * pv3AffectedNormal) const
{
	if (!m_pMapOutdoor)
		return;
