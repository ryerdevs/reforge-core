...
void CTerrain::LoadMiniMapTexture(const char * c_pchMiniMapFileName)
{
	...
	const auto pImage = (CGraphicImage *) CResourceManager::Instance().GetResourcePointer(c_pchMiniMapFileName);
...
{
	const DWORD dwStart = ELTimer_GetMSec();
	const auto pImage = (CGraphicImage *) CResourceManager::Instance().GetResourcePointer(ShadowFileName);
	...
	{
		TraceError(" CTerrain::LoadShadowTexture - ShadowTexture is Empty %s", ShadowFileName); // @warme003 filepath
...
bool CTerrain::LoadShadowMap(const char * c_pszFileName)
{
	...
	const DWORD dwShadowMapSize = sizeof(WORD) * 256 * 256;
...
WORD CTerrain::WE_GetHeightMapValue(short sX, short sY) const
{
...
{
	const long lMapWidth = XSIZE * CELLSCALE;
	const long lMapHeight = YSIZE * CELLSCALE;
	while (ix < 0)
		ix += lMapWidth;
...
bool CTerrain::isAttrOn(WORD wCoordX, WORD wCoordY, BYTE byAttrFlag) const
{
...
BYTE CTerrain::GetAttr(WORD wCoordX, WORD wCoordY) const
{
	if (wCoordX >= ATTRMAP_XSIZE || wCoordY >= ATTRMAP_YSIZE)
	{
...
void CTerrain::GetWaterHeight(BYTE byWaterNum, long * plWaterHeight) const
{
...
bool CTerrain::GetWaterHeight(WORD wCoordX, WORD wCoordY, long * plWaterHeight) const
{
...
void CTerrain::RAW_AllocateSplats(bool bBGLoading)
{
	...
	const DWORD dwTexCount = GetTextureSet()->GetTextureCount();
...
void CTerrain::RAW_CountTiles()
{
	...
	{
		const long lPatchIndexY = min(max((y-1)/PATCH_TILE_YSIZE,0), PATCH_YCOUNT - 1);
		for (long x = 0; x < TILEMAP_RAW_XSIZE; ++x)
		{
			const long lPatchIndexX = min(max((x-1)/(PATCH_TILE_XSIZE), 0), PATCH_XCOUNT - 1);
		...
		if (rSplat.NeedsUpdate)
		{
			if (m_TerrainSplatPatch.TileCount[i] > 0)
			{
				...
				for (long y = 0; y < SPLATALPHA_RAW_YSIZE; ++y)
				{
					...
					{
						const long lTileMapOffset = y * TILEMAP_RAW_XSIZE + x;
...
LPDIRECT3DTEXTURE9 CTerrain::AddTexture32(BYTE byImageNum, BYTE * pbyImage, long lTextureWidth, long lTextureHeight)
{
	...
	const bool bResizedAndSuccess = false;

	IDirect3DTexture9* pkTex= nullptr;

	const UINT uiNewWidth = 256;
	const UINT uiNewHeight = 256;
	...
	{
		const UINT uDstSize=uSrcSize>>1;
...
void CTerrain::PutImage32(BYTE *src, BYTE *dst, long src_pitch, long dst_pitch, long texturewidth, long textureheight, bool bResize) const
{
	for (int y = 0; y < textureheight; ++y)
    {
		for (int x = 0; x < texturewidth; ++x)
		{
			const DWORD packed_pixel = src[x] << 24;
...
void CTerrain::PutImage16(BYTE *src, BYTE *dst, long src_pitch, long dst_pitch, long texturewidth, long textureheight, bool bResize) const
{
	for (int y = 0; y < textureheight; ++y)
    {
		for (int x = 0; x < texturewidth; ++x)
		{
			const WORD packed_pixel = src[x] << 8;
