...
void CMapOutdoor::__RenderTerrain_RenderHardwareTransformPatch()
{
	...
	const std::pair<float, long> fog_far(fFogFarDistance+1600.0f, 0);
	const std::pair<float, long> fog_near(fFogNearDistance-3200.0f, 0);
...
void CMapOutdoor::__HardwareTransformPatch_RenderPatchNone(long patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType) const
{
	...
	const CGraphicVertexBuffer* pkVB=pTerrainPatchProxy->HardwareTransformPatch_GetVertexBufferPtr();
	if (!pkVB)
		return;
