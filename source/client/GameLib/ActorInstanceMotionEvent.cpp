...
void CActorInstance::SoundEventProcess(BOOL bCheckFrequency) const
{
	if (!m_pkCurRaceMotionData)
		return;
...
BOOL CActorInstance::__IsHiding() const
{
	return m_isHiding;
}
...
void CActorInstance::ProcessMotionEventEffectEvent(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	auto c_pEffectData = (const CRaceMotionData::TMotionEffectEventData *)c_pData;
...
void CActorInstance::ProcessMotionEventEffectToTargetEvent(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	const auto c_pEffectToTargetData = (const CRaceMotionData::TMotionEffectToTargetEventData *)c_pData;
	...
	else
	{
		...
		{
			const auto pTargetInstance = (CActorInstance *)m_kFlyTarget.GetFlyTarget();
			const D3DXVECTOR3 v3Position(	c_pEffectToTargetData->v3EffectPosition.x,
		...
		else
		{
			...
			const D3DXVECTOR3 v3Position(	c_rv3FlyTarget.x + c_pEffectToTargetData->v3EffectPosition.x,
...
void CActorInstance::ProcessMotionEventSpecialAttacking(int iMotionEventIndex, const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	const auto c_pAttackingData = (const CRaceMotionData::TMotionAttackingEventData *)c_pData;

	const float fRadian = D3DXToRadian(270.0f + 360.0f - GetRotation());
...
void CActorInstance::ProcessMotionEventSound(const CRaceMotionData::TMotionEventData * c_pData) const
{
	if (CRaceMotionData::MOTION_EVENT_TYPE_SOUND != c_pData->iType)
		return;

	const auto c_pSoundData = (const CRaceMotionData::TMotionSoundEventData *)c_pData;
...
void CActorInstance::ProcessMotionEventFly(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	auto c_pFlyData = (const CRaceMotionData::TMotionFlyEventData *)c_pData;
...
void CActorInstance::ProcessMotionEventWarp(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		const D3DXVECTOR3 v3MainPosition(m_x, m_y, m_z);
		...
		const TPixelPosition DestPixelPosition = c_rv3TargetPosition - (v3Distance * sc_fDistanceFromTarget);
...
void CActorInstance::ProcessMotionEventUnk11(DWORD dwcurFrame, int iIndex, const CRaceMotionData::TMotionEventData * c_pData) // AniSpeed ON
{
	...
	const auto c_pUnk11TargetData = (const CRaceMotionData::TMotionUnk11EventData *)c_pData;
	...
	// workaround
	{
		...
		if (m_kFlyTarget.IsValidTarget())
		{
			...
			D3DXVECTOR3 v3Distance = c_rv3TargetPosition - v3MainPosition;
			D3DXVec3Normalize(&v3Distance, &v3Distance);
			const TPixelPosition DestPixelPosition = c_rv3TargetPosition - (v3Distance * (sc_fDistanceFromTarget+c_pUnk11TargetData->iAniSpeed));
...
void CActorInstance::ProcessMotionEventUnk12(DWORD dwcurFrame, int iIndex, const CRaceMotionData::TMotionEventData * c_pData) const
// AniSpeed OFF
{
	if (CRaceMotionData::MOTION_EVENT_TYPE_UNK12 != c_pData->iType)
		return;

	auto c_pUnk12TargetData = (const CRaceMotionData::TMotionUnk12EventData *)c_pData;
