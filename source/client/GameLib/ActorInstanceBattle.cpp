...
bool CActorInstance::CanAct() const
{
	if (IsDead())
		return false;
...
BOOL CActorInstance::IsClickableDistanceDestInstance(CActorInstance & rkInstDst, float fDistance)
{
	...
	const D3DXVECTOR3 kD3DVct3Src(kPPosSrc);
	...
	for (i=rkLstkDefPtInst.begin(); i!=rkLstkDefPtInst.end(); ++i)
	{
		...
		for (j=rkVctkDefSphere.begin(); j!=rkVctkDefSphere.end(); ++j)
		{
			...
			const float fMovDistance=D3DXVec3Length(&val);
			const float fAtkDistance=rkSphere.fRadius+fDistance;
...
bool CActorInstance::InputComboAttackCommand(float fDirRot)
{
	...
	{
		const float fElapsedTime = GetAttackingElapsedTime();
...
void CActorInstance::__RunNextCombo()
{
    ...
    const WORD wComboIndex = m_dwcurComboIndex;
    const WORD wComboType = __GetCurrentComboType();
    ...
    const DWORD dwComboArrayIndex = wComboIndex - 1;
    ...
    const WORD wcurComboMotionIndex = pComboData->ComboIndexVector[dwComboArrayIndex];
...
BOOL CActorInstance::isAttacking() const
{
...
BOOL CActorInstance::CanCheckAttacking() const
{
	if (isAttacking())
		return true;
...
BOOL CActorInstance::isNormalAttacking() const
{
	if (!m_pkCurRaceMotionData)
		return FALSE;
...
BOOL CActorInstance::isComboAttacking() const
{
	if (!m_pkCurRaceMotionData)
		return FALSE;
...
BOOL CActorInstance::IsSplashAttacking() const
{
	if (!m_pkCurRaceMotionData)
		return FALSE;
...
BOOL CActorInstance::__IsMovingSkill(WORD wSkillNumber) const
{
	enum
	{
...
BOOL CActorInstance::IsUsingMovingSkill() const
{
	return __IsMovingSkill(m_kCurMotNode.uSkill);
}

DWORD CActorInstance::GetComboIndex() const
{
...
DWORD CActorInstance::__GetOwnerVID() const
{
	return m_dwOwnerVID;
}
...
{
	if (uiSkill != 0) // skip skills
		return c_rAttackData.fInvisibleTime;

	// if it's NRaceData::MOTION_TYPE_COMBO, you can literally return 0.0f here and it will still be fine

	if (m_fAtkSpd < 1.0f) // skip negative speed attacks
		return c_rAttackData.fInvisibleTime;

	if (c_rAttackData.fInvisibleTime < 0.10f) // skip low values
		return c_rAttackData.fInvisibleTime;

	const auto adjustedIT = c_rAttackData.fInvisibleTime / m_fAtkSpd;
	//TraceError("GetInvisibleTimeAdjust fInvisibleTime(%.3f) m_fAtkSpd(%.3f) adjustedIT(%.3f)", c_rAttackData.fInvisibleTime, m_fAtkSpd, adjustedIT);
	return adjustedIT;
}
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	const float fHeight = D3DXToDegree(atan2(-vec3Effect.x + v3Pos.x,+vec3Effect.y - v3Pos.y));
...
void CActorInstance::__HitStone(CActorInstance& rVictim) const
{
	if (rVictim.IsStun())
	{
		rVictim.Die();
...
void CActorInstance::__HitGood(CActorInstance& rVictim)
{
	...
	else
	{
		...
		if (!rVictim.isLock())
		{
			const float fRotRad = D3DXToRadian(GetRotation());
			const float fVictimRotRad = D3DXToRadian(rVictim.GetRotation());
			...
			const float fScalar = D3DXVec2Dot(&v2Normal, &v2VictimNormal);
...
void CActorInstance::__PushCircle(CActorInstance & rVictim)
{
	...
	const D3DXVECTOR3 v3SrcPos(c_rkPPosAtk.x, -c_rkPPosAtk.y, c_rkPPosAtk.z);
...
void CActorInstance::__PushDirect(CActorInstance & rVictim) const
{
	D3DXVECTOR3 v3Direction;
...
bool CActorInstance::__isInvisible() const
{
	if (IsDead())
		return true;
