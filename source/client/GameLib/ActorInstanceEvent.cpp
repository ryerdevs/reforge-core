...
void CActorInstance::__OnMoving()
{
	...
	const float distance=sqrt(kPPosDir.x*kPPosDir.x+kPPosDir.y*kPPosDir.y);
...
void CActorInstance::__OnHit(UINT uSkill, CActorInstance& rkActorVictm, BOOL isSendPacket) const
{
	IEventHandler& rkEventHandler=__GetEventHandlerRef();
	rkEventHandler.OnHit(uSkill, rkActorVictm, isSendPacket);
}

void CActorInstance::__OnClearAffects() const
{
	IEventHandler& rkEventHandler=__GetEventHandlerRef();
	rkEventHandler.OnClearAffects();
}

void CActorInstance::__OnSetAffect(UINT uAffect) const
{
	IEventHandler& rkEventHandler=__GetEventHandlerRef();
	rkEventHandler.OnSetAffect(uAffect);
}

void CActorInstance::__OnResetAffect(UINT uAffect) const
{
	IEventHandler& rkEventHandler=__GetEventHandlerRef();
	rkEventHandler.OnResetAffect(uAffect);
}

CActorInstance::IEventHandler& CActorInstance::__GetEventHandlerRef() const
{
	assert(m_pkEventHandler!=nullptr && "CActorInstance::GetEventHandlerRef");
	return *m_pkEventHandler;
}

CActorInstance::IEventHandler* CActorInstance::__GetEventHandlerPtr() const
{
	return m_pkEventHandler;
}
