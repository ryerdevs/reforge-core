...
class CRaceManager : public CSingleton<CRaceManager>
{
	...
	public:
		...
		const char * GetFullPathFileName(const char* c_szFileName) const;

		// Handling
		void CreateRace(DWORD dwRaceIndex);
		void SelectRace(DWORD dwRaceIndex);
		CRaceData * GetSelectedRaceDataPointer() const;
		// Handling
	...
	protected:
		...
		bool __LoadRaceMotionList(CRaceData& rkRaceData, const char* pathName, const char* motionListFileName) const;
