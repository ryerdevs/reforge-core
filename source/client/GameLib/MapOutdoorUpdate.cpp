...
struct FPCBlockerDistanceSort
{
	...
	{
		const auto val1 = (plhs->GetPosition() - m_v3Eye);
		const auto val2 = (prhs->GetPosition() - m_v3Eye);
...
void CMapOutdoor::__Game_UpdateArea(D3DXVECTOR3& v3Player)
{
	...
	const CCamera * pCamera = rCmrMgr.GetCurrentCamera();
	if (!pCamera)
		return;

	const float fDistance = pCamera->GetDistance();

	D3DXVECTOR3 v3View= pCamera->GetView();
	D3DXVECTOR3 v3Target = pCamera->GetTarget();
	D3DXVECTOR3 v3Eye= pCamera->GetEye();

	auto v3Light = D3DXVECTOR3(1.732f, 1.0f, -3.464f);
...
#else
void CMapOutdoor::__UpdateAroundAreaList() const
#endif
{
#ifdef __PERFORMANCE_CHECKER__
...
#ifdef ENABLE_AREA_OPTIMIZATION
	m_bAreaChangedThisFrame = bAreaChanged;
#endif
#ifdef __PERFORMANCE_CHECKER__
...
struct FGetShadowReceiverFromHeightData
{
	...
	CGraphicObjectInstance* GetCollectItem(UINT uIndex) const
	{
...
struct PCBlocker_SInstanceList
{
	...
	PCBlocker_SInstanceList(PCBlocker_CDynamicSphereInstanceVector* pkDSIVector)
	{
		...
		const D3DXVECTOR3 m_v3View = m_pCamera->GetView();
		const D3DXVECTOR3 m_v3Target = m_pCamera->GetTarget();
	...
	void __DEBUG_ShowInstanceMaxCount() const
	{
...
void CMapOutdoor::FPushTerrainToDeleteVector::operator () (CTerrain * pTerrain)
{
	...
	const WORD wReferenceCoordX = m_CurCoordinate.m_sTerrainCoordX;
	const WORD wReferenceCoordY = m_CurCoordinate.m_sTerrainCoordY;
...
{
	constexpr DWORD dwTerrainEraseInterval = 1000 * 60;
...
void CMapOutdoor::UpdateAreaList(long lCenterX, long lCenterY)
{
	...
	if (!rPushTerrainToDeleteVector.m_ReturnTerrainVector.empty())
	{
		...
		for (DWORD dwIndex = 0; dwIndex < rPushTerrainToDeleteVector.m_ReturnTerrainVector.size(); ++dwIndex)
		{
			...
			{
				const CTerrain * pTerrain = *aTerrainPtrItertor;
	...
	if (!rPushAreaToDeleteVector.m_ReturnAreaVector.empty())
	{
		...
		for (DWORD dwIndex = 0; dwIndex < rPushAreaToDeleteVector.m_ReturnAreaVector.size(); ++dwIndex)
		{
			...
			{
				const CArea * pArea = *aAreaPtrItertor;
...
void CMapOutdoor::ConvertTerrainToTnL(long lx, long ly)
{
	...
	const long diameter = m_lViewRadius * 2;
	...
	const long xw = x1 - x0 + 1;     /* Figure out how many patches are needed */
	const long yw = y1 - y0 + 1;

	const long ex = lx + diameter;
	const long ey = ly + diameter;
...
void CMapOutdoor::AssignPatch(long lPatchNum, long x0, long y0, long x1, long y1) const
{
...
void CMapOutdoor::UpdateQuadTreeHeights(CTerrainQuadtreeNode *Node)
{
	...
	for (long y = Node->y0; y <= Node->y1; y++)
	{
		...
		{
			const long patch = y * m_wPatchCount + x;
