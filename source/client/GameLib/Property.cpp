...
DWORD CProperty::GetCRC() const
{
	return m_dwCRC;
}

const char * CProperty::GetFileName() const
{
...
bool CProperty::GetString(const char * c_pszKey, const char ** c_ppString)
{
	...
	const auto it = m_stTokenMap.find(stTempKey.c_str());
...
void CProperty::PutString(const char * c_pszKey, const char * c_pszString)
{
	...
	const auto itor = m_stTokenMap.find(stTempKey);
...
bool CProperty::Save(const char * c_pszFileName)
{
	...
	const DWORD fourcc = MAKEFOURCC('Y', 'P', 'R', 'T');
	...
	if (0 == m_stCRC.length())
	{
		...
		GetTimeString(szCRC, time(nullptr));
...
{
	auto pcData = (const char *) c_pvData;
