...
void CActorInstance::UpdatePointInstance(TCollisionPointInstance * pPointInstance) const
{
	if (!pPointInstance)
	{
	...
	if (pPointInstance->isAttached)
	{
		...
		auto pmatBone = (D3DXMATRIX *)pModelInstance->GetBoneMatrixPointer(pPointInstance->dwBoneIndex);
...
bool CActorInstance::CreateCollisionInstancePiece(DWORD dwAttachingModelIndex, const NRaceData::TAttachingData * c_pAttachingData, TCollisionPointInstance * pPointInstance) const
{
	if (!c_pAttachingData)
	{
...
BOOL CActorInstance::__SplashAttackProcess(CActorInstance & rVictim)
{
	...
	if (rVictim.CheckCollisionDetection(&m_kSplashArea.SphereInstanceVector, &v3HitPosition))
	{
		...
		const int iMaxHitCount = (0 == c_rAttackData.iHitLimitCount ? 16 : c_rAttackData.iHitLimitCount);
		//Tracef(" ------------------- Splash Hit : %d\n", iCurrentHitCount);
...
BOOL CActorInstance::TestPhysicsBlendingCollision(CActorInstance & rVictim)
{
	...
	const auto v3Distance = D3DXVECTOR3(rVictim.m_x - kPPosLast.x, rVictim.m_y - kPPosLast.y, rVictim.m_z - kPPosLast.z);
	const float fDistance = D3DXVec3LengthSq(&v3Distance);
			...
			for (DWORD i = 0; i < c_rMainSphereVector.size(); ++i)
			{
				...
				for( int i = 1; i <= nSubCheckCount; ++ i )
				{
					...
					for (DWORD j = 0; j < c_rVictimSphereVector.size(); ++j)
					{
						...
						{
							const BOOL bResult = GetVector3Distance(c_rMainSphere.v3Position, c_rVictimSphere.v3Position) <= GetVector3Distance(c_rMainSphere.v3LastPosition, c_rVictimSphere.v3Position);
...
BOOL CActorInstance::TestActorCollision(CActorInstance & rVictim)
{
	...
	// Check Distance
	const auto v3Distance = D3DXVECTOR3(rVictim.m_x - m_x, rVictim.m_y - m_y, rVictim.m_z - m_z);
	const float fDistance = D3DXVec3LengthSq(&v3Distance);
	if (fDistance > 800.0f*800.0f)
		return FALSE;
