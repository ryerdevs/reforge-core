...
bool DetectCollisionDynamicZCylinderVSDynamicZCylinder(const CDynamicSphereInstance & c_rSphere1, const CDynamicSphereInstance & c_rSphere2)
{
	...
	const float r = c_rCylinder1.fRadius+c_rCylinder2.fRadius;
	const float rsq = r*r;
...
{
	const float r = c_rSphere1.fRadius+c_rSphere2.fRadius;
	const float rsq = r*r;
...
{
	const int fValue = abs((int) (360.0f - end + begin));
