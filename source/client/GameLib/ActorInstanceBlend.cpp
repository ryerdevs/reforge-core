...
float CActorInstance::GetAlphaValue() const
{
...
void CActorInstance::__BlendAlpha_Update()
{
	...
	const float fElapsedTime=__BlendAlpha_GetElapsedTime();

	if (fElapsedTime<m_kBlendAlpha.m_fDuration)
	{
		const float fCurAlpha=m_kBlendAlpha.m_fBaseAlpha+(m_kBlendAlpha.m_fDstAlpha-m_kBlendAlpha.m_fBaseAlpha)*fElapsedTime/m_kBlendAlpha.m_fDuration;
		SetBlendRenderMode();
...
{
	const float fCurTime=GetLocalTime();
