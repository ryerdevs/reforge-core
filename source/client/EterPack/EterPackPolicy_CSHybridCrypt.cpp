...
bool EterPackPolicy_CSHybridCrypt::GenerateCryptKey(const std::string& rfileName )
{
	...
	const DWORD  dwExtHash  = stringhash().GetHash(extName);

	const TCSHybridCryptKeyMap::const_iterator cit = m_mapHybridCryptKey.find( dwExtHash );
...
bool EterPackPolicy_CSHybridCrypt::GetPerFileCryptKey(const std::string& rfileName, eHybridCipherAlgorithm& eAlgorithm, TEncryptKey& key, TEncryptIV& iv )
{
	...
	const std::string extName = GetFileExt(fileNamelower);

	const TCSHybridCryptKeyMap::const_iterator cit = m_mapHybridCryptKey.find( stringhash().GetHash(extName));
	...
	const DWORD dwfileNameCrc = GetCRC32(fileNamelower.c_str(), fileNamelower.size());
...
void EterPackPolicy_CSHybridCrypt::WriteCryptKeyToFile( CFileBase& rFile )
{
	...
	const DWORD dwCryptKeySize = m_mapHybridCryptKey.size();
...
bool EterPackPolicy_CSHybridCrypt::GenerateSupplementaryDataBlock(const std::string& rfilename, const std::string& strMapName, IN const BYTE* pSrcData, IN int iSrcLen, OUT LPBYTE& pDestData, OUT int& iDestLen )
{
	...
	const DWORD dwFileNameHash  = stringhash().GetHash(fileNamelower);
	const TSupplementaryDataBlockMap::const_iterator cit = m_mapSDBMap.find( dwFileNameHash );
	...
	const TSupplementaryDataBlockInfo info;
...
void EterPackPolicy_CSHybridCrypt::WriteSupplementaryDataBlockToFile( CFileBase& rFile )
{
	...
	const DWORD dwSDBMapSize = m_mapSDBMap.size();
