...
class CEterPackManager : public CSingleton<CEterPackManager>
{
	...
	public:
		...
		//THEMIDA
		bool GetFromFile(CMappedFile & rMappedFile, const char * c_szFileName, LPCVOID * pData) const;
	...
	public:
		void ArrangeMemoryMappedPack() const;

	protected:
		int ConvertFileName(const char * c_szFileName, std::string & rstrFileName) const;
		bool CompareName(const char * c_szDirectoryName, DWORD iLength, const char * c_szFileName) const;
