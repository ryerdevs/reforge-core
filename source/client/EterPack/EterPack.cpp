...
void CMakePackLog::Writef(const char* c_szFormat, ...)
{
	...
	char szBuf[1024];
	const int nBufLen = _vsnprintf(szBuf, sizeof(szBuf), c_szFormat, args);
	szBuf[nBufLen] = '\0';
...
void CMakePackLog::FlushError() const
{
	...
	const std::istream_iterator <std::wstring, wchar_t> iit(iFile);
	const std::istream_iterator <std::wstring, wchar_t> eos;

    std::vector <std::wstring> vText;

    std::copy (iit, eos, std::back_inserter(vText));

	const std::ostream_iterator <std::wstring, wchar_t, std::char_traits <wchar_t> > oit(std::wcout);

    std::sort (vText.begin(), vText.end());
    std::copy (vText.begin(), vText.end(), oit);
}
#ifdef __MAKE_PACK__
FILE * CEterPack::ms_PackLogFile = nullptr;
#endif
///////////////////////////////////////////////////////////////////////////////
...
CEterPack::~CEterPack()
{
	...
	m_pCSHybridCryptPolicy = nullptr;
}
...
bool CEterPack::Create(CEterFileDict& rkFileDict, const char * dbname, const char* pathName, bool bReadOnly, const BYTE* iv)
{
	...
	const bool bOverwrite = (iv != nullptr);
...
bool CEterPack::DecryptIV(DWORD dwPanamaKey)
{
	...
	const auto ivs = (DWORD*)&m_stIV_Panama[0];
...
bool CEterPack::EncryptIndexFile()
{
	...
	auto pbData = new BYTE[file.Size()];
...
bool CEterPack::__BuildIndex(CEterFileDict& rkFileDict, bool bOverwrite)
{
	...
	const DWORD fourcc = *(DWORD *) pvData;
...
bool CEterPack::Get(CMappedFile& out_file, const char * filename, LPCVOID * data)
{
	...
	const bool bIsSecurityCheckRequired = ( index->compressed_type == COMPRESSED_TYPE_SECURITY ||
	...
	if( bIsSecurityCheckRequired )
	{
		...
		const DWORD dwCrc32 = GetCRC32((const char*)(*data), index->data_size);
...
bool CEterPack::GetNames(std::vector<std::string>* retNames)
{
	...
	for (auto i = m_DataPositionMap.begin();
	     ...
	     ++i)
	{
		const TEterPackIndex* index = i->second;
...
int CEterPack::GetFreeBlockIndex(long size) const
{
...
void CEterPack::PushFreeIndex(TEterPackIndex* index)
{
	...
	{
		const auto i = m_DataPositionMap.find(index->filename_crc);
	...
	const int blockidx = GetFreeBlockIndex(index->real_data_size);
...
{
	const long pos = (file.Size() - eterpack::c_HeaderSize) / sizeof(TEterPackIndex);
	++m_indexCount;
	return (pos);
}
...
TEterPackIndex* CEterPack::NewIndex(CFileBase& file, const char* filename, long size)
{
	...
	const int block_size = size + (DATA_BLOCK_SIZE - (size % DATA_BLOCK_SIZE));
//		return index;

	const int blockidx = GetFreeBlockIndex(block_size);
...
TEterPackIndex* CEterPack::FindIndex(const char * filename)
{
	...
	const DWORD filename_crc = GetCRC32(tmpFilename, strlen(tmpFilename));
	const auto i = m_DataPositionMap.find(filename_crc);
...
long CEterPack::GetNewDataPosition(CFileBase& file) const
{
	return file.Size();
}

bool CEterPack::ReadData(CFileBase & file, TEterPackIndex* index, LPVOID data, long maxsize) const
{
...
bool CEterPack::WriteData(CFileBase & file, TEterPackIndex* index, LPCVOID data) const
{
	file.Seek(index->data_position);
...
bool CEterPack::WriteNewData(CFileBase& file, TEterPackIndex* index, LPCVOID data) const
{
	file.Seek(index->data_position);
	...
	const auto empty_buf = (char *) calloc(empty_size, sizeof(char));
...
void CEterPack::__CreateFileNameKey_Panama(const char * filename, BYTE * key, unsigned int keySize) const
{
	if (keySize != 32)
		return;

	const std::string SrcStringForKey(filename);
	const unsigned int idx = GetCRC32(SrcStringForKey.c_str(), SrcStringForKey.length()) & 3;
	...
	const unsigned int idx2 = *(unsigned int*) key;
...
void CEterFileDict::UpdateItem(CEterPack* pkPack, TEterPackIndex* pkInfo)
{
	...
	const auto f = m_dict.find(pkInfo->filename_crc);
	if (f == m_dict.end())
		m_dict.insert(TDict::value_type(pkInfo->filename_crc, item));
	else
	{
...
{
	const std::pair<TDict::iterator, TDict::iterator> iter_pair = m_dict.equal_range(dwFileNameHash);
