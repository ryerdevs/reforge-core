...
bool CGuildMarkDownloader::__LoginState_Process()
{
	...
	const UINT needPacketSize = __GetPacketSize(header);
...
// MARK_BUG_FIX
UINT CGuildMarkDownloader::__GetPacketSize(UINT header) const
{
	switch (header)
	{
...
bool CGuildMarkDownloader::__LoginState_RecvMarkBlock()
{
	...
	if (kPacket.count > 0)
	{
		...
		if (CGuildMarkManager::Instance().GetMarkImageFilename(kPacket.imgIdx, imagePath))
		{
			...
			{
				const auto pkGrpImg=static_cast<CGraphicImage*>(pResource);
				pkGrpImg->Reload();
			}
		}
	}
...
bool CGuildMarkDownloader::__LoginState_RecvSymbolData()
{
	...
	const WORD wDataSize = kPacketSymbolData.size - sizeof(kPacketSymbolData);
	const DWORD dwGuildID = kPacketSymbolData.guild_id;
