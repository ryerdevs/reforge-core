...
bool CPythonPlayer::FindSkillSlotIndexBySkillIndex(DWORD dwSkillIndex, DWORD * pdwSkillSlotIndex) const
{
	for (int i = 0; i < SKILL_MAX_NUM; ++i)
	{
		const TSkillInstance & rkSkillInst = m_playerStatus.aSkill[i];
...
void CPythonPlayer::ChangeCurrentSkillNumberOnly(DWORD dwSlotIndex)
{
	...
	const TSkillInstance & rkSkillInst = m_playerStatus.aSkill[dwSlotIndex];
...
bool CPythonPlayer::__CheckShortMana(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	...
	const int iNeedSP = rkSkillData.GetNeedSP(rkSkillInst.fcurEfficientPercentage);
...
bool CPythonPlayer::__CheckShortLife(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	...
	const DWORD dwNeedHP = rkSkillData.GetNeedSP(rkSkillInst.fcurEfficientPercentage);
...
bool CPythonPlayer::__CheckRestSkillCoolTime(DWORD dwSlotIndex) const
{
	if (!m_sysIsCoolTime)
		return false;
	...
	const float fElapsedTime = CTimer::Instance().GetCurrentSecond() - m_playerStatus.aSkill[dwSlotIndex].fLastUsedTime;
...
bool CPythonPlayer::__CheckDashAffect(CInstanceBase& rkInstMain) const
{
...
{
	const DWORD dwSkillTargetRange=__GetSkillTargetRange(rkSkillData);
	const float fSkillTargetRange = float(dwSkillTargetRange);
...
bool CPythonPlayer::__CanUseSkill()
{
	...
	// @fixme054 BEGIN
	DWORD slotIndex{};
	if (pkInstMain->IsMountingHorse() && GetSkillSlotIndex(c_iSkillIndex_Riding, &slotIndex) && (GetSkillGrade(slotIndex) < 1 && GetSkillLevel(slotIndex) < 20))
		return false;
	// @fixme054 END
...
bool CPythonPlayer::__UseSkill(DWORD dwSlotIndex)
{
		...
		pSkillData->IsAutoSearchTarget())
	{
		...
		if (pkInstTarget)
		{
			if (pSkillData->IsOnlyForAlliance())
			{
				...
				{
					const DWORD dwSkillRange = __GetSkillTargetRange(*pSkillData);

					if (dwSkillRange > 0)
					{
						const float fDistance=pkInstMain->GetDistance(pkInstTarget);
	...
	const DWORD dwTargetMaxCount = pSkillData->GetTargetCount(rkSkillInst.fcurEfficientPercentage);
	const DWORD dwRange = __GetSkillTargetRange(*pSkillData);
	...
	const DWORD dwTargetVID=pkInstTarget ? pkInstTarget->GetVirtualID() : 0;
...
{
	const TSkillInstance & rkSkillInst = m_playerStatus.aSkill[dwSkillSlotIndex];
...
BYTE CPythonPlayer::__GetSkillType(DWORD dwSkillSlotIndex) const
{
	const TSkillInstance & rkSkillInst = m_playerStatus.aSkill[dwSkillSlotIndex];
...
void CPythonPlayer::UseGuildSkill(DWORD dwSkillSlotIndex)
{
	...
	const DWORD dwSkillIndex = rkSkillInst.dwIndex;
	...
	const DWORD dwMotionIndex = pSkillData->GetSkillMotionIndex();
...
bool CPythonPlayer::__CheckSpecialSkill(DWORD dwSkillIndex)
{
	...
	else if (c_iSkillIndex_Combo == dwSkillIndex)
	{
		...
		const int iLevel = GetSkillLevel(dwSlotIndex);
		if (iLevel > 0)
		{
