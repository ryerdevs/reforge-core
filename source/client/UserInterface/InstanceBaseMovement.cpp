...
{
	const float fDirRot=CInstanceBase_GetDegreeFromPosition(c_rkPPosDir.x, -c_rkPPosDir.y);
	const float fClampDirRot=ClampDegree(fDirRot);
...
{
	const float frotDifference = GetDegreeDifference(GetRotation(), fRotation);
...
bool CInstanceBase::NEW_MoveToDestPixelPositionDirection(const TPixelPosition & c_rkPPosDst)
{
	TPixelPosition kPPosCur;
	NEW_GetPixelPosition(&kPPosCur);
	const float fDstRot = NEW_GetAdvancingRotationFromPixelPosition(kPPosCur, c_rkPPosDst);
...
void CInstanceBase::NEW_MoveToDirection(float fDirRot)
{
	...
	const D3DXVECTOR3 kD3DVt3Cur(kPPosCur.x, -kPPosCur.y, kPPosCur.z);
