...
{
	SEffectContainer::Dict& rkDctEftID=__EffectContainer_GetDict();
	const auto f=rkDctEftID.find(dwEftKey);
	if (rkDctEftID.end()!=f)
		return 0;
...
void CInstanceBase::__AttachEmpireEffect(DWORD eEmpire)
{
	...
	const CInstanceBase* pkInstMain=__GetMainInstancePtr();
...
bool CInstanceBase::UpdateDeleting()
{
	...
	const float fAlpha = __GetAlphaValue() - (rApp.GetGlobalElapsedTime() * 1.5f);
	__SetAlphaValue(fAlpha);
...
{
	const DWORD dwPVPKey=__GetPVPKey(dwVIDSrc, dwVIDDst);
...
{
	const DWORD dwPVPReadyKey=__GetPVPKey(dwVIDSrc, dwVIDDst);
...
{
	const DWORD dwGVGKey = __GetPVPKey(dwSrcGuildVID, dwDstGuildVID);
...
bool CInstanceBase::__FindPVPKey(DWORD dwVIDSrc, DWORD dwVIDDst)
{
	...
	if (!g_kSet_dwPVPKey.contains(dwPVPKey))
		return false;
...
bool CInstanceBase::__FindPVPReadyKey(DWORD dwVIDSrc, DWORD dwVIDDst)
{
	...
	if (!g_kSet_dwPVPReadyKey.contains(dwPVPKey))
		return false;
...
{
	const DWORD dwGVGKey=__GetPVPKey(dwSrcGuildID, dwDstGuildID);

	if (!g_kSet_dwGVGKey.contains(dwGVGKey))
		return false;
...
{
	const DWORD dwDUELKey=__GetPVPKey(dwVIDSrc, dwVIDDst);

	if (!g_kSet_dwDUELKey.contains(dwDUELKey))
		return false;
...
{
	const DWORD dwVIDSrc=GetVirtualID();
	const DWORD dwVIDDst=rkInstSel.GetVirtualID();

	const DWORD dwGuildIDSrc=GetGuildID();
	const DWORD dwGuildIDDst=rkInstSel.GetGuildID();
...
UINT CInstanceBase::GetNameColorIndex()
{
	if (IsPC())
	{
		...
		if (__IsExistMainInstance() && !__IsMainInstance())
		{
			...
			const DWORD dwVIDMain=pkInstMain->GetVirtualID();
			const DWORD dwVIDSelf=GetVirtualID();
			...
			if (pkInstMain->IsSameEmpire(*this))
			{
				...
				const DWORD dwGuildIDMain=pkInstMain->GetGuildID();
				const DWORD dwGuildIDSelf=GetGuildID();
...
{
	const UINT uGrade = GetAlignmentGrade();
	if ( uGrade >= TITLE_NUM)
	{
...
void CInstanceBase::AttachTextTail()
{
	...
	static auto s_kD3DXClrTextTail=D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
{
	static auto s_kLevelColor = D3DXCOLOR(152.0f/255.0f, 255.0f/255.0f, 51.0f/255.0f, 1.0f);
...
void CInstanceBase::RefreshTextTail()
{
	...
	const int iAlignmentGrade = GetAlignmentGrade();
...
void CInstanceBase::__SetNormalAffectFlagContainer(const CAffectFlagContainer& c_rkAffectFlagContainer)
{
	...
	{
		const bool isOldSet=m_kAffectFlagContainer.IsSet(i);
		const bool isNewSet=c_rkAffectFlagContainer.IsSet(i);
...
void CInstanceBase::__SetStoneSmokeFlagContainer(const CAffectFlagContainer& c_rkAffectFlagContainer)
{
	...
	if (m_kAffectFlagContainer.IsSet(STONE_SMOKE8))
		eSmoke=3;
	else if (m_kAffectFlagContainer.IsSet(STONE_SMOKE5)||m_kAffectFlagContainer.IsSet(STONE_SMOKE6)||m_kAffectFlagContainer.IsSet(STONE_SMOKE7)) // @fixme045
		eSmoke=2;
	else if (m_kAffectFlagContainer.IsSet(STONE_SMOKE2)||m_kAffectFlagContainer.IsSet(STONE_SMOKE3)||m_kAffectFlagContainer.IsSet(STONE_SMOKE4)) // @fixme045
		eSmoke=1;
	else
		eSmoke=0;
...
bool CInstanceBase::IsPossibleEmoticon() const
{
	...
	{
		const DWORD effectID = ms_adwCRCAffectEffect[EFFECT_EMOTICON+eEmoticon];
...
void CInstanceBase::SetEmoticon(UINT eEmoticon)
{
	...
	if (IsPossibleEmoticon())
	{
		...
		const D3DXVECTOR3 v3Dir = (pCamera->GetEye()-v3Pos)*9/10;
...
DWORD CInstanceBase::__AttachEffect(UINT eEftType)
{
	...
	{
		const std::string & rstrBoneName = ms_astAffectEffectAttachBone[eEftType];
