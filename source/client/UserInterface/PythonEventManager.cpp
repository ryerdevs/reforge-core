...
{
	const int ix = atoi(GetArgument("x", rArgList));
	const int iy = atoi(GetArgument("y", rArgList));
	const int iz = atoi(GetArgument("z", rArgList));
	const int iUpDir = atoi(GetArgument("up", rArgList));
	const int iViewDir = atoi(GetArgument("view", rArgList));
	const int iCrossDir = atoi(GetArgument("cross", rArgList));
	const int iDistance = atoi(GetArgument("distance", rArgList));
	const int iRot = atoi(GetArgument("rot", rArgList));
	const int iPitch = atoi(GetArgument("pitch", rArgList));
...
int CPythonEventManager::RegisterEventSet(const char * c_szFileName)
{
	...
	const int iEmptySlotIndex = GetEmptyEventSetSlot();
...
int CPythonEventManager::RegisterEventSetFromString(const string& strScript)
{
	...
	if (pEventSet->ScriptGroup.ReadCmd(ScriptCommand))
	{
		if (GetScriptEventIndex(ScriptCommand.name.c_str(), &pEventPosition, &iEventType))
		{
			...
			{
				const std::string strFileName = GetArgument("value", ScriptCommand.argList);
...
void CPythonEventManager::__ClearEventSetp(TEventSet * pEventSet)
{
	...
	for (const auto & rkLine : pEventSet->ScriptTextLineList)
	{
...
int CPythonEventManager::GetEventSetLocalYPosition(int iIndex)
{
	...
	const TEventSet * pEventSet = m_EventSetVector[iIndex];
	if (!pEventSet)
		return 0;
...
void CPythonEventManager::UpdateEventSet(int iIndex, int ix, int iy)
{
	...
	{
		const int iLeftTime = max(0, pEventSet->iConfirmEndTime - timeGetTime()/1000);
...
bool CPythonEventManager::ProcessEventSet(TEventSet * pEventSet)
{
	...
	switch (iEventType)
	{
		...
		{
			const int x = atoi(GetArgument("x", ScriptCommand.argList));
			const int y = atoi(GetArgument("y", ScriptCommand.argList));
		...
		case EVENT_TYPE_INSERT_IMAGE:
		{
			...
			const int index = atoi(GetArgument("index", ScriptCommand.argList));
			const int total = atoi(GetArgument("total", ScriptCommand.argList));

			if (imageFile.empty())
			{
				const char * imageType = GetArgument("image_type", ScriptCommand.argList);
				const int iItemIndex = atoi(GetArgument("idx", ScriptCommand.argList));
		...
		case EVENT_TYPE_QUEST_BUTTON:
		{
			...
			const int idx = atoi(GetArgument("idx", ScriptCommand.argList));
		...
		case EVENT_TYPE_BLEND_CAMERA:
		{
			...
			const float fBlendTime = atoi(GetArgument("blendtime", ScriptCommand.argList));
		...
		{
			const float fSpeed = (float)atof(GetArgument("speed", ScriptCommand.argList));
		...
		{
			const int killstone_count = atoi(GetArgument("killstone_count", ScriptCommand.argList));
			const int killmob_count = atoi(GetArgument("killmob_count", ScriptCommand.argList));
			const int find_hidden = atoi(GetArgument("find_hidden", ScriptCommand.argList));
			const int hidden_total = atoi(GetArgument("hidden_total", ScriptCommand.argList));
			const int use_potion = atoi(GetArgument("use_potion", ScriptCommand.argList));
			const int is_revived = atoi(GetArgument("is_revived", ScriptCommand.argList));
			const int killallmob = atoi(GetArgument("killallmob", ScriptCommand.argList));
			const int total_time = atoi(GetArgument("total_time", ScriptCommand.argList));
			const int bonus_exp = atoi(GetArgument("bonus_exp", ScriptCommand.argList));
		...
		{
			const int iIndex = atoi(GetArgument("value", ScriptCommand.argList));
			CItemData * pItemData;
		...
		{
			const int iWidth = atoi(GetArgument("width", ScriptCommand.argList));
			const int iHeight = atoi(GetArgument("height", ScriptCommand.argList));
		...
		{
			const int iTimeOut = atoi(GetArgument("timeout", ScriptCommand.argList));
...
int CPythonEventManager::GetLineHeight(int iIndex)
{
	...
	const TTextLine& rkLine = pEventSet->ScriptTextLineList.front();
	CGraphicTextInstance* pInstance = rkLine.pInstance;
...
bool CPythonEventManager::GetScriptEventIndex(const char * c_szName, int * pEventPosition, int * pEventType)
{
	...
	const auto it = EventTypeMap.find(c_szEventName);
...
CPythonEventManager::CPythonEventManager()
	: m_poInterface(nullptr), m_strLeftTimeString("Time remaining: %d seconds")
{
