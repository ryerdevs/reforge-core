...
void CPythonApplication::__SetFullScreenWindow(HWND hWnd, DWORD dwWidth, DWORD dwHeight, DWORD dwBPP) const
{
	DEVMODE DevMode;
	...
	const LONG Error = ChangeDisplaySettings(&DevMode, CDS_FULLSCREEN);
...
void CPythonApplication::__MinimizeFullScreenWindow(HWND hWnd, DWORD dwWidth, DWORD dwHeight) const
{
	ChangeDisplaySettings(nullptr, 0);
				...
				case SIZE_MAXIMIZED:
					{
						...
						const UINT uWidth=rcWnd.right-rcWnd.left;
						const UINT uHeight=rcWnd.bottom-rcWnd.left;
