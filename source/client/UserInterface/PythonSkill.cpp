...
int SplitLine(const char * c_szText, CTokenVector* pstTokenVector, const char * c_szDelimeter)
{
	...
	std::string stToken;
	const std::string stLine = c_szText;
...
void string_replace_word(const char* base, int base_len, const char* src, int src_len, const char* dst, int dst_len, std::string& result)
{
	...
	while (cur < base_len)
	{
		// BOUNDS FIX (crash 2026-08-09): memcmp() must never read past the end
		// of base. Reading garbage beyond base_len could spuriously "match"
		// and produce corrupted skill formulas (heap corruption 0xc0000374
		// later in world load; AppVerifier caught the over-read at login).
...
bool CPythonSkill::RegisterSkillDesc(const char * c_szFileName)
{
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		// Type
		{
			...
			auto it = m_SkillTypeIndexMap.find(strJob.c_str());
		...
		if (TokenVector.size() > DESC_TOKEN_TYPE_MAX_LEVEL)
		{
			...
			constexpr DWORD LevelLimitEmpty = 1;
...
bool CPythonSkill::RegisterSkill(DWORD dwSkillIndex, const char * c_szFileName)
{
	...
	if (TextFileLoader.GetTokenString("type", &strTypeName))
	{
		...
		auto it = m_SkillTypeIndexMap.find(strTypeName.c_str());
		if (m_SkillTypeIndexMap.end() == it)
		{
	...
	if (TextFileLoader.GetTokenVector("attribute", &pAttributeTokenVector))
	{
		...
		{
			std::string & rstrToken = *it;
			auto it2 = m_SkillAttributeIndexMap.find(rstrToken.c_str());
			if (m_SkillAttributeIndexMap.end() == it2)
			{
	...
	if (TextFileLoader.GetTokenVector("needweapon", &pNeedWeaponVector))
	{
		...
		{
			std::string & rstrToken = *it;
			auto it2 = m_SkillNeedWeaponIndexMap.find(rstrToken.c_str());
			if (m_SkillNeedWeaponIndexMap.end() == it2)
			{
...
{
	const auto it = m_SkillDataMap.find(dwSkillIndex);
...
bool CPythonSkill::GetSkillDataByName(const char * c_szName, TSkillData ** ppSkillData)
{
	...
	{
		const TSkillData * pData = &(itor->second);
...
CPythonSkill::CPythonSkill()
{
	...
	SSkillData::ms_NewMaxStatusNameMap.emplace("wep", POINT_MAX_WEP); // @fixme041 moved from min to max
...
BOOL CPythonSkill::SSkillData::IsIgnoreTarget() const
{
...
bool CPythonSkill::SSkillData::GetState(const char * c_szStateName, int * piState, int iMinMaxType) const
{
	...
	const auto it = pStatusNameMap->find(c_szStateName);
...
const char * CPythonSkill::SSkillData::GetAffectDescription(DWORD dwIndex, float fSkillLevel) const
{
...
DWORD CPythonSkill::SSkillData::GetSkillCoolTime(float fSkillPoint) const
{
...
DWORD CPythonSkill::SSkillData::GetTargetCount(float fSkillPoint) const
{
...
DWORD CPythonSkill::SSkillData::GetMotionLoopCount(float fSkillPoint) const
{
...
DWORD CPythonSkill::SSkillData::GetContinuationSP(float fSkillPoint) const
{
...
DWORD CPythonSkill::SSkillData::GetDuration(float fSkillPoint) const
{
...
PyObject * skillGetSkillRequirementData(PyObject * poSelf, PyObject * poArgs)
{
	...
	const int ireqLevel = (int)ceil(float(c_pSkillData->byRequireSkillLevel)/float(max(1, pRequireSkillData->byLevelUpPoint)));
...
PyObject * skillCanLevelUpSkill(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (c_pSkillData->isRequirement)
	{
		...
		if (CPythonSkill::Instance().GetSkillDataByName(c_pSkillData->strRequireSkillName.c_str(), &pRequireSkillData))
		{
			...
			if (CPythonPlayer::Instance().FindSkillSlotIndexBySkillIndex(pRequireSkillData->dwSkillIndex, &dwRequireSkillSlotIndex))
			{
				const int iSkillGrade = CPythonPlayer::Instance().GetSkillGrade(dwRequireSkillSlotIndex);
				const int iSkillLevel = CPythonPlayer::Instance().GetSkillLevel(dwRequireSkillSlotIndex);
				if (iSkillGrade <= 0)
