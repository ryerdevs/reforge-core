...
void AppendMonsterList(const CPythonNonPlayer::TMobTableList & c_rMobTableList, const char * c_szHeader, int iType)
{
	...
	auto itor = c_rMobTableList.begin();
...
bool SplitToken(const char * c_szLine, CTokenVector * pstTokenVector, const char * c_szDelimeter = " ")
{
	...
	std::string stToken;
	const std::string strLine = c_szLine;
...
void CPythonNetworkStream::ServerCommand(const char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "cube"))
	{
		...
		if ("open" == TokenVector[1])
		{
			...
			const DWORD npcVNUM = (DWORD)atoi(TokenVector[2].c_str());
		...
		else if ("info" == TokenVector[1])
		{
			...
			const UINT gold = atoi(TokenVector[2].c_str());
			const UINT itemVnum = atoi(TokenVector[3].c_str());
			const UINT count = atoi(TokenVector[4].c_str());
		...
		else if ("success" == TokenVector[1])
		{
			...
			const UINT itemVnum = atoi(TokenVector[2].c_str());
			const UINT count = atoi(TokenVector[3].c_str());
		...
		else if ("m_info" == TokenVector[1])
		{
			...
			const UINT requestStartIndex = (UINT)atoi(TokenVector[2].c_str());
			const UINT resultCount = (UINT)atoi(TokenVector[3].c_str());
	...
	else if (!strcmpi(szCmd, "ObserverCount"))
	{
		...
		const UINT uObserverCount= atoi(TokenVector[1].c_str());
	...
	else if (!strcmpi(szCmd, "ObserverMode"))
	{
		...
		const UINT uMode= atoi(TokenVector[1].c_str());
	...
	else if (!strcmpi(szCmd, "StoneDetect"))
	{
		...
		const DWORD dwVID = atoi(TokenVector[1].c_str());
	...
	else if (!strcmpi(szCmd, "StartStaminaConsume"))
	{
		...
		const DWORD dwConsumePerSec = atoi(TokenVector[1].c_str());
		const DWORD dwCurrentStamina = atoi(TokenVector[2].c_str());
	...
	else if (!strcmpi(szCmd, "StopStaminaConsume"))
	{
		...
		const DWORD dwCurrentStamina = atoi(TokenVector[1].c_str());
	...
	{
		const int iVID1 = atoi(TokenVector[1].c_str());
		const int iVID2 = atoi(TokenVector[2].c_str());
	...
	{
		const int iVID = atoi(TokenVector[1].c_str());
		IAbstractCharacterManager & rkChrMgr = IAbstractCharacterManager::GetSingleton();
	...
	else
	{
		...
		const auto f = s_emotionDict.find(szCmd);
		...
		{
			const int emotionIndex = f->second;
