...
{
	const auto f=m_kMap_dwVID_kObserver.find(dwVID);
...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		const float fDistanceFromCenterX = (kInstancePosition.x - m_fCenterX) * fooCellScale * m_fScale;
		const float fDistanceFromCenterY = (kInstancePosition.y - m_fCenterY) * fooCellScale * m_fScale;
		if (fabs(fDistanceFromCenterX) >= m_fMiniMapRadius || fabs(fDistanceFromCenterY) >= m_fMiniMapRadius)
			continue;

		const float fDistanceFromCenter = sqrtf(fDistanceFromCenterX * fDistanceFromCenterX + fDistanceFromCenterY * fDistanceFromCenterY );
...

	{
		...
		for (; itor != m_AtlasWayPointInfoVector.end(); ++itor)
		{
			...
			constexpr float c_fMiniMapWindowRadius = 55.0f;
			...
			{
				const float fRadianX = acosf(fDistanceFromCenterX / fDistanceFromCenter);
				const float fRadianY = asinf(fDistanceFromCenterY / fDistanceFromCenter);
...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	for (BYTE byTerrainNum = 0; byTerrainNum < AROUND_AREA_NUM; ++byTerrainNum)
	{
		const LPDIRECT3DTEXTURE9 pMiniMapTexture = m_lpMiniMapTexture[byTerrainNum];
		STATEMANAGER.SetTexture(0, pMiniMapTexture);
		if (pMiniMapTexture)
		{
			CStateManager& rkSttMgr=CStateManager::Instance();
			rkSttMgr.DrawIndexedPrimitive(D3DPT_TRIANGLELIST, byTerrainNum * 4, 4, byTerrainNum * 6, 2, 0);
		}
		else
		{
			STATEMANAGER.SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
			STATEMANAGER.DrawIndexedPrimitive(D3DPT_TRIANGLELIST, byTerrainNum * 4, 4, byTerrainNum * 6, 2, 0);
	...
	if (m_fScale >= 2.0f)
	{
		...
		{
			const TMarkPosition & rPosition = *aIterator;
	...
	// Target
	{
		...
		for (; itor != m_AtlasWayPointInfoVector.end(); ++itor)
		{
			const TAtlasMarkInfo & rAtlasMarkInfo = *itor;
	...
	const CCamera* pkCmrCur=CCameraManager::Instance().GetCurrentCamera();
...
bool CPythonMiniMap::Create()
{
	...
	auto pImage = (CGraphicImage *) CResourceManager::Instance().GetResourcePointer(strImageFilter.c_str());
	...
	auto pSubImage = (CGraphicSubImage *) CResourceManager::Instance().GetResourcePointer(strPlayerMark.c_str());
...
#pragma pack(pop)
	...
	{
		auto pchMiniMapVertex = (char *)lpOrigMiniMapVertex;
		...
		for (int iY = -3; iY <= 1; ++iY)
		{
			if (0 == iY%2)
				continue;
			const float fY = 0.5f * ((float)iY);
			for (int iX = -3; iX <= 1; ++iX)
			{
				if (0 == iX%2)
					continue;
				const float fX = 0.5f * ((float)iX);
				lpMiniMapVertex = (LPMINIMAPVERTEX) pchMiniMapVertex;
...
void CPythonMiniMap::RegisterAtlasMark(BYTE byType, const char * c_szName, long lx, long ly)
{
	...
	switch(byType)
	{
		...
		case CActorInstance::TYPE_WARP:
			...
			{
				const int iPos = aAtlasMarkInfo.m_strText.find(" ");
				if (iPos >= 0)
...
DWORD CPythonMiniMap::GetGuildAreaID(DWORD x, DWORD y)
{
	...
	{
		const TGuildAreaInfo & rAreaInfo = *itor;
...
void CPythonMiniMap::__LoadAtlasMarkInfo()
{
	...
	const CMapOutdoor & rkMap=rkBG.GetMapOutdoorRef();
	...
	for (DWORD i = 0; i < stTokenVectorMap.size(); ++i)
	{
		...
		if (rVector.size() == 3)
		{
			...
			const int iVNum = atoi(c_rstrText.c_str());
...
bool CPythonMiniMap::LoadAtlas()
{
	...
	const auto playerMarkFileName = "d:/ymir work/ui/minimap/playermark.sub";
	...
	const auto pkGrpImgAtlas = (CGraphicImage *) CResourceManager::Instance().GetResourcePointer(atlasFileName);
	if (pkGrpImgAtlas)
	{
...
void CPythonMiniMap::RenderAtlas(float fScreenX, float fScreenY)
{
	...
	{
		const TAtlasMarkInfo & rAtlasMarkInfo = *m_AtlasMarkInfoVectorIterator;
...
bool CPythonMiniMap::GetPickedInstanceInfo(float fScreenX, float fScreenY, std::string & rReturnName, float * pReturnPosX, float * pReturnPosY, DWORD * pdwTextColor) const
{
	const float fDistanceFromMiniMapCenterX = fScreenX - m_fScreenX - m_fWidth * 0.5f;
	const float fDistanceFromMiniMapCenterY = fScreenY - m_fScreenY - m_fHeight * 0.5f;
	...
	const float fRealX = m_fCenterX + fDistanceFromMiniMapCenterX / m_fScale * ((float) CTerrainImpl::CELLSCALE);
	const float fRealY = m_fCenterY + fDistanceFromMiniMapCenterY / m_fScale * ((float) CTerrainImpl::CELLSCALE);
...
bool CPythonMiniMap::GetAtlasInfo(float fScreenX, float fScreenY, std::string & rReturnString, float * pReturnPosX, float * pReturnPosY, DWORD * pdwTextColor, DWORD * pdwGuildID)
{
	...
	*pReturnPosY = fRealY;
	...
	const float fCheckWidth = (m_fAtlasMaxX / m_fAtlasImageSizeX) * 5.0f;
	const float fCheckHeight = (m_fAtlasMaxY / m_fAtlasImageSizeY) * 5.0f;
