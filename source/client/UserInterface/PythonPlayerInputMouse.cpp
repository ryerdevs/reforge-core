...
bool CPythonPlayer::NEW_MoveToMouseScreenDirection()
{
	...
	const long lScrWidth = rkWndMgr.GetScreenWidth();
	const long lScrHeight = rkWndMgr.GetScreenHeight();
	const float fMouseX=lMouseX/float(lScrWidth);
	const float fMouseY=lMouseY/float(lScrHeight);
...
void CPythonPlayer::NEW_SetMouseCameraState(int eMBS)
{
	...
	if (pkCmrCur)
	{
		...
		{
			const bool isCameraDrag=pkCmrCur->EndDrag();
...
void CPythonPlayer::__ShowPickedEffect(const TPixelPosition& c_rkPPosPickedGround) const
{
...
int CPythonPlayer::NEW_GetMouseFunc(int eMBT) const
{
	if (eMBT>=MBT_NUM)
		return false;
...
bool CPythonPlayer::NEW_SetMouseState(int eMBT, int eMBS)
{
	...
	const int eMBF=m_aeMBFButton[eMBT];
	switch (eMBF)
	{
		case MBF_MOVE:
			if (__CanMove())
		...
		case MODE_CLICK_ITEM:
		{
			...
			if (rkIT.GetGroundItemPosition(m_dwIIDReserved, &kPPosPickedItem))
			{
				...
				if (pkInstMain->NEW_GetDistanceFromDestPixelPosition(kPPosPickedItem) < pickDistance)
				{
					...
					const float fCurRot=pkInstMain->GetRotation();
...
BOOL CPythonPlayer::__IsRightButtonSkillMode() const
{
