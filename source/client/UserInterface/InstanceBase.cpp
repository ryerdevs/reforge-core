...
constexpr float c_fDefaultRotationSpeed = 1200.0f;
constexpr float c_fDefaultHorseRotationSpeed = 1200.0f; // @fixme056 (default 300.0f)
...
UINT CInstanceBase::SHORSE::GetLevel() const
{
	if (m_pkActor)
...
{
	const int nMainAlpha=(__GetAlphaValue() < 1.0f) ? 1 : 0;
	const int nTestAlpha=(pkInst->__GetAlphaValue() < 1.0f) ? 1 : 0;
	...
	const UINT uLeftLODLevel=__LessRenderOrder_GetLODLevel();
	const UINT uRightLODLevel=pkInst->__LessRenderOrder_GetLODLevel();
...
constexpr float PC_DUST_RANGE = 2000.0f;
constexpr float NPC_DUST_RANGE = 1000.0f;
...
DWORD CInstanceBase::GetEmpireID() const
{
	return m_dwEmpireID;
}
...
BYTE CInstanceBase::GetPKMode() const
{
	return m_byPKMode;
}
...
bool CInstanceBase::__IsMainInstance() const
{
...
{
#ifdef ENABLE_CANSEEHIDDENTHING_FOR_GM
	const CInstanceBase * pInstance = __GetMainInstancePtr();
...
float CInstanceBase::__GetBowRange() const
{
...
CInstanceBase* CInstanceBase::__FindInstancePtr(DWORD dwVID) const
{
	CPythonCharacterManager& rkChrMgr=CPythonCharacterManager::Instance();
	return rkChrMgr.GetInstancePtr(dwVID);
}

bool CInstanceBase::__FindRaceType(DWORD dwRace, BYTE* pbType) const
{
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	if (IsGuildWall())
	{
		...
		const float center_z = __GetBackgroundHeight(center_x, center_y);
	...
	if (!IsPC())
	{
		const DWORD dwBodyColor = CPythonNonPlayer::Instance().GetMonsterColor(c_rkCreateData.m_dwRace);
		if (0 != dwBodyColor)
		{
	...
	constexpr int c_iGuildSymbolRace = 14200;
...
void CInstanceBase::__Create_SetWarpName(const SCreateData& c_rkCreateData)
{
	...
	if (CPythonNonPlayer::Instance().GetName(c_rkCreateData.m_dwRace, &c_szName))
	{
		...
		const int iFindingPos = strName.find_first_of(" ", 0);
		if (iFindingPos > 0)
		{
...
{
	constexpr int c_iWeddingDressShape = 201;
	return c_iWeddingDressShape == m_eShape;
}

BOOL CInstanceBase::IsHoldingPickAxe() const
{
	constexpr int c_iPickAxeStart = 29101;
	constexpr int c_iPickAxeEnd = 29110;
...
int CInstanceBase::GetHorseVnum() const
{
...
DWORD CInstanceBase::GetPart(CRaceData::EParts part) const
{
	assert(part >= 0 && part < CRaceData::PART_MAX_NUM);
	return m_awPart[part];
}

DWORD CInstanceBase::GetShape() const
{
	return m_eShape;
}
...
void CInstanceBase::StateProcess()
{
	while (1)
	{
		...
		const DWORD dwDstChkTime = m_kQue_kCmdNew.front().m_dwChkTime;
		const DWORD dwCurChkTime = ELTimer_GetServerFrameMSec();
		...
		const FLOAT fRotDst = kCmdTop.m_fDstRot;
		const UINT eFunc = kCmdTop.m_eFunc;
		const UINT uArg = kCmdTop.m_uArg;
		const UINT uVID = GetVirtualID();
		const UINT uTargetVID = kCmdTop.m_uTargetVID;
		...
		*/
		...
		const TPixelPosition kPPosDir = kPPosDst - kPPosCur;
		const float fDirLen = (float)sqrt(kPPosDir.x * kPPosDir.x + kPPosDir.y * kPPosDir.y);
...
void CInstanceBase::MovementProcess()
{
	...
	const TPixelPosition kPPosDeltaSC = kPPosCur - NEW_GetSrcPixelPositionRef();
	const TPixelPosition kPPosDeltaSN = kPPosNext - NEW_GetSrcPixelPositionRef();
	const TPixelPosition kPPosDeltaSD = NEW_GetDstPixelPositionRef() - NEW_GetSrcPixelPositionRef();

	const float fCurLen = sqrtf(kPPosDeltaSC.x * kPPosDeltaSC.x + kPPosDeltaSC.y * kPPosDeltaSC.y);
	const float fNextLen = sqrtf(kPPosDeltaSN.x * kPPosDeltaSN.x + kPPosDeltaSN.y * kPPosDeltaSN.y);
	const float fTotalLen = sqrtf(kPPosDeltaSD.x * kPPosDeltaSD.x + kPPosDeltaSD.y * kPPosDeltaSD.y);
	const float fRestLen = fTotalLen - fCurLen;

	if (__IsMainInstance())
	{
		if (m_isGoing && IsWalking())
		{
			const float fDstRot = NEW_GetAdvancingRotationFromPixelPosition(NEW_GetSrcPixelPositionRef(), NEW_GetDstPixelPositionRef());
			...
			if (fRestLen<=0.0)
			{
				...
				{
					const DWORD dwMotionNumber = m_kMovAfterFunc.uArg;
					const DWORD dwTargetVID = m_kMovAfterFunc.uArgExpanded;
	...
	else
	{
		if (m_isGoing && IsWalking())
		{
			...
			if (fRestLen < -100.0f)
			{
				...
				const float fDstRot = NEW_GetAdvancingRotationFromPixelPosition(kPPosCur, NEW_GetDstPixelPositionRef());
				SetAdvancingRotation(fDstRot);
	...
	if (IsWalking() || m_GraphicThingInstance.IsUsingMovingSkill())
	{
		...
		if (__IsInDustRange())
		{
			const float fDustDistance = NEW_GetDistanceFromDestPixelPosition(m_kPPosDust);
			if (IsMountingHorse())
			{
...
void CInstanceBase::__ProcessFunctionEmotion(DWORD dwMotionNumber, DWORD dwTargetVID, const TPixelPosition & c_rkPosDst)
{
	...
	if (pTargetInstance)
	{
		...
		const WORD wMotionNumber1 = HIWORD(dwMotionNumber);
		const WORD wMotionNumber2 = LOWORD(dwMotionNumber);

		const int src_job = RaceToJob(GetRace());
...
void CInstanceBase::Update()
{
	...
	if (m_GraphicThingInstance.IsMovement())
	{
		...
		const DWORD dwCurTime=ELTimer_GetFrameMSec();
...
void CInstanceBase::RenderToShadowMap()
{
	...
	constexpr float SHADOW_APPLY_DISTANCE = 2500.0f;
...
DWORD CInstanceBase::GetLevel() const
{
	return m_dwLevel;
}
#endif
...
DWORD CInstanceBase::GetRace() const
{
	return m_dwRace;
}

bool CInstanceBase::IsConflictAlignmentInstance(CInstanceBase& rkInstVictim) const
{
...
DWORD CInstanceBase::GetDuelMode() const
{
	return m_dwDuelMode;
}
...
void CInstanceBase::SetHair(DWORD eHair)
{
	...
	const auto it = setAllowedHair.find(eHair);
...
void CInstanceBase::SetArmor(DWORD dwArmor)
{
	...
	if (__ArmorVnumToShape(dwArmor, &dwShape))
	{
		...
		{
			const float fSpecularPower=pItemData->GetSpecularPowerf();
...
DWORD CInstanceBase::GetWeaponType() const
{
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
	...
	case CItemData::ITEM_TYPE_ARMOR:
		...
		{
			const DWORD vnum = pItem->GetIndex();
...
bool CInstanceBase::ChangeArmor(DWORD dwArmor)
{
	...
	const DWORD eHair = GetPart(CRaceData::PART_HAIR);
	const DWORD eWeapon = GetPart(CRaceData::PART_WEAPON);
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	const DWORD dwAcce = GetPart(CRaceData::PART_ACCE);
#endif
	const float fRot = GetRotation();
	const float fAdvRot = GetAdvancingRotation();
...
DWORD CInstanceBase::__GetRaceType() const
{
	return m_eRaceType;
}
...
#ifdef ENABLE_RACE_HEIGHT
float CInstanceBase::GetBaseHeight() const
{
	const CActorInstance* pkHorse = m_kHorse.GetActorPtr();
	if (!m_kHorse.IsMounting() || !pkHorse)
		return 0.0f;

	const DWORD dwHorseVnum = m_kHorse.m_pkActor->GetRace();
	if ((dwHorseVnum >= 20101 && dwHorseVnum <= 20109) ||
		(dwHorseVnum == 20029 || dwHorseVnum == 20030))
		return 100.0f;

	const float fRaceHeight = CRaceManager::instance().GetRaceHeight(dwHorseVnum);
	if (fRaceHeight == 0.0f)
		return 100.0f;
	return fRaceHeight;
}
