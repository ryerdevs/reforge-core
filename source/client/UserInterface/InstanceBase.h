...
class CInstanceBase
{
	...
	protected:
		...
		bool __IsMainInstance() const;
		bool __MainCanSeeHiddenThing() const;
		float __GetBowRange() const;
	...
	public:
		...
		bool					IsAffect(UINT uAffect) const;
		BOOL					IsInvisibility();
		BOOL					IsParalysis();
	...
	public:
		...
		void					ProcessHitting(DWORD dwMotionKey, CInstanceBase * pVictimInstance) const;
		void					ProcessHitting(DWORD dwMotionKey, BYTE byEventIndex, CInstanceBase * pVictimInstance) const;
	...
	protected:
		...
		CInstanceBase*			__FindInstancePtr(DWORD dwVID) const;

		bool  __FindRaceType(DWORD dwRace, BYTE* pbType) const;
		DWORD __GetRaceType() const;
