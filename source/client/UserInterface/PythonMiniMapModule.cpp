...
{
	const bool bShow = CPythonMiniMap::Instance().CanShow();
...
{
	const bool bShow = CPythonMiniMap::Instance().CanShowAtlas();
	return Py_BuildValue("b", bShow);
}

PyObject * miniMapIsAtlas(PyObject * poSelf, PyObject * poArgs)
{
	const bool isData = CPythonMiniMap::Instance().IsAtlas();
...
PyObject* miniMapGetGuildAreaID(PyObject* poSelf, PyObject* poArgs)
{
	...
	const DWORD dwGuildID = CPythonMiniMap::Instance().GetGuildAreaID(fx, fy);
