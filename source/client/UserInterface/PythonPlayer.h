...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	public:
		...
		typedef struct SPlayerStatus
		{
			...
			long GetPoint(UINT ePoint) const;
		} TPlayerStatus;
	...
	public:
		...
		int		NEW_GetMouseFunc(int eMBT) const;
		...
		CInstanceBase* NEW_FindActorPtr(DWORD dwVID) const;
		...
		void	RequestDeleteGlobalQuickSlot(DWORD dwGlobalSlotIndex) const;
		void	RequestUseLocalQuickSlot(DWORD dwLocalSlotIndex);
		DWORD	LocalQuickSlotIndexToGlobalQuickSlotIndex(DWORD dwLocalSlotIndex) const;
		...
		char	IsItem(TItemPos SlotIndex) const;

#ifdef ENABLE_NEW_EQUIPMENT_SYSTEM
		bool    IsBeltInventorySlot(TItemPos Cell) const;
#endif
		bool	IsInventorySlot(TItemPos SlotIndex) const;
		bool	IsEquipmentSlot(TItemPos SlotIndex) const;
		...
		int		GetSkillIndex(DWORD dwSlotIndex) const;
		int		GetSkillGrade(DWORD dwSlotIndex) const;
		int		GetSkillLevel(DWORD dwSlotIndex) const;
		float	GetSkillCurrentEfficientPercentage(DWORD dwSlotIndex) const;
		float	GetSkillNextEfficientPercentage(DWORD dwSlotIndex) const;
		...
		BOOL	IsToggleSkill(DWORD dwSlotIndex) const;
		void	ClickSkillSlot(DWORD dwSlotIndex);
		void	ChangeCurrentSkillNumberOnly(DWORD dwSlotIndex);
		bool	FindSkillSlotIndexBySkillIndex(DWORD dwSkillIndex, DWORD * pdwSkillSlotIndex) const;

		void	SetSkillCoolTime(DWORD dwSkillIndex);
		void	EndSkillCoolTime(DWORD dwSkillIndex);

		float	GetSkillCoolTime(DWORD dwSlotIndex) const;
		float	GetSkillElapsedCoolTime(DWORD dwSlotIndex) const;
		BOOL	IsSkillActive(DWORD dwSlotIndex) const;
		...
		void	OpenCharacterMenu(DWORD dwVictimActorID) const;
		DWORD	GetTargetVID() const;
	...
	protected:
		...
		DWORD	__GetLevelAtk();
		DWORD	__GetStatAtk();
		DWORD	__GetWeaponAtk(DWORD dwWeaponPower) const;
		...
		void	__OnClickItem(CInstanceBase& rkInstMain, DWORD dwPickedItemID) const;
		...
		bool	__GetPickedActorPtr(CInstanceBase** pkInstPicked) const;

		bool	__GetPickedActorID(DWORD* pdwActorID) const;
		bool	__GetPickedItemID(DWORD* pdwItemID) const;
		...
		void	__ShowPickedEffect(const TPixelPosition& c_rkPPosPickedGround) const;
		...
		void	NEW_ShowEffect(int dwEID, TPixelPosition kPPosDst) const;
		...
		bool	__CheckRestSkillCoolTime(DWORD dwSkillSlotIndex) const;
		...
		bool	__CheckDashAffect(CInstanceBase& rkInstMain) const;

		void	__SendUseSkill(DWORD dwSkillSlotIndex, DWORD dwTargetVID);
		void	__RunCoolTime(DWORD dwSkillSlotIndex);

		BYTE	__GetSkillType(DWORD dwSkillSlotIndex) const;

		bool	__IsReservedUseSkill(DWORD dwSkillSlotIndex) const;
		...
		void				__ClearTarget();
		DWORD				__GetTargetVID() const;
		void				__SetTargetVID(DWORD dwVID);
		bool				__IsSameTargetVID(DWORD dwVID) const;
		bool				__IsTarget() const;
		bool				__ChangeTargetToPickedInstance();

		CInstanceBase *		__GetSkillTargetInstancePtr(CPythonSkill::TSkillData& rkSkillData) const;
		CInstanceBase *		__GetAliveTargetInstancePtr() const;
		CInstanceBase *		__GetDeadTargetInstancePtr() const;

		BOOL				__IsRightButtonSkillMode() const;
...
constexpr inline DWORD c_iSkillIndex_Tongsol = 121;
constexpr inline DWORD c_iSkillIndex_Combo = 122;
constexpr inline DWORD c_iSkillIndex_Fishing = 123;
constexpr inline DWORD c_iSkillIndex_Mining = 124;
constexpr inline DWORD c_iSkillIndex_Making = 125;
constexpr inline DWORD c_iSkillIndex_Language1 = 126;
constexpr inline DWORD c_iSkillIndex_Language2 = 127;
constexpr inline DWORD c_iSkillIndex_Language3 = 128;
