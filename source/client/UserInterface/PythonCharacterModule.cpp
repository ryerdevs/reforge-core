...
PyObject * chrHasInstance(PyObject* poSelf, PyObject* poArgs)
{
	...
	const bool bFlag = CPythonCharacterManager::Instance().GetInstancePtr(iVirtualID) ? TRUE : FALSE;
...
PyObject * chrIsGameMaster(PyObject* poSelf, PyObject* poArgs)
{
	...
	const CInstanceBase * pInstance = (iVirtualID) ? CPythonCharacterManager::Instance().GetInstancePtr(iVirtualID) : CPythonCharacterManager::Instance().GetMainInstancePtr();
	if (!pInstance)
		return Py_BuildValue("i", 0);
...
{
	const DWORD VirtualID = CPythonCharacterManager::Instance().PickAll();
	return Py_BuildValue("i", VirtualID);
}
...
PyObject * chrAttachEffectByName(PyObject* poSelf, PyObject* poArgs)
{
	...
	const CInstanceBase * pkInst = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
...
PyObject * chrGetRotation(PyObject* poSelf, PyObject* poArgs)
{
	...
	const float fRotation = pCharacterInstance->GetRotation();
