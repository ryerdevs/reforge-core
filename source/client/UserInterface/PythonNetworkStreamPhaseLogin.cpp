...
void CPythonNetworkStream::SetLoginPhase()
{
	...
	if (__DirectEnterMode_IsSet())
	{
		...
		// NOTE: do NOT ClearLoginInfo() here. DirectEnter and the warp
		// reconnect (RecvWarpPacket) each re-send LOGIN3, and the server
		// re-authenticates the password on every channel connection.
		// Clearing m_stPassword here would fail the next reconnect.
	...
	else
	{
		...
		// NOTE: do NOT ClearLoginInfo() here. The DirectEnter reconnect
		// (ConnectGameServer) and the world warp reconnect reuse
		// m_stPassword; clearing it here leaves the client stuck on the
		// character select screen (server-side: "RESULT_LOGIN: no account").
		CAccountConnector & rkAccountConnector = CAccountConnector::Instance();
		rkAccountConnector.ClearLoginInfo();
