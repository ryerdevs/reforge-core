...
void SNetworkActorData::UpdatePosition()
{
	...
	const DWORD dwElapsedTime=dwClientCurTime-m_dwClientSrcTime;

	if (dwElapsedTime<m_dwDuration)
	{
		const float fRate=float(dwElapsedTime)/float(m_dwDuration);
...
void CNetworkActorManager::__OLD_Update()
{
	...
	for (i=m_kNetActorDict.begin(); i!=m_kNetActorDict.end(); ++i)
	{
		...
		const CInstanceBase * pkInstFind = rkChrMgr.GetInstancePtr(rkNetActorData.m_dwVID);
		if (!pkInstFind)
		{
...
{
	const LONG dx = lPosX-m_lMainPosX;
	const LONG dy = lPosY-m_lMainPosY;
	const LONG len = (LONG)sqrt(double(dx * dx + dy * dy));
...
bool CNetworkActorManager::__IsMainActorVID(DWORD dwVID) const
{
...
void CNetworkActorManager::__RemoveCharacterManagerActor(SNetworkActorData& rkNetActorData) const
{
	CPythonCharacterManager & rkChrMgr = CPythonCharacterManager::Instance();
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
	...
	const DWORD dwVID=rkNetActorData.m_dwVID;
	...
	if (pOldInstance)
	{
		...
		const BOOL bIsMountingHorse = pOldInstance->IsMountingHorse();
...
void CNetworkActorManager::AppendActor(const SNetworkActorData& c_rkNetActorData)
{
	if (__IsMainActorVID(c_rkNetActorData.m_dwVID))
	{
		...
		const CInstanceBase* pMainInstance = CPythonCharacterManager::Instance().GetInstancePtr(c_rkNetActorData.m_dwVID);
		if( pMainInstance )
		{
			const BOOL bIsMountingHorse = pMainInstance->IsMountingHorse();
...
{
	const auto f=m_kNetActorDict.find(dwVID);
...
{
	const auto f=m_kNetActorDict.find(c_rkNetUpdateActorData.m_dwVID);
	if (m_kNetActorDict.end()==f)
	{
#ifdef _DEBUG
...
{
	const auto f=m_kNetActorDict.find(c_rkNetMoveActorData.m_dwVID);
	if (m_kNetActorDict.end()==f)
	{
#ifdef _DEBUG
...
{
	const auto f=m_kNetActorDict.find(dwVictimVID);
	if (m_kNetActorDict.end()==f)
	{
#ifdef _DEBUG
