...
bool CPythonAcce::GetAttachedItem(BYTE bPos, BYTE & bHere, WORD & wCell) const
{
	if (bPos >= ACCE_WINDOW_MAX_MATERIALS)
		return false;
...
void CPythonAcce::GetResultItem(DWORD & dwItemVnum, DWORD & dwMinAbs, DWORD & dwMaxAbs) const
{
...
PyObject * GetAcceAttachedItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	BYTE bHere;
	WORD wCell;
	const bool bGet = CPythonAcce::Instance().GetAttachedItem(bPos, bHere, wCell);
	if (!bGet)
	{
		bHere = 0;
		wCell = 0;
...
PyObject * SendAcceRefineRequest(PyObject * poSelf, PyObject * poArgs)
{
	BYTE bHere;
	WORD wCell;
	const bool bGet = CPythonAcce::Instance().GetAttachedItem(1, bHere, wCell);
	if (bGet)
	{
		if (bHere)
		{
