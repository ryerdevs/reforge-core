...
namespace Discord
{
	...
	inline DCDATA GetNameData()
	{
		...
		if (!DCmapname.contains(WarpName))
		{
		...
		auto MapName = "Location: " + (DCmapname.contains(WarpName) ? DCmapname.at(WarpName) : WarpName);
	...
	{
		const auto pInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();
		if (!pInstance)
			return { "","" };
