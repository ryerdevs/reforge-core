...
DWORD CPythonBackground::GetRenderShadowTime() const
{
...
void CPythonBackground::SelectViewDistanceNum(int eNum)
{
	...
	const auto env = ((TEnvironmentData *) mc_pcurEnvironmentData);
...
void CPythonBackground::SetViewDistanceSet(int eNum, float fFarClip)
{
	...
	const float fSkyBoxScale						= fFarClip * 0.6f;//0.5773502f;
...
void CPythonBackground::Update(float fCenterX, float fCenterY, float fCenterZ)
{
	...
	// Target Effect Process
	{
		...
		{
			const DWORD dwTargetID = itor->first;
			const DWORD dwChrID = itor->second;
	...
	// Reserve Target Effect
	{
		...
		{
			const DWORD dwID = itor->first;
			const SReserveTargetEffect & rReserveTargetEffect = itor->second;

			const float ilx = float(rReserveTargetEffect.ilx);
			const float ily = float(rReserveTargetEffect.ily);

			const float fHeight = rkMap.GetHeight(ilx, ily);
			if (0.0f == fHeight)
			{
				++itor;
				continue;
...
void CPythonBackground::RenderCharacterShadowToTexture()
{
		...
		m_eShadowLevel == SHADOW_GROUND_AND_SOLO)
	{
		...
		const bool canRender=rkMap.BeginRenderCharacterShadowToTexture();
		if (canRender)
		{
...
{
	const auto f = std::find_if(m_kVct_kMapInfo.begin(), m_kVct_kMapInfo.end(), FFindWarpMapName(dwGlobalX, dwGlobalY));
...
{
	const TMapInfo* pkMapInfo = GlobalPositionToMapInfo(dwX, dwY);
	if (!pkMapInfo)
	{
	...
	RefreshShadowLevel();
	const TMapInfo & rMapInfo = *pkMapInfo;
...
void CPythonBackground::CreateTargetEffect(DWORD dwID, long lx, long ly)
{
	...
	const int ilx = +(lx-int(dwBaseX));
	const int ily = -(ly-int(dwBaseY));

	const float fHeight = rkMap.GetHeight(float(ilx), float(ily));
