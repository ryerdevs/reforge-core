...
PyObject* appEnablePerformanceTime(PyObject* poSelf, PyObject* poArgs)
{
	...
	const bool isEnable=nEnable ? true : false;
...
PyObject* appLoadLocaleAddr(PyObject* poSelf, PyObject* poArgs)
{
	...
	const auto enc = (char*)_alloca(size);
	fseek(fp, 0, SEEK_SET);
	fread(enc, size, 1, fp);
	fclose(fp);
...
PyObject* appIsExistFile(PyObject* poSelf, PyObject* poArgs)
{
	...
	const bool isExist=CEterPackManager::Instance().isExist(szFileName);
...
PyObject* appGetFileList(PyObject* poSelf, PyObject* poArgs)
{
	...
	const HANDLE hFind = FindFirstFile(szFilter, &wfd);
...
PyObject * appRunPythonFile(PyObject * poSelf, PyObject * poArgs)
{
	...
	const bool ret = CPythonLauncher::Instance().RunFile(szFileName);
...
PyObject * appLoadCameraSetting(PyObject * poSelf, PyObject * poArgs)
{
	...
	const bool bResult = CPythonApplication::Instance().LoadCameraSetting(szFileName);
...
PyObject * appCloseTextFile(PyObject * poSelf, PyObject * poArgs)
{
	...
	const auto pTextFileLoader = (CTextLineLoader *)iHandle;
...
PyObject * appSetGuildMarkPath(PyObject * poSelf, PyObject * poArgs)
{
    ...
    char newPath[256];
	const char * ext = strstr(path, ".tga");

    if (ext)
    {
	    const int extPos = ext - path;
...
PyObject* appLogoOpen(PyObject* poSelf, PyObject* poArgs)
{
	...
	const int nIsSuccess = 1; //CPythonApplication::Instance().OnLogoOpen(szName);
...
{
	const int nIsRun = 0; //CPythonApplication::Instance().OnLogoUpdate();
