...
bool CPythonSystem::GetResolution(int index, OUT DWORD *width, OUT DWORD *height, OUT DWORD *bpp) const
{
	if (index >= m_ResolutionCount)
		return false;
...
int	CPythonSystem::GetResolutionIndex(DWORD width, DWORD height, DWORD bit) const
{
...
DWORD CPythonSystem::GetBPP() const
{
	return m_Config.bpp;
}
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp(command, "NO_ANTIALIASING"))
			m_Config.iAntialiasing = atoi(value);
	...
	// @fixme047 BEGIN
	const auto screen_width = m_Config.bWindowed ? GetSystemMetrics(SM_CXFULLSCREEN) : GetSystemMetrics(SM_CXSCREEN);
	const auto screen_height = m_Config.bWindowed ? GetSystemMetrics(SM_CYFULLSCREEN) : GetSystemMetrics(SM_CYSCREEN);
