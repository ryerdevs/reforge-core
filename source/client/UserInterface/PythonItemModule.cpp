...
PyObject * itemGetIconImageFileName(PyObject * poSelf, PyObject * poArgs)
{
	...
	const CGraphicSubImage * pImage = pItemData->GetIconImage();
	if (!pImage)
...
PyObject * itemIsDetachScroll(PyObject * poSelf, PyObject * poArgs)
{
	...
	const int iType = pItemData->GetType();
	const int iSubType = pItemData->GetSubType();
...
PyObject* itemSaveLootingSettings(PyObject* poSelf, PyObject* poArgs)
{
	char* jsonData;
	if (!PyTuple_GetString(poArgs, 0, &jsonData))
		return Py_BadArgument();

	CPythonItem::Instance().SaveLootingSettings(jsonData);
	return Py_BuildNone();
}
#endif
