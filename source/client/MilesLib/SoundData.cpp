...
const char * CSoundData::GetFileName() const
{
	return m_filename;
}
...
ULONG CSoundData::GetSize() const
{
	return m_size;
}
...
DWORD CSoundData::GetPlayTime() const
{
	return m_dwPlayTime;
}
...
DWORD CSoundData::GetAccessTime() const
{
...
bool CSoundData::ReadFromDisk()
{
	...
	switch (type)
	{
		...
		case AILFILETYPE_MPEG_L1_AUDIO: // @fixme059
		case AILFILETYPE_MPEG_L2_AUDIO: // @fixme059
...
{
	const int iIndex = GetEmptySlotIndex();
...
U32 AILCALLBACK CSoundData::read_callback(U32 file_handle, void * buffer, U32 bytes)
{
	...
	const DWORD dwRealSize = min(ms_SoundFile[file_handle].Size(), bytes);
