...
BOOL CSoundManager::Create() const
{
...
void CSoundManager::SetDirection(float fxDir, float fyDir, float fzDir, float fxUp, float fyUp, float fzUp) const
{
	...
	for (int i = 0; i < CSoundManagerStream::MUSIC_INSTANCE_MAX_NUM; ++i)
	{
		...
		switch (rMusicInstance.MusicState)
		{
			case MUSIC_STATE_FADE_IN:
				...
				{
					const CSoundInstanceStream * pInstance = ms_SoundManagerStream.GetInstance(i);
					if (pInstance)
...
void CSoundManager::UpdateSoundData(DWORD dwcurTime, const NSound::TSoundDataVector * c_pSoundDataVector) const
{
	assert(!"CSoundManager::UpdateSoundData");
}

void CSoundManager::UpdateSoundData(float fx, float fy, float fz, DWORD dwcurTime, const NSound::TSoundDataVector * c_pSoundDataVector) const
{
	assert(!"CSoundManager::UpdateSoundData");
}
...
void CSoundManager::UpdateSoundInstance(DWORD dwcurFrame, const NSound::TSoundInstanceVector * c_pSoundInstanceVector) const
{
	for (DWORD i = 0; i < c_pSoundInstanceVector->size(); ++i)
	{
		const NSound::TSoundInstance & c_rSoundInstance = c_pSoundInstanceVector->at(i);
...
float CSoundManager::GetSoundScale() const
{
...
float CSoundManager::__ConvertRatioVolumeToApplyVolume(float fRatioVolume) const
{
...
float CSoundManager::__ConvertGradeVolumeToApplyVolume(int nGradeVolume) const
{
...
{
	const float fVolume=__ConvertGradeVolumeToApplyVolume(iGrade);
	SetSoundVolume(fVolume);
}

void CSoundManager::SetSoundVolumeRatio(float fRatio)
{
	const float fVolume = __ConvertRatioVolumeToApplyVolume(fRatio);
	SetSoundVolume(fVolume);
}
...
float CSoundManager::GetSoundVolume() const
{
	return m_fSoundVolume;
}

float CSoundManager::GetMusicVolume() const
{
	return m_fMusicVolume;
}

BOOL CSoundManager::GetSoundInstance2D(const char * c_szSoundFileName, ISoundInstance ** ppInstance) const
{
...
BOOL CSoundManager::GetSoundInstance3D(const char * c_szFileName, ISoundInstance ** ppInstance) const
{
	const int iIndex = ms_SoundManager3D.SetInstance(c_szFileName);
...
void CSoundManager::PlaySound2D(const char * c_szFileName) const
{
...
void CSoundManager::PlaySound3D(float fx, float fy, float fz, const char * c_szFileName, int iPlayCount) const
{
	if (0.0f == GetSoundVolume())
		return;
	...
	const ISoundInstance * pInstance = ms_SoundManager3D.GetInstance(iIndex);
	if (!pInstance)
		return;
...
int CSoundManager::PlayAmbienceSound3D(float fx, float fy, float fz, const char * c_szFileName, int iPlayCount) const
{
	if (0.0f == GetSoundVolume())
		return -1;
...
void CSoundManager::PlayCharacterSound3D(float fx, float fy, float fz, const char * c_szFileName, BOOL bCheckFrequency)
{
	...
	if (bCheckFrequency)
	{
		...
		const float fdx = (fx - m_fxPosition) * (fx - m_fxPosition);
		const float fdy = (fy - m_fyPosition) * (fy - m_fyPosition);

		if (fdx+fdy > s_fLimitDistance)
			return;

		const auto itor = m_PlaySoundHistoryMap.find(c_szFileName);
		if (m_PlaySoundHistoryMap.end() != itor)
		{
			const float fTime = itor->second;
...
void CSoundManager::StopSound3D(int iIndex) const
{
...
void CSoundManager::SetSoundVolume3D(int iIndex, float fVolume) const
{
	const ISoundInstance * pInstance = ms_SoundManager3D.GetInstance(iIndex);
...
void CSoundManager::FadeInMusic(const char * c_szFileName, float fVolumeSpeed)
{
	...
	for (int i = 0; i < CSoundManagerStream::MUSIC_INSTANCE_MAX_NUM; ++i)
	{
		const TMusicInstance & rMusicInstance = m_MusicInstances[i];
		if (MUSIC_STATE_OFF != rMusicInstance.MusicState)
			continue;
...
void CSoundManager::SaveVolume()
{
	...
	const float fBackupMusicVolume = m_fMusicVolume;
	const float fBackupSoundVolume = m_fSoundVolume;
...
void CSoundManager::PlayMusic(DWORD dwIndex, const char * c_szFileName, float fVolume, float fVolumeSpeed)
{
	...
	const CSoundInstanceStream * pInstance = ms_SoundManagerStream.GetInstance(dwIndex);
	if (!pInstance)
	{
...
BOOL CSoundManager::GetMusicIndex(const char * c_szFileName, DWORD * pdwIndex) const
{
	std::string strFileName;
	StringPath(c_szFileName, strFileName);
