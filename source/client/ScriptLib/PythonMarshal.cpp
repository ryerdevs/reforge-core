...
static PyObject * r_long64(RFILE *p)
{
	...
	unsigned char buf[8];
	int one = 1;
	const int is_little_endian = (int)*(char*)&one;
	if (is_little_endian) {
		memcpy(buf, &lo4, 4);
		memcpy(buf+4, &hi4, 4);
	}
...
static PyObject * r_object(RFILE *p)
{
	...
	case TYPE_CODE:
		if (PyEval_GetRestricted()) {
			...
			const int argcount = r_short(p);
			const int nlocals = r_short(p);
			const int stacksize = r_short(p);
			const int flags = r_short(p);
...
#define REASONABLE_FILE_LIMIT (1L << 18)
	...
	if (filesize > 0) {
		...
		if (pBuf != nullptr) {
			PyObject* v;
			const size_t n = fread(pBuf, 1, filesize, fp);
