...
static string* StringSplit(string strOrigin, string strTok)
{
    size_t cutAt;
    int index = 0;
    const auto strResult = new string[30];
...
int get_Item_Type_Value(std::string inputString)
{
	...
	const auto it = std::find(std::begin(arItemType), std::end(arItemType), inputString);
...
int get_Item_SubType_Value(int type_value, std::string inputString)
{
	...
	const auto itType = mapItemSubType.find(type_value);
	...
	const auto itSubtype = std::find(vecSubtype.begin(), vecSubtype.end(), inputString);
...
int get_Item_AntiFlag_Value(string inputString)
{
	...
	int retValue = 0;
	const string* arInputString = StringSplit(inputString, "|");
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
...

	{
		...
		{
			const std::string s_start_vnum (s.substr(0, pos));
			const std::string s_end_vnum (s.substr(pos +1 ));
	...
	const auto it = nameMap.find(itemTable->dwVnum);
	if (it != nameMap.end()) {
		const char * localeName = it->second;
	...
	itemTable->bSpecular = dataArray[30]; //@fixme328
