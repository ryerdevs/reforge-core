...
class CPrivManager : public singleton<CPrivManager>
{
	...
	// ADD_GUILD_PRIV_TIME
	void SendChangeGuildPriv(DWORD guild_id, BYTE type, int value, time_t end_time_sec) const;
	// END_OF_ADD_GUILD_PRIV_TIME

	// ADD_EMPIRE_PRIV_TIME
	void SendChangeEmpirePriv(BYTE empire, BYTE type, int value, time_t end_time_sec) const;
	// END_OF_ADD_EMPIRE_PRIV_TIME

	void SendChangeCharPriv(DWORD pid, BYTE type, int value) const;
