...
static bool __InitializeDefaultPriv()
{
	if (bCleanOldPriv)
	{
		const auto pCleanStuff(CDBManager::instance().DirectQuery("DELETE FROM priv_settings WHERE value <= 0 OR duration <= NOW();", SQL_COMMON));
		printf("DEFAULT_PRIV_EMPIRE: removed %u expired priv settings.\n", pCleanStuff->Get()->uiAffectedRows);
	}
	const auto pMsg(CDBManager::instance().DirectQuery("SELECT priv_type, id, type, value, UNIX_TIMESTAMP(duration) FROM priv_settings", SQL_COMMON));
...
static bool __UpdateDefaultPriv(const char* priv_type, DWORD id, BYTE type, int value, time_t duration_sec)
{
	...
	const auto pMsg(CDBManager::instance().DirectQuery(szQuery, SQL_COMMON));
...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	const CMonarch::VEC_MONARCHCANDIDACY & rVecMonarchCandidacy = CMonarch::instance().GetVecMonarchCandidacy();
...
void CClientManager::QUERY_PLAYER_COUNT(CPeer * pkPeer, TPlayerCountPacket * pPacket) const
{
	pkPeer->SetUserCount(pPacket->dwCount);
}

void CClientManager::QUERY_QUEST_SAVE(CPeer * pkPeer, TQuestTable * pTable, DWORD dwLen) const
{
...
void CClientManager::QUERY_SAFEBOX_LOAD(CPeer * pkPeer, DWORD dwHandle, TSafeboxLoadPacket * packet, bool bMall) const
{
	const auto pi = new ClientHandleInfo(dwHandle);
...
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
	auto pi = (ClientHandleInfo *) qi->pvData;
	const DWORD dwHandle = pi->dwHandle;
	...
	if (pi->account_index == 0)
	{
		...
		const auto pSafebox = new TSafeboxTable;
		...
		{
			const MYSQL_ROW row = mysql_fetch_row(res->pSQLResult);
	...
	else
	{
		...
		{
			const CGrid grid(5, MAX(1, pi->pSafebox->bSize) * 9);
			bool bEscape = false;
			...
			if (!bEscape)
			{
				...
				while (it != pSet->end())
				{
...

					{
						...
						const TItemTable* item_table = it->second;
					...
					const auto pmsg(CDBManager::instance().DirectQuery(szQuery));
...
void CClientManager::QUERY_SAFEBOX_CHANGE_SIZE(CPeer * pkPeer, DWORD dwHandle, TSafeboxChangeSizePacket * p) const
{
	const auto pi = new ClientHandleInfo(dwHandle);
...
void CClientManager::RESULT_SAFEBOX_CHANGE_SIZE(CPeer * pkPeer, SQLMsg * msg) const
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
	const auto p = (ClientHandleInfo *) qi->pvData;
	const DWORD dwHandle = p->dwHandle;
	const BYTE bSize = p->account_index;
...
void CClientManager::QUERY_SAFEBOX_CHANGE_PASSWORD(CPeer * pkPeer, DWORD dwHandle, TSafeboxChangePasswordPacket * p) const
{
	const auto pi = new ClientHandleInfo(dwHandle);
...
void CClientManager::RESULT_SAFEBOX_CHANGE_PASSWORD(CPeer * pkPeer, SQLMsg * msg) const
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
	const auto p = (ClientHandleInfo *) qi->pvData;
	const DWORD dwHandle = p->dwHandle;
	...
	if (msg->Get()->uiNumRows > 0)
	{
		...
		{
			char szQuery[QUERY_MAX_LEN];
			char escape_pwd[SAFEBOX_PASSWORD_MAX_LEN * 2 + 1]; // @fixme330 escape buffer fix
...
void CClientManager::RESULT_SAFEBOX_CHANGE_PASSWORD_SECOND(CPeer * pkPeer, SQLMsg * msg) const
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
	const auto p = (ClientHandleInfo *) qi->pvData;
	const DWORD dwHandle = p->dwHandle;
...
{
	const auto pReqInfo = (TItemPricelistReqInfo*)static_cast<CQueryInfo*>(pMsg->pvUserData)->pvData;
...
{
	const auto pUpdateTable = (TItemPriceListTable*)static_cast<CQueryInfo*>(pMsg->pvUserData)->pvData;
...
void CClientManager::QUERY_SAFEBOX_SAVE(CPeer * pkPeer, TSafeboxTable * pTable) const
{
	char szQuery[QUERY_MAX_LEN];
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, DWORD dwHandle, TEmpireSelectPacket * p) const
{
	char szQuery[QUERY_MAX_LEN];
	...
	sys_log(0, "EmpireSelect: %s", szQuery);
	{
		...
		if (pRes->uiNumRows)
		{
			...
			const MYSQL_ROW row = mysql_fetch_row(pRes->pSQLResult);
			DWORD pids[PLAYER_PER_ACCOUNT]{}; // @fixme310

			const UINT g_start_map[4] =
			{
				0,  // reserved
				1,
				21,
			...
			const DWORD g_start_position[4][2]=
			{
				{      0,      0 },
				{ 469300, 964200 },
				{  55700, 157900 },
...
{
	const auto p = (TPacketGDSetup *) c_pData;
	...
	for (DWORD c = 0; c < p->dwLoginCount; ++c, ++pck) // todo: dead code
	{
...
{
	const DWORD dwID = *(DWORD *) c_pData;
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		if (c)
		{
			const auto it = m_map_pkItemCacheSetPtr.find(c->Get()->owner);
		...
		CDBManager::instance().ReturnQuery(itemQuery.c_str(), QID_ITEM_SAVE, pkPeer->GetHandle(), nullptr);
	}
	else
	{
		if (g_test_server)
...
{
	const auto it = m_map_itemCache.find(id);
...
{
	const auto it = m_mapItemPriceListCache.find(dwID);
...
void CClientManager::QUERY_ITEM_DESTROY(CPeer * pkPeer, const char * c_pData)
{
	...
	const DWORD dwPID = *(DWORD *) c_pData;
...
void CClientManager::AddGuildPriv(TPacketGiveGuildPriv* p) const
{
...
void CClientManager::AddEmpirePriv(TPacketGiveEmpirePriv* p) const
{
...
void CClientManager::AddCharacterPriv(TPacketGiveCharacterPriv* p) const
{
...
void CClientManager::MoneyLog(TPacketMoneyLog* p) const
{
...
CLoginData * CClientManager::GetLoginDataByLogin(const char * c_pszLogin)
{
	...
	const auto it = m_map_pkLoginDataByLogin.find(szLogin);
...
{
	const auto it = m_map_pkLoginDataByAID.find(dwAID);
...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	{
		const auto pkLD = new CLoginData;
...
void CClientManager::GuildDepositMoney(TPacketGDGuildMoney* p) const
{
	CGuildManager::instance().DepositMoney(p->dwGuild, p->iGold);
}

void CClientManager::GuildWithdrawMoney(CPeer* peer, TPacketGDGuildMoney* p) const
{
	CGuildManager::instance().WithdrawMoney(peer, p->dwGuild, p->iGold);
}

void CClientManager::GuildWithdrawMoneyGiveReply(TPacketGDGuildMoneyWithdrawGiveReply* p) const
{
	CGuildManager::instance().WithdrawMoneyReply(p->dwGuild, p->bGiveSuccess, p->iChangeGold);
}

void CClientManager::GuildWarBet(TPacketGDGuildWarBet * p) const
{
...
{
	const DWORD dwID = pdw[0];
	const DWORD dwGuild = pdw[1];
...
void CClientManager::BlockChat(TPacketBlockChat* p)
{
	...
	{
		const MYSQL_ROW row = mysql_fetch_row(pRes->pSQLResult);
		const DWORD pid = strtoul(row[0], nullptr, 10);
...
void CClientManager::MarriageAdd(TPacketMarriageAdd * p) const
{
...
void CClientManager::MarriageUpdate(TPacketMarriageUpdate * p) const
{
...
void CClientManager::MarriageRemove(TPacketMarriageRemove * p) const
{
...
void CClientManager::WeddingEnd(TPacketWeddingEnd * p) const
{
...
void CClientManager::MyshopPricelistUpdate(const TItemPriceListTable* pPacket) // @fixme403 (TPacketMyshopPricelistHeader to TItemPriceListTable)
{
	...
	{
		const auto pUpdateTable = new TItemPriceListTable;
...
void CClientManager::MyshopPricelistRequest(CPeer* peer, DWORD dwHandle, DWORD dwPlayerID)
{
	if (CItemPriceListTableCache* pCache = GetItemPriceListCache(dwPlayerID))
	{
		...
		const TPacketMyshopPricelistHeader header =
		{
			pTable->dwOwnerID,
			pTable->byCount
		};
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_LOGIN:
				QUERY_LOGIN(peer, dwHandle, (SLoginPacket *) data);
				break;
...
{
	if (m_setAuthPeer.contains(pPeer))
	{
		m_setAuthPeer.erase(pPeer);
	}
	else
	{
...
CPeer * CClientManager::GetAnyPeer() const
{
	if (m_peerList.empty())
		return nullptr;
...
void UsageLog()
{
	...
	const int         avg = g_dwUsageAvg / 3600;
...
int CClientManager::Process()
{
	...
	while (pulses--)
	{
		...
		if (!(thecore_heart->pulse % thecore_heart->passes_per_sec))
		{
			...
			const DWORD dwCount = CClientManager::instance().GetUserCount();
	...
	const int num_events = fdwatch(m_fdWatcher, nullptr);
	int idx;
	CPeer * peer;
	...
	for (idx = 0; idx < num_events; ++idx)
	{
		...
		switch (fdwatch_check_event(m_fdWatcher, peer->GetFd(), idx))
		{
			case FDW_READ:
				...
				{
					if (m_setAuthPeer.contains(peer))
						if (g_log)
			...
			case FDW_WRITE:
				...
				else if (peer->GetSendLength() == 0)
				{
					/* fdwatch keeps the WRITE interest once registered and a
					   writable socket then reports FDW_WRITE on every loop
					   iteration, starving the main loop (queued async SQL
					   results never dispatch). Re-register with READ only
					   after the output drains; the peer send paths re-add
					   FDW_WRITE when new data is queued. */
					fdwatch_del_fd(m_fdWatcher, peer->GetFd());
					fdwatch_add_fd(m_fdWatcher, peer->GetFd(), peer, FDW_READ, false);
				}
...
DWORD CClientManager::GetUserCount() const
{
...
void CClientManager::SendNotice(const char * c_pszFormat, ...)
{
	...
	const int len = vsnprintf(szBuf, sizeof(szBuf), c_pszFormat, args);
	va_end(args);
	szBuf[len] = '\0';
...
DWORD CClientManager::GetItemID() const
{
	return m_itemRange.dwUsableItemIDMin;
}
// ITEM_UNIQUE_ID_END
	...
	while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != nullptr)
	{
		...
		if (strcmp(locale.szKey, "LOCALE") == 0)
		{
			if (strcmp(locale.szValue, "cibn") == 0)
			{
				...
				g_stLocale = "gb2312";
				g_stLocaleNameColumn = "locale_name"; // @fixme350 old gb2312name
			}
			...
			else if (strcmp(locale.szValue, "latin1") == 0)
			{
				sys_log(0, "locale[LOCALE] = %s", locale.szValue);
				g_stLocale = "latin1";
				g_stLocaleNameColumn = "locale_name";
...
bool CClientManager::__GetAdminInfo(const char *szIP, std::vector<tAdminInfo> & rAdminVec) const
{
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery),
...
bool CClientManager::__GetHostInfo(std::vector<std::string> & rIPVec) const
{
	char szQuery[512];
...
void CClientManager::ReloadAdmin(CPeer*, TPacketReloadAdmin* p)
{
	...
	const DWORD dwPacketSize = sizeof(WORD) + sizeof (WORD) + sizeof(tAdminInfo) * vAdmin.size() +
		  sizeof(WORD) + sizeof(WORD) + 16 * vHost.size();
...
//BREAK_MARRIAGE
void CClientManager::BreakMarriage(CPeer * peer, const char * data) const
{
	DWORD pid1, pid2;
...
void CClientManager::Election(CPeer * peer, DWORD dwHandle, const char* data) const
{
	DWORD idx;
	DWORD selectingpid;
...
void CClientManager::Candidacy(CPeer *  peer, DWORD dwHandle, const char* data)
{
	...
	if (!CMonarch::instance().AddCandidacy(pid, data))
	{
		...
		peer->Encode(nullptr, sizeof(int));
		peer->Encode(data, 32);
		return;
	}
	else
...
void CClientManager::TakeMonarchMoney(CPeer * peer, DWORD dwHandle, const char * data) const
{
	const int Empire = *(int *) data;
	data += sizeof(int);

	const DWORD pid = *(DWORD *) data;
	data += sizeof(int);
...
void CClientManager::ComeToVote(CPeer * peer, DWORD dwHandle, const char * data) const
{
...
void CClientManager::ChangeMonarchLord(CPeer * peer, DWORD dwHandle, TPacketChangeMonarchLord* info)
{
	...
	if (pMsg->Get()->uiNumRows != 0)
	{
		...
		const auto pMsg2 = CDBManager::instance().DirectQuery(szQuery, SQL_PLAYER);
...
void CClientManager::SendSpareItemIDRange(CPeer* peer) const
{
...
// delete gift notify icon
void CClientManager::DeleteAwardId(TPacketDeleteAwardID *data) const
{
...
{
	const auto it = m_mChannelStatus.find(pData->nPort);
...
void CClientManager::ChargeCash(const TRequestChargeCash* packet) const
{
	char szQuery[512];
