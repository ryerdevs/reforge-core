...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer * pkPeer, DWORD dwHandle, TPacketGDLoginByKey * p)
{
	...
	{
		sys_err("LOGIN_BY_KEY without request %s", r.login);
	...
	const auto pkTab = new TAccountTable;
	...
	// Language System: LOGIN_BY_KEY builds the table from the in-memory
	// CLoginData (created during auth without lang), so fetch the account
	// language to keep the TAccountTable sent in HEADER_DG_LOGIN_SUCCESS
	// consistent with the classic QUERY_LOGIN path.
	{
		char szLangQuery[QUERY_MAX_LEN];
		snprintf(szLangQuery, sizeof(szLangQuery), "SELECT lang FROM account%s WHERE id=%u", GetTablePostfix(), r.id);
		const auto pLangMsg(CDBManager::instance().DirectQuery(szLangQuery, SQL_ACCOUNT));

		if (pLangMsg->Get()->uiNumRows > 0)
		{
			const MYSQL_ROW langRow = mysql_fetch_row(pLangMsg->Get()->pSQLResult);
			if (langRow[0] && *langRow[0])
				strlcpy(pkTab->lang, langRow[0], sizeof(pkTab->lang));
		}
	}

	const auto info = new ClientHandleInfo(dwHandle);
...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg) const
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
	const auto info = (ClientHandleInfo *) qi->pvData;
	...
	{
		const DWORD account_id = info->pAccountTable->id;
		char szQuery[QUERY_MAX_LEN];
...
// PLAYER_INDEX_CREATE_BUG_FIX
void CClientManager::RESULT_PLAYER_INDEX_CREATE(CPeer * pkPeer, SQLMsg * msg) const
{
	const auto qi = (CQueryInfo *) msg->pvUserData;
...
TAccountTable * CreateAccountTableFromRes(MYSQL_RES * res)
{
	...
	// Language System: account language (e.g. "es"), 13th column of QUERY_LOGIN.
	if (row[col] && *row[col])
		strlcpy(pkTab->lang, row[col], sizeof(pkTab->lang));
	col++;
...
void CreateAccountPlayerDataFromRes(MYSQL_RES * pRes, TAccountTable * pkTab)
{
	...
	{
		const MYSQL_ROW row = mysql_fetch_row(pRes);
		int col = 0;
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				const TPlayerTable * pt = pc ? pc->Get(false) : nullptr;
...
void CClientManager::QUERY_LOGIN(CPeer * peer, DWORD dwHandle, SLoginPacket * data)
{
	char szLogin[LOGIN_MAX_LEN + 1];
	char szPasswd[PASSWD_MAX_LEN + 1];

	trim_and_lower(data->login, szLogin, sizeof(szLogin));
	strlcpy(szPasswd, data->passwd, sizeof(szPasswd));

	if (g_test_server)
		sys_log(0, "QUERY_LOGIN %s", szLogin);

	char szQuery[QUERY_MAX_LEN];
	// Column layout MUST match CreateAccountTableFromRes():
	// hash, id, login, password, social_id, bEmpire, player_id[PLAYER_PER_ACCOUNT], status, lang
	// The player ids and the empire live in player_index (pid1..pid5, empire).
	// NOTE: the account.lang column is added by the Language System ALTER:
	//   ALTER TABLE account ADD COLUMN lang varchar(4) NOT NULL DEFAULT 'es';
	snprintf(szQuery, sizeof(szQuery),
			"SELECT mysql_hash_password('%s'), a.id, a.login, a.password, a.social_id, pi.empire, "
			"pi.pid1, pi.pid2, pi.pid3, pi.pid4, pi.pid5, a.status, a.lang "
			"FROM account%s a LEFT JOIN player.player_index%s pi ON pi.id = a.id "
			"WHERE a.login='%s' AND a.password=mysql_hash_password('%s')",
			szPasswd, GetTablePostfix(), GetTablePostfix(), szLogin, szPasswd);

	const auto info = new ClientHandleInfo(dwHandle);
	strlcpy(info->ip, "", sizeof(info->ip));

	// Account queries must run on the SQL_ACCOUNT connection (the account
	// schema); the default slot is SQL_PLAYER and would resolve FROM account
	// as player.account -> "Table 'player.account' doesn't exist".
	CDBManager::instance().ReturnQuery(szQuery, QID_LOGIN, peer->GetHandle(), info, SQL_ACCOUNT);
}
...
{
	const auto packet = (TLogoutPacket*)data;
	...
	pLoginData->SetAllowLoginByKey(packet->bCanUseLoginByKey); // @fixme353
...
void CClientManager::QUERY_CHANGE_NAME(CPeer * peer, DWORD dwHandle, TPacketGDChangeName * p) const
{
	char queryStr[QUERY_MAX_LEN];
	snprintf(queryStr, sizeof(queryStr),
		"SELECT COUNT(*) as count FROM player%s WHERE name='%s' AND id <> %u", GetTablePostfix(), p->name, p->pid);

	const auto pMsg(CDBManager::instance().DirectQuery(queryStr, SQL_PLAYER));
