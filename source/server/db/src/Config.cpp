...
void CConfig::NextLine(FILE *fp) const
{
	int c;
...
bool CConfig::GetWord(FILE *fp, char *tar) const
{
	int i = 0;
	int c;
...
bool CConfig::GetLine(FILE* fp, char*dest) const
{
	int c;
	int i = 0;
	while ((c = fgetc(fp)) != EOF)
...
{
	const std::string * pstStr = Search(key);
	if (!pstStr)
		return false;
