...
void CPrivManager::Update()
{
	...
	{
		const TPrivGuildData* p = m_pqPrivGuild.top().second;
		m_pqPrivGuild.pop();
	...
	{
		const TPrivEmpireData* p = (m_pqPrivEmpire.top().second);
		m_pqPrivEmpire.pop();

		if (p->value != 0 && !p->bRemoved)
			SendChangeEmpirePriv(p->empire, p->type, 0, 0);

		if (p == m_aaPrivEmpire[p->type][p->empire]) // @fixme351
	...
	{
		const TPrivCharData* p = (m_pqPrivChar.top().second);
		m_pqPrivChar.pop();
...
// ADD_GUILD_PRIV_TIME
void CPrivManager::SendChangeGuildPriv(DWORD guild_id, BYTE type, int value, time_t end_time_sec) const
{
...
// ADD_EMPIRE_PRIV_TIME
void CPrivManager::SendChangeEmpirePriv(BYTE empire, BYTE type, int value, time_t end_time_sec) const
{
	CClientManager::instance().for_each_peer(FSendChangeEmpirePriv(empire, type, value, end_time_sec));
}
// END_OF_ADD_EMPIRE_PRIV_TIME

void CPrivManager::SendChangeCharPriv(DWORD pid, BYTE type, int value) const
{
...
void CPrivManager::SendPrivOnSetup(CPeer* peer)
{
	for (int i = 1; i < MAX_PRIV_NUM; ++i)
	{
		...
		{
			// ADD_EMPIRE_PRIV_TIME
			const TPrivEmpireData* pPrivEmpireData = m_aaPrivEmpire[i][e];
			if (pPrivEmpireData)
			{
