...
void fdwatch_add_fd(LPFDWATCH fdw, socket_t fd, void* client_data, int rw, int oneshot)
{
	...
	if (idx < 0) {
		/* Reuse a hole if any, else append. */
		int i;
		for (i = 0; i < fdw->nselect_fds; ++i) {
			if (fdw->select_fds[i] == (socket_t)-1) {
				idx = i;
				break;
			}
		}
...
void fdwatch_del_fd(LPFDWATCH fdw, socket_t fd)
{
	...
	const int idx = fdwatch_get_fdidx(fdw, fd);
	if (idx < 0) {
		return;
	}

	/* Mark the slot as a hole (fd -1) instead of swapping the last entry
	   into it. Swapping corrupts pending event indices when descriptors
	   are closed/reused while events are being processed (the auth/chan
	   processes close P2P descriptors constantly). Holes are skipped by
	   fdwatch(), and nselect_fds is recomputed lazily by compacting at
	   the start of fdwatch(). */
	fdw->select_fds[idx] = (socket_t)-1;
	fdw->fd_data[idx] = nullptr;
	fdw->fd_rw[idx] = 0;

	/* Compact trailing holes so nselect_fds stays correct. */
	while (fdw->nselect_fds > 0 && fdw->select_fds[fdw->nselect_fds - 1] == (socket_t)-1)
	{
		--fdw->nselect_fds;
		fdw->select_fds[fdw->nselect_fds] = 0;
		fdw->fd_data[fdw->nselect_fds] = nullptr;
...
int fdwatch(LPFDWATCH fdw, struct timeval *timeout)
{
    ...
    /* select() requires nfds = highest fd + 1; passing 0 (as the old
       FreeBSD-oriented code did) makes Linux inspect no descriptors at
       all, so no read/write/accept event is ever reported. */
    for (i = 0; i < fdw->nselect_fds; ++i)
	if (fdw->select_fds[i] > maxfd)
	    maxfd = fdw->select_fds[i];

    if (!timeout)
    {
	tv.tv_sec = 0;
	tv.tv_usec = 0;
	r = select(maxfd + 1, &fdw->working_rfd_set, &fdw->working_wfd_set, (fd_set*) nullptr, &tv);
    }
    else
    {
	tv = *timeout;
	...
	if (fdwatch_check_fd(fdw, fdw->select_fds[i]))
	    fdw->select_rfdidx[event_idx++] = fdw->select_fds[i];
    }
...
void * fdwatch_get_client_data(LPFDWATCH fdw, unsigned int event_idx)
{
	const socket_t fd = fdw->select_rfdidx[event_idx];
	const int idx = fdwatch_get_fdidx(fdw, fd);
	if (idx < 0) {
		return nullptr;
	}
...
int fdwatch_get_ident(LPFDWATCH fdw, unsigned int event_idx)
{
	return (int)fdw->select_rfdidx[event_idx];
}

void fdwatch_clear_event(LPFDWATCH fdw, socket_t fd, unsigned int event_idx)
{
	const socket_t rfd = fdw->select_rfdidx[event_idx];
	if (fd != rfd) {
		return;
	}
...
int fdwatch_check_event(LPFDWATCH fdw, socket_t fd, unsigned int event_idx)
{
	...
	const int result = fdwatch_check_fd(fdw, fd);
	if (result & FDW_READ) {
		return FDW_READ;
