...
// System headers (needed on Linux; on Windows they were inside the #else branch)
#include <stdio.h>
#include <string.h>
#include <stdarg.h>
#include <errno.h>
...
#endif
/* select() backend: raise the fd limit above the default 64 so the
   server can register the listening sockets, all P2P peers, the DB
   connection and game clients without silently dropping fds. */
#ifndef FD_SETSIZE
