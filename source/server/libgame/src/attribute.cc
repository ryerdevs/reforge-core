...
int CAttribute::GetDataType() const
{
    return dataType;
}

void * CAttribute::GetDataPtr() const
{
    return data;
}
...
void CAttribute::Remove(DWORD x, DWORD y, DWORD attr) const
{
    if (x > width || y > height)
	return;
...
DWORD CAttribute::Get(DWORD x, DWORD y) const
{
    if (x > width || y > height)
	return 0;
...
void CAttribute::CopyRow(DWORD y, DWORD * row) const
{
    if (!data)
    {
	for (DWORD x = 0; x < width; ++x)
