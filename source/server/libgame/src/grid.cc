...
{
    m_pGrid = new char[m_iWidth * m_iHeight];
    const int iSize = std::min(w * h, pkGrid->m_iWidth * pkGrid->m_iHeight);
...
void CGrid::Clear() const
{
    memset(m_pGrid, 0, sizeof(char) * m_iWidth * m_iHeight);
}

int CGrid::FindBlank(int w, int h) const
{
	...
	{
		const int iIndex = iRow * m_iWidth + iCol;
...
bool CGrid::Put(int iPos, int w, int h) const
{
    if (!IsEmpty(iPos, w, h))
	return false;
    ...
    for (int y = 0; y < h; ++y)
    {
	    const int iStart = iPos + (y * m_iWidth);
	m_pGrid[iStart] = true;
...
void CGrid::Get(int iPos, int w, int h) const
{
...
bool CGrid::IsEmpty(int iPos, int w, int h) const
{
    if (iPos < 0)
	return false;

    const int iRow = iPos / m_iWidth;
...
unsigned int CGrid::GetSize() const
{
