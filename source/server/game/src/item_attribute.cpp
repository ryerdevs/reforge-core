...
{
	const int index = FindAttribute(bType);
...
void CItem::CopyAttributeTo(LPITEM pItem) const
{
	pItem->SetAttributes(m_aAttr);
}

int CItem::GetRareAttrCount() const
{
	int ret = 0;
...
bool CItem::AddRareAttribute()
{
	...
	const int pos = count + ITEM_ATTRIBUTE_RARE_START;
