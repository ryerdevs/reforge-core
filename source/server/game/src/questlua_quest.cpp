...
namespace quest
{
	...
	ALUA(quest_setstate)
	{
		...
		if (pPC)
		{
			const std::string stCurrentState = lua_tostring(L,-1);
			if ( test_server )
	...
	ALUA(quest_coroutine_yield)
	{
		...
		if (q.IsInOtherPCBlock())
		{
			...
			const QuestState* pQS = pPC->GetRunningQuestState();
	...
	ALUA(quest_get_current_quest_index)
	{
		...
		const int idx = q.GetQuestIndexByName(pPC->GetCurrentQuestName());
	...
	{
		CQuestManager& q = CQuestManager::instance();
		const DWORD pid = lua_tonumber(L, -1);
