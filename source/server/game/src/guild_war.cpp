...
bool GuildWar_IsWarMap(unsigned type)
{
	...
	const unsigned mapIndex = GuildWar_GetTypeMapIndex(type);
...
void CGuild::NotifyGuildMaster(const char* msg) const
{
...
bool CGuild::WaitStartWar(DWORD dwOppGID)
{
	...
	const DWORD lMapIndex = CWarMapManager::instance().CreateWarMap(rkGuildWarInfo, id1, id2);
	if (!lMapIndex)
	{
...
void CGuild::RefuseWar(DWORD dwOppGID)
{
	...
	{
		const BYTE type = it->second.type;
...
void CGuild::GuildWarEntryAsk(DWORD dwOppGID)
{
	...
	while (it != m_memberOnline.end())
	{
		...
		const unsigned int questIndex=CQuestManager::instance().GetQuestIndexByName("guild_war_join");
		if (questIndex)
		{
...
void CGuild::ChangeLadderPoint(int iChange) const
{
