...
namespace quest
{
	...
	static ALUA(item_select_cell)
	{
		...
		const DWORD cell = (DWORD) lua_tonumber(L, 1);

		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const LPITEM item = ch ? ch->GetInventoryItem(cell) : nullptr;
	...
	static ALUA(item_select)
	{
		...
		const DWORD id = (DWORD) lua_tonumber(L, 1);
		const LPITEM item = ITEM_MANAGER::instance().Find(id);
	...
	{
		CQuestManager& q = CQuestManager::instance();
		const LPITEM item = q.GetCurrentDNDItem();
		if (item != nullptr) {
	...
	static ALUA(item_has_flag)
	{
		...
		const long lCheckFlag = (long) lua_tonumber(L, 1);
	...
	{
		CQuestManager& q = CQuestManager::instance();
		const LPITEM pItem = q.GetCurrentDNDItem();
	...
	static ALUA(item_copy_and_give_before_remove)
	{
		...
		const DWORD vnum = (DWORD)lua_tonumber(L, 1);

		CQuestManager& q = CQuestManager::instance();
		const LPITEM pItem = q.GetCurrentDNDItem();
		const LPCHARACTER pChar = q.GetCurrentCharacterPtr();

		const LPITEM pkNewItem = ITEM_MANAGER::instance().CreateItem(vnum, 1, 0, false);
		...
		if (pkNewItem)
		{
			...
			const auto itemPos = pItem->GetItemPos(); // @warme019
			ITEM_MANAGER::instance().RemoveItem(pItem, "REMOVE (COPY SUCCESS)");

			pkNewItem->AddToCharacter(pChar, itemPos); // @warme019
