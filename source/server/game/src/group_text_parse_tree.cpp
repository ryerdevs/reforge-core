...
const char * CGroupTextParseTreeLoader::GetFileName() const
{
...
bool CGroupTextParseTreeLoader::Load(const char * c_szFileName)
{
	...
	const auto pData = M2_NEW char[fileSize];
...
bool CGroupTextParseTreeLoader::LoadGroup(CGroupNode * pGroupNode)
{
	...
	for (; m_dwcurLineIndex < m_fileLoader.GetLineCount(); ++m_dwcurLineIndex)
	{
		...
		if (0 == stTokenVector[0].compare("group"))
		{
			...
			const auto pNewNode = new CGroupNode();
...
CGroupNode* CGroupTextParseTreeLoader::GetGroup(const char * c_szGroupName) const
{
	if (nullptr == m_pRootGroupNode)
		return nullptr;
...
DWORD CGroupNode::GetChildNodeCount() const
{
...
bool CGroupNode::SetChildNode(const char * c_szKey, CGroupNode* pChildGroup)
{
	...
	const auto it = m_mapChildNodes.find(c_szKey);
...
{
	const auto it = m_mapChildNodes.find(c_rstrKey);
	if (it != m_mapChildNodes.end())
		return it->second;
	else
		return nullptr;
...
{
	return m_map_rows.contains(c_rstrKey);
}

int CGroupNode::GetRowCount() const
{
...
{
	const auto row_it = m_map_rows.find(c_rstrRowKey);
...
{
	const CGroupNode* pChildGroup = GetChildNode(stGroupName);
	if (nullptr != pChildGroup)
	{
...
{
	const auto it = m_map_columnNameToIndex.find(stName);
