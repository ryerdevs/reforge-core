...
void CDungeon::QuitParty(LPPARTY pParty)
{
	...
	const auto it = m_map_pkParty.find(pParty); // @warme011 boom! crash!
...
{
	const auto info = dynamic_cast<dungeon_id_info*>( event->info );
	...
	const LPDUNGEON pDungeon = CDungeonManager::instance().Find(info->dungeon_id);
	if (pDungeon == nullptr) {
		return 0;
	}
...
void CDungeon::IncPartyMember(LPPARTY pParty, LPCHARACTER ch)
{
	...
	const auto it = m_map_pkParty.find(pParty);
...
void CDungeon::JumpAll(long lFromMapIndex, int x, int y) const
{
	x *= 100;
	y *= 100;
...
void CDungeonManager::Destroy(CDungeon::IdType dungeon_id)
{
	...
	const LPDUNGEON pDungeon = Find(dungeon_id);
	if (pDungeon == nullptr) {
		return;
	}
	...
	const DWORD server_timer_arg = lMapIndex;
...
{
	const auto it_to = m_map_Area.find(pos_to);
	...
	const auto it_from = m_map_Area.find(pos_from);
...
{
	const auto it = m_map_Area.find(pos);
...
void CDungeon::Spawn(DWORD vnum, const char* pos)
{
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX+dx, pkSectreeMap->m_setting.iBaseY+dy, 0, false, dir);
	if (ch)
		ch->SetDungeon(this);
}
...
LPCHARACTER CDungeon::SpawnMob(DWORD vnum, int x, int y, int dir)
{
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX+x*100, pkSectreeMap->m_setting.iBaseY+y*100, 0, false, dir == 0 ? -1 : (dir - 1) * 45);
...
LPCHARACTER CDungeon::SpawnMob_ac_dir(DWORD vnum, int x, int y, int dir)
{
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX+x*100, pkSectreeMap->m_setting.iBaseY+y*100, 0, false, dir);
...
void CDungeon::SpawnNameMob(DWORD vnum, int x, int y, const char* name)
{
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX+x, pkSectreeMap->m_setting.iBaseY+y, 0, false, -1);
	if (ch)
	{
		ch->SetName(name);
		ch->SetDungeon(this);
...
void CDungeon::SpawnGotoMob(long lFromX, long lFromY, long lToX, long lToY)
{
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(MOB_GOTO_VNUM, m_lMapIndex, lFromX, lFromY, 0, false, -1);
...
LPCHARACTER CDungeon::SpawnGroup(DWORD vnum, long x, long y, float radius, bool bAggressive, int count)
{
	...
	while (count--)
	{
		const LPCHARACTER chLeader = CHARACTER_MANAGER::instance().SpawnGroup(vnum, m_lMapIndex, sx, sy, ex, ey, nullptr, bAggressive, this);
		if (chLeader && !ch)
			ch = chLeader;
	}
...
void CDungeon::SpawnMoveGroup(DWORD vnum, const char* pos_from, const char* pos_to, int count) const
{
	const auto it_to = m_map_Area.find(pos_to);
...
{
	const LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
	if (pkMap == nullptr) {
		sys_err("CDungeon: SECTREE_MAP not found for #%ld", m_lMapIndex);
		return;
	}
...
{
	const LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lOrigMapIndex);
...
{
	const LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(m_lWarpMapIndex);
...
bool CDungeon::IsAllPCNearTo(int x, int y, int dist) const
{
	const LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
