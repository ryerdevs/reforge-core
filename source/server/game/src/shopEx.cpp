...
{
	const BYTE tabIdx = pos / SHOP_HOST_ITEM_MAX_NUM;
	const BYTE slotPos = pos % SHOP_HOST_ITEM_MAX_NUM;
	...
	case SHOP_COIN_TYPE_SECONDARY_COIN:
		{
			const DWORD count = ch->CountSpecifyTypeItem(ITEM_SECONDARY_COIN);
			if (count < dwPrice)
			{
