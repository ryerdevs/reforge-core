...
namespace quest
{
	...
	{
		CQuestManager& q = CQuestManager::instance();
		const LPDESC d = q.GetCurrentCharacterPtr()->GetDesc();
		if (d)
		{
			const BYTE header = HEADER_GC_REQUEST_MAKE_GUILD;
			d->Packet(&header, 1);
		}
		return 0;
	}
	...
	ALUA(game_drop_item)
	{
		...
		const int count = (int) lua_tonumber(L, 2);
		const long x = ch->GetX();
		const long y = ch->GetY();

		const LPITEM item = ITEM_MANAGER::instance().CreateItem(item_vnum, count);
	...
	ALUA(game_drop_item_with_ownership)
	{
		...
		if (lua_isnumber(L, 3))
		{
			const int sec = (int) lua_tonumber(L, 3);
			if (sec <= 0)
			{
