...
namespace quest
{
	...
	ALUA(affect_add)
	{
		...
		const BYTE applyOn = (BYTE) lua_tonumber(L, 1);
		...
		const long value = (long) lua_tonumber(L, 2);
		const long duration = (long) lua_tonumber(L, 3);
	...
	ALUA(affect_get_apply_on)
	{
		...
		const DWORD affectType = lua_tonumber(L, 1);

		const CAffect* pkAff = ch->FindAffect(affectType);
	...
	ALUA(affect_add_collect_point)
	{
		...
		const BYTE point_type = (BYTE) lua_tonumber(L, 1);
