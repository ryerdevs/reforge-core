...
void P2P_MANAGER::Boot(LPDESC d) const
{
...
void P2P_MANAGER::Login(LPDESC d, const TPacketGGLogin * p)
{
	...
	if (UpdateP2P) {
		const std::string name(pkCCI->szName);
...
{
	const auto it = m_map_dwPID_pkCCI.find(pid);
