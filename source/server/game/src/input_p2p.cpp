...
void CInputP2P::Login(LPDESC d, const char * c_pData) const
{
	P2P_MANAGER::instance().Login(d, (TPacketGGLogin *) c_pData);
}

void CInputP2P::Logout(LPDESC d, const char * c_pData) const
{
	const auto p = (TPacketGGLogout *) c_pData;
	P2P_MANAGER::instance().Logout(p->szName);
}

int CInputP2P::Relay(LPDESC d, const char * c_pData, size_t uiBytes) const
{
	const auto p = (TPacketGGRelay *) c_pData;
	...
	if (*c_pbData == HEADER_GC_WHISPER)
	{
		...
		const auto p2 = (TPacketGCWhisper*) buf;

		const BYTE bToEmpire = (p2->bType >> 4);
...
int CInputP2P::Notice(LPDESC d, const char * c_pData, size_t uiBytes, bool bBigFont) const
#else
int CInputP2P::Notice(LPDESC d, const char * c_pData, size_t uiBytes)
#endif
{
	const auto p = (TPacketGGNotice *) c_pData;
...
#ifdef ENABLE_FULL_NOTICE
	SendNoticeRaw(szBuf, bBigFont); // already translated at the origin peer
#else
	SendNoticeRaw(szBuf, false);
#endif
	return (p->lSize);
}

int CInputP2P::MonarchNotice(LPDESC d, const char * c_pData, size_t uiBytes) const
{
	const auto p = (TPacketGGMonarchNotice *) c_pData;
...
int CInputP2P::MonarchTransfer(LPDESC d, const char* c_pData) const
{
	const auto p = (TPacketMonarchGGTransfer*) c_pData;
	const LPCHARACTER pTargetChar = CHARACTER_MANAGER::instance().FindByPID(p->dwTargetPID);
...
int CInputP2P::Guild(LPDESC d, const char* c_pData, size_t uiBytes) const
{
	const auto p = (TPacketGGGuild *) c_pData;
		...
		case GUILD_SUBHEADER_GG_CHAT:
			{
				...
				const auto p = (TPacketGGGuildChat *) c_pData;
...
void CInputP2P::Shout(const char * c_pData) const
{
	const auto p = (TPacketGGShout *) c_pData;
	SendShout(p->szText, p->bEmpire);
}

void CInputP2P::Disconnect(const char * c_pData) const
{
	const auto p = (TPacketGGDisconnect *) c_pData;

	const LPDESC d = DESC_MANAGER::instance().FindByLoginName(p->szLogin);
...
void CInputP2P::Setup(LPDESC d, const char * c_pData) const
{
	const auto p = (TPacketGGSetup *) c_pData;
	sys_log(0, "P2P: Setup %s:%d", d->GetHostName(), p->wPort);
	d->SetP2P(d->GetHostName(), p->wPort, p->bChannel);
}

void CInputP2P::MessengerAdd(const char * c_pData) const
{
...
void CInputP2P::MessengerRemove(const char * c_pData) const
{
	const auto p = (TPacketGGMessenger *) c_pData;
...
{
	const auto p = (TPacketGGFindPosition*) c_pData;
...
void CInputP2P::WarpCharacter(const char* c_pData) const
{
...
void CInputP2P::GuildWarZoneMapIndex(const char* c_pData) const
{
	const auto p = (TPacketGGGuildWarMapIndex*) c_pData;
...
void CInputP2P::Transfer(const char * c_pData) const
{
	const auto p = (TPacketGGTransfer *) c_pData;

	const LPCHARACTER ch = CHARACTER_MANAGER::instance().FindPC(p->szName);

	if (ch)
		ch->WarpSet(p->lX, p->lY);
}

void CInputP2P::XmasWarpSanta(const char * c_pData) const
{
	const auto p =(TPacketGGXmasWarpSanta *) c_pData;

	if (p->bChannel == g_bChannel && map_allow_find(p->lMapIndex))
	{
		const int	iNextSpawnDelay = 50 * 60;
...
void CInputP2P::XmasWarpSantaReply(const char* c_pData) const
{
	const auto p = (TPacketGGXmasWarpSantaReply*) c_pData;
...
void CInputP2P::LoginPing(LPDESC d, const char * c_pData) const
{
	const auto p = (TPacketGGLoginPing *) c_pData;
...
// BLOCK_CHAT
void CInputP2P::BlockChat(const char * c_pData) const
{
	const auto p = (TPacketGGBlockChat *) c_pData;
...
void CInputP2P::IamAwake(LPDESC d, const char * c_pData) const
{
