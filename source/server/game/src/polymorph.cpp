...
{
	const BYTE bySkillLevel = pChar->GetSkillLevel(POLYMORPH_SKILL_ID);
	DWORD dwDuration = 0;
...
bool CPolymorphUtils::UpdateBookPracticeGrade(LPCHARACTER pChar, LPITEM pItem) const
{
...
bool CPolymorphUtils::GiveBook(LPCHARACTER pChar, DWORD dwMobVnum, DWORD dwPracticeCount, BYTE BookLevel, BYTE LevelLimit) const
{
	if (pChar == nullptr)
		return false;

	const LPITEM pItem = pChar->AutoGiveItem(POLYMORPH_BOOK_ID, 1);
...
bool CPolymorphUtils::BookUpgrade(LPCHARACTER pChar, LPITEM pItem) const
{
	if (pChar == nullptr || pItem == nullptr)
		return false;
