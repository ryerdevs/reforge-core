...
class CPartyManager : public singleton<CPartyManager>
{
	...
	public:
		...
		bool		IsEnablePCParty() const { return m_bEnablePCParty; }
...
class CParty
{
	...
	public:
		...
		LPCHARACTER	GetLeader() const { return m_pkChrLeader; }

		DWORD		GetMemberCount() const;
		DWORD		GetNearMemberCount() const { return m_iCountNearPartyMember; }

		bool		IsMember(DWORD pid) { return m_memberMap.contains(pid); }

		bool		IsNearLeader(DWORD pid);

		bool		IsPositionNearLeader(LPCHARACTER ch) const;
		...
		int		GetPartyBonusExpPercent() const { return m_iExpBonus; }
		...
		void		ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd) const;
		...
		LPCHARACTER	GetExpCentralizeCharacter() const;

		void		RequestSetMemberLevel(DWORD pid, BYTE level) const;
	...
	protected:
		...
		void		SendParameter(LPCHARACTER ch) const;
	...
	public:
		...
		LPDUNGEON GetDungeon_for_Only_party() const;
};
...
inline int CParty::ComputePartyBonusAttackGrade() const
{
	/*
...
inline int CParty::ComputePartyBonusDefenseGrade() const
{
	/*
	   if (GetNearMemberCount() <= 1)
	   return 0;
...
struct FPartyDropDiceRoll
{
	...
	void Process(const LPCHARACTER mobVictim)
	{
		...
		{
			const LPPARTY pParty = m_itemOwner->GetParty();
	...
	LPCHARACTER GetItemOwner() const
	{
		return m_itemOwner;
	}
	const LPITEM GetItemDrop() const
	{
		return m_itemDrop;
	}
