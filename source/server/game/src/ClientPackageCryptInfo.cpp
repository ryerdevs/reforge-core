...
bool CClientPackageCryptInfo::LoadPackageCryptFile( const char* pCryptFile )
{
	...
	const int iCryptKeySize = iSDBDataOffset - 2*sizeof(int);

	{
		...
		{
			const int nCurKeySize = (int)m_vecPackageCryptKeys.size();
	...
	for( int i = 0; i < iSDBPackageCnt; ++i )
	{
		...
		for( int j = 0; j < (int)dwSDBFileCnt; ++j )
		{
			...
			//reconstruct it
			auto it = m_mapPackageSDB.find( strRelatedMapName );
			if (it == m_mapPackageSDB.end())
			{
				const TPerFileSDBInfo fileSDBInfo;
...
{
	const int nCryptKeySize = m_vecPackageCryptKeys.size();
	const int iStreamSize   = sizeof(int)+nCryptKeySize;
...
bool CClientPackageCryptInfo::GetRelatedMapSDBStreams(const char* pMapName, BYTE** ppData, int& iDataSize )
{
	...
	const auto it = m_mapPackageSDB.find( strLowerMapName.c_str() );
