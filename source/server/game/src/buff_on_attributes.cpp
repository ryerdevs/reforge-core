...
void CBuffOnAttributes::RemoveBuffFromItem(LPITEM pItem)
{
	...
	if (nullptr != pItem)
	{
		if (pItem->GetCell() < INVENTORY_MAX_NUM)
			return;
		const auto it = find (m_p_vec_buff_wear_targets->begin(), m_p_vec_buff_wear_targets->end(), pItem->GetCell() - INVENTORY_MAX_NUM);
...
bool CBuffOnAttributes::On(BYTE bValue)
{
	...
	for (int i = 0; i < n; i++)
	{
		const LPITEM pItem = m_pBuffOwner->GetWear(m_p_vec_buff_wear_targets->at(i));
		if (nullptr != pItem)
		{
			const int m = pItem->GetAttributeCount();
			for (int j = 0; j < m; j++)
