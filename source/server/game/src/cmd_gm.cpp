#include "stdafx.h"
#include <cstdarg> // va_list / va_copy for the multi-language notices
#include "utils.h"
#include "config.h"
...
ACMD(do_transfer)
{
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Target(%s) is in %d channel (my channel %d)"), arg1, pkCCI->bChannel, g_bChannel);
				return;
			}
...
bool CHARACTER_GoToName(LPCHARACTER ch, BYTE empire, int mapIndex, const char* gotoName)
{
	...
	for (i = gs_vec_gotoInfo.begin(); i != gs_vec_gotoInfo.end(); ++i)
	{
		...
		{
			const int x = c_eachGotoInfo.x * 100;
			const int y = c_eachGotoInfo.y * 100;
...
ACMD(do_warp)
{
	...
	else
	{
		...
		if (nullptr == tch)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("There is no one(%s) by that name"), arg1);
			}
...
#ifdef ENABLE_CMD_WARP_IN_DUNGEON
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You warp to ( %d, %d, %d )"), x, y, mapIndex);
...
ACMD(do_rewarp)
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You warp to ( %d, %d )"), ch->GetX(), ch->GetY());
...
ACMD(do_item)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: item <item vnum>"));
		return;
	}
	...
	else
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("#%u item not exist by that vnum(%s)."), dwVnum, arg1);
			return;
		}
	}
...
ACMD(do_group_random)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: grrandom <group vnum>"));
		return;
	}
...
ACMD(do_mob_coward)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: mc <vnum>"));
		return;
	}
	...
	if (vnum == 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("No such mob(%s) by that vnum"), arg1);
		return;
	}
	...
	iCount = MINMAX(1, iCount, 20); // @fixme339
...
ACMD(do_mob_map)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Syntax: mm <vnum>"));
		return;
	}

	DWORD vnum = 0;
	str_to_number(vnum, arg1);
	const LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRandomPosition(vnum, ch->GetMapIndex());
...
ACMD(do_mob_aggresive)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: mob <mob vnum>"));
		return;
	}
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent) const
	{
		...
		const int iDist = DISTANCE_APPROX(pkChr->GetX() - m_pkGM->GetX(), pkChr->GetY() - m_pkGM->GetY());
...
ACMD(do_purge)
{
	...
	const LPSECTREE sectree = ch->GetSectree();
	if (sectree) // #431
...
ACMD(do_item_purge)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: ipurge <window>"));
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("List of the available windows:"));
...
ACMD(do_state)
{
	...
	const LPSECTREE pSec = SECTREE_MANAGER::instance().Get(tch->GetMapIndex(), tch->GetX(), tch->GetY());
	if (pSec)
	{
		const TMapSetting& map_setting = SECTREE_MANAGER::instance().GetMap(tch->GetMapIndex())->m_setting;
	...
	for (int i = 0; i < MAX_PRIV_NUM; ++i)
	{
		...
		{
			const int iByEmpire = CPrivManager::instance().GetPrivByEmpire(tch->GetEmpire(), i);
			int iByGuild = 0;

			if (tch->GetGuild())
				iByGuild = CPrivManager::instance().GetPrivByGuild(tch->GetGuild()->GetID(), i);

			const int iByPlayer = CPrivManager::instance().GetPrivByCharacter(tch->GetPlayerID(), i);
...
// Multi-language notice functor. m_fmt is the UNTRANSLATED format string (a
// locale key); it is resolved per player with the language of each desc, so
// every client receives the notice in its own language. Callers must not pass
// already-translated text here — use SendNoticeRaw() for that.
struct notice_packet_func
{
	const char * m_fmt;    // untranslated format string / locale key
	va_list m_args;        // copy of the variadic arguments (re-copied per desc)
	bool m_bBigFont;

	notice_packet_func(const char * fmt, va_list args, bool bBigFont=false)
		: m_fmt(fmt), m_bBigFont(bBigFont)
	{
		va_copy(m_args, args);
	}
	...
	void operator () (LPDESC d) const
	{
		...
		// Translate the format for this player, then interpolate this
		// player's own copy of the arguments (va_copy per call so the
		// stored list is never consumed).
		char szBuf[256];

		va_list args;
		va_copy(args, m_args);
		vsnprintf(szBuf, sizeof(szBuf), LC_TEXT_LANG(d->GetLang(), m_fmt), args);
		va_end(args);

		d->GetCharacter()->ChatPacket(m_bBigFont ? CHAT_TYPE_BIG_NOTICE : CHAT_TYPE_NOTICE, "%s", szBuf);
	}
};

// Verbatim functor for already-translated / raw text: no per-desc translation.
struct notice_raw_packet_func
{
	const char * m_text;
	bool m_bBigFont;

	notice_raw_packet_func(const char * text, bool bBigFont=false)
		: m_text(text), m_bBigFont(bBigFont)
	{
	}
	...
	void operator () (LPDESC d) const
	{
		...
		d->GetCharacter()->ChatPacket(m_bBigFont ? CHAT_TYPE_BIG_NOTICE : CHAT_TYPE_NOTICE, "%s", m_text);
	}
};
...
// The first argument is the untranslated format string / locale key; it is
// translated per player inside notice_packet_func.
void SendNotice(const char * c_pszFmt, ...)
{
	va_list args;
	va_start(args, c_pszFmt);

	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), notice_packet_func(c_pszFmt, args));

	va_end(args);
}

void SendNoticeBig(const char * c_pszFmt, ...)
{
	va_list args;
	va_start(args, c_pszFmt);

	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), notice_packet_func(c_pszFmt, args, true));

	va_end(args);
}

// Already-translated / raw text (from another process, an admin command or a
// quest script): sent verbatim, no per-desc translation.
void SendNoticeRaw(const char * c_pszText, bool bBigFont)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), notice_raw_packet_func(c_pszText, bBigFont));
}
...
#ifdef ENABLE_FULL_NOTICE
	// BroadcastNotice carries raw (already formatted / typed) text: no translation.
	SendNoticeRaw(c_pszBuf, bBigFont);
#else
	SendNoticeRaw(c_pszBuf, false);
#endif
}
...
ACMD(do_poison)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ex) /poison <player name>"));
		return;
	}
...
ACMD(do_bleeding)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ex) /bleeding <player name>"));
		return;
	}
...
#ifdef ENABLE_NEWSTUFF
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("List of the fields available:"));
		...
		case DoSetTypes::GOLD:	// gold
			{
				...
				if (gold < 0 && (-gold) > tch->GetGold()) // fix negative yang set
					gold = -tch->GetGold();
...
ACMD(do_makeguild)
{
	...
	[[maybe_unused]] const auto guildID = gm.CreateGuild(cp);
...
ACMD(do_refine_rod)
{
	...
	const LPITEM item = ch->GetInventoryItem(cell);
	if (item)
...
ACMD(do_qf)
{
	...
	const std::string questname = pPC->GetCurrentQuestName();

	if (!questname.empty())
	{
		const int value = quest::CQuestManager::Instance().GetQuestStateIndex(questname, arg1);

		pPC->SetFlag(questname + ".__status", value);
		pPC->ClearTimer();

		quest::PC::QuestInfoIterator it = pPC->quest_begin();
		const unsigned int questindex = quest::CQuestManager::instance().GetQuestIndexByName(questname);
...
ACMD(do_b1)
{
	...
	for (int i = 0; i < 30; ++i)
	{
		const int rot = number(0, 359);
		float fx, fy;
		GetDeltaByDegree(rot, 800, &fx, &fy);

		const LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(number(701, 706),
		                                                                    ch->GetMapIndex(),
		                                                                    304222 + (int)fx,
		                                                                    742858 + (int)fy,
		                                                                    304222 + (int)fx,
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		const LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(8009,
		                                                                    ch->GetMapIndex(),
		                                                                    304222 + (int)fx,
		                                                                    742858 + (int)fy,
		                                                                    304222 + (int)fx,
...
ACMD(do_set_state)
{
	...
	{
		char szHelp[128];
		snprintf(szHelp, sizeof(szHelp), "%s%s",
			LC_TEXT("Syntax: set_state <questname> <statename>"),
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "%s", szHelp);
		return;
	}
	...
	quest::PC* pPC = quest::CQuestManager::instance().GetPCForce(ch->GetPlayerID());
#endif
	const std::string questname = arg1;
	const std::string statename = arg2;

	if (!questname.empty())
	{
		const int value = quest::CQuestManager::Instance().GetQuestStateIndex(questname, statename);
...
ACMD(do_polymorph)
{
	...
	if (*arg1)
	{
		...
		if (ch->FindAffect(AFFECT_POLYMORPH)) // @fixme321
		{
			ch->SetPolymorph(0);
...
ACMD(do_polymorph_item)
{
	...
	if (*arg1)
	{
		...
		const LPITEM item = ITEM_MANAGER::instance().CreateItem(70104, 1, 0, true);
		if (item)
		{
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		const CGuild * g = CGuildManager::instance().FindGuildByName(arg1);
...
ACMD(do_socket_item)
{
	...
	if (*arg1)
	{
		...
		const LPITEM item = ch->AutoGiveItem(dwVnum);
...
ACMD(do_block_chat)
{
	...
	const long lBlockDuration = parse_time_str(argument);

	if (lBlockDuration < 0)
	{
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("�߸��� ������ �ð��Դϴ�. h, m, s�� �ٿ��� ������ �ֽʽÿ�."));
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("��) 10s, 10m, 1m 30s"));
		}
		return;
	}
...
ACMD(do_build)
{
	...
	const BYTE GMLevel = ch->GetGMLevel();
	...
	switch (LOWER(*arg1))
	{
		case 'c':
			{
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						const DWORD dwItemVnum = t->kMaterials[i].dwItemVnum;
						const DWORD dwItemCount = t->kMaterials[i].dwCount;
				...
				const float x_rot = atof(arg4);
				const float y_rot = atof(arg5);
				const float z_rot = atof(arg6);
				...
				const bool isSuccess = pkLand->RequestCreateObject(dwVnum,
				                                                   ch->GetMapIndex(),
				                                                   map_x,
				                                                   map_y,
				                                                   x_rot,
		...
		case 'w' :
			if (GMLevel > GM_PLAYER)
			{
				const int mapIndex = ch->GetMapIndex();
...
ACMD(do_change_attr)
{
	const LPITEM weapon = ch->GetWear(WEAR_WEAPON);
	if (weapon)
...
ACMD(do_end_duel)
{
	...
	const LPCHARACTER pChar = CHARACTER_MANAGER::instance().FindPC(szName);
	if (pChar == nullptr)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("�������� �ʴ� ĳ���� �Դϴ�."));
		return;
...
ACMD(do_duel)
{
	...
	const LPCHARACTER pChar1 = CHARACTER_MANAGER::instance().FindPC(szName1);
	const LPCHARACTER pChar2 = CHARACTER_MANAGER::instance().FindPC(szName2);
...
ACMD(do_stat_plus_amount)
{
	...
	const int nRemainPoint = ch->GetPoint(POINT_STAT);
...
struct FCountInMap
{
	...
	int GetCount(BYTE bEmpire) const { return m_Count[bEmpire]; }
};
...
ACMD(do_threeway_war_info)
{
	...
	for (int n = 1; n < 4; ++n)
	{
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap(GetSungziMapIndex());
...
ACMD(do_rmcandidacy)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: rmcandidacy <name>"));
		return;
	}
...
ACMD(do_setmonarch)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: setmonarch <name>"));
		return;
	}
...
ACMD(do_rmmonarch)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: rmmonarch <name>"));
		return;
	}
...
ACMD(do_check_monarch_money)
{
	...
	int empire = 0;
	str_to_number(empire, arg1);
	const int NationMoney = CMonarch::instance().GetMoney(empire);

	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("����: %d ��"), NationMoney);
}
...
ACMD(do_flush)
{
	...
	const DWORD pid = (DWORD) strtoul(arg1, nullptr, 10);
	const auto tch = CHARACTER_MANAGER::instance().FindByPID(pid);
	if (!tch) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("player_id %d not found"), pid);
		return;
	}
	tch->Save();
}
...
struct FMobCounter
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			const auto pChar = static_cast<LPCHARACTER>(ent);
...
{
	const LPSECTREE_MAP pSectree = SECTREE_MANAGER::instance().GetMap(ch->GetMapIndex());
...
ACMD(do_set_stat)
{
	...
	const LPCHARACTER tch = CHARACTER_MANAGER::instance().FindPC(szName);

	if (!tch)
	{
		const CCI * pkCCI = P2P_MANAGER::instance().Find(szName);
	...
	else
	{
		...
		const int nRemainPoint = tch->GetPoint(POINT_STAT);
		const int nCurPoint = tch->GetRealPoint(subcmd);
		int nChangeAmount = 0;
...
ACMD(do_set_socket)
{
	...
	const LPITEM item = ITEM_MANAGER::instance().Find (item_id);
	if (item)
...
ACMD (do_ds_list)
{
	...
	{
		const TItemPos cell(DRAGON_SOUL_INVENTORY, i);

		const LPITEM item = ch->GetItem(cell);
		if (item != nullptr)
			ch->ChatPacket(CHAT_TYPE_INFO, "cell : %d, name : %s, id : %d", item->GetCell(), item->GetName(), item->GetID());
	}
}
