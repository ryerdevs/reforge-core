...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	const auto pmsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM guild%s WHERE name = '%s'",
...
{
	const auto it = m_map_pkGuildByPID.find(pid);
...
{
	const auto it = m_map_pkGuildByPID.find(ch->GetPlayerID());
...
{
	const auto it = m_mapGuild.find(guild_id);
	if (it == m_mapGuild.end())
	{
		return nullptr;
	}
...
void CGuildManager::Initialize()
{
	...
	const auto pmsg(DBManager::instance().DirectQuery("SELECT id FROM guild%s", get_table_postfix()));
...
// Guild War
/////////////////////////////////////////////////////////////////////
void CGuildManager::RequestCancelWar(DWORD guild_id1, DWORD guild_id2) const
{
...
void CGuildManager::RequestEndWar(DWORD guild_id1, DWORD guild_id2) const
{
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		// @warme005
		{
			SendNotice("%s 길드가 %s 길드에 선전포고를 하였습니다!", TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
		}
	}
}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		SendNotice("%s 길드와 %s 길드가 잠시 후 전쟁을 시작합니다!", g1->GetName(), g2->GetName());
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	SendNotice("%s 길드와 %s 길드가 전쟁을 시작하였습니다!", g1->GetName(), g2->GetName());
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		if (bDraw)
		{
			SendNotice("%s 길드와 %s 길드 사이의 전쟁이 무승부로 끝났습니다.", g1->GetName(), g2->GetName());
		}
		else
		{
			if ( g1->GetWarScoreAgainstTo( g2->GetID() ) > g2->GetWarScoreAgainstTo( g1->GetID() ) )
			{
				SendNotice("%s 길드가 %s 길드와의 전쟁에서 승리 했습니다.", g1->GetName(), g2->GetName());
			}
			else
			{
				SendNotice("%s 길드가 %s 길드와의 전쟁에서 승리 했습니다.", g2->GetName(), g1->GetName());
			}
		}
...
bool CGuildManager::EndWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	const std::pair<DWORD, DWORD> k = std::make_pair(guild_id1, guild_id2);

	const auto it = m_GuildWar.find(k);
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	const CGuild * g1 = FindGuild(guild_id1);
	const CGuild * g2 = FindGuild(guild_id2);

	if (g1)
	{
		const LPCHARACTER master1 = g1->GetMasterCharacter();
	...
	if (g2)
	{
		const LPCHARACTER master2 = g2->GetMasterCharacter();
	...
	if (g1 && g2)
	{
		SendNotice("%s 길드와 %s 길드 사이의 전쟁이 취소되었습니다.", g1->GetName(), g2->GetName());
	}
}
...
void CGuildManager::Kill(LPCHARACTER killer, LPCHARACTER victim) const
{
	if (!killer->IsPC())
		return;
...
{
	const auto iter = m_mapGuild.find(dwGID);
...
uint64_t CGuildManager::GenerateTokenHashNumber() const
{
	return msl::random_number<uint64_t>(1);
}

bool CGuildManager::IsCorrectGuildToken(DWORD guildID, uint64_t hashID) const
{
	if (!guildID || !hashID)
		return false;

	const auto guild = CGuildManager::Instance().FindGuild(guildID);
	if (!guild)
		return false;
...
void CGuildManager::SendGuildToken(LPCHARACTER ch, uint64_t token) const
{
	if (!ch)
		return;

	const auto d = ch->GetDesc();
	if (!d)
		return;
...
void CGuildManager::GuildRelink(DWORD guildID, LPCHARACTER ch)
{
	...
	const auto pGuild = FindGuild(guildID);
	if (pGuild)
		ch->SetGuild(pGuild);
}
#endif
