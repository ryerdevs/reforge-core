...
class DBManager : public singleton<DBManager>
{
	public:
		...
		void			SendMoneyLog(BYTE type, DWORD vnum, int gold) const;

		void			LoginPrepare(BYTE bBillType, DWORD dwBillID, long lRemainSecs, LPDESC d, DWORD * pdwClientKey, int * paiPremiumTimes = nullptr);
		void			SendAuthLogin(LPDESC d);
		void			SendLoginPing(const char * c_pszLogin) const;

		void			InsertLoginData(CLoginData * pkLD);
		void			DeleteLoginData(CLoginData * pkLD);
		CLoginData *		GetLoginData(DWORD dwKey);

		DWORD			CountQuery() const { return m_sql.CountQuery(); }
		DWORD			CountQueryResult() const { return m_sql.CountResult(); }
...
template <class Functor> void DBManager::FuncQuery(Functor f, const char* c_pszFormat, ...)
{
	...
	const auto p = M2_NEW CFuncQueryInfo;
...
template <class Functor> void DBManager::FuncAfterQuery(Functor f, const char* c_pszFormat, ...)
{
	...
	const auto p = M2_NEW CFuncAfterQueryInfo;
...
class AccountDB : public singleton<AccountDB>
{
	...
	private:
		SQLMsg * PopResult();
		void AnalyzeReturnQuery(SQLMsg * pMsg) const;
