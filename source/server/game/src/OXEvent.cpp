...
{
	const BYTE ret = quest::CQuestManager::instance().GetEventFlag("oxevent_status");
...
void COXEventManager::SetStatus(OXEventStatus status) const
{
	BYTE val = 0;
...
{
	static BYTE flag = 0;
	const auto info = dynamic_cast<OXEventInfoData*>(event->info);
