...
namespace quest
{
	...
	void NPC::Set(unsigned int vnum, const string & script_name)
	{
		...
		while (itEventName != CQuestManager::instance().m_mapEventName.end())
		{
			...
			for (itertype(g_setQuestObjectDir) itObjectDir = g_setQuestObjectDir.begin(); itObjectDir != g_setQuestObjectDir.end(); ++itObjectDir)
			{
				...
				//sys_log(0, "XXX %s", buf);
				const int event_index = it->second;
	...
	bool NPC::OnTarget(PC & pc, DWORD dwQuestIndex, const char * c_pszTargetName, const char * c_pszVerb, bool & bRet)
	{
		...
		const PC::QuestInfoIterator itPCQuest = pc.quest_find(dwQuestIndex);
		...
		const int iTargetLen = strlen(c_pszTargetName);
	...
	{
		return HandleReceiveAllNoWaitEvent(pc, QUEST_SERVER_TIMER_EVENT); // @fixme336
	}
	...
	{
		return HandleReceiveAllNoWaitEvent(pc, QUEST_DIE_EVENT); // @fixme336
	}
#endif
	...
	{
		return HandleReceiveAllNoWaitEvent(pc, QUEST_BOOT_EVENT); // @fixme336
	}
#endif
	...
	struct FuncMissHandleReceiveAllEvent
	{
		...
		void operator() (PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap)
		{
			const DWORD dwQuestIndex = itQuestMap->first;
	...
	struct FuncMatchHandleReceiveAllEvent
	{
		...
		{
			const NPC::AQuestScriptType& QuestScript = itQuestMap->second;
			const int iPCState = itPCQuest->second.st;
	...
	{
		void operator()(PC::QuestInfoIterator& itPCQuest, NPC::QuestMapType::iterator& itQuestMap) const
		{
		}
	};
				...
				if (it != QuestScript.end())
				{
					...
					if (CQuestManager::ExecuteQuestScript(
								*pPC,
								...
								it->second.GetSize()))
					{
						sys_err("QUEST NOT END RUNNING on NoWait - %s",
	...
	bool NPC::OnInfo(PC & pc, unsigned int quest_index)
	{
		...
		const PC::QuestInfoIterator itPCQuest = pc.quest_find(quest_index);
		...
		QuestMapType & rmapEventOwnQuest = m_mapOwnQuest[EventIndex];
		const auto itQuestMap = rmapEventOwnQuest.find(quest_index);
		...
		const auto itQuestScript = itQuestMap->second.find(itPCQuest->second.st);
	...
	bool NPC::OnButton(PC & pc, unsigned int quest_index)
	{
		...
		const auto itQuestScript=itQuestMap->second.find(iState);
	...
	struct FuncMissChatEvent
	{
		...
		void operator()(PC::QuestInfoIterator& itPCQuest, NPC::ArgQuestMapType::iterator& itQuestMap) const
		{
			auto oldquestname = rPC.GetCurQuest(); // @fixme317 store old value
			auto oldstate = rPC.GetRunningQuestStateRef(); // @fixme317 store old value
			...
			if (CQuestManager::instance().CanStartQuest(itQuestMap->first) && NPC::HasStartState(itQuestMap->second))
			{
				...
				{
					auto questName = CQuestManager::instance().GetQuestNameByIndex(itQuestMap->second[QUEST_START_STATE_INDEX][i].quest_index);
			...
			rPC.SetCurQuest(oldquestname); // @fixme317 restore old value
			rPC.SetRunningQuestStateRef(oldstate); // @fixme317 restore old value
		}
	...
	struct FuncMatchChatEvent
	{
		...
		void operator()(PC::QuestInfoIterator& itPCQuest, NPC::ArgQuestMapType::iterator& itQuestMap) const
		{
			...
			if (itQuestScript != itQuestMap->second.end())
			{
				...
				{
					auto questName = CQuestManager::instance().GetQuestNameByIndex(itQuestMap->second[iState][i].quest_index);
					rPC.SetCurQuest(questName); // @fixme317 set current quest
					rPC.SetRunningQuestStateRef(&itPCQuest->second); // @fixme317 set current quest
	...
	bool NPC::OnChat(PC& pc)
	{
		...
		FuncMatchChatEvent fMatch(AvailScript, pc);
		FuncMissChatEvent fMiss(AvailScript, pc);
