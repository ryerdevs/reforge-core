...
struct FFindStone
{
	...
	void operator()(LPENTITY pEnt)
	{
		...
		{
			const auto pChar = (LPCHARACTER)pEnt;
...
)
{
	...
	case EQUIPMENT:
		{
			...
			const LPITEM pOld = m_PlayerSlots->pItems[wCell];
	...
	case DRAGON_SOUL_INVENTORY:
		{
			const LPITEM pOld = m_PlayerSlots->pDSItems[wCell];
			...
			if (pOld)
			{
				if (wCell < DRAGON_SOUL_INVENTORY_MAX_NUM)
				{
					for (int i = 0; i < pOld->GetSize(); ++i)
					{
						const int p = wCell + (i * DRAGON_SOUL_BOX_COLUMN_NUM);
...
void CHARACTER::ClearItem()
{
	...
	MSL_FOR_LOOP(INVENTORY_AND_EQUIP_SLOT_MAX)
	{
	...
	MSL_FOR_LOOP(DRAGON_SOUL_INVENTORY_MAX_NUM)
	{
...
bool CHARACTER::IsEmptyItemGrid(TItemPos Cell, BYTE bSize, int iExceptionCell) const
{
	...
	case INVENTORY:
		{
			const WORD bCell = Cell.cell;

			++iExceptionCell;

			if (Cell.IsBeltInventoryPosition())
			{
				const LPITEM beltItem = GetWear(WEAR_BELT);
			...
			if (m_PlayerSlots->bItemGrid[bCell])
			{
				if (m_PlayerSlots->bItemGrid[bCell] == iExceptionCell)
				{
					...
					int j = 1;
					const BYTE bPage = bCell / (INVENTORY_MAX_NUM / INVENTORY_PAGE_COUNT);

					do
					{
						const BYTE p = bCell + (5 * j);
...
int CHARACTER::GetEmptyInventoryEx(LPITEM item) const
{
	if (!item)
		return -1;
...
{
	MSL_FOR_LOOP(INVENTORY_MAX_NUM)
	{
...
int CHARACTER::GetEmptyDragonSoulInventory(LPITEM pItem) const
{
	...
	const BYTE bSize = pItem->GetSize();
	const WORD wBaseCell = DSManager::instance().GetBasePosition(pItem);
...
void TransformRefineItem(LPITEM pkOldItem, LPITEM pkNewItem)
{
	...
	{
		MSL_FOR_LOOP(ITEM_SOCKET_MAX_NUM)
		{
...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	const DWORD result_vnum = item->GetRefinedVnum();

	// REFINE_COST
	int cost = ComputeRefineFee(prt->cost);

	const int RefineChance = GetQuestFlag("main_quest_lv7.refine_chance");
...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	const DWORD result_vnum = item->GetRefinedVnum();
	const DWORD result_fail_vnum = item->GetRefineFromVnum();
	...
	{
		const LPITEM pkNewItem = ITEM_MANAGER::instance().CreateItem(result_fail_vnum, 1, 0, false);
...
bool CHARACTER::RefineInformation(BYTE bCell, BYTE bType, int iAdditionalCell)
{
	...
	const LPITEM item = GetInventoryItem(bCell);
...
bool CHARACTER::GiveRecallItem(LPITEM item)
{
	...
	{
		const LPITEM item2 = ITEM_MANAGER::instance().CreateItem(item->GetVnum(), 1);
...
{
	if (m_bNoOpenedShop) {
		const DWORD dwPlayerID = GetPlayerID();
		...
		case ITEM_SKILLBOOK:
			{
				...
				{
					item->SetCount(item->GetCount() - 1); // @fixme344
					...
					case USE_AFFECT :
						{
							...
							{
								auto * pkAff = FindAffect(AFFECT_LOOTING_SYSTEM);
								auto lDur = item->GetValue(3);
								...
								if (pkAff)
								{
									if (lDur > 0)
									{
										...
										{
											ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("The duration of the affect cannot be increased any more."));
											return false;
										}
									}
									else
									...
									RemoveAffect(pkAff);
									ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Loot filter duration extended."));
								}
								else
								{
									ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Loot filter activated."));
								}
							...
							case 71052 :
								{
									...
									#endif
									{
										LogManager::instance().ItemLog(this, item, "CHANGE_RARE_ATTR", std::to_string(item2->GetID()).c_str());
										item->SetCount(item->GetCount() - 1);
									}
									else
									{
					...
					case USE_RESET_COSTUME_ATTR:
						{
							...
							switch (item->GetSubType())
							{
								case USE_CHANGE_COSTUME_ATTR:
									...
									LogManager::instance().ItemLog(this, item, "CHANGE_COSTUME_ATTR", std::to_string(item2->GetID()).c_str());
									break;
								case USE_RESET_COSTUME_ATTR:
									item2->ClearAttribute();
									item2->AlterToMagicItem();
									LogManager::instance().ItemLog(this, item, "RESET_COSTUME_ATTR", std::to_string(item2->GetID()).c_str());
									break;
							}
								...
								case USE_CLEAN_SOCKET:
									{
										...
										LogManager::instance().ItemLog(this, item, "CLEAN_SOCKET", std::to_string(item2->GetID()).c_str());
										...
										if (item2->IsAccessoryForSocket())
										{
											if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
												...
												#endif
												{
													...
													LogManager::instance().ItemLog(this, item, "ADD_SOCKET_SUCCESS", std::to_string(item2->GetID()).c_str());
												}
												else
												{
													ChatPacket(CHAT_TYPE_INFO, LC_TEXT("소켓 추가에 실패하였습니다."));
													LogManager::instance().ItemLog(this, item, "ADD_SOCKET_FAIL", std::to_string(item2->GetID()).c_str());
												}
									...
									if (item2->IsAccessoryForSocket() && item->CanPutInto(item2))
									{
										if (item2->GetAccessorySocketGrade() < item2->GetAccessorySocketMaxGrade())
										{
											if (number(1, 100) <= aiAccessorySocketPutPct[item2->GetAccessorySocketGrade()])
											{
												...
												LogManager::instance().ItemLog(this, item, "PUT_SOCKET_SUCCESS", std::to_string(item2->GetID()).c_str());
											}
											else
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("장착에 실패하였습니다."));
												LogManager::instance().ItemLog(this, item, "PUT_SOCKET_FAIL", std::to_string(item2->GetID()).c_str());
											}
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (IS_SET(item->GetFlag(), ITEM_FLAG_LOG))
	{
		...
		const DWORD oldCount = item->GetCount();
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	if (Cell.IsEquipPosition())
	{
		...
		{
			const LPITEM costumeWeapon = GetWear(WEAR_COSTUME_WEAPON);
	...
	else
	{
		...
		else if (count < item->GetCount())
		{
			...
			item->SetCount(item->GetCount() - count);
			const LPITEM item2 = ITEM_MANAGER::instance().CreateItem(item->GetVnum(), count);
...
void CHARACTER::GiveGold(int iAmount)
{
	...
	if (GetParty())
	{
		const LPPARTY pParty = GetParty();

		const DWORD dwTotal = iAmount;
		...
		{
			const DWORD dwShare = dwTotal / funcCountNearMember.total;
...
bool CHARACTER::PickupItem(DWORD dwVID)
{
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			{
				const auto finalItem = AutoStackItem(item);
				if (finalItem)
				{
				...
				const int iEmptyCell = GetEmptyInventoryEx(item);
				if (iEmptyCell == -1)
				{
...
bool CHARACTER::SwapItem(BYTE bCell, BYTE bDestCell)
{
	...
	const TItemPos srcCell(INVENTORY, bCell), destCell(INVENTORY, bDestCell);
	...
	{
		const BYTE bEquipCell = item2->GetCell() - INVENTORY_MAX_NUM;
		const BYTE bInvenCell = item1->GetCell();
	...
	{
		const BYTE bCell1 = item1->GetCell();
		const BYTE bCell2 = item2->GetCell();
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	else
	{
		...
		{
			const BYTE bOldCell = item->GetCell();
...
{
	const auto it = m_map_buff_on_attrs.find(bType);
	...
	else if(0 == bOldValue)
	{
		...
		if (m_map_buff_on_attrs.end() == it)
		{
			...
			case POINT_ENERGY:
				{
					static std::vector<BYTE> vec_slots = { WEAR_BODY, WEAR_HEAD, WEAR_FOOTS, WEAR_WRIST, WEAR_WEAPON, WEAR_NECK, WEAR_EAR, WEAR_SHIELD,
			...
			case POINT_COSTUME_ATTR_BONUS:
				{
					static std::vector<BYTE> vec_slots = {
						WEAR_COSTUME_BODY,
						WEAR_COSTUME_HAIR,
...
{
	const auto func = [&](auto inventoryMaxNum, auto getInventoryItem) -> LPITEM {
		MSL_FOR_LOOP(inventoryMaxNum)
		{
			auto pkItem = getInventoryItem(i);
...
#ifdef ENABLE_ITEM_DS_INVENTORY_PROCESS
	if (ITEM_MANAGER::Instance().IsDragonSoul(vnum))
		return func(DRAGON_SOUL_INVENTORY_MAX_NUM, [this](auto wCell) { return this->GetDragonSoulInventoryItem(wCell); });
#endif
	return func(INVENTORY_MAX_NUM, [this](auto wCell) { return this->GetInventoryItem(wCell); });
}
...
LPITEM CHARACTER::FindItemByID(DWORD id) const
{
	...
	MSL_FOR_LOOP(BELT_INVENTORY_SLOT_START)
	{
		auto pkItem = GetInventoryItem(i);
		if (pkItem && pkItem->GetID() == id)
			return pkItem;
...
{
	const auto func = [&](auto inventoryMaxNum, auto getInventoryItem) -> int {
		int	count = 0;
		MSL_FOR_LOOP(inventoryMaxNum)
		{
...
void CHARACTER::RemoveSpecifyItem(DWORD vnum, DWORD count, const std::unordered_set<LPITEM> * skipList) const
{
	if (0 == count)
		return;

	const auto func = [&](auto inventoryMaxNum, auto getInventoryItem) -> void {
		MSL_FOR_LOOP(INVENTORY_MAX_NUM)
		{
			auto pkItem = getInventoryItem(i);
			if (!pkItem)
			...
			if(m_pkMyShop)
			{
				const bool isItemSelling = m_pkMyShop->IsSellingItem(pkItem->GetID());
				if (isItemSelling)
					continue;
			}
...
int CHARACTER::CountSpecifyTypeItem(BYTE type) const
{
	const auto func = [&](auto inventoryMaxNum, auto getInventoryItem) -> int {
		...
		MSL_FOR_LOOP(inventoryMaxNum)
		{
			const auto pItem = getInventoryItem(i);
...
void CHARACTER::RemoveSpecifyTypeItem(BYTE type, DWORD count) const
{
	if (0 == count)
		return;
...
#ifdef ENABLE_ITEM_DS_INVENTORY_PROCESS
	if (type == ITEM_DS) {
		func(DRAGON_SOUL_INVENTORY_MAX_NUM, [this](auto wCell) { return this->GetDragonSoulInventoryItem(wCell); });
		return;
	}
#endif
	func(INVENTORY_MAX_NUM, [this](auto wCell) { return this->GetInventoryItem(wCell); });
}
...
std::pair<bool, LPITEM> CHARACTER::AutoStackItemProto(DWORD dwItemVnum, BYTE & bCount) const
{
	...
	if (p->dwFlags & ITEM_FLAG_STACKABLE && p->bType != ITEM_BLEND)
	{
		MSL_FOR_LOOP(INVENTORY_MAX_NUM)
		{
			...
			if (item->GetVnum() == dwItemVnum && FN_check_item_socket(item))
			{
				...
				const auto bCount2 = MIN(g_bItemCountLimit - item->GetCount(), bCount);
				bCount -= bCount2;
...
bool CHARACTER::AutoStackItemEx(LPITEM & item, bool bAutoGive, int iEmptyPos)
{
	if (!item)
		return false;

	#ifdef ENABLE_ITEM_AUTOSTACK_EX
	...
	if (!finalItem)
	{
		...
		{
			if (iEmptyPos < 0)
				iEmptyPos = GetEmptyInventoryEx(item);
			if (iEmptyPos >= 0)
				item->AddToCharacter(this, TItemPos(item->GetWindowInventoryEx(), iEmptyPos));
			else
			{
				sys_err("AutoStackItemEx item %d %d position not found", item->GetID(), item->GetVnum());
				if (item->GetOwner())
					item->RemoveFromCharacter();
				M2_DESTROY_ITEM(item);
				item = nullptr;
...
LPITEM CHARACTER::AutoStackItem(LPITEM item) const
{
	if (!item->IsBusy() && item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
	{
		auto bCount = item->GetCount();
		MSL_FOR_LOOP(INVENTORY_MAX_NUM)
		{
			const auto item2 = GetInventoryItem(i);
			if (!item2)
				continue;
			...
			if (item2->GetVnum() == item->GetVnum())
			{
				...
				const auto bCount2 = MIN(g_bItemCountLimit - item2->GetCount(), bCount);
				bCount -= bCount2;
...
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, BYTE bCount, int iRarePct, bool bMsg)
{
	...
	const auto finalItem = AutoStackItem(item); // @fixme316
	if (finalItem)
	{
		if (bMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s"), finalItem->GetName());
	...
	if (item->GetType() == ITEM_BLEND)
	{
		...
		{
			const LPITEM inv_item = GetInventoryItem(i);
...
bool CHARACTER::GiveItem(LPCHARACTER victim, TItemPos Cell)
{
	...
	const LPITEM item = GetItem(Cell);
			...
			case CSpecialItemGroup::MOB:
				{
					...
					const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(dwCount, GetMapIndex(), x, y, 0, true, -1);
					if (ch)
						ch->SetAggressive();
					bSuccess = true;
				}
...
bool CHARACTER::ItemProcess_Polymorph(LPITEM item)
{
	...
	const DWORD dwVnum = item->GetSocket(0);
...
{
	const LPITEM Unique1 = GetWear(WEAR_UNIQUE1);
	const LPITEM Unique2 = GetWear(WEAR_UNIQUE2);
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
	const LPITEM MountCostume = GetWear(WEAR_COSTUME_MOUNT);
#endif
...
void CHARACTER::AutoRecoveryItemProcess(const EAffectTypes type)
{
	...
	if (nullptr != pAffect)
	{
		const LPITEM pItem = FindItemByID(pAffect->dwFlag);
...
{
	const BYTE window_type = Pos.window_type;
	const WORD cell = Pos.cell;
