...
int DESC::ProcessOutput()
{
	...
	/* socket_write() returns: >0 = bytes actually written (== bytes_to_write on
	   full success), 0 = EAGAIN (nothing written), -1 = error. Consume only the
	   bytes that really left the buffer, otherwise the output buffer grows
	   without limit and the peer never receives its responses. */
	if (result > 0)
	{
...
void DESC::SendKeyAgreement()
{
	...
	const size_t agreed_length = cipher_.Prepare(packet.data, &data_length);
	if (agreed_length == 0) {
