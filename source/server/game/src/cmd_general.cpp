...
ACMD(do_user_horse_feed)
{
	...
	const auto foodTable = ITEM_MANAGER::instance().GetTable(dwFood);
	const auto foodName = (foodTable) ? foodTable->szLocaleName : "NONAME"; // @fixme307
...
{
	const auto info = dynamic_cast<shutdown_event_data*>( event->info );
...
void Shutdown(int iSec)
{
	...
	SendNotice("%d초 후 게임이 셧다운 됩니다.", iSec);
...
{
	const auto info = dynamic_cast<TimedEventInfo *>( event->info );
...
ACMD(do_autostack)
{
	MSL_FOR_RLOOP(INVENTORY_MAX_NUM)
	{
		const auto item = ch->GetInventoryItem(i);
		if (!item)
			continue;
		ch->AutoStackItem(item);
	}
}
...
ACMD(do_monarch_tax)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: monarch_tax <1-50>"));
		return;
	}
...
ACMD(do_monarch_mob)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Usage: mmob <mob name>"));
		return;
	}
