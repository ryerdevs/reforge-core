...
{
	const int rv = number(1, g_prob_sum[prob_idx]);
	const int * p = std::lower_bound(g_prob_accumulate[prob_idx], g_prob_accumulate[prob_idx]+ MAX_FISH, rv);
	const int fish_idx = p - g_prob_accumulate[prob_idx];
	return fish_idx;
}
...
int DetermineFish(LPCHARACTER ch)
{
	...
	{
		const int fish_pct_value = MINMAX(0, quest::CQuestManager::instance().GetEventFlag("fish_miss_pct"), 200);
		adjust = (100-fish_pct_value) * fish_info[0].prob[prob_idx] / 100;
	}

	const int rv = number(adjust + 1, g_prob_sum[prob_idx]);

	const int * p = std::lower_bound(g_prob_accumulate[prob_idx], g_prob_accumulate[prob_idx] + MAX_FISH, rv);
	const int fish_idx = p - g_prob_accumulate[prob_idx];

	{
		const DWORD vnum = fish_info[fish_idx].vnum;
...
{
	const LPITEM rod = ch->GetWear(WEAR_WEAPON);
...
{
	const auto info = dynamic_cast<fishing_event_info *>( event->info );
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(info->pid);
...
int Compute(DWORD fish_id, DWORD ms, DWORD* item, int level)
{
	...
	const int time_step = MINMAX(0,((ms + 99) / 200), MAX_FISHING_TIME_COUNT - 1);
...
void Take(fishing_event_info* info, LPCHARACTER ch)
{
	if (info->step == 1)
	{
		const long ms = (long) ((get_dword_time() - info->hang_time));
		DWORD item_vnum = 0;
		const int ret = Compute(info->fish_id, ms, &item_vnum, GetFishingLevel(ch));
		...
		switch (ret)
		{
			...
			case 0:
				if (item_vnum)
				{
					...
					const LPITEM item = ch->AutoGiveItem(item_vnum, 1, -1, false);
					if (item)
					{
...
void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
{
	...
	{
		const int fish_id = DetermineFishByProbIndex(prob_idx);
		DWORD item = 0;
	...
	for (auto it = fished.begin(); it != fished.end(); ++it)
...
{
	const int idx = item->GetVnum() - fish_info[2].vnum+2;

	if (idx<=1 || idx >= MAX_FISH)
		return;

	const int r = number(1, 10000);
	...
	else
	{
		...
		const int u_index = std::lower_bound(s_acc_prob, s_acc_prob + NUM_USE_RESULT_COUNT, r) - s_acc_prob;
...
int RealRefineRod(LPCHARACTER ch, LPITEM item)
{
	...
	const int iAdv = rod->GetValue(0) / 10;
