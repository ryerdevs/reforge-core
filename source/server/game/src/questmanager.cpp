...
namespace quest
{
	...
	int CQuestManager::ReadQuestCategoryFile(WORD q_index) const
	{
	...
	void CQuestManager::Select(unsigned int pc, unsigned int selection)
	{
		...
		if ((pPC = GetPC(pc)) && pPC->IsRunning() && pPC->GetRunningQuestState()->suspend_state==SUSPEND_STATE_SELECT)
		{
			...
			if (!pPC->GetRunningQuestState()->chat_scripts.empty())
			{
				...
				{
					const AArgScript* pas = pPC->GetRunningQuestState()->chat_scripts[selection];
...
#define ENABLE_PARTYKILL
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			const LPCHARACTER ch = GetCurrentCharacterPtr();
			const LPPARTY pParty = ch->GetParty();
			const LPCHARACTER leader = pParty ? pParty->GetLeaderCharacter() : ch;
			if (leader)
			{
	...
	bool CQuestManager::DND(uint32_t pc, LPITEM item_dnd, LPITEM item_victim, bool bReceiveAll)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
			{
				const LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(pc);
				if (ch)
	...
	bool CQuestManager::GiveItemToPC(unsigned int pc, LPCHARACTER pkChr)
	{
		...
		if (pPC)
		{
			...
			const TargetInfo * pInfo = CTargetManager::instance().GetTargetInfo(pc, TARGET_TYPE_VID, pkChr->GetVID());
	...
	bool CQuestManager::Click(unsigned int pc, LPCHARACTER pkChrTarget)
	{
		...
		if (pPC)
		{
			...
			const DWORD dwCurrentNPCRace = pkChrTarget->GetRaceNum();
	...
	void CQuestManager::LoadStartQuest(const string& quest_name, unsigned int idx)
	{
		for (itertype(g_setQuestObjectDir) it = g_setQuestObjectDir.begin(); it != g_setQuestObjectDir.end(); ++it)
		{
			...
			if (inf.is_open())
			{
				...
				const istreambuf_iterator<char> ib(inf), ie;
	...
	bool CQuestManager::CanEndQuestAtState(const string& quest_name, const string& state_name) const
	{
		return false;
	}
	...
	bool CQuestManager::HasQuestRunning(unsigned int pc)
	{
		auto it = m_mapPC.find(pc);
		if (it == m_mapPC.end())
			return false;
	...
	PC * CQuestManager::GetPC(unsigned int pc)
	{
		...
		const LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindByPID(pc);
	...
	const char* CQuestManager::GetQuestStateName(const string& quest_name, const int state_index) const
	{
		const int x = lua_gettop(L);
	...
	int CQuestManager::GetQuestStateIndex(const string& quest_name, const string& state_name) const
	{
		const int x = lua_gettop(L);
		lua_getglobal(L, quest_name.c_str());
		if (lua_isnil(L,-1))
		...
		const int v = (int)rint(lua_tonumber(L,-1));
		lua_settop(L, x);
		if ( test_server )
	...
	{
		const auto it = m_hmQuestName.find(name);
	...
	void CQuestManager::RequestSetEventFlag(const string& name, int value) const
	{
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		else if (name == "newyear_boom")
		{
			...
			{
				const LPCHARACTER ch = (*it)->GetCharacter();
		...
		else if (name == "new_xmas_event")
		{
			...
			if (value != 0)
			{
				...
				{
					const LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap(1);
				...
				{
					const LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap(21);
				...
				{
					const LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap(41);
	...
	{
		const auto it = m_mapEventFlag.find(name);
	...
	void CQuestManager::BroadcastEventFlagOnLogin(LPCHARACTER ch) const
	{
		int iEventFlagValue;
	...
	DWORD CQuestManager::GetServerTimerArg() const
	{
	...
	void CQuestManager::EndOtherPCBlock()
	{
		...
		const DWORD pc = m_vecPCStack.back();
	...
	bool CQuestManager::IsInOtherPCBlock() const
	{
		return !m_vecPCStack.empty();
	}

	PC*	CQuestManager::GetOtherPCBlockRootPC() const
	{
