...
void CInputLogin::Login(LPDESC d, const char * data) const
{
	const auto pinfo = (TPacketCGLogin *) data;
...
void CInputLogin::LoginByKey(LPDESC d, const char * data) const
{
...
void CInputLogin::ChangeName(LPDESC d, const char * data) const
{
	const auto p = (TPacketCGChangeName *) data;
...
void CInputLogin::CharacterSelect(LPDESC d, const char * data) const
{
	const auto pinfo = (struct command_player_select *) data;
	const TAccountTable & c_r = d->GetAccountTable();
		...
		BYTE bDex)
{
	...
	if (china_event_server)
	{
		...
		for (int i = 1; i < 35; ++i)
		{
			const int iHP = number(JobInitialPoints[job].hp_per_lv_begin, JobInitialPoints[job].hp_per_lv_end);
			const int iSP = number(JobInitialPoints[job].sp_per_lv_begin, JobInitialPoints[job].sp_per_lv_end);
			table->sRandomHP += iHP;
			table->sRandomSP += iSP;
			table->stat_point += 3;
		}
...
void CInputLogin::CharacterCreate(LPDESC d, const char * data) const
{
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data) const
{
	const auto pinfo = (struct command_player_delete *) data;
	const TAccountTable & c_rAccountTable = d->GetAccountTable();
...
void CInputLogin::Entergame(LPDESC d, const char * data) const
{
	LPCHARACTER ch;
...
int CInputLogin::GuildSymbolUpload(LPDESC d, const char* c_pData, size_t uiBytes) const
{
	...
	const auto p = (TPacketCGGuildSymbolUpload*) c_pData;
...
void CInputLogin::GuildSymbolCRC(LPDESC d, const char* c_pData) const
{
...
void CInputLogin::GuildMarkUpload(LPDESC d, const char* c_pData) const
{
	const auto p = (TPacketCGMarkUpload *) c_pData;
...
void CInputLogin::GuildMarkIDXList(LPDESC d, const char* c_pData) const
{
...
void CInputLogin::GuildMarkCRCList(LPDESC d, const char* c_pData) const
{
	const auto pCG = (TPacketCGMarkCRCList *) c_pData;
