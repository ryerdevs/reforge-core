...
bool CUBE_DATA::can_make_item (LPITEM *items, WORD npc_vnum) const
{
	DWORD	i, end_index;
	DWORD	need_vnum;
	int		need_count;
...
void CUBE_DATA::remove_material (LPCHARACTER ch) const
{
	DWORD	i, end_index;
	DWORD	need_vnum;
	int		need_count;
...
void Cube_open (LPCHARACTER ch)
{
	...
	if (!FN_check_valid_npc(npc->GetRaceNum()))
	{
		if (test_server)
...
void Cube_init()
{
	const CUBE_DATA * p_cube = nullptr;
...
static bool FN_update_cube_status(LPCHARACTER ch)
{
	...
	const CUBE_DATA* cube = FN_find_cube(ch->GetCubeItem(), npc->GetRaceNum());
...
bool Cube_make (LPCHARACTER ch)
{
	...
	const int cube_gold = cube_proto->gold;
	...
	const CUBE_VALUE	*reward_value = cube_proto->reward_value();
	...
	if ( percent_number<=cube_proto->percent)
	{
		...
		new_item = ITEM_MANAGER::instance().CreateItem(reward_value->vnum, reward_value->count);
		ch->AutoStackItemEx(new_item, true); // @fixme316
...
void Cube_InformationInitialize()
{
	...
	{
		const CUBE_DATA* cubeData = s_cube_proto[i];
		...
		for (auto iter = resultList.begin(); resultList.end() != iter; ++iter)
		{
			...
			if (reward.vnum == existInfo.reward.vnum)
			{
				...
				{
					const TItemTable* existMaterialProto = ITEM_MANAGER::Instance().GetTable(existMaterialIter->vnum);
					...
					if (0 < existItemInfo.level)
					{
						...
						{
							const TItemTable* currentMaterialProto = ITEM_MANAGER::Instance().GetTable(currentMaterialIter->vnum);
...
void Cube_request_result_list(LPCHARACTER ch)
{
	...
	const DWORD npcVNUM = npc->GetRaceNum();
