...
{
	const auto info = dynamic_cast<war_map_info*>( event->info );
...
bool CWarMap::GetTeamIndex(DWORD dwGuildID, BYTE & bIdx) const
{
...
DWORD CWarMap::GetGuildID(BYTE bIdx) const
{
	assert(bIdx < 2);
	return m_TeamData[bIdx].dwID;
}

CGuild * CWarMap::GetGuild(BYTE bIdx) const
{
	return m_TeamData[bIdx].pkGuild;
}

long CWarMap::GetMapIndex() const
{
	return m_kMapInfo.lMapIndex;
}

BYTE CWarMap::GetType() const
{
	return m_kMapInfo.bType;
}

DWORD CWarMap::GetGuildOpponent(LPCHARACTER ch) const
{
	if (ch->GetGuild())
	{
		const DWORD gid = ch->GetGuild()->GetID();
		BYTE idx;
...
DWORD CWarMap::GetWinnerGuild() const
{
	DWORD win_gid = 0;
...
void CWarMap::UsePotion(LPCHARACTER ch, LPITEM item)
{
	...
	const int iPrice = item->GetProto()->dwGold;
...
int CWarMap::STeamData::GetAccumulatedJoinerCount() const
{
	return set_pidJoiner.size();
}

int CWarMap::STeamData::GetCurJointerCount() const
{
	return iMemberCount;
}
...
void CWarMap::DecMember(LPCHARACTER ch)
{
	...
	if (!ch->IsObserverMode())
	{
		...
		{
			const CAffect * pkAff = ch->FindAffect(AFFECT_WAR_FLAG);
...
int CWarMap::GetRewardGold(BYTE bWinnerIdx) const
{
...
{
	const FPacket f(p, size);
	std::for_each(m_set_pkChr.begin(), m_set_pkChr.end(), f);
}

void CWarMap::SendWarPacket(LPDESC d) const
{
	TPacketGCGuild pack;
...
bool CWarMap::IsFlagOnBase(BYTE bIdx) const
{
	assert(bIdx < 2);

	const TeamData & r = m_TeamData[bIdx];
...
bool CWarMapManager::GetStartPosition(long lMapIndex, BYTE bIdx, PIXEL_POSITION & pos)
{
	...
	const TWarMapInfo * pi = GetWarMapInfo(lMapIndex);
	...
	if (!pi)
	{
		...
		{
			const PIXEL_POSITION& cur=it->second->posStart[bIdx];
...
{
	const long mapIdx = pMap->GetMapIndex();
