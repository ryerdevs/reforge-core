...
SECTREE_MAP::~SECTREE_MAP()
{
	...
	while (it != map_.end()) {
		const LPSECTREE sectree = (it++)->second;
		M2_DELETE(sectree);
	}
...
{
	const LPSECTREE_MAP pkSectreeMap = GetMap(dwIndex);
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
int SECTREE_MANAGER::LoadSettingFile(long lMapIndex, const char * c_pszSettingFileName, TMapSetting & r_setting) const
{
...
LPSECTREE_MAP SECTREE_MANAGER::BuildSectreeFromSetting(TMapSetting & r_setting) const
{
	const auto pkMapSectree = M2_NEW SECTREE_MAP;
...
bool SECTREE_MANAGER::LoadAttribute(LPSECTREE_MAP pkMapSectree, const char * c_pszFileName, TMapSetting & r_setting) const
{
	...
	constexpr auto maxMemSize = LZOManager::GetMaxCompressedSize(sizeof(DWORD) * (SECTREE_SIZE / CELL_SIZE) * (SECTREE_SIZE / CELL_SIZE));
...
#endif
	const auto attr = M2_NEW DWORD[maxMemSize];
	...
	for (int y = 0; y < iHeight; ++y)
		for (int x = 0; x < iWidth; ++x)
		{
			...
			const LPSECTREE tree = pkMapSectree->Find(id.package);
...
int SECTREE_MANAGER::Build(const char * c_pszListFileName, const char* c_pszMapBasePath)
{
	...
	const msl::file_ptr f(c_pszListFileName);
	if (!f.is_open())
		return 0;
	...
	for (auto & line : lines)
	{
		...
		if (loadedMaps.contains(iIndex)) // @fixme308
		{
...
bool SECTREE_MANAGER::IsAttackablePosition(long lMapIndex, long x, long y) const
{
	const LPSECTREE tree = SECTREE_MANAGER::instance().Get(lMapIndex, x, y);
...
bool SECTREE_MANAGER::IsMovablePosition(long lMapIndex, long x, long y) const
{
	const LPSECTREE tree = SECTREE_MANAGER::instance().Get(lMapIndex, x, y);
...
bool SECTREE_MANAGER::GetMovablePosition(long lMapIndex, long x, long y, PIXEL_POSITION & pos) const
{
	int i = 0;
	...
	do
	{
		...
		const LPSECTREE tree = SECTREE_MANAGER::instance().Get(lMapIndex, dx, dy);
...
{
	const LPSECTREE_MAP pkSectreeMap = GetMap(lMapIndex);
...
BYTE SECTREE_MANAGER::GetEmpireFromMapIndex(long lMapIndex) const
{
...
class FRemoveIfAttr
{
	public:
		...
		void operator () (LPENTITY entity) const
		{
...
bool SECTREE_MANAGER::ForAttrRegionCell( long lMapIndex, long lCX, long lCY, DWORD dwAttr, EAttrRegionMode mode )
{
	...
	const LPSECTREE pSec = Get( lMapIndex, id.package );
	if ( !pSec )
		return false;
...
bool SECTREE_MANAGER::ForAttrRegionFreeAngle( long lMapIndex, long lCX, long lCY, long lCW, long lCH, long lRotate, DWORD dwAttr, EAttrRegionMode mode )
{
	...
	const float fTilt1 = float(fdy1) / float(fdx1);
	const float fTilt2 = float(fdy2) / float(fdx2);
...
bool SECTREE_MANAGER::SaveAttributeToImage(int lMapIndex, const char * c_pszFileName, LPSECTREE_MAP pMapSrc)
{
	...
	const auto pdwLine = M2_NEW DWORD[SECTREE_SIZE / CELL_SIZE];
	...
	for (y = 0; y < 4 * iMapHeight; ++y)
	{
		for (y2 = 0; y2 < SECTREE_SIZE / CELL_SIZE; ++y2)
		{
			for (x = 0; x < 4 * iMapWidth; ++x)
			{
				...
				const LPSECTREE pSec = pMap->Find(id.package);
...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent) const
	{
		...
		{
			const auto lpChar = (LPCHARACTER)ent;
...
void SECTREE_MANAGER::PurgeMonstersInMap(long lMapIndex) const
{
	const LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);
...
void SECTREE_MANAGER::PurgeStonesInMap(long lMapIndex) const
{
	const LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);
...
void SECTREE_MANAGER::PurgeNPCsInMap(long lMapIndex) const
{
	const LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);
...
size_t SECTREE_MANAGER::GetMonsterCountInMap(long lMapIndex) const
{
	const LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);
...
size_t SECTREE_MANAGER::GetMonsterCountInMap(long lMapIndex, DWORD dwVnum) const
{
	const LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);
...
std::pair<long, long> SECTREE_MAP::CoordinateToLocal(long x, long y) const
{
	const auto nx = x - m_setting.iBaseX;
	const auto ny = y - m_setting.iBaseY;
	return { nx / 100, ny / 100 };
}

std::pair<long, long> SECTREE_MAP::CoordinateToGlobal(long x, long y) const
{
