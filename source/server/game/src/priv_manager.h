...
class CPrivManager : public singleton<CPrivManager>
{
	public:
		...
		void RequestGiveGuildPriv(DWORD guild_id, BYTE type, int value, time_t dur_time_sec) const;
		void RequestGiveEmpirePriv(BYTE empire, BYTE type, int value, time_t dur_time_sec) const;
		void RequestGiveCharacterPriv(DWORD pid, BYTE type, int value) const;
