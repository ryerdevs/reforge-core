...
void CHARACTER::CowardEscape()
{
	...
	for (int iDistIdx = 2; iDistIdx >= 0; --iDistIdx)
		for (int iTryCount = 0; iTryCount < 8; ++iTryCount)
		{
			...
			float fx, fy;
			const float fDist = number(iDist[iDistIdx], iDist[iDistIdx+1]);
			...
			const int iDestX = GetX() + (int) fx;
			const int iDestY = GetY() + (int) fy;
...
void CHARACTER::__StateIdle_NPC()
{
	...
	if (IsGuardNPC())
	{
		if (!quest::CQuestManager::instance().GetEventFlag("noguard"))
		{
			...
			const LPCHARACTER victim = f.GetVictim();
	...
	else
	{
		if (GetRaceNum() == xmas::MOB_SANTA_VNUM)
		{
			...
			{
				const int	next_warp_time = 2 * 1000;
		...
		{
			const LPCHARACTER pkChrProtege = GetProtege();
			...
			if (!number(0, 6))
			{
				...
				float fx, fy;
				const float fDist = number(200, 400);
...
void CHARACTER::__StateIdle_Monster()
{
	...
	if (!no_wander && !IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
	{
		if (!number(0, 6))
		{
			...
			float fx, fy;
			const float fDist = number(300, 700);
...
void CHARACTER::StateMove()
{
	...
	const int x = (int) ((float) (m_posDest.x - m_posStart.x) * fRate + m_posStart.x);
	const int y = (int) ((float) (m_posDest.y - m_posStart.y) * fRate + m_posStart.y);
	...
	else
	{
		...
		{
			const LPCHARACTER victim = GetVictim();
		...
		if (IsMonster() && GetMobRank() >= MOB_RANK_BOSS && GetVictim())
		{
			...
			if (GetRaceNum() == 2191 && number(1, 20) == 1 && get_dword_time() - m_pkMobInst->m_dwLastWarpTime > 1000)
			{
				...
				const long new_x = victim->GetX() + (long)fx;
				const long new_y = victim->GetY() + (long)fy;
...
void CHARACTER::StateBattle()
{
	...
	if (!victim || (victim->IsStun() && IsGuardNPC()) || victim->IsDead())
	{
		...
		{
			const LPCHARACTER new_victim = FindVictim(this, m_pkMobData->m_table.wAggressiveSight);
			...
			if (!new_victim)
			{
				switch (GetMobBattleType())
				{
					...
					case BATTLE_TYPE_TANKER:
						{
							float fx, fy;
							const float fDist = number(400, 1500);
	...
	if (IsSummonMonster() && !IsDead() && !IsStun())
	{
		...
		if (bPct && pParty->CountMemberByVnum(GetSummonVnum()) < SUMMON_MONSTER_COUNT)
		{
			...
			const LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(GetSummonVnum(), GetMapIndex(), sx, sy, ex, ey, true, true);
	...
	const float fDist = DISTANCE_APPROX(GetX() - victim->GetX(), GetY() - victim->GetY());
	...
	const DWORD dwDuration = CalculateDuration(GetLimitPoint(POINT_ATT_SPEED), 2000);
		...
		for (unsigned int iSkillIdx = 0; iSkillIdx < MOB_SKILL_MAX_NUM; ++iSkillIdx)
		{
			if (CanUseMobSkill(iSkillIdx))
			{
				...
				if (UseMobSkill(iSkillIdx))
				{
					...
					const float fDuration = CMotionManager::instance().GetMotionDuration(GetRaceNum(), MAKE_MOTION_KEY(MOTION_MODE_GENERAL, MOTION_SPECIAL_1 + iSkillIdx));
...
void CHARACTER::StateHorse()
{
	...
	const DWORD	STATE_DURATION = (DWORD)PASSES_PER_SEC(0.5);
	...
	const LPCHARACTER victim = GetRider();
