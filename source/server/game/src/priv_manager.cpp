...
void CPrivManager::RequestGiveGuildPriv(DWORD guild_id, BYTE type, int value, time_t duration_sec) const
{
...
void CPrivManager::RequestGiveEmpirePriv(BYTE empire, BYTE type, int value, time_t duration_sec) const
{
	if (MAX_PRIV_NUM <= type)
	{
...
void CPrivManager::RequestGiveCharacterPriv(DWORD pid, BYTE type, int value) const
{
	if (MAX_PRIV_NUM <= type)
	{
...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	const CGuild* g = CGuildManager::instance().FindGuild(guild_id);

	if (g)
	{
		if (value)
		{
			SendNotice("%s 길드의 %s이 %d%% 증가했습니다!", g->GetName(), GetPrivName(type), value);
		}
		else
		{
			SendNotice("%s 길드의 %s이 정상으로 돌아왔습니다.", g->GetName(), GetPrivName(type));
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		if (empire)
			SendNotice("%s의 %s이 %d%% 증가했습니다!", GetEmpireName(empire), GetPrivName(type), value);
		else
		{
			char buf[100];
	...
	{
		if (empire)
			SendNotice("%s의 %s이 정상으로 돌아왔습니다.", GetEmpireName(empire), GetPrivName(type));
		else
		{
			char buf[100];
...
{
	const int val_ch = GetPrivByCharacter(ch->GetPlayerID(), type);
...
{
	const SPrivEmpireData* pkPrivEmpireData = GetPrivByEmpireEx(bEmpire, type);
