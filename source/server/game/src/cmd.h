...
// Multi-language notices: the first argument is the UNTRANSLATED format
// string / locale key; it is resolved per player inside cmd_gm.cpp.
extern void SendNotice(const char * c_pszFmt, ...);
extern void SendNoticeBig(const char * c_pszFmt, ...);
// Verbatim notice for already-translated / raw text (P2P relays, admin input).
extern void SendNoticeRaw(const char * c_pszText, bool bBigFont);
