...
namespace quest
{
	ALUA(marriage_engage_to)
	{
		...
		const LPCHARACTER ch_you = CHARACTER_MANAGER::instance().Find(vid);
		if (ch_you)
		{
	...
	{
		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const marriage::TMarriage* pMarriage = marriage::CManager::instance().Get(ch->GetPlayerID());
		if (!pMarriage)
		{
	...
	ALUA(marriage_find_married_vid)
	{
		...
		if (pMarriage)
		{
			const LPCHARACTER you = CHARACTER_MANAGER::instance().FindByPID(pMarriage->GetOther(ch->GetPlayerID()));
			if (you)
				vid = you->GetVID();
		}
	...
	ALUA(marriage_join_wedding)
	{
		...
		const DWORD pid1 = (DWORD) lua_tonumber(L, 1);
		const DWORD pid2 = (DWORD) lua_tonumber(L, 2);

		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		const marriage::TMarriage* pMarriage = marriage::CManager::instance().Get(pid1);
		if (!pMarriage)
		{
			sys_err("pid[%d:%s] is not exist couple", ch->GetPlayerID(), ch->GetName());
			return 0;
