...
{
	const auto info = dynamic_cast<TPoisonEventInfo *>( event->info );
	...
	const LPCHARACTER pkAttacker = CHARACTER_MANAGER::instance().FindByPID(info->attacker_pid);

	const int dam = ch->GetMaxHP() * GetPoisonDamageRate(ch) / 1000;
...
{
	const auto info = dynamic_cast<TBleedingEventInfo *>( event->info );
	...
	const int dam = ch->GetMaxHP() * GetBleedingDamageRate(ch) / 1000;
...
{
	const auto info = dynamic_cast<TFireEventInfo *>( event->info );
