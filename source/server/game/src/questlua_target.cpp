...
namespace quest
{
	...
	ALUA(target_pos)
	{
		...
		const int iQuestIndex = CQuestManager::instance().GetCurrentPC()->GetCurrentQuestIndex();
		...
		const int x = pos.x + (int) lua_tonumber(L, 2) * 100;
		const int y = pos.y + (int) lua_tonumber(L, 3) * 100;
	...
	{
		const LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const DWORD dwQuestIndex = CQuestManager::instance().GetCurrentPC()->GetCurrentQuestIndex();
		...
		const LPEVENT pkEvent = CTargetManager::instance().GetTargetEvent(ch->GetPlayerID(), dwQuestIndex, (const char *) lua_tostring(L, 1));
