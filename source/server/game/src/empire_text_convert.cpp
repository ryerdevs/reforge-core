...
bool LoadEmpireTextConvertTable(DWORD dwEmpireID, const char* c_szFileName)
{
	...
	const DWORD dwEngCount = 26;
	const DWORD dwHanCount = (0xC8 - 0xB0+1) * (0xFE - 0xA1+1);
	const DWORD dwHanSize = dwHanCount * 2;
...
void ConvertEmpireText(DWORD dwEmpireID, char* szText, size_t len, int iPct)
{
	...
	for (auto pbText = reinterpret_cast<BYTE*>(szText) ; len > 0 && *pbText != '\0' ; --len, ++pbText)
	{
