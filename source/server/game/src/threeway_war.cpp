...
{
	const auto info = dynamic_cast<regen_mob_event_info *>(event->info);
	...
	const int iMapIndex = info->dwMapIndex;

	char filename[128];
	std::string szFilename(GetSungziMapPath());

	const int choice = quest::CQuestManager::instance().GetEventFlag("threeway_war_choice");
	if (0 == choice)
...
{
	return MapIndexSet_.contains(iMapIndex);
}
...
void CThreeWayWar::RandomEventMapSet() const
{
...
{
	const auto iter = RegisterUserMap_.find(PlayerID);
...
void CThreeWayWar::onDead(LPCHARACTER pChar, LPCHARACTER pkKiller)
{
	...
	else if (1 == GetRegenFlag())
	{
		...
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap(pChar->GetMapIndex());
		...
		for (int n = 0; n < quest::CQuestManager::instance().GetEventFlag("threeway_war_boss_count");)
		{
			...
			const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(
...
struct FDestroyAllEntity
{
	void operator() (LPENTITY ent) const
	{
		...
		{
			const auto ch = static_cast<LPCHARACTER>(ent);
			if (!ch->IsPC())
...
void CThreeWayWar::RemoveAllMonstersInThreeWay() const
{
	...
	{
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( *iter );
