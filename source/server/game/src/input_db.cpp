...
void CInputDB::LoginSuccess(DWORD dwHandle, const char *data) const
{
	sys_log(0, "LoginSuccess");

	const auto pTab = (TAccountTable *) data;

	const LPDESC d = DESC_MANAGER::instance().FindByHandle(dwHandle);
	...
	// Language System: propagate the account language (e.g. "es") to the desc
	// and to the current global context, so LC_TEXT() calls made while the
	// session is handled resolve to the player's language.
	BYTE bLang = LANGUAGE_DEFAULT;

	for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
	{
		if (!strcasecmp(pTab->lang, arstLocaleStringNames[i].c_str()))
		{
			bLang = (BYTE) i;
			break;
		}
	...
	d->SetLang(bLang);
	g_iCurrentLang = d->GetLang();

	sys_log(0, "InputDB::login_success: lang '%s' -> %d for %s", pTab->lang, bLang, pTab->login);
#endif
...
void CInputDB::PlayerCreateFailure(LPDESC d, BYTE bType) const
{
	if (!d)
		return;
...
void CInputDB::PlayerCreateSuccess(LPDESC d, const char * data) const
{
	if (!d)
		return;
	...
	if (china_event_server)
	{
		...
		const int job = pPacketDB->player.byJob;
...
void CInputDB::PlayerDeleteSuccess(LPDESC d, const char * data) const
{
	if (!d)
		return;
...
void CInputDB::PlayerDeleteFail(LPDESC d) const
{
	if (!d)
		return;
...
void CInputDB::ChangeName(LPDESC d, const char * data) const
{
	if (!d)
		return;

	const auto p = (TPacketDGChangeName *) data;
...
void CInputDB::PlayerLoad(LPDESC d, const char * data) const
{
	const auto pTab = (TPlayerTable *) data;
	...
	{
		const LPCHARACTER p = d->GetCharacter();
	...
	const LPCHARACTER ch = CHARACTER_MANAGER::instance().CreateCharacter(pTab->name, pTab->id);
	...
	const long lPublicMapIndex = lMapIndex >= 10000 ? lMapIndex / 10000 : lMapIndex;
...
{
	const auto info = dynamic_cast<quest_login_event_info*>( event->info );
	...
	{
		sys_err("input_db.cpp:quest_login_event INVALID PHASE pid %d", ch->GetPlayerID()); // @warme016 previously nulled
		return 0;
	}
}

void CInputDB::QuestLoad(LPDESC d, const char * c_pData) const
{
	if (nullptr == d)
		return;
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;

	const auto p = (TSafeboxTable *) c_pData;
...
void CInputDB::SafeboxChangeSize(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;

	const BYTE bSize = *(BYTE *) c_pData;
...
//
//
void CInputDB::SafeboxWrongPassword(LPDESC d) const
{
	if (!d)
		return;
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData) const
{
	if (!d)
		return;
	...
	const auto p = (TSafeboxChangePasswordPacketAnswer*) c_pData;
	if (p->flag)
	{
...
void CInputDB::MallLoad(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;
...
void CInputDB::LoginAlready(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;

	{
		const auto p = (TPacketDGLoginAlready *) c_pData;

		const LPDESC d2 = DESC_MANAGER::instance().FindByLoginName(p->szLogin);
...
void CInputDB::EmpireSelect(LPDESC d, const char * c_pData) const
{
...
void CInputDB::MapLocations(const char * c_pData) const
{
...
void CInputDB::P2P(const char * c_pData) const
{
	extern LPFDWATCH main_fdw;

	const auto p = (TPacketDGP2P *) c_pData;
...
void CInputDB::GuildLoad(const char * c_pData) const
{
	CGuildManager::instance().LoadGuild(*(DWORD *) c_pData);
}

void CInputDB::GuildSkillUpdate(const char* c_pData) const
{
	const auto p = (TPacketGuildSkillUpdate *) c_pData;
...
void CInputDB::GuildWar(const char* c_pData) const
{
	const auto p = (TPacketGuildWar*) c_pData;
...
void CInputDB::GuildWarScore(const char* c_pData) const
{
	const auto p = (TPacketGuildWarScore*) c_pData;
	CGuild * g = CGuildManager::instance().TouchGuild(p->dwGuildGainPoint);
	g->SetWarScoreAgainstTo(p->dwGuildOpponent, p->lScore);
}

void CInputDB::GuildSkillRecharge() const
{
	CGuildManager::instance().SkillRecharge();
}

void CInputDB::GuildExpUpdate(const char* c_pData) const
{
	const auto p = (TPacketGuildSkillUpdate *) c_pData;
...
void CInputDB::GuildAddMember(const char* c_pData) const
{
	const auto p = (TPacketDGGuildMember *) c_pData;
...
void CInputDB::GuildRemoveMember(const char* c_pData) const
{
	const auto p=(TPacketGuild*)c_pData;
	CGuild* g = CGuildManager::instance().TouchGuild(p->dwGuild);
...
void CInputDB::GuildChangeGrade(const char* c_pData) const
{
	const auto p=(TPacketGuild*)c_pData;
	CGuild* g = CGuildManager::instance().TouchGuild(p->dwGuild);
...
void CInputDB::GuildChangeMemberData(const char* c_pData) const
{
	sys_log(0, "Recv GuildChangeMemberData");
	const auto p = (TPacketGuildChangeMemberData *) c_pData;
	CGuild * g = CGuildManager::instance().TouchGuild(p->guild_id);
...
void CInputDB::GuildDisband(const char* c_pData) const
{
	const auto p = (TPacketGuild*) c_pData;
	CGuildManager::instance().DisbandGuild(p->dwGuild);
}

void CInputDB::GuildLadder(const char* c_pData) const
{
	const auto p = (TPacketGuildLadder*) c_pData;
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData) const
{
	LPCHARACTER ch;
...
void CInputDB::AffectLoad(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;
	...
	const DWORD dwPID = decode_4bytes(c_pData);
	c_pData += sizeof(DWORD);
...
void CInputDB::PartyCreate(const char* c_pData) const
{
	const auto p = (TPacketPartyCreate*) c_pData;
	CPartyManager::instance().P2PCreateParty(p->dwLeaderPID);
}

void CInputDB::PartyDelete(const char* c_pData) const
{
	const auto p = (TPacketPartyDelete*) c_pData;
	CPartyManager::instance().P2PDeleteParty(p->dwLeaderPID);
}

void CInputDB::PartyAdd(const char* c_pData) const
{
	const auto p = (TPacketPartyAdd*) c_pData;
	CPartyManager::instance().P2PJoinParty(p->dwLeaderPID, p->dwPID, p->bState);
}

void CInputDB::PartyRemove(const char* c_pData) const
{
	const auto p = (TPacketPartyRemove*) c_pData;
	CPartyManager::instance().P2PQuitParty(p->dwPID);
}

void CInputDB::PartyStateChange(const char* c_pData) const
{
	const auto p = (TPacketPartyStateChange *) c_pData;
	const LPPARTY pParty = CPartyManager::instance().P2PCreateParty(p->dwLeaderPID);
...
void CInputDB::PartySetMemberLevel(const char* c_pData) const
{
	const auto p = (TPacketPartySetMemberLevel*) c_pData;
	const LPPARTY pParty = CPartyManager::instance().P2PCreateParty(p->dwLeaderPID);
...
void CInputDB::Time(const char * c_pData) const
{
...
void CInputDB::GuildSkillUsableChange(const char* c_pData) const
{
	const auto p = (TPacketGuildSkillUsableChange*) c_pData;
...
void CInputDB::AuthLogin(LPDESC d, const char * c_pData) const
{
	if (!d)
		return;

	const BYTE bResult = *(BYTE *) c_pData;
...
void CInputDB::ChangeEmpirePriv(const char* c_pData) const
{
	const auto p = (TPacketDGChangeEmpirePriv*) c_pData;
...
void CInputDB::ChangeGuildPriv(const char* c_pData) const
{
	const auto p = (TPacketDGChangeGuildPriv*) c_pData;
...
void CInputDB::ChangeCharacterPriv(const char* c_pData) const
{
	const auto p = (TPacketDGChangeCharacterPriv*) c_pData;
	CPrivManager::instance().GiveCharacterPriv(p->pid, p->type, p->value, p->bLog);
}

void CInputDB::MoneyLog(const char* c_pData) const
{
	const auto p = (TPacketMoneyLog *) c_pData;
...
void CInputDB::GuildMoneyChange(const char* c_pData) const
{
	const auto p = (TPacketDGGuildMoneyChange*) c_pData;
...
void CInputDB::GuildWithdrawMoney(const char* c_pData) const
{
	const auto p = (TPacketDGGuildMoneyWithdraw*) c_pData;

	const CGuild* g = CGuildManager::instance().TouchGuild(p->dwGuild);
	if (g)
	{
...
void CInputDB::SetEventFlag(const char* c_pData) const
{
	const auto p = (TPacketSetEventFlag*) c_pData;
	quest::CQuestManager::instance().SetEventFlag(p->szFlagName, p->lValue);
}

void CInputDB::CreateObject(const char * c_pData) const
{
	using namespace building;
	CManager::instance().LoadObject((TObject *) c_pData);
}

void CInputDB::DeleteObject(const char * c_pData) const
{
	using namespace building;
	CManager::instance().DeleteObject(*(DWORD *) c_pData);
}

void CInputDB::UpdateLand(const char * c_pData) const
{
	using namespace building;
	CManager::instance().UpdateLand((TLand *) c_pData);
}

void CInputDB::Notice(const char * c_pData) const
{
	char szBuf[256+1];
	...
	SendNoticeRaw(szBuf, false); // text arrives pre-formatted from the db process
}

void CInputDB::GuildWarReserveAdd(TGuildWarReserve * p) const
{
	CGuildManager::instance().ReserveWarAdd(p);
}

void CInputDB::GuildWarReserveDelete(DWORD dwID) const
{
	CGuildManager::instance().ReserveWarDelete(dwID);
}

void CInputDB::GuildWarBet(TPacketGDGuildWarBet * p) const
{
	CGuildManager::instance().ReserveWarBet(p);
}

void CInputDB::MarriageAdd(TPacketMarriageAdd * p) const
{
...
void CInputDB::MarriageUpdate(TPacketMarriageUpdate * p) const
{
...
void CInputDB::MarriageRemove(TPacketMarriageRemove * p) const
{
	sys_log(0, "MarriageRemove %u %u", p->dwPID1, p->dwPID2);
	marriage::CManager::instance().Remove(p->dwPID1, p->dwPID2);
}

void CInputDB::WeddingRequest(TPacketWeddingRequest* p) const
{
	marriage::WeddingManager::instance().Request(p->dwPID1, p->dwPID2);
}

void CInputDB::WeddingReady(TPacketWeddingReady* p) const
{
...
void CInputDB::WeddingStart(TPacketWeddingStart* p) const
{
	sys_log(0, "WeddingStart %u %u", p->dwPID1, p->dwPID2);
	marriage::CManager::instance().WeddingStart(p->dwPID1, p->dwPID2);
}

void CInputDB::WeddingEnd(TPacketWeddingEnd* p) const
{
...
// MYSHOP_PRICE_LIST
void CInputDB::MyshopPricelistRes(LPDESC d, const TPacketMyshopPricelistHeader* p ) const
{
	LPCHARACTER ch;
...
//RELOAD_ADMIN
void CInputDB::ReloadAdmin(const char * c_pData ) const
{
	gm_new_clear();
	...
	for (int n = 0; n < size; ++n )
	{
		...
		const LPCHARACTER pChar = CHARACTER_MANAGER::instance().FindPC(rAdminInfo.m_szName );
		if (pChar )
		{
			pChar->SetGMLevel();
		}
...
void CInputDB::AddMonarchMoney(LPDESC d, const char * data ) const
{
	...
	const DWORD pid = CMonarch::instance().GetMonarchPID(Empire);
...
void CInputDB::DecMonarchMoney(LPDESC d, const char * data) const
{
	const int Empire = *(int *) data;
	data += sizeof(int);
...
void CInputDB::TakeMonarchMoney(LPDESC d, const char * data) const
{
	const int Empire = *(int *) data;
	data += sizeof(int);
...
void CInputDB::ChangeMonarchLord(TPacketChangeMonarchLordACK* info) const
{
	SendNotice("%s�� ���ְ� %s ������ ��ü�Ǿ����ϴ�.", EMPIRE_NAME(info->bEmpire), info->szName);
}

void CInputDB::UpdateMonarchInfo(TMonarchInfo* info) const
{
	CMonarch::instance().SetMonarchInfo(info);
	sys_log(0, "MONARCH INFO UPDATED");
}

void CInputDB::GuildChangeMaster(TPacketChangeGuildMaster* p) const
{
	CGuildManager::instance().ChangeMaster(p->dwGuildID);
}

void CInputDB::DetailLog(const TPacketNeedLoginLogInfo* info) const
{
	const LPCHARACTER pChar = CHARACTER_MANAGER::instance().FindByPID( info->dwPlayerID );
...
void CInputDB::ItemAwardInformer(TPacketItemAwardInfromer *data) const
{
	const LPDESC d = DESC_MANAGER::instance().FindByLoginName(data->login);
...
void CInputDB::RespondChannelStatus(LPDESC desc, const char* pcData) const
{
	if (!desc) {
		return;
	}
	...
	const BYTE bHeader = HEADER_GC_RESPOND_CHANNELSTATUS;
