...
class CGuildManager : public singleton<CGuildManager>
{
	public:
		...
		void		RequestEndWar(DWORD guild_id1, DWORD guild_id2) const;
		void		RequestCancelWar(DWORD guild_id1, DWORD guild_id2) const;
		...
		void		Kill(LPCHARACTER killer, LPCHARACTER victim) const;
		...
		int		GetDisbandDelay() const;
		int		GetWithdrawDelay() const;

		void		ChangeMaster(DWORD dwGID);

		#ifdef ENABLE_GUILD_TOKEN_AUTH
		uint64_t	GenerateTokenHashNumber() const;
		bool		IsCorrectGuildToken(DWORD guildID, uint64_t hashID) const;
		void		SendGuildToken(LPCHARACTER ch, uint64_t token) const;
