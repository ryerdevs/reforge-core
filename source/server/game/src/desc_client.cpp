...
bool CLIENT_DESC::IsRetryWhenClosed() const
{
...
void CLIENT_DESC::UpdateChannelStatus(DWORD t, bool fForce)
{
	...
	const DWORD tLCSUP = m_tLastChannelStatusUpdateTime+CHANNELSTATUS_UPDATE_PERIOD;
	if (fForce || tLCSUP < t) {
		int iTotal;
...
void CLIENT_DESC::Reset()
{
	...
	const LPFDWATCH fdw = m_lpFdw;
	const std::string host = m_stHost;
	const WORD port = m_wPort;
