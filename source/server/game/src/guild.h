...
class CGuild
{
	public:
		...
		int		GetMaxSP() const { return m_data.max_power; }
		DWORD		GetMasterPID() const	{ return m_data.master_pid; }
		LPCHARACTER	GetMasterCharacter() const;
		...
		void		RequestDisband(DWORD pid) const;
		void		Disband();
		...
		void		SendAllGradePacket(LPCHARACTER ch) const;
		void		SendListPacket(LPCHARACTER ch);
		void		SendListOneToAll(DWORD pid);
		void		SendListOneToAll(LPCHARACTER ch);
		void		SendLoginPacket(LPCHARACTER ch, LPCHARACTER chLogin) const;
		void		SendLogoutPacket(LPCHARACTER ch, LPCHARACTER chLogout) const;
		void		SendLoginPacket(LPCHARACTER ch, DWORD pid) const;
		void		SendLogoutPacket(LPCHARACTER ch, DWORD pid) const;
		...
		void		AddComment(LPCHARACTER ch, const std::string& str) const;
		void		DeleteComment(LPCHARACTER ch, DWORD comment_id);

		void		RefreshComment(LPCHARACTER ch) const;
		void		RefreshCommentForce(DWORD player_id) const;

		int			GetSkillLevel(DWORD vnum) const;
		...
		void		SendDBSkillUpdate(int amount = 0) const;

		void		SkillRecharge() const;
		bool		ChargeSP(LPCHARACTER ch, int iSP) const;
		...
		// Guild Money
		void		RequestDepositMoney(LPCHARACTER ch, int iGold) const;
		void		RequestWithdrawMoney(LPCHARACTER ch, int iGold) const;

		void		RecvMoneyChange(int iGold);
		void		RecvWithdrawMoneyGive(int iChangeGold) const;
		...
		// War state relative
		void		NotifyGuildMaster(const char* msg) const;
		...
		void		ChangeLadderPoint(int iChange) const;
	...
	public:
		...
		LPDUNGEON GetDungeon_for_Only_guild() const;
#endif
#ifdef ENABLE_NEWSTUFF
	public:
		void SetSkillLevel(DWORD dwVnum, BYTE level, BYTE point = 0);
		DWORD GetSkillPoint() const;
