...
int check_name_latin1(const char * str)
{
	...
	for (tmp = str; *tmp; ++tmp)
	{
		...
		const unsigned char uc_tmp = *tmp;
...
void LocaleService_LoadLocaleStringFile()
{
	...
	// Language System: load one locale_string file per supported language.
	// Runtime layout: <base>/locale_string_XX.txt with the language code in
	// UPPERCASE (e.g. locale/spain/locale_string_ES.txt). Files are optional;
	// locale_init_lang() returns the number of loaded entries (0 = missing or
	// empty file). fprintf is used (not sys_log) because at config_init time
	// the log file is not open yet and sys_log is silent on stdout when
	// DB_LOG_LEVEL <= 1 — these lines are the boot evidence of the load.
	for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
	{
		const std::string stLocaleStringFile = LocaleService_GetBasePath() + "/locale_string_" + arstLocaleStringNames[i] + ".txt";
		const int iLoaded = locale_init_lang((BYTE) i, stLocaleStringFile.c_str());
		fprintf(stdout, "Load LocaleString %s (%d entries)\n", stLocaleStringFile.c_str(), iLoaded);
	}
#else
...
bool LC_IsEurope()
{
	const eLocalization val = LC_GetLocalType();
