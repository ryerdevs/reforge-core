...
namespace marriage
{
	...
	{
		const int point = MINMAX(50, GetMarriagePoint(), 100);
		if (point < 65)
			return 0;
		else if (point < 80)
			return 1;
	...
	int TMarriage::GetMarriagePoint()
	{
		if (test_server)
		{
			const int value = quest::CQuestManager::instance().GetEventFlag("lovepoint");
			if (value)
	...
	{
		const auto info = dynamic_cast<near_check_event_info*>( event->info );
	...
	void TMarriage::Update(DWORD point)
	{
		...
		if (point > 0 && is_married)
		{
			bSave = true;
			// @fixme126
			const uint64_t llActualPoints = static_cast<uint64_t>(love_point) + point;
	...
	void TMarriage::WarpToWeddingMap(DWORD dwPID) const
	{
		if (!pWeddingInfo)
			return;
	...
	bool CManager::IsMarriageUniqueItem(DWORD dwItemVnum) const
	{
	...
	{
		const TMarriage* pkMarriageFinded=Get(dwPlayerID);
	...
	bool CManager::Initialize() const
	{
		return true;
	}

	void CManager::Destroy() const
	{
	}
	...
	void CManager::RequestAdd(DWORD dwPID1, DWORD dwPID2, const char* szName1, const char* szName2) const
	{
		if (dwPID1 > dwPID2)
		{
			std::swap(dwPID1, dwPID2);
	...
	void CManager::RequestRemove(DWORD dwPID1, DWORD dwPID2) const
	{
		Align(dwPID1, dwPID2);
	...
	{
		const TMarriage* pMarriage = Get(dwPID1);
		if (!pMarriage || pMarriage->GetOther(dwPID1) != dwPID2)
		{
			sys_err("wrong marriage %u, %u", dwPID1, dwPID2);
			return;
		...
		const TWeddingInfo * pwi = pMarriage->pWeddingInfo;
	...
	void CManager::RequestEndWedding(DWORD dwPID1, DWORD dwPID2) const
	{
		TPacketWeddingEnd p;
		p.dwPID1 = dwPID1;
		p.dwPID2 = dwPID2;
