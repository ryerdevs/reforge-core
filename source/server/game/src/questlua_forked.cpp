...
namespace quest
{
	...
	{
		const int iEmpire 	= (int)lua_tonumber(L, 1);
	...
	{
		const auto info = dynamic_cast<warp_all_to_map_event_info *>(event->info);
		...
		const LPSECTREE_MAP pSecMap = SECTREE_MANAGER::instance().GetMap( info->m_iMapIndexFrom );
		if ( pSecMap )
		{
			FWarpInMap f;
	...
	{
		const int iMapIndexFrom	= (int)lua_tonumber(L, 1 );
		const int iMapIndexTo		= (int)lua_tonumber(L, 2 );
		const int ix				= (int)lua_tonumber(L, 3 );
		const int iy				= (int)lua_tonumber(L, 4 );
		const int iTime			= (int)lua_tonumber(L, 5 );
