...
{
	const int ret = lzo1x_1_compress(src, srcsize, dest, puiDestSize, GetWorkMemory());
...
{
	const int ret = lzo1x_decompress_safe(src, srcsize, dest, puiDestSize, GetWorkMemory());
