...
class CDungeon
{
	...
	int	CountMonster() const { return m_iMonsterCount; }
	int	CountRealMonster() const;
	...
	void	SpawnMoveGroup(DWORD vnum, const char* pos_from, const char* pos_to, int count=1) const;
	...
	void	JumpAll(long lFromMapIndex, int x, int y) const;
	void	WarpAll(long lFromMapIndex, int x, int y) const;
