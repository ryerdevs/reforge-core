...
static float MOB_GetNormalAttackDuration(TMobTable* mobTable)
{
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (strcmp(mode, "GENERAL") == 0 && strncmp(type, "NORMAL_ATTACK", 13) == 0)
		{
			...
			const float curDuration = MSA_GetNormalAttackDuration(msaPath);
...
static void LoadMotion(CMotionSet* pMotionSet, TMobTable* mob_table, EPublicMotion motion)
{
	...
	const auto pMotion = M2_NEW CMotion;
...
{
	const auto it = m_map_pkMotionSet.find(dwVnum);
...
{
	const auto f = m_map_normalAttackDuration.find(dwVnum);
...
{
	const auto it = m_map_pkMotion.find(dwKey);
...
{
	const auto pkMotion = M2_NEW CMotion;
