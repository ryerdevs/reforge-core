...
namespace marriage
{
	...
	{
		const auto info = dynamic_cast<wedding_map_info*>( event->info );
	...
	void WeddingMap::SendLocalEvent(LPCHARACTER ch) const
	{
		char szCommand[256];
	...
	const char* WeddingMap::__BuildCommandPlayMusic(char* szCommand, size_t nCmdLen, BYTE bSet, const char* c_szMusicFileName) const
	{
		if (nCmdLen < 1)
		{
			szCommand[0] = '\0';
	...
	bool WeddingManager::IsWeddingMap(DWORD dwMapIndex) const
	{
	...
	DWORD WeddingManager::__CreateWeddingMap(DWORD dwPID1, DWORD dwPID2)
	{
		...
		// LOCALE_SERVICE
		const LPSECTREE_MAP pkSectreeMap = rkSecTreeMgr.GetMap(dwMapIndex);
	...
	void WeddingManager::Request(DWORD dwPID1, DWORD dwPID2)
	{
		...
		{
			const DWORD dwMapIndex = __CreateWeddingMap(dwPID1, dwPID2);
