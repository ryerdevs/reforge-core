...
class SECTREE_MAP
{
	public:
		...
		std::pair<long, long> CoordinateToLocal(long x, long y) const;
		std::pair<long, long> CoordinateToGlobal(long x, long y) const;
...
class SECTREE_MANAGER : public singleton<SECTREE_MANAGER>
{
	public:
		...
		int		LoadSettingFile(long lIndex, const char * c_pszSettingFileName, TMapSetting & r_setting) const;
		...
		LPSECTREE_MAP BuildSectreeFromSetting(TMapSetting & r_setting) const;
		bool		LoadAttribute(LPSECTREE_MAP pkMapSectree, const char * c_pszFileName, TMapSetting & r_setting) const;
		...
		bool		GetMovablePosition(long lMapIndex, long x, long y, PIXEL_POSITION & pos) const;
		bool		IsAttackablePosition(long lMapIndex, long x, long y) const;
		bool		IsMovablePosition(long lMapIndex, long x, long y) const;
		...
		BYTE		GetEmpireFromMapIndex(long lMapIndex) const;

		void		PurgeMonstersInMap(long lMapIndex) const;
		void		PurgeStonesInMap(long lMapIndex) const;
		void		PurgeNPCsInMap(long lMapIndex) const;
		size_t		GetMonsterCountInMap(long lMapIndex) const;
		size_t		GetMonsterCountInMap(long lMpaIndex, DWORD dwVnum) const;
