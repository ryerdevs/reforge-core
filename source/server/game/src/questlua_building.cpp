...
namespace quest
{
	...
	ALUA(building_get_land_id)
	{
		...
		const CLand * pkLand = CManager::instance().FindLand((int) lua_tonumber(L, 1), (int) lua_tonumber(L, 2), (int) lua_tonumber(L, 3));
	...
	ALUA(building_get_land_info)
	{
		...
		if (lua_isnumber(L, 1))
		{
			...
			const CLand * pkLand = CManager::instance().FindLand((DWORD) lua_tonumber(L, 1));
	...
	ALUA(building_has_land)
	{
		...
		const auto pmsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM land%s WHERE guild_id = %d", get_table_postfix(), (DWORD)lua_tonumber(L,1)));
	...
	ALUA(building_reconstruct)
	{
		...
		const DWORD dwNewBuilding = (DWORD)lua_tonumber(L, 1);
		...
		const CGuild* pGuild = npc->GetGuild();
		if (!pGuild)
			return 0;
		...
		const LPOBJECT pObject = pLand->FindObjectByNPC(npc);
		if (!pObject)
			return 0;
