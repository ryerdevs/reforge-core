...
{
	if (m_pProto && map_allow_find(GetMapIndex())) // @fixme315
	{
...
void CObject::RegenNPC()
{
	...
	const DWORD dwGuildID = m_pkLand->GetOwner();
	CGuild* pGuild = CGuildManager::instance().FindGuild(dwGuildID);

	if (!pGuild)
		return;

	const int x = m_pProto->lNPCX;
	const int y = m_pProto->lNPCY;
	int newX, newY;

	const float rot = m_data.zRot * 2.0f * M_PI / 360.0f;
...
void CLand::Destroy()
{
	...
	{
		const LPOBJECT pkObj = (it++)->second;
		...
		if (r)
		{
			...
			if (CHARACTER_MANAGER::instance().GetCharactersByRaceNum(20040, i))
			{
				...
				while (it != i.end())
				{
					...
					const int x = ch->GetX() - r->sx;
					const int y = ch->GetY() - r->sy;
...
{
	const auto it = m_map_pkObject.find(dwID);
...
bool CLand::RequestCreateObject(DWORD dwVnum, long lMapIndex, long x, long y, float xRot, float yRot, float zRot, bool checkAnother) const
{
	SECTREE_MANAGER& rkSecTreeMgr = SECTREE_MANAGER::instance();
	const TObjectProto * pkProto = CManager::instance().GetObjectProto(dwVnum);
	...
	const int sx = r->sx + m_data.x;
	const int ex = sx + m_data.width;
	const int sy = r->sy + m_data.y;
	const int ey = sy + m_data.height;

	const int osx = x + pkProto->lRegion[0];
	const int osy = y + pkProto->lRegion[1];
	const int oex = x + pkProto->lRegion[2];
	const int oey = y + pkProto->lRegion[3];

	const float rad = zRot * 2.0f * M_PI / 360.0f;

	const int tsx = (int)(pkProto->lRegion[0] * cosf(rad) + pkProto->lRegion[1] * sinf(rad) + x);
	const int tsy = (int)(pkProto->lRegion[0] * -sinf(rad) + pkProto->lRegion[1] * cosf(rad) + y);

	const int tex = (int)(pkProto->lRegion[2] * cosf(rad) + pkProto->lRegion[3] * sinf(rad) + x);
	const int tey = (int)(pkProto->lRegion[2] * -sinf(rad) + pkProto->lRegion[3] * cosf(rad) + y);
...
void CLand::RequestUpdate(DWORD dwGuild) const
{
	DWORD a[2];
...
{
	const auto it = m_map_pkLand.find(dwID);
...
// BUILD_WALL
void CLand::DrawWall(DWORD dwVnum, long nMapIndex, long& x, long& y, char length, float zRot) const
{
	int rot = (int)zRot;
	rot = ((rot%360) / 90) * 90;
...
bool CLand::RequestCreateWall(long nMapIndex, float rot) const
{
	const bool 	WALL_ANOTHER_CHECKING_ENABLE = false;

	const TLand& land = GetData();

	const int center_x = land.x + land.width  / 2;
	const int center_y = land.y + land.height / 2;

	const int wall_x = center_x;
	const int wall_y = center_y;
	const int wall_half_w = 1000;
	const int wall_half_h = 1362;

	if (rot == 0.0f)
	{
		const int door_x = wall_x;
		const int door_y = wall_y + wall_half_h;
	...
	{
		const int door_x = wall_x + wall_half_h;
		const int door_y = wall_y;
...
void CLand::RequestDeleteWall()
{
	...
	while (iter != m_map_pkObject.end())
	{
		const unsigned id   = iter->second->GetID();
		const unsigned vnum = iter->second->GetVnum();
...
bool CLand::RequestCreateWallBlocks(DWORD dwVnum, long nMapIndex, char wallSize, bool doorEast, bool doorWest, bool doorSouth, bool doorNorth) const
{
	const TLand & r = GetData();
	...
	const DWORD corner = dwVnum - 4;
	const DWORD wall   = dwVnum - 3;
	const DWORD door   = dwVnum - 1;

	const bool checkAnother = false;
	long* ptr = nullptr;
	int delta = 1;
	int rot = 270;
...
void CLand::RequestDeleteWallBlocks(DWORD dwID)
{
	...
	const DWORD corner = dwID - 4;
	const DWORD wall = dwID - 3;
	const DWORD door = dwID - 1;
	DWORD dwVnum = 0;
