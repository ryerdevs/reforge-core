...
void CGuild::RequestAddMember(LPCHARACTER ch, int grade)
{
	...
	if (m_member.contains(ch->GetPlayerID()))
	{
...
void CGuild::SendAllGradePacket(LPCHARACTER ch) const
{
	const LPDESC d = ch->GetDesc();
	if (!d)
		return;
...
void CGuild::SendListOneToAll(DWORD pid)
{
	...
	const auto cit = m_member.find(pid);
	if (cit == m_member.end())
		return;
...
void CGuild::SendLoginPacket(LPCHARACTER ch, LPCHARACTER chLogin) const
{
	SendLoginPacket(ch, chLogin->GetPlayerID());
}

void CGuild::SendLoginPacket(LPCHARACTER ch, DWORD pid) const
{
	/*
	   Login Packet
	   header 4
...
void CGuild::SendLogoutPacket(LPCHARACTER ch, LPCHARACTER chLogout) const
{
	SendLogoutPacket(ch, chLogout->GetPlayerID());
}

void CGuild::SendLogoutPacket(LPCHARACTER ch, DWORD pid) const
{
	/*
	   Logout Packet
	   header 4
...
void CGuild::LoadGuildMemberData(SQLMsg* pmsg)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		const BYTE grade = (BYTE) strtoul(row[1], (char**)nullptr, 10);
		BYTE is_general = 0;

		if (row[2] && *row[2] == '1')
			is_general = 1;

		const DWORD offer = strtoul(row[3], (char**)nullptr, 10);
		const BYTE level = (BYTE)strtoul(row[4], (char**)nullptr, 10);
		const BYTE job = (BYTE)strtoul(row[5], (char**)nullptr, 10);
		char * name = row[6];
...
void CGuild::LoadGuildGradeData(SQLMsg* pmsg)
{
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		const DWORD auth = strtoul(row[2], nullptr, 10);
...
void CGuild::SendDBSkillUpdate(int amount) const
{
	TPacketGuildSkillUpdate guild_skill;
	guild_skill.guild_id = m_data.guild_id;
...
{
	const auto it = m_member.find(pid);
	if (it==m_member.end())
		return nullptr;
...
{
	const auto cit = m_member.find(ch->GetPlayerID());
...
void CGuild::RequestDisband(DWORD pid) const
{
...
void CGuild::AddComment(LPCHARACTER ch, const std::string& str) const
{
#ifdef ENABLE_GUILD_COMMENT_ANTIFLOOD
...
void CGuild::RefreshComment(LPCHARACTER ch) const
{
	RefreshCommentForce(ch->GetPlayerID());
}

void CGuild::RefreshCommentForce(DWORD player_id) const
{
	...
	const auto pmsg(DBManager::instance().DirectQuery("SELECT id, name, content FROM guild_comment%s WHERE guild_id = %u ORDER BY notice DESC, id DESC LIMIT %d", get_table_postfix(), m_data.guild_id, GUILD_COMMENT_MAX_COUNT));
	...
	const BYTE count = pmsg->Get()->uiNumRows;
...
bool CGuild::ChangeMemberGeneral(DWORD pid, BYTE is_general)
{
	...
	{
		const LPDESC d = (*(itOnline++))->GetDesc();
...
{
	const DWORD dwRealVnum = dwVnum - GUILD_SKILL_START;
	...
	switch (dwVnum)
	{
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				...
				{
					const CCI * pcci = P2P_MANAGER::instance().FindByPID(pid);
...
int CGuild::GetSkillLevel(DWORD vnum) const
{
	const DWORD dwRealVnum = vnum - GUILD_SKILL_START;
...
void CGuild::SkillRecharge() const
{
}
...
LPCHARACTER CGuild::GetMasterCharacter() const
{
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP) const
{
	int gold = iSP * 100;
...
{
	const DWORD dwRealVnum = dwSkillVnum - GUILD_SKILL_START;
...
void CGuild::BroadcastMemberCountBonus() const
{
	TPacketGGGuild p1;
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold) const
{
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold) const
{
	if (false==ch->CanDeposit())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 잠시후에 이용해주십시오"));
...
void CGuild::RecvWithdrawMoneyGive(int iChangeGold) const
{
	const LPCHARACTER ch = GetMasterCharacter();
...
{
	const auto pInfo = dynamic_cast<TInviteGuildEventInfo*>( event->info );
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	if (m_GuildInviteEventMap.contains( pchInvitee->GetPlayerID() ) )
		return;
...
{
	const auto itFind = m_GuildInviteEventMap.find( pchInvitee->GetPlayerID() );
...
{
	const auto itFind = m_GuildInviteEventMap.find( dwPID );
	if ( itFind == m_GuildInviteEventMap.end() )
	{
...
CGuild::GuildJoinErrCode CGuild::VerifyGuildJoinableCondition( const LPCHARACTER pchInvitee )
{
	...
	{
		const auto pMsg( DBManager::instance().DirectQuery("SELECT value FROM guild_invite_limit WHERE id=%d", GetID()) );
		if ( pMsg->Get()->uiNumRows > 0 )
		{
			const MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
			time_t limit_time=0;
...
LPDUNGEON CGuild::GetDungeon_for_Only_guild() const
{
...
DWORD CGuild::GetSkillPoint() const
{
