	...
	def SetBasePos(self, basePos):
		if app.__BL_CLIP_MASK__:
			...
			for item in self.itemList:
				...
				if localeInfo.IsARABIC():
					item.SetPosition(self.GetWidth() - item.GetWidth() - 10, curPos - fromPos)
				else:
...
class PythonScriptLoader(object):
	...
	def LoadChildren(self, parent, dicChildren):
		...
		for ElementValue in ChildrenList:
			...
			if Type == "window":
			...
			elif app.ENABLE_UI_CIRCLE and Type == "circle":
				...
				dbg.TraceError("Window Type {} not handled. Skipped.".format(Type))
				Index += 1
				continue
	...
	def LoadElementButton(self, window, value, parentWindow):
		...
		if app.ENABLE_AUTO_L2R and value.has_key("l2r") and value["l2r"]:
